...
def _merge_3way_dfs_on_key(current,injecting,ancestor,key,visited=_A):
	H=visited;G=ancestor;C=injecting;B=current;A=key;I=A in B;J=A in C;K=A in G;M=K and not J and I;N=not K and J and not I;L=J and I and B[A]!=C[A];O=L and K and B[A]==G[A]
	if M:del B[A]
	elif N:D=C[A];B[A]=_deepcopy_or_default(D,D)
	elif O:E=B[A];D=C[A];B[A]=_deepcopy_or_default(D,E)
	elif L:
		E=B[A];F=E
		if not is_composite_type(F):F=C[A];B[A]=_deepcopy_or_default(F,E)
		else:0
		P,H=check_already_visited_obj_id(F,H)
		if not P:merge_3way(B[A],C.get(A,{}),G.get(A,{}),H)
