...
from funpinpin_cli.core.util.context import which
...
class Node(object):
    ...
    def build(self, verbose):
        ...
        cmd = f"git clone https://git.funpinpin.cn/apps/apps-template.git {self.name}"
        os.system(cmd)
        root = os.path.join(os.getcwd(), self.name)
        os.chdir(root)
        cmd = "npm --user-config .npmrc config set @funpinpin:registry https://registry.yarnpkg.com"
        os.system(cmd)
