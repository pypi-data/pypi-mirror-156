...
dag = DAG(
    dag_id='airflow_init_procedure_code',
...
query1 = f"CALL GCWB_WDB.DM.P_MMKD_PNT_PAY_DED_RSN_I();",


def select_query(**context):
    dwh_hook = SnowflakeHook(snowflake_conn_id="SNOWFLAKE_CONN_ID")
    result = dwh_hook.get_first("SELECT COUNT(*) FROM GCWB_WDB.DM.MMKD_PNT_PAY_DED_RSN;")
    logging.info("Number of rows in `GCWB_WDB.DM.MMKD_PNT_PAY_DED_RSN`  - %s", result[0])

t1 = SnowflakeOperator(
    task_id= 'P_MMKD_PNT_PAY_DED_RSN_I',
    sql=query1,
