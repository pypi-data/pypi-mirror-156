...
dag = DAG(
    dag_id='airflow_init_procedure_behavior',
...
query1 = f"CALL GCWB_WDB.DM.P_MOBS_BHV_STEP_I({yesterday.strftime('%Y%m%d')});",


def select_query(**context):
    dwh_hook = SnowflakeHook(snowflake_conn_id="SNOWFLAKE_CONN_ID")
    result = dwh_hook.get_first("SELECT COUNT(*) FROM GCWB_WDB.DM.MOBS_BHV_STEP")
    logging.info("Number of rows in `GCWB_WDB.DM.MOBS_BHV_STEP`  - %s", result[0])

t1 = SnowflakeOperator(
    task_id= 'MOBS_BHV_STEP',
    sql=query1,
