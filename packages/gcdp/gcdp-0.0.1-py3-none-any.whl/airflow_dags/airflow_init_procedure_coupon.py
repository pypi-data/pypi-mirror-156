...
dag = DAG(
    dag_id='airflow_init_procedure_coupon',
...
query1 = f"CALL GCWB_WDB.DM.P_MMKD_CPN_I({yesterday.strftime('%Y%m%d')});",


def select_query(**context):
    dwh_hook = SnowflakeHook(snowflake_conn_id="SNOWFLAKE_CONN_ID")
    result = dwh_hook.get_first("SELECT COUNT(*) FROM GCWB_WDB.DM.MMKD_CPN")
    logging.info("Number of rows in `GCWB_WDB.DM.MMKD_CPN`  - %s", result[0])
