...
dag = DAG(
    dag_id='airflow_init_procedure_health_topic',
...
query1 = "CALL GCWB_WDB.DM.P_MMBD_INTRST_SUBJ_I();",


def select_query(**context):
    dwh_hook = SnowflakeHook(snowflake_conn_id="SNOWFLAKE_CONN_ID")
    result = dwh_hook.get_first("SELECT COUNT(*) FROM GCWB_WDB.DM.MMBD_INTRST_SUBJ;")
    logging.info("Number of rows in `GCWB_WDB.DM.MMBD_INTRST_SUBJ`  - %s", result[0])

t1 = SnowflakeOperator(
    task_id= 'P_MMBD_INTRST_SUBJ_I',
    sql=query1,
