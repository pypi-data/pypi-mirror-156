...
def get_GA_PNT_BEHAVIOR(START_DATE, END_DATE):
    ...
    GA_PNT_BEHAVIOR = API_Result_DF(Dict_input)
    GA_PNT_BEHAVIOR = GA_PNT_BEHAVIOR.astype({'sessions': int})
    GA_PNT_BEHAVIOR.columns = [col.upper() for col in GA_PNT_BEHAVIOR.columns.values]
    return GA_PNT_BEHAVIOR
...
def main(start_day, end_day):
    ...
    O_GA_PNT_BEHAVIOR.to_csv(f"{os.path.join(set_downloads_folder('ga'),f'{service_name}.csv')}", encoding='utf-8-sig', index=False)
