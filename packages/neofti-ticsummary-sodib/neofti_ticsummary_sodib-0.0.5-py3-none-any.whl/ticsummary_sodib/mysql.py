...
def __getRecordByRandomDebug__():
    result = np.zeros(shape=(1024,21),dtype=int)
    for i in range(21):
        for j in range(1024):
            if i in range(16):
                result[j][i] = int(np.random.rand()*100*(8/(np.abs(i - 8)+0.5)))
            if i == 16:
                result[j][i] = int(np.random.rand()*100*1)
            if i == 17:
                result[j][i] = int(np.random.rand()*100*1)
            if i == 18:
                result[j][i] = int(np.random.rand()*100*1)
            if i == 19:
                result[j][i] = int(np.random.rand()*100*1)
            if i == 20:
                result[j][i] = int(np.random.rand()*100*4)
    return result
    
def getRecordByIdFirstBank(table, connector, id):
    query = "SELECT {0},{1},{2},{3},{4} FROM {5} WHERE {6}={7}".format(
        TABLE_3FIELD_NAME,
        TABLE_6FIELD_NAME,
        TABLE_7FIELD_NAME,
        TABLE_10FIELD_NAME,
...
def getRecordByIdFirstBankWithGS(table,connector,id):
    query = "SELECT {0},{1},{2},{3},{4},{5} FROM {6} WHERE {7}={8}".format(
        TABLE_3FIELD_NAME,
        TABLE_6FIELD_NAME,
        TABLE_7FIELD_NAME,
        TABLE_10FIELD_NAME,
    ...
    with connector.cursor() as cursor:
        try:
            cursor.execute(query)
            result = cursor.fetchall()
            return __getDataBySTRWithGS__(result[0])  
        except Error as e:
            print("Error while connecting to MySQL:")
...
def getRecordByIdSecondBank(table, connector, id):
    ...
    with connector.cursor() as cursor:
        try:
            ...
            print("Error while connecting to MySQL:")
def getRecordByIdSecondBankWithGS(table, connector, id):
    query = "SELECT {0},{1},{2},{3},{4},{5} FROM {6} WHERE {7}={8}".format(
        TABLE_3FIELD_NAME,
        TABLE_8FIELD_NAME,
        TABLE_9FIELD_NAME,
...
def __getDataBySTRWithGS__(str):
    listMatrixX=json.loads(str[3])
    listMatrixY=json.loads(str[4])
    listMatrixGS=json.loads(str[5])
    resultArray = np.zeros(shape=(len(listMatrixX),len(listMatrixX[0])+len(listMatrixY[0])+len(listMatrixGS[0])),dtype=int)
    for i in range(0,len(listMatrixX)):
        for j in range(0,len(listMatrixX[0])):
            resultArray[i][j] = listMatrixX[i][j]
    for i in range(0,len(listMatrixY)):
        for j in range(len(listMatrixX[0]),len(listMatrixX[0])+len(listMatrixY[0])):
            resultArray[i][j] = listMatrixY[i][j-len(listMatrixX[0])]
    for i in range(0,len(listMatrixGS)):
        for j in range(len(listMatrixX[0])+len(listMatrixY[0]),len(listMatrixX[0])+len(listMatrixY[0])+len(listMatrixGS[0])):
            resultArray[i][j] = listMatrixGS[i][j-(len(listMatrixX[0])+len(listMatrixY[0]))]
    return DownloadData(
        id=id,
        acsn=0,
        threshold=None,
...
def getQDatabaseByParameters(parameters):
    db = QtSql.QSqlDatabase.addDatabase("QMYSQL")
    db.setHostName(parameters.host)
...
def getSearchModelByParameters(parameters):
    ...
    model.setQuery(str, getQDatabaseByParameters(parameters))
    return model
...
def getRunId(id,table,connector):
