...
class Model(QtCore.QObject):
    ...
    def __initValue__(self):
        ...
        self.chargeDevice = dataTIC.DescriptionDevice("charge", 69, 69)
    ...
    def setData(self,data:mysql.DownloadData):
        ...
        self.dataCharge = inputDataHandler.getMatrixByFromToFilter(data.matrix, self.chargeDevice.channelFrom, self.chargeDevice.channelTo)
    
        x = np.zeros(shape=np.size(self.dataScL))
        for i in range(np.size(self.dataScL)):
            x[i] = i * float(data.timeslice/10**6)
        self.mainWindow.setDataMCP(self.dataProfile, float(data.timeslice/10**6), 2)
        self.mainWindow.setDataGalo(self.dataScL, self.dataScR, self.dataScT, self.dataScB, self.dataScC,self.dataCharge, x,self.__coefGaloCalibration)
