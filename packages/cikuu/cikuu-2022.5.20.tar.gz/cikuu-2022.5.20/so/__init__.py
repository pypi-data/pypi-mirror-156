...
if not hasattr(requests, 'eshost'):
	requests.eshost	= os.getenv("eshost", "172.17.0.1") 
...
config = {  
		...
		"mappings" : {
			...
			"snt": { "type": "text", "analyzer": "standard","store":"true"}
		  }
		}
	}
...
rows	= lambda query: requests.post(f"http://{requests.eshost}:{requests.esport}/_sql",json={"query": query}).json().get('rows',[]) 
sql		= lambda query: requests.post(f"http://{requests.eshost}:{requests.esport}/_sql",json={"query": query}).json().get('rows',[]) 
...
warmup  = lambda : requests.put(f"http://{requests.eshost}:{requests.esport}/_cluster/settings", json={"persistent": {"search.max_buckets": 1000000}}).text
...
def ids(_ids:list, cp:str='inau'): 
	...
	return requests.post(f"http://{requests.eshost}:{requests.esport}/{cp}/_search/", json=sql).json()
...
def hybchunk(hyb:str='the _NNS of', index:str='gzjc', size:int= -1, topk:int=10):
  ...
  "size":  size
	...
	res = requests.post(f"http://{requests.eshost}:{requests.esport}/{index}/_search/", json=sql).json()
	si = Counter()
	repat = rehyb(hyb)
