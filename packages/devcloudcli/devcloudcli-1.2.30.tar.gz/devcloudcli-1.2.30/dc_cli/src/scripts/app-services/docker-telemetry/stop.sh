...
if [ "$(echo "intel123" | sudo docker ps -q -f name=^cadvisor$)" ]; then
    ...
    if [ $1=="all-services" ]; then 
        ...
        sudo -E docker-compose -f /usr/local/lib/python$ver/dist-packages/src/scripts/app-services/docker-telemetry/docker-compose-app-service.yaml stop influxdb grafana cadvisor prometheus
