...
class Track:
    def __init__(self, name, ntype=2, col=0xffffff, montype=0, dname='', unit='', fmt=1, srate=0, gain=1.0, offset=0.0, mindisp=0, maxdisp=0, recs=None):
        self.type = ntype  # 1: wav, 2: num, 5: str
        self.fmt = fmt
        self.name = name
        self.srate = srate
        self.unit = unit
            ...
            for path in ipath:
                ...
                if self.dtstart == 0:  # open the first file
                    ...
                    for dname, dev in vf.devs.items():
                        if dname not in self.devs:  # new device
                            self.devs[dname] = dev
                    for dtname, trk in vf.trks.items():
                        if dtname in self.trks:  # track exists
            ...
            while True:
                ...
                if packet_type == 9:  # devinfo
                    ...
                    if cmd == 6:  # reset events
                        ...
                        self.order = [tid_dtnames[tid] for tid in tids]
                        pos += cnt * 2
...
def vital_recs(ipath, track_names=None, interval=None, return_timestamp=False, return_datetime=False, return_pandas=False, exclude=None):
    ...
    :param track_names: list of track names, eg) ['SNUADC/ECG', 'Solar 8000/HR']
    :param interval: interval of each samples. if None, maximum resolution. if there is no wave tracks, 1 sec
...
if __name__ == '__main__':
    ...
    print(time.time()-dtstart)
    dtstart = time.time()
    vf.to_vital('1.vital')
    print(time.time()-dtstart)
    #df = vf.to_pandas(TRACK_NAMES, 1/60, return_datetime=True)
    #df.to_csv('00001.csv', index=False, encoding='utf-8-sig')
    quit()
