...
class ReHelper(object):
    ...
    def extract_all_image_src(str_source: str, pre_url: str = '') -> list:
        ...
        if str_source:
            ...
            if len(images) == 0:
                images = re.findall(r'<img.*?src=\"(.*?)\".*?/?>', str_source, flags=re.IGNORECASE)
        if pre_url:
            return [image if (pre_url in image or 'http' in image) else f'{pre_url}{image}' for image in images]
        else:
            return images
    ...
    @staticmethod
    def get_array_first_str(arry_str: list) -> [str, None]:
        """
        获取数组中第一个元素并删除元素中的空格和换行符
        :param arry_str: 原始数组
        :return: str
        """
        if arry_str:
            if isinstance(arry_str, list):
                return ReHelper.delete_tab_and_br(''.join(arry_str))
        return None
    ...
    @staticmethod
    def get_price(price_str: str) -> [float, None]:
        pass
...
if __name__ == '__main__':
    ...
    print(ReHelper.delete_html_tag_and_content(source_s))
