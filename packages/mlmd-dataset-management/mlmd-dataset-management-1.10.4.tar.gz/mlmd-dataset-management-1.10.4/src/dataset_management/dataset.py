...
class Dataset():
    ...
    def remove_files(self, filelist):
        ...
        if len(image_metadata) > 0:
            print("Updating {} image log".format(len(image_metadata)))
            log_metadata = [(image_id, exe_id, self.uncommitted_version, self.id) for image_id in image_metadata_ids]
            if len(log_metadata) > 0:
                insert_image_log_bulk(log_metadata)
        else:
            print("No image_id matched")
    ...
    def get_filelist(self, metadata_filter=None, get_annotation=False, file_path=None, get_metadata=False, encode_annotation=False):
        ...
        for key,value in self.global_changelist.items():            
            ...
            if value["changes"][0] == addtype.id:
                if get_annotation or get_metadata:
                    ...
                    if encode_annotation:
                        ann = annotation_dict.get(value["annotation"])
                    if get_metadata:
                        yield ret_key, ann, value["metadata"]
                    else:
                        yield ret_key, ann
                else:
                    yield ret_key
