...
class MysqlPool(BaseConnectionPool):
    ...
    def sync_create_pool(self):
        pool = PooledDB(creator=pymysql, host=self.conn_info.host, port=self.conn_info.port, user=self.conn_info.user,
                        ...
                        mincached=self.conn_info.min_size, maxcached=self.conn_info.max_size,
                        ping=1, maxconnections=self.conn_info.max_size, maxshared=self.conn_info.min_size,
                        maxusage=self.conn_info.min_size, charset="utf8mb4", autocommit=True)
        return pool
