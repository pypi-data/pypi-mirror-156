...
TRANSCRIPTION_ENDPOINT = "https://api.houndify.com/v1/transcription"
VERSION = "2.0.9"
...
class TextHoundClient(_BaseHoundClient):
    ...
    def __init__(self, clientID, clientKey, userID, requestInfo = dict(), endpoint = TEXT_ENDPOINT,
      ...
      _BaseHoundClient.__init__(self, clientID, clientKey, userID, endpoint, proxyHost, proxyPort, proxyHeaders, timeout, saveQuery)
...
class HoundListener(object):
    ...
    def onPartialTranscriptTokens(self, transcript):
      """
      onPartialTranscriptTokens is fired when the server has a partial transcript.
      """
      pass
          ...
          if "Format" in parsedMsg:
            if parsedMsg["Format"] == "SoundHoundVoiceSearchParialTranscript" or parsedMsg["Format"] == "HoundVoiceQueryPartialTranscript":
              try:
                ...
                pass
              try:
                listener.onPartialTranscriptTokens(parsedMsg["PartialTranscriptTokens"])
              except:
                pass
              try:
                listener.onFinalPartialTranscriptTokens(parsedMsg["FinalPartialTranscriptTokens"])
              except:
                pass
