...
class FirthLogisticRegression(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y, mask=None):
        ...
        self.coef_, self.loglik_, self.n_iter_ = _firth_newton_raphson(
            X, y, self.max_iter, self.max_stepsize, self.max_halfstep, self.tol, mask
        )

        if not mask:
            self.bse_ = _bse(X, self.coef_)
            self.pval = _lrt(self.loglik_, X, y)
            if self.fit_intercept:
...
def _lrt(full_loglik, X, y):
    mask = 1
    # X = np.delete(X, mask, 1)
    b, null_loglik, niter = _firth_newton_raphson(X, y, 25, 5, 1000, 0.0001, mask)
    lr_stat = 2 * (full_loglik - null_loglik)
    p_value = scipy.stats.chi2.sf(lr_stat, df=1)
    print(full_loglik, null_loglik, p_value, niter)
    print(b)
    return p_value
# 1-pchisq(2*(fit.full$loglik-fit.i$loglik),1)


if __name__ == '__main__':
    import scipy
    X = np.loadtxt("tests/sex2.csv", delimiter=",", skiprows=1)
    y = X[:, 0]
    X = X[:, 1:]
    # X = np.loadtxt("tests/diabetes.csv", delimiter=",", skiprows=1)
    # y = X[:, -1]
    # X = X[:, :-1]
    fl = FirthLogisticRegression()
    fl.fit(X, y)
    # print(fl.coef_.shape)
    # print(list(range(1, fl.coef_.shape[0]+1)))
    # print(fl.bse_)
    # print(_firth_newton_raphson(X, y, 250, 5, 1000, 0.0001, 1))
