...
def general_path_sorter(path_list, pattern):
    ...
    """
    if '?P<group1>' not in pattern:
        raise ValueError('The regex pattern must have a group called "group1".'
                         'E.g. image(?P<group1>[0-9]+) will match 123 from image123 in the filename.')
...
def save_images(images, image_format='jpg', names="", directory=None):
    ...
    for name, image in zip(names, images):
        imsave(os.path.join(directory, os.path.splitext(str(name))[0] + f'.{image_format}'), image,
               check_contrast=False)


def plot_cracks(image, cracks, linewidth=1, color='red', comparison=False, **kwargs):
    """
