...
from azureml.mlflow._common._authentication.azureml_token_authentication import AzureMLTokenAuthentication
from azureml.mlflow._common._authentication.databricks_cluster_authentication import _DatabricksClusterAuthentication
from azureml.mlflow._common._authentication.arcadia_authentication import _ArcadiaAuthentication
from azureml.mlflow._client.artifact.run_artifact_client import RunArtifactsClient
from azureml.mlflow._client.artifact.local_artifact_client import LocalArtifactClient
from azure.identity import DefaultAzureCredential, ChainedTokenCredential, DeviceCodeCredential, \
...
class ServiceContext(object):
    ...
    def __init__(
            self, host_url, subscription_id, workspace_name, resource_group_name, auth, location=None, cloud=None):
...
def get_service_context_from_tracking_url_default_auth(parsed_url):
    ...
    auth = ChainedTokenCredential(
        *default_auth.credentials,
        ...
        InteractiveBrowserCredential(authority=cloud._get_authority()),
        DeviceCodeCredential()
    )
    service_context = ServiceContext(
        subscription_id=subscription_id, resource_group_name=resource_group_name,
        workspace_name=workspace_name, auth=auth, host_url="https://" + parsed_url.netloc, cloud=cloud
    )
...
def get_service_context_from_tracking_url_mlflow_env_vars(parsed_url):
    ...
    service_context = ServiceContext(
        subscription_id=subscription_id, resource_group_name=resource_group_name,
        workspace_name=workspace_name, auth=auth, host_url=host_url, cloud=cloud
    )
    # TODO Create host url from tracking URL select scheme based on dev mode or no
...
def get_artifact_repository_client(artifact_uri):
    ...
    if _EXP_NAME in parsed_artifacts_path and _RUN_ID in parsed_artifacts_path:
        ...
        artifacts_client = RunArtifactsClient(get_v2_service_context(service_context), experiment_name, run_id, path)
    else:
        origin = parsed_artifacts_path[_ORIGIN]
        container = parsed_artifacts_path[_CONTAINER]
        path = parsed_artifacts_path[_ARTIFACT_PATH]
        artifacts_client = LocalArtifactClient(get_v2_service_context(service_context), origin, container, path)
...
def get_v2_service_context(service_context):
    return ServiceContext(
        ...
        cloud=_get_cloud_or_default(service_context.get_auth()._cloud_type.name)
    )
