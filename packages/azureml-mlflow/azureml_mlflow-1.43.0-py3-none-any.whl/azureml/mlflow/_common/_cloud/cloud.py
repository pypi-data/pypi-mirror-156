...
PUBLIC_CLOUD_METADATA_URL = "https://management.azure.com/metadata/endpoints?api-version=2019-05-01"
DEFAULT_TIMEOUT = 30
...
HARD_CODED_CLOUD_DICT = {
    AZURE_PUBLIC_CLOUD.name: AZURE_PUBLIC_CLOUD,
    AZURE_CHINA_CLOUD.name: AZURE_CHINA_CLOUD,
    AZURE_US_GOV_CLOUD.name: AZURE_US_GOV_CLOUD,
    AZURE_GERMAN_CLOUD.name: AZURE_GERMAN_CLOUD
}

KNOWN_CLOUDS = HARD_CODED_CLOUD_DICT
...
def _convert_arm_to_cli(arm_cloud_metadata_dict):
    ...
    for cloud in arm_cloud_metadata_dict:
        ...
        for cloud in cli_cloud_metadata_dict.keys():
            logger.debug('Active directory endpoint loaded from {0} metadata is {1}'.format(cloud, cli_cloud_metadata_dict[cloud].endpoints.active_directory))
            cli_cloud_metadata_dict[cloud].endpoints.active_directory = cli_cloud_metadata_dict[cloud].endpoints.active_directory.rstrip('/')
            logger.debug('Active directory endpoint for cloud {0} set to {1}'.format(cloud, cli_cloud_metadata_dict[cloud].endpoints.active_directory))
...
def _get_suffix_fallback_value(cloud_name):
    def _get_cloud_suffix_fallback_value(suffix_name):
        suffix_mapper = {c.name: c.suffixes.__dict__.get(suffix_name, None) for c in KNOWN_CLOUDS}
...
def _get_endpoint_fallback_value(cloud_name):
    def _get_cloud_endpoint_fallback_value(endpoint_name):
        endpoint_mapper = {v.name: v.endpoints.__dict__.get(endpoint_name, None) for c, v in HARD_CODED_CLOUD_DICT.items()}
...
def _get_cloud_or_default(cloud_name=None):
    if not cloud_name:
        ...
        if cloud_name:
            ...
            logger.info("Cloud name was not provided and environment variable {} is not set. Falling back to {}"
                        .format(AZUREML_CLOUD_ENV_NAME, cloud_name))

    metadata_url = os.getenv(ARM_CLOUD_METADATA_URL, PUBLIC_CLOUD_METADATA_URL)

    if cloud_name not in KNOWN_CLOUDS.keys():
        clouds = _get_clouds_by_metadata_url(metadata_url, DEFAULT_TIMEOUT)
        for cloud in clouds:
            KNOWN_CLOUDS.update({cloud.name: cloud})

    if cloud_name in KNOWN_CLOUDS.keys():
        return KNOWN_CLOUDS[cloud_name]

    raise Exception("{} cloud metadata could not be fetched or is not available at {}".format(cloud_name, metadata_url))
