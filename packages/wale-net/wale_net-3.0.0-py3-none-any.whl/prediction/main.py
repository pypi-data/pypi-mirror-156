...
from prediction.utils.dataset import get_scenario_list
    ...
    def __init__(self, scenario, mpl_backend="Agg", online_args=None, verbose=True):
        ...
        if self.pred_common_args["use_cuda"]:
            try:
                ...
                print("Warning: Failed using CUDA Device. Switching to CPU.")

                saved_model_dict = torch.load(
                    self.model_path, map_location=torch.device("cpu")
                )
                self.pred_common_args["use_cuda"] = False
        else:
            saved_model_dict = torch.load(
                self.model_path, map_location=torch.device("cpu")
            )
...
if __name__ == "__main__":
    ...
    log_name = os.path.join(
        "logs", now.strftime("%Y%m%d%H%M%S") + scenario_list[0] + ".log"
    )
