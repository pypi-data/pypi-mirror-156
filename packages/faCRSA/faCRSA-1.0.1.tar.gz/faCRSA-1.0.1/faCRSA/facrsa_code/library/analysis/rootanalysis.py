    ...
    def __init__(self, img_list, conf, file_array, tid):
        ...
        self.df = pd.DataFrame(
            columns=["Image_Name", "Total_Root_Length(cm)", "Total_Root_Projected_Area(cm2)",
                     ...
                     'Total_Root_Volume(cm3)', "Main_Root_Length(cm)", "Main_Root_Projected_Area(cm2)",
                     "Main_Root_Surface_Area(cm2)", "Main_Root_Volume(cm3)",
                     'Convex_Hull_Area(cm2)', 'Max_Root_Depth(cm)', "Total_Pixels_Number(L)",
                     "Total_Pixels_Number(A)", "Main_Pixels_Number(L)", "Main_Pixels_Number(A)"])
    ...
    def img_analysis_b_m(self, img):
        ...
        try:
            ...
            data = {
                ...
                "Main_Root_Length(cm)": round(m_length, 3),
                "Main_Root_Projected_Area(cm2)": round(m_area, 3),
                'Main_Root_Surface_Area(cm2)': round(m_surface_area, 3),
                'Main_Root_Volume(cm3)': round(m_volume, 3),
                'Convex_Hull_Area(cm2)': round(convex_area, 3),
                'Max_Root_Depth(cm)': round(max_root_depth, 3),
                "Total_Pixels_Number(L)": all_pnum_l,
                "Total_Pixels_Number(A)": all_pnum_a,
                "Main_Pixels_Number(L)": m_pnum_l,
                "Main_Pixels_Number(A)": m_pnum_a,
            }
