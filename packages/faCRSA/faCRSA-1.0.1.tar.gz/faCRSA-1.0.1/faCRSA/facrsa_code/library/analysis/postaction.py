...
class postAction():
    ...
    def move_file(self):
        ...
        for file in filelist:
            shutil.move(self.conf["out_path"] + file, self.conf["out_path"] + self.conf["uid"] + "_" + self.conf["tid"] + "/" + file)
    ...
    def uplaod_file(self):
        local_file_path = self.conf["out_path"] + self.conf["uid"] + "_" + self.conf["tid"] + ".zip"
        Key = self.conf[8] + "_" + self.conf[9] + ".zip"
        try:
            upload(local_file_path, Key)
        except BaseException:
            try:
                upload(local_file_path, Key)
            except BaseException:
    ...
    def set_status(self):
        with sqliteUtil() as um:
            sql = 'update task set status=' + str(
                self.status) + ' where tid =' + self.tid + ' and user ="' + self.user + '"'
            um.update(sql)


def upload(local_file_path, Key):
    pass
