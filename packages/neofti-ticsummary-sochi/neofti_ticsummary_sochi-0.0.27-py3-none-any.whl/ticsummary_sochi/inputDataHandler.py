...
def getSumDataByLoadMultiData(dataList): #(dataB1.matrix,dataB1.timeslice,dataB2.matrix,dataB2.timeslice,dataB1,dataB2)
    sumDataB1 = np.zeros(shape=(np.size(dataList[0][0],0),np.size(dataList[0][0],1)))
    sumDataB2 = np.zeros(shape=(np.size(dataList[0][2],0),np.size(dataList[0][2],1)))
    for i in range(len(dataList)):
        sumDataB1 += dataList[i][0]
        sumDataB2 += dataList[i][2]
    return (sumDataB1,dataList[0][1],sumDataB2,dataList[0][3],dataList[0].dateTime + "-" + dataList[-1].dateTime)
