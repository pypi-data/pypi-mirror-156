...
from ticsummary_sochi.ui import  viewGraphicsDialog,profileBeamDock,settingsColorBarDialog,galoBeam, uic as uicFile
#Third party module
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def setDataGalo(self,scRU,scLU,scRB,scLB,scCT,x,calibrationGalo = None):
        self.dGaloLine.setData(scRU, scLU, scRB, scLB, scCT,x)
        sumScRU = np.sum(np.sum(scRU))
        sumScLU = np.sum(np.sum(scLU))
        sumScRB = np.sum(np.sum(scRB))
        sumScLB = np.sum(np.sum(scLB))
        sumScCT = np.sum(np.sum(scCT))

        self.dGaloCounter.setData(sumScRU, sumScLU, sumScRB, sumScLB, sumScCT)
        if calibrationGalo:
            self.dGaloMonitoring.setData(time.time(), calibrationGalo.getCalibratedValueByGaloValue(sumScLU, sumScRU, sumScLB, sumScRB))
        else:
            self.dGaloMonitoring.setData(time.time(), (sumScRU+sumScLU+sumScRB+sumScLB)/4)  
