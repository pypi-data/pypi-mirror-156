...
#Project module
from ticsummary_sochi import handlerData
from ticsummary_sochi.ui import settingsColorBarDialog
...
class ProfileBeamDock(Dock):
    def __init__(self,keyEventHandler=None, *args, **kwargs):
        ...
        self.__initGraph__()
    def setData(self,data,scaleX,scaleY):
        self.scaleX = scaleX
        self.scaleY = scaleY
        self.imageData = data
        ...
        tr.translate(0, -np.size(projectionX)/2)
        self.image.setTransform(tr)
        self.bar._update_items()
        self.setOrientation("horizontal")

        irregularityData,irregularitY = handlerData.getIrregularityByProfile(projectionX,scaleY)
        self.histogramIrregularityMapItem.setOpts(y=irregularitY,width = irregularityData,height=1)
        strTickList = list()
        for i in range(np.size(irregularitY)):
            strTickList.append(str(irregularitY[i]))
            #strTickList[i] = "{0}-{1}".format(str(irregularitY[i]),str(irregularityData[i]))
        #self.irregularityMapHistPlot.getAxis("left").setTicks(strTickList)
        self.irregularityMapHistPlot.getAxis("left").setTicks([[(y, "{0:.0f}mm-{1:.0f}%".format(y,data)) for y,data in zip(irregularitY,irregularityData) ]])
        
        self.__setValueCrossHairProfile__(0,0,0)

        self.bar.setMaxMinImage(np.min(data),np.max(data))
    def setEnabled(self,enabled:bool):
        self.layoutPlot.setEnabled(enabled)
    def updateStyle(self):
        Dock.updateStyle(self)
        self.setOrientation("horizontal")
    def keyPressEvent(self,event):
        if self.keyEventHandler != None:
    ...
    class __ColorBarItemWithDoubleClick__(pg.ColorBarItem):
        ...
        def mouseDoubleClickEvent(self, e):
            self.settingsDialog = SettingsColorBarDialog(self.parent,*self.levels(),self.cmap.name,self.autoscale,self.__setNewparameter__)
    ...
    def __initGraph__(self):
        ...
        self.layoutPlot.addItem(self.labelDescription, row=0,col=1)
        ...
        self.irregularityMapHistPlot:pg.PlotItem = self.layoutPlot.addPlot(row=2,col=2,rowspan=1,colspan=1)
        self.irregularityMapHistPlot.setFixedWidth(150)
        self.irregularityMapHistPlot.hideAxis("right")
        self.histogramIrregularityMapItem = pg.BarGraphItem(x0 = 0, y = (), height = 0.6, width=(), brush ='g')
        self.irregularityMapHistPlot.addItem(self.histogramIrregularityMapItem)
        #self.irregularityMapHistPlot.showAxis('right')
        ...
        self.projectionMapHistPlot.setLabel('bottom',"count",units="qty",**labelAxisStyle)
        #self.intensityMapLinePlot.setTitle("Intensity", **self.titlePlotStyle)
        self.intensityMapLinePlot.setLabel('left',"count",units="qty",**labelAxisStyle)
        self.intensityMapLinePlot.setLabel('bottom',"time",units="sec",**labelAxisStyle)

        self.linePlotIntensityMcpitem:pg.PlotDataItem = self.intensityMapLinePlot.plot(pen=(0,120,120),name="Intensity")
        self.histPlotProjectionMcpItem:pg.PlotDataItem = self.projectionMapHistPlot.plot(pen=(0,120,120),name='Projection')
    ...
    def __mouseMoved__(self,evt):
        ...
        if self.colorMapPlot.sceneBoundingRect().contains(pos):
            ...
            if indexX >= 0 and indexX < np.size(self.imageData,0):
                if indexY >= 0 and indexY < np.size(self.imageData,1):
                    self.__setValueCrossHairProfile__(indexX * self.scaleX,indexY,self.imageData[indexX][indexY])
