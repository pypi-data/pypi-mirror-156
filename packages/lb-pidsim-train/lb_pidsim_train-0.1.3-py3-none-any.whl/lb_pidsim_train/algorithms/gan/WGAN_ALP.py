...
class WGAN_ALP (GAN):   # TODO add class description
  ...
  def compile ( self , 
    ...
    ## data-type control
    try:
      v_adv_dir_updt = int (v_adv_dir_updt)
    except:
      raise TypeError ("The number of virtual adversarial direction updates should be an integer.")

    try:
      adv_lp_penalty = float (adv_lp_penalty)
    except:
      raise TypeError ("The adversarial Lipschitz penalty should be a float.")
