...
def copy_admin_sshkeys(**kw):
    ...
    ceph_obj = Metadata().__dict__.get("ceph")
    ips = ceph_obj.ceph_node.get_ip_address()
    ...
    host_list = ceph_obj.ceph_node.get_host_name()
    for i in range(len(host_list)):
        env_config_copy["hosts"] = ceph_obj.ceph_node.get_ip_address_by_node_name(
            host_list[i]
        )
        logger.info("Updating fqdnhostnames to shortnames")
        fabfile.run_command(f"hostname {host_list[i]}", env_config_copy)
