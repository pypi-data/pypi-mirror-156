...
from ..wolf_array import WolfArray,WolfArrayMNAP,header_wolf
...
class prev_simulation():
    '''#> Simulation "ancien WOLF"
    ## @date 2016-12-21-09.35
    ...
    def __init__(self,fn) -> None:
        ...
        self.mymnap= WolfArrayMNAP(fn)

        self.nb_infil_blocs = 0   ##< Nombre de blocs infiltrants
        self.which_infil=0         ##< type d'infiltration
        
        # self.cont_sauv:Zones
        # self.filaire:Zones
        
    def read_param_mnap(self):
        '''#> Lecture des données multi-blocs via le fichier MNAP
        '''
        self.myblocfile.read_file()
        self.myparam.read_file()
    
    def get_header_MB(self):
        '''#> Renvoit un header avec les infos multi-blocs'''
        myheader = self.mymnap.get_header()
        for curblock in self.mymnap.myblocks.values():
            myheader.head_blocks[curblock.blockindex] = curblock.get_header()
        return  myheader
        
    def get_header(self):
        '''#> Renvoit un header de matrice "fine" non MB'''
        return self.mymnap.get_header()
    ...
    def read_fine_array(self,which=''):
        ...
        myarray.filename = self.filename+which
        myarray.read_data()

    def read_finenap(self):
        nbx=self.myparam.nxfin
        nby=self.myparam.nyfin        
        
        with open(self.filename +'.napbin', 'rb') as f:
            self.mynap = np.frombuffer(f.read(nbx*nby*2), dtype=np.int16).copy()
        
        self.mynap.reshape((nbx,nby), order='F')

class prev_parameters_blocks:
    ''' #> Paramètres de calcul propres aux blocs
    # ## @date 2016-12-02-09.51
    # ## @author Pierre Archambeau
    # '''
    ...
    def __init__(self,lines=None) -> None:
        ...
        self.vdebug:list #		    #< vecteur des 60 paramètres debug par bloc (stockage en real*8)
        
        if lines is list:
            ##reconstruction et limitation
            self.ntyprec            =int(lines[0])		#=0 si rec constante, 1 si rec linéaire
            self.ntyprecfront       =int(lines[0])      #=0 si rec cst, 1 si rec lin non limitée, 2 si rec lin limitée des bords frontière
            self.ntyprecbord        =int(lines[0])      #type de reconstruction des bords libres (0 = cst, 1 = lin limitée (non implémenté), 2 = lin non limitée)
            self.nvoislimit         =int(lines[0])	    #nbre de voisins pour la limitation
            self.nrechzh            =int(lines[0])		#=0 si limitation de h, 1 si limitation de h+z
            self.ntraitefront       =int(lines[0])      #type de traitement des frontières (1 = rien, 0 = moyenne et décentrement unique)
            self.ntypflux           =int(lines[0])	    #type de spliting :
            #paramètres de calcul
            self.nbvar              =int(lines[0])		#nbre d'inconnues du calcul
            self.nbequ              =int(lines[0])		#nbre d'equations à résoudre
            self.froudemax          =float(lines[0])	#froude max pour limitation des résultats
            self.nirv               =int(lines[0])		#inégale répartition de vitesse
            #options
            self.nconflit           =int(lines[0])	    #gestion des conflits (0), ou juste en centré (1) ou pas (2)
            self.ncouvdecouv        =int(lines[0])	    #gestion des éléments couvrants-découvrants ou non
            self.ntypetopo          =int(lines[0])	    #type d'agglomération de la topo (max, min ou moy)
            #options
            self.frotimpl           =float(lines[0])	#frottement : 0 = expl, 1 = impl simple,
                                                        #			 [-1,0[ = impl complet pondéré par ||
            self.ninfil             =int(lines[0])		#infiltration avec mod qté mvt (=2), classique (=1) ou non (=0)
            self.hfilref            =float(lines[0])	#hauteur de référence pour infiltration modifiée
            self.nincli_axes        =int(lines[0])      #type d'inclinaison d'axe : 0 si pas d'inclinaison
            self.iegal              =int(lines[0])		#=1 si égalisation d'altitude
            self.alti               =float(lines[0])	#altitude de surface libre à laquelle égaliser
            self.istat              =int(lines[0])		#arrêt stationnaire (=1 si critère sur v, =2 si sur h)
            self.epsstat            =float(lines[0])	#epsilon à vérifier pour l'arrêt stationnaire
            self.nvartop            =int(lines[0])		#topo variable ou non
    ...
    def read_file_debug(self):
        '''#> lecture du fichier de paramètres - valeurs de debug'''
        pass
...
class prev_boundary_condition():
	...
	## @author Pierre Archambeau
    '''
    def __init__(self,x:int,y:int,ntype:int,val:float) -> None:
        self.x=x		    # indice de colonne dans le plus petit maillage
        self.y=y		    # indice de ligne   dans le plus petit maillage
...
class prev_boundary_conditions():
    ...
    def __init__(self,parent:prev_simulation) -> None:
        self.parent = parent
        self.mybc = []
    ...
    def read_file(self,lines:list):
        ...
        for curline in lines:
            tmp=curline.split(',')
            self.mybc.append(prev_boundary_condition(int(tmp[0]),
                                                     int(tmp[1]),
                                                     int(tmp[2]),
                                                     float(tmp[3])))
...
class prev_parameters_simul:
    ...
    def read_file(self):
        ...
        with open(self.parent.filename+'.par') as f:
            lines=f.read().splitlines()

        #Lecture des PARAMETRES GLOBAUX
        #Durée de la simulation et résultats
        self.npas           = int(lines[0])		    #nbre de pas de simulation à réaliser
        ...
        self.clf.read_file(lines[decal:decal+self.impfgen])
        decal+=self.impfgen
        self.clfbx.read_file(lines[decal:decal+self.impfbxgen])
        decal+=self.impfbxgen
        self.clfby.read_file(lines[decal:decal+self.impfbygen])
        ...
        for nbblocs in range(self.nblocs):
            self.my_param_blocks[nbblocs].vdebug=[]
            for i in range(60):
                self.my_param_blocks[nbblocs].vdebug.append(float(lines[decal+i]))
            decal+=60
...
class prev_infiltration():
    ...
    def __init__(self,parent:prev_simulation) -> None:
        ...
        self.which_bloc=0  # #< bloc
        self.consistent = True # #< Consistence entre le fichier .fil et le fichier matriciel .inf
        self.consistent_error = 0 ##< erreur de consistance

        self.myarray:WolfArray      ##< matrice d'infiltration (en entiers)
        self.cont_infil:vector   ##< contours des zones
...
class bloc_file():
    '''    #> objet général permettant la lecture et la manipulation d'un fichier .BLOC'''

    def __init__(self,parent:prev_simulation) -> None:
        
        self.parent=parent
        self.filename = self.parent.filename + '.bloc'
        
        self.nb_blocs=0   #     #< nombre de blocs
        self.max_size_cont= 0#  #< taille maximale des contours
        self.interior=True #        #< gestion des contours intérieurs --> ne tient pas compte des allers-retours vers les contours internes lors des calculs d'intersection
        ...
        self.external_border=vector(is2D=True,name='external border')    ##< vecteur du contour externe

        self.my_blocs_array:WolfArray  ##< matrice MB en réels contenant le maillage des blocs
        self.my_array_gen:WolfArray    ##< matrice full en réels contenant le maillage du contour extérieur avec la taille de maille minimale

        self.my_blocs=[] #    #< vecteur des objets blocs
        pass
    ...
    def read_file(self):
        ...
        self.nb_blocs  = int(tmpline[0])
        self.max_size_cont = int(tmpline[1])

        # #lecture du nombre de points du contour extérieur
        nb= int(lines[1])

        # #si nb est négatif, il existe une troisième colonne qui dit si le segment est utile ou pas (maillage de zones intérieures)
        if nb<0:
            self.interior=True
        ...
        if(self.interior) :
            decal=2
            for i in range(decal,nb+decal+1):
        ...
        self.external_border.close_force()

        #lecture des données par bloc
        decal = nb+2
        for i in range(self.nb_blocs):
            curextent=bloc_extent(self,lines[decal:])
            self.my_blocs.append(curextent)
            decal+=curextent.nb+3

        #lecture des tailles de maille pour tous les blocs
        for i in range(self.nb_blocs):
            tmp=lines[decal].split(',')
            self.my_blocs[i].dx = float(tmp[0])
            self.my_blocs[i].dy = float(tmp[1])
        
class bloc_extent():
    
    def __init__(self,parent:bloc_file,lines=[]) -> None:

        self.parent=parent # #< objetparent

        self.xmin=0.               ##< position minimale selon X du contour une fois qu'il a été "accroché" sur le gri magnétique
        self.xmax=0.               ##< position maximale selon X du contour une fois qu'il a été "accroché" sur le gri magnétique
        self.ymin=0.               ##< position minimale selon Y du contour une fois qu'il a été "accroché" sur le gri magnétique
        self.ymax=0.               ##< position maximale selon Y du contour une fois qu'il a été "accroché" sur le gri magnétique
        self.dx=0.                 ##< taille de discrétisation selon X
        self.dy=0.                 ##< taille de discrétisation selon Y

        self.external_border:vector     #   #< contour externe du bloc (en coordonnées réelles, c-à-d non translatée vis-à-vis du bloc général)
        ...
        ##emprise du bloc en accord avec le grid magnétique
