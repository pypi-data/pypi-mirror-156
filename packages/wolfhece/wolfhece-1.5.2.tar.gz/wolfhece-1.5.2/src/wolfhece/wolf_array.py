...
class WolfArray(header_wolf):
    ...
    def __init__(self,fname = None,mold = None,masknull=True,crop=None,whichtype=WOLF_ARRAY_FULL_SINGLE):
        super().__init__()
    ...
    def get_header(self,abs=True) -> header_wolf():
        if abs:
            curhead:header_wolf()
            curhead = super(header_wolf,self)
    ...
    def get_ij_from_xy(self,x,y,z=0.,scale=1.,aswolf=False,abs=True):
        ...
        if isinstance(x, (list, tuple, np.ndarray)):
            ...
            i = int(locx / (self.dx*scale))
            j = int(locy / (self.dy*scale))
    ...
    def get_xy_from_ij(self,i,j,k=0,scale=1.,aswolf=False,abs=True):
        ...
        if isinstance(i, (list, tuple, np.ndarray)):        
            ...
            if abs:
                ...
                x= (i + .5) * (self.dx*scale) + self.origx
                y= (j + .5) * (self.dy*scale) + self.origy
            ...
            if self.nbdims==3:
                ...
                if abs:
                    ...
                    z= (k - .5) * (self.dz*scale) + self.origz
