...
def sort_grid_np(fn_in,fn_out,bounds,gridsize,chunksize=5000000):
...
def get_Orthos_Walonmap(xbounds,ybounds,fn,cat='IMAGERIE/ORTHO_2012_2013',size=3000):
    '''
    ...
    elif resx>.15 & resy <=.15:    
        nbx = math.ceil(resx/.15)
        liste=[]
        x1=xbounds[0]
        dx = dx/float(nbx)
        for i in range(nbx):
