...
for package in sorted([package.key for package in pkg_resources.working_set]):
    apps_module = f'{package}.apps'.replace('-', '_')
    try:
        module = importlib.import_module(apps_module)
    except ImportError:
        continue
    if hasattr(module, 'HuscyApp'):
        INSTALLED_APPS.append(f'{apps_module}.HuscyApp')
