...
class NavParser:
    ...
    def __init__(self, nmea: str, is_file=False):
        ...
        self._nmea_buffer_flags = {
            NMEASentectID.RMC: False,
            NMEASentectID.GGA: False,
            NMEASentectID.VTG: False
        }
        self._points = dict()
        if is_file:
            self.read_file(nmea)
    ...
    def process_NMEA_block(self, block):
        ...
        for line in lines:
            ...
            if sentenceID == NMEASentectID.GGA:
                GGA = data
            if sentenceID == NMEASentectID.RMC:
                RMC = data
            if sentenceID == NMEASentectID.VTG:
                VTG = data
