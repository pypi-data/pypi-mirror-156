...
class Mapper(object):
    ...
    def field(self, _field):
        if self.mod == 'all':
            datas = self.mapper.fields(str(_field)).all(isdict=False)
            return [i[0] for i in datas] if datas else datas
        elif self.mod == 'first':
            data = self.mapper.fields(str(_field)).first(isdict=False)
...
class Table(object):
    ...
    @classmethod
    def map(cls, **kwargs):
        return Mapper(SelectTable(cls._db(), cls._table()).where(**kwargs), mod='all')

    @classmethod
    def map_one(cls, **kwargs):
        return Mapper(SelectTable(cls._db(), cls._table()).where(**kwargs), mod='first')
