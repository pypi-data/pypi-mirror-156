...
def collapse_taxon_table(df):

    df_out = pd.DataFrame()
    taxa = df[['Phylum', 'Class', 'Order', 'Family', 'Genus', 'Species']].values.tolist()
    taxa.sort()
    samples = df.columns.tolist()[10:]
    seq_loc = df.columns.get_loc("seq")

    for taxon in taxa:
        df_sub = df.loc[(df['Phylum'] == taxon[0]) & (df['Class'] == taxon[1]) & (df['Order'] == taxon[2]) & (df['Family'] == taxon[3]) & (df['Genus'] == taxon[4]) & (df['Species'] == taxon[5])]
        df_new = df_sub[samples].agg(['sum'])
        df_new[['Phylum', 'Class', 'Order', 'Family', 'Genus', 'Species']] = taxon
