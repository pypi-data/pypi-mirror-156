        ...
        for k in results:
            ...
            for a in range(0, StopCount - 1):
                ...
                if res1=='GGA':
                    ...
                    print(res)
                    x=GPGSVrec(res)
                    print("Number of Messages1: "+x.nofmsg+" Message Number: "+x.msgno+" Satellites in View: "+x.sinV+" Satellite ID Ch1: "+x.idch1+" Elevation (degree) Ch1: "+x.evch1+" Azimuth (degree) ch1: "+x.azch1+" SNR ch1 (dBHz): "+x.snrch1+" Satellite ID Ch2: "+x.idch2+" Elevation (degree) Ch2: "+x.evch2+" Azimuth (degree) ch2: "+x.azch2+" SNR ch2 (dBHz): "+x.snrch2+" Satellite ID Ch3: "+x.idch3+" Elevation (degree) Ch3: "+x.evch3+" Azimuth (degree) ch3: "+x.azch3+" SNR ch3 (dBHz): "+x.snrch3+" Satellite ID Ch4: "+x.idch4+" Elevation (degree) Ch4: "+x.evch4+" Azimuth (degree) ch4: "+x.azch4+" SNR ch4 (dBHz): "+x.snrch4+"\n")
...
class imp:
    def docs():
        ...
        print("#$GPGGA,124632.122,5231.537,N,01320.203,E,1,12,1.0,0.0,M,0.0,M,,*6A \n #$GPGSA,A,3,01,02,03,04,05,06,07,08,09,10,11,12,1.0,1.0,1.0*30\n #$GPRMC,124633.122,A,5231.938,N,01324.385,E,2228.2,011.3,020622,000.0,W*48\n #$GPZDA,181813,14,10,2003,00,00*4F\n #$GPVTG,309.62,T, ,M,0.13,N,0.2,K,A*23\n #$GPGSV,2,1,07,07,79,048,42,02,51,062,43,26,36,256,42,27,27,138,42*71")
         ...
         print("""           
          GPGSVrec:
                ...
                Satellites in View\n"
                  ...
                  v.print=("Number of Messages1: "+x.nofmsg+" Message Number: "+x.msgno+" Satellites in View: "+x.sinV+
              " Satellite ID Ch1: "+x.idch1+" Elevation (degree) Ch1: "+x.evch1+" Azimuth (degree) ch1: "+x.azch1+" SNR ch1 (dBHz): "+x.snrch1+
              " Satellite ID Ch2: "+x.idch2+" Elevation (degree) Ch2: "+x.evch2+" Azimuth (degree) ch2: "+x.azch2+" SNR ch2 (dBHz): "+x.snrch2+
              " Satellite ID Ch3: "+x.idch3+" Elevation (degree) Ch3: "+x.evch3+" Azimuth (degree) ch3: "+x.azch3+" SNR ch3 (dBHz): "+x.snrch3+
              " Satellite ID Ch4: "+x.idch4+" Elevation (degree) Ch4: "+x.evch4+" Azimuth (degree) ch4: "+x.azch4+" SNR ch4 (dBHz): "+x.snrch4+"\n")
          ...
          GPGGArec:
                ...
                print("Global Positioning System Fixed Data\n")
                  ...
                  v.longd  
                      ...
                      v.view='Real Time Kinematic'
                elif d[6]=='5':
                      v.view='Float RTK'
                elif d[6]=='6':
                      v.view='Estimated'
                elif d[6]=='7':
                      v.view='Manual Input mode'
                elif d[6]=='8':
                      v.view='Simulation Mode'
                else:
                      v.view='NA'
                  v.nofsat   # No of sat
                  v.hdop=d[8]
                  ...
                  v.geoidunit=d[12]
                  v.timeS=d[13]
                  v.DGPSid=d[14].split('*')[0]
                  
                  v.print=("Fix taken at UTC: "+x.fix+" Latitude: "+x.lat+" Latitude Direction: "+x.latd+" Long: "+x.long+" Long Direction: "+x.longd+" Fix Quality: "+x.view+"No of Satellite: "+x.nofsat+" Horizontal dilution of position: "+x.hdop+" Altitude above mean sea level: "+x.alt+" Units of altitude: "+x.altunit+" Height of geoid (mean sea level) above WGS84: "+x.geoid+" units of geoid: "+x.geoidunit+" time in seconds since last DGPS update: "+x.timeS+" DGPS station ID number: "+x.DGPSid+"\n")
          ...
          GPZDArec:
                ...
                print("SiRF Timing Message\n")
                  ...
                  v.print=("UTC time: "+x.time+" Day: "+x.d+" Month: "+x.m+" Year: "+x.y+" Local zone hour: "+x.lzh+" Local zone minutes: "+x.lzm+"\n"
          ...
          GPGSArec:
                ...
                if d[1]=='A':
                      v.mode='Automatic, 3D/2D'
                elif d[1]=='M':
                      v.mode='Manual, forced to operate in 2D or 3D'
                else:
                      v.mode='Mode Not Identified'
                if d[2]=='1':
                      v.view='Fix not available'
                elif d[2]=='2':
                      v.view='2D'
                elif d[2]=='3':
                      v.view='3D'
                      ...
                      v.view='NA'
                  v.svid=d[3:15] # Space Vehicle SV ID
                  v.pdop=d[15]
                  v.hdop=d[16]
                  v.vdop=d[17].split('*')[0]
                  
                  v.print=("Mode: "+x.mode+" View: "+x.view+" IDs of SVs used in position fix: "+str(x.svid)+" PDOP (dilution of precision): "+x.pdop+" Horizontal dilution of precision(HDOP): "+x.hdop+" Vertical dilution of precision (VDOP): "+x.vdop+"\n")
          ...
          GPRMCrec:
                print("Recommended minimum specific GPS/Transit data\n")
                  v.res=res
                s=res.split(',')
                  v.time=s[1][0:2]+':'+s[1][2:4]+':'+s[1][4:]+' UTC'
                if s[2]=='A':
                      v.validity="Valid"
                elif s[2]=='V':
                      v.validity="InValid"
                  v.lat=s[3]#current Latitude
                  v.latd=s[4]
                  v.lon=s[5]
                  v.lond=s[6]
                  v.speed=s[7]
                  v.tcourse=s[8]
                  v.date=s[9][0:2]+'/'+s[9][2:4]+'/'+s[9][4:6]
                  v.magVar=s[10]
                  v.magVarDir=s[11].split('*')[0]
                  
                  v.print=("Fix taken at: "+x.time+" Validity: "+x.validity+" Latitude: "+x.lat+" Latitude Direction "+x.latd+" Longitude: "+x.lon+" Longitude Direction: "+x.lond+" Speed in knots: "+x.speed+" Track angle in degrees True: "+x.tcourse+" Date: "+x.date+" Magnetic Variation: "+x.magVar+" Magnetic Variation Direction: "+x.magVarDir+"\n") 
          ...
          GPVTGrec:
                ...
                s=res.split(',')
                  ...
                  v.print=("True track made good: "+x.ttm+" Magnetic track made good: "+x.mtm+" Ground speed, knots: "+x.gspeedn+" Ground speed, Kilometers per hour: "+x.gspeedk+"\n")
