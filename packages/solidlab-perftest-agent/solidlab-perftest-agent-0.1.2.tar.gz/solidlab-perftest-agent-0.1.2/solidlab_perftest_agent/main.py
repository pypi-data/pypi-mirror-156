...
from solidlab_perftest_common.util import call_external_command_async

from solidlab_perftest_agent.api_calls import Api
...
class CommandExecutor:
    ...
    def __init__(self, command: AgentCommand, config: Config, api: Api):
        ...
        self.env = {
            "PERFTEST_AGENT_API_ENDPOINT": config.api_endpoint,
            "PERFTEST_AGENT_MACHINE_ID": config.machine_id,
            "PERFTEST_AGENT_TEST_ENV_ID": config.test_env_id,
            "PERFTEST_AGENT_AUTH_TOKEN": config.auth_token,
        }
        if self.config.css_config_file:
            self.env["PERFTEST_AGENT_CSS_CONFIG_FILE"] = self.config.css_config_file
        if self.config.css_host:
            self.env["PERFTEST_AGENT_CSS_HOST"] = self.config.css_host
    ...
    def get_status(self, status: AgentCommandResultStatus) -> AgentCommandResult:
        return AgentCommandResult(
            ...
            error_msg=self.err_s,
            trace=None,
            debug_msg=self.debug_s,
            stdout=self.stdout_s,
            stderr=self.stderr_s,
        )
    ...
    async def execute(self) -> bool:
        ...
        self.called = True
        try:
            if self.command.type == AgentCommandType.SET_CSS_CONFIG:
                assert isinstance(self.command.data, str)
                async with aiofiles.open(self.config.css_config_file, mode="w") as f:
                    await f.write(self.command.data)

                await self.api.report_command_status(
                    self.command, AgentCommandResultStatus.SUCCESS
                )
                return True
            ...
            await self.api.report_command_result(
                self.command, self.get_status(AgentCommandResultStatus.FAILURE)
            )
            return False
            ...
            for command in commands:
                ...
                try:
                    command_success = await asyncio.wait_for(
                        command_executor.execute(), timeout=command.timeout_s
                    )
                    ...
                    logging.exception(f"Failed to execute command: command timed out")
                    await api.report_command_result(
                        command,
                        command_executor.get_status(AgentCommandResultStatus.TIMEOUT),
                    )
...
def main() -> int:
    ...
    if len(sys.argv) == 8:
        ...
        css_config_file = sys.argv[5].strip() if sys.argv[5].strip() else None
        css_host = sys.argv[6].strip() if sys.argv[6].strip() else None
        css_port = (
            int(sys.argv[7].strip())
    ...
    if show_help:
        print(
            f"""Usage: {sys.argv[0]} <api_endpoint> <machine_id> <test_env_id> <auth_token> <css_config_file> <css_host> <css_port>
