...
{
    pop();
    if(p=='A')for(j=strlen(prod[0])-1;j>=3;j--)push(prod[0][j]);
    else for(j=strlen(prod[1])-1;j>=3;j--)push(prod[1][j]);
}
void main()
{
    char c; int i;
    ...
    {
        printf("\n");
        for(l=top;l>=0;l--)printf("%c",stack[l]);
        printf("\t");
        for(l=i;l<strlen(input);l++)printf("%c",input[l]);
        printf("\t");
        if(stack[top]=='A'){
            printf("A->aBa");
            stackpush('A');
