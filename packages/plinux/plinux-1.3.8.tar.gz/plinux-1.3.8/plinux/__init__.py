...
class Plinux:
    ...
    def run_cmd(self, cmd: str, sudo: bool = False, timeout: int = 30) -> ResponseParser:
        ...
        try:
            ...
            self.logger.info(f'[{self.host}] <- {exited}\n{out}')
    ...
    def stat_file(self, path: str, sudo: bool = False) -> dict:
        ...
        :param sudo: 664 or -rw-rw-r--
        :return:
        """

        params = ('access_rights:%a|access_rights_human:%A|group:%g|group_human:%G|user:%u|user_human:%U|file_type:%F|'
                  'inode:%i|size:%s')
