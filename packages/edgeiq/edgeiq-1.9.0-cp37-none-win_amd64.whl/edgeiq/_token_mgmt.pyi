from _typeshed import Incomplete
from edgeiq._load_cfg import JsonFile as JsonFile

LOGGER: Incomplete
CREDENTIALS_DIR: Incomplete
CREDENTIALS_PATH: Incomplete
DEVICE_CONFIG_PATH: Incomplete
DEV_HOST_CONFIG_PATH: Incomplete
SYSTEM_ID_OPTIONS: Incomplete
DAYS_TO_SECONDS: int

class ErrorCode:
    def __init__(self) -> None: ...
    @property
    def list(self): ...
    def log(self, code) -> None: ...

ERROR_CODE: Incomplete

def reset_errors() -> None: ...
def log_error(error_code) -> None: ...

ERROR_NO_CREDENTIALS_FILE: int
ERROR_DEVICE_SYSTEM_ID: int
ERROR_DEVICE_ID: int
ERROR_NO_DEVICE_TOKENS: int
ERROR_DEVICE_TOKEN_REFRESH_FAILURE_EXCEPTION: int
ERROR_DEVICE_TOKEN_REFRESH_FAILURE_STATUS: int
ERROR_DEVICE_TOKEN_REFRESH_FAILURE_FORMAT: int
ERROR_NO_DEV_HOST_TOKENS: int
ERROR_DEV_HOST_SYSTEM_ID: int
ERROR_DEV_HOST_TOKEN_REFRESH_FAILURE_EXCEPTION: int
ERROR_DEV_HOST_TOKEN_REFRESH_FAILURE_FORMAT: int
ERROR_NO_INTERNET_CONNECTION: int
ERROR_TOKEN_HEADERS_FAILURE: int
ERROR_NO_PUBLIC_KEY: int
ERROR_TOKEN_SIGNATURE_FAILED: int
ERROR_TOKEN_EXPIRED: int
ERROR_MALFORMED_TOKEN: int
ERROR_NO_ID_TOKEN_OR_KEYS: int
ERROR_UNHANDLED: int
ERROR_DEV_HOST_ID: int
USER_POOL_CONFIG: Incomplete
REGION: str
REFRESH_DEVICE_ENDPOINT: Incomplete

class CredentialsFile(JsonFile):
    def __init__(self) -> None: ...
    @property
    def device_id(self): ...
    @property
    def device_refresh_token(self): ...
    @property
    def device_id_token(self): ...
    def get_dev_host_refresh_token(self, user_pool_client_id): ...
    def get_dev_host_id_token(self, user_pool_client_id): ...
    def update(self, data_to_update): ...

def validate_system_id(system_id): ...

class DeviceConfigFile(JsonFile):
    def __init__(self) -> None: ...
    @property
    def system_id(self): ...

class DevHostConfigFile(JsonFile):
    def __init__(self) -> None: ...
    @property
    def system_id(self): ...

class Token:
    token: Incomplete
    def __init__(self, token, keys) -> None: ...
    def decode(self): ...
    def validate_expiry(self): ...
    def get_audience(self, is_id_token: bool = ...): ...
    @property
    def claims(self): ...

class TokenGroup:
    def __init__(self, config, system_id, refresh_token, undecoded_id_token) -> None: ...
    def exists(self): ...
    def decode(self): ...
    def validate(self): ...
    def get_username(self): ...

class DeviceTokens(TokenGroup):
    mode: str
    def __init__(self, token_file, system_id, device_id) -> None: ...
    def decode(self): ...
    def refresh(self): ...

class DevelopmentHostTokens(TokenGroup):
    def __init__(self, token_file, system_id) -> None: ...
    def refresh(self): ...

def validate_device_tokens(token_file): ...
def validate_development_host_tokens(token_file): ...
def validate_credentials() -> None: ...
