...
async def attach_metadata(device):
    ...
    try:
        if hasattr(device, '_hw_version_major_property') and hasattr(device, '_hw_version_minor_property') and hasattr(device, '_hw_version_variant_property'):
            ...
            if version[0] == 3:
                device._product = 'ODrive v{}.{}-{}V'.format(*version)
                device._model = 'ODrive'
            elif version[0] == 4:
                device._product = 'ODrive Pro v{}.{}-{}V'.format(*version)
                device._model = 'ODrive Pro'
            elif version[0] == 5:
                if version[1] == 0:
                    device._product = 'ODrive S1 X1'
                elif version[1] == 1:
                    device._product = 'ODrive S1 X3'
                else:
                    device._product = 'Unknown ODrive S1'
                device._model = 'ODrive S1'
            else:
                device._product = 'Unknown ODrive'
                device._model = 'ODrive'
            ...
            try:
                metadata = _db.get_product(device._product)
...
def format_errors(odrv, clear=False):
    ...
    if hasattr(odrv, 'issues') and hasattr(odrv.issues, 'length') and hasattr(odrv.issues, 'get'):
        if odrv.issues.length == 0:
            ...
            lines.append("internal issues: " + RichText(str(odrv.issues.length), *STYLE_BAD))
...
def start_liveplotter(get_var_callback: Callable, legend: List[str] = None):
    ...
    def plot_data():
        ...
        while not cancellation_token.is_set():
            plt.clf()
            plt.plot(vals)
            plt.legend(legend_vals, loc=('lower left' if legend else 'best'))
            fig.canvas.draw()
