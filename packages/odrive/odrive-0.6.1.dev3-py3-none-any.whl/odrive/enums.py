...
class ProcedureResult(enum.Enum):
    SUCCESS                                  = 0
    BUSY                                     = 1
    CANCELLED                                = 2
    DISARMED                                 = 3
...
class SpiEncoderMode(enum.Enum):
    DISABLED                                 = 0
    RLS                                      = 1
    AMS                                      = 2
    CUI                                      = 3
...
def _extend_enum(t, new_vals):
    ...
    if isinstance(t, enum.Enum):
        return enum.Enum(t.__name__, all)
    else:
...
# Removed in 0.4.12 => 0.5.1
ControlMode = _extend_enum(ControlMode, {
...
def _load_legacy_names():
    ...
    for var in legacy_enum_names:
        ...
        for item in t:
            setattr(module, ('ODRIVE_ERROR' if var == 'LegacyODriveError' else to_macro_case(var)) + '_' + item.name, item.value)
