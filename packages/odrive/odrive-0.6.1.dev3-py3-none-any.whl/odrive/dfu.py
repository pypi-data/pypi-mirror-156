...
class ODriveInDfuMode(DfuDevice):
    ...
    def init(self, logger, ask):
        ...
        if is_odrive3:
            ...
            if ask:
                guess = 'ODrive Pro v4.4-58V' if 'OTP Memory' in self.memories else 'ODrive S1 X3'
                if not odrive.utils.yes_no_prompt('Hardware version detection not yet supported for this ODrive. Is this an {}?'.format(guess), False):
