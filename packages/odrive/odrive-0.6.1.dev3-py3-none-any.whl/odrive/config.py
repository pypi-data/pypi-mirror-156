...
from odrive.enums import EncoderId, AxisState, ComponentStatus, ProcedureResult, ODriveError
...
class DeviceRef(EntityRef):
    ...
    def get_children(self):
        product_info = _db.get_product(self.config.product)
...
class EncoderConfig(ConfigType):
    ...
    def parse_addr(self, unit=''): return int(self.addr or '0x54', 0) # default for AMT21: 0x54
    def protocol(self):
        if self.db_ref == "":
...
class MotorConfig(ConfigType):
    ...
    @staticmethod
    def from_db(db_ref: str, use_thermistor: bool = None):
        return MotorConfig.from_json({'db_ref': db_ref, 'use_thermistor': use_thermistor})
...
class AxisConfig(ConfigType):
    ...
    can_protocol: str = ""
    can_node_id: str = ""
    can_node_id_extended: bool = False
...
class DeviceConfig(ConfigType):
    ...
    def __init__(self, product: str, **kwargs):
        product_info = _db.get_product(product) # verify that this is a known product
        ConfigType.__init__(self, product=product, **kwargs)
...
#        self._rs485_connections.append([enc_ref, rs485_intf_ref])
    ...
    def get_status(self, odrives):
        ...
        output_configs = [{'config': {
                **{f'inverter{i}': {}
                for i in range(len(_db.get_product(d.product)['inverters']))},
                **{f'brake_resistor{i}': {}
                for i in range(len(_db.get_product(d.product)['aux_inverters']))}
            },
            **{
                f'axis{i}': {'controller': {'config': {}}, 'config': {'motor': {}, 'can': {}}}
                for i in range(len(_db.get_product(d.product)['inverters']))
            }}
        ...
        for dev_ref in ref.devices():
            ...
            if dev_config.serial_number is None:
                ...
                product = dev._product
                if dev_config.product != product:
                    issues.append(dev_ref, 'Expected {} but found {}.'.format(dev_config.product, product))
                else:
        ...
        for net in self.get_nets(NetType.RS485):
            ...
            for enc_ref in enc_refs:
                ...
                if encoder_config.protocol() != 'amt21':
                    issues.append(enc_ref, f"RS485 protocol {encoder_config.protocol()} not supported.")
                ...
                if f'amt21_encoder_group{rs485_intf_ref.key}' in odrv_output_config:
                    amt21_encoder_group_config = odrv_output_config[f'amt21_encoder_group{rs485_intf_ref.key}']
                else:
                    amt21_encoder_group_config = {'config': {'enable': True, 'rs485': rs485_intf_ref.key}}
                    odrv_output_config[f'amt21_encoder_group{rs485_intf_ref.key}'] = amt21_encoder_group_config

                if 'addr0' in amt21_encoder_group_config['config']:
                    issues.append(rs485_intf_ref.parent, rs485_intf_ref.parent.num, f"Current firmware does not support more than AMT21 encoder.")
                    continue # ignore encoder

                amt21_encoder_group_config['config']['addr0'] = encoder_config.parse_addr() # use default AMT21 address if unspecified
                encoder_ids[rs485_intf_ref.parent.num][enc_ref] = EncoderId.AMT21_ENCODER0
        ...
        for net in self.get_nets(NetType.THREE_PHASE):
            ...
            inv_data = _db.get_product(self.devices[inv_ref.parent.key].product)['inverters'][inv_ref.key]
            ...
            axis_output_config['config']['motor']['phase_resistance_inductance_valid'] = True

            calibrated = (motor_config.phase_resistance != "") and (motor_config.phase_inductance != "")
            if not odrv_list[inv_ref.parent.key] is None:
                calib = functools.partial(_run_motor_calibration, getattr(odrv_list[inv_ref.parent.key], 'axis{}'.format(inv_ref.key)), motor_config)
            else:
                calib = None
            axis_calib[motor_ref.parent.key].append(CalibrationTask(
                "Motor Calibration",
                calib,
                CalibrationStatus.OK if calibrated else CalibrationStatus.RECOMMENDED
                ))
        ...
        for net in self.get_nets(NetType.THERMISTOR):
            ...
            motor_data = _db.get_motor(motor_config['type'])
            ...
            odrv_data = _db.get_product(odrv_config.product)
            ...
            axis_output_config['motor_thermistor'] = {
                'config': {
        ...
        for dev_num, dev_config in enumerate(self.devices):
            for inv_num in range(len(_db.get_product(dev_config.product)['inverters'])):
        ...
        for dev_num, dev_config in enumerate(self.devices):
            ...
            for inv_num in range(len(_db.get_product(dev_config.product)['aux_inverters'])):
                output_config['config'][f'brake_resistor{inv_num}']['enable'] = False
        ...
        for axis_ref in ref.axes():
            ...
            if axis_config.can_protocol == "":
                ...
                output_configs[inv_ref.parent.num]['config']['enable_can_a'] = True
                axis_output_config['config']['can']['node_id'] = int(axis_config.can_node_id)
                axis_output_config['config']['can']['is_extended'] = bool(axis_config.can_node_id_is_extended)
            else:
            ...
            axis_calib[axis_ref.num].append(CalibrationTask(
                "Encoder Offset Calibration",
                None if axis is None else functools.partial(_run_encoder_offset_calibration, axis),
                (CalibrationStatus.OK if ((not axis is None) and ComponentStatus(axis.commutation_mapper.status) == ComponentStatus.NOMINAL) else
                CalibrationStatus.NEEDED if ((not axis is None) and ComponentStatus(axis.commutation_mapper.status) == ComponentStatus.RELATIVE_MODE) else
...
#
    ...
    def format_status(self, odrives) -> RichText:
        ...
        for axis_ref in ref.axes():
            ...
            for calib in axis_calib[axis_ref.num]:
                if calib.status == CalibrationStatus.OK:
                    lines.append("  " + check_sign + " " + RichText(str(calib.name) + " ok", Color.GREEN))
                elif calib.status == CalibrationStatus.RECOMMENDED:
                    lines.append("  " + info_sign + " " + RichText(str(calib.name) + " recommended"))
                elif calib.status == CalibrationStatus.NEEDED:
                    lines.append("  " + warning_sign + " " + RichText(str(calib.name) + " needed", style=Style.BOLD))
                elif calib.status == CalibrationStatus.UNKNOWN:
    ...
    def calibrate(self, odrives, include_optional = False):
        ...
        for axis_num, axis_calib in enumerate(axis_calibs):
            for calib in axis_calib:
                if (calib.status == CalibrationStatus.RECOMMENDED and include_optional) or calib.status == CalibrationStatus.NEEDED:
...
def _run_state(axis, state):
    ...
    if result == ProcedureResult.DISARMED:
        raise Exception("Device failed with {}".format(ODriveError(axis.disarm_reason)))
...
def _run_motor_calibration(axis, motor_config):
    _run_state(axis, AxisState.MOTOR_CALIBRATION)
    motor_config['phase_resistance'] = axis.config.motor.phase_resistance
    motor_config['phase_inductance'] = axis.config.motor.phase_inductance

def _run_encoder_offset_calibration(axis):
    _run_state(axis, AxisState.ENCODER_OFFSET_CALIBRATION)
