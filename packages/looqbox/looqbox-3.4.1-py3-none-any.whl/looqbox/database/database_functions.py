...
def sql_execute(connection, query, replace_parameters=None, close_connection=True, show_query=False,
    ...
    try:
        if _is_connection_jdbc(connection):
            ...
            query_dataframe.data = _get_mongo_query_result(connection, query)
            query_dataframe.metadata = {}

        elif _is_connection_big_query(connection):
            query_dataframe.data = _get_BQ_query_result(connection, query)
            query_dataframe.metadata = {}
        ...
        GlobalCalling.log_query({"connection": str(_get_connection_alias()), "query": query,
...
def _get_mongo_query_result(connection, query):
    ...
    dataframe = pd.DataFrame(collection.find(query.get("query"), query.get("fields")))
