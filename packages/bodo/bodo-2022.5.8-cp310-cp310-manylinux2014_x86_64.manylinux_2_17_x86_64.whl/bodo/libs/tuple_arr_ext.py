...
class TupleArrayType(types.ArrayCompatible):
    ...
    @property
    def dtype(self):
        return types.BaseTuple.from_types(tuple(xuj__viq.dtype for xuj__viq in
            self.data))
...
class TupleArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        atcvt__zcfd = [('data', StructArrayType(fe_type.data))]
        models.StructModel.__init__(self, dmm, fe_type, atcvt__zcfd)
...
def init_tuple_arr(typingctx, data_typ=None):
    ...
    areo__gwse = TupleArrayType(data_typ.data)

    def codegen(context, builder, sig, args):
        fpe__zqwm, = args
        jbky__rgh = context.make_helper(builder, areo__gwse)
        jbky__rgh.data = fpe__zqwm
        context.nrt.incref(builder, data_typ, fpe__zqwm)
        return jbky__rgh._getvalue()
    return areo__gwse(data_typ), codegen


@unbox(TupleArrayType)
def unbox_tuple_array(typ, val, c):
    data_typ = StructArrayType(typ.data)
    dnknf__dfq = unbox_struct_array(data_typ, val, c, is_tuple_array=True)
    fpe__zqwm = dnknf__dfq.value
    jbky__rgh = c.context.make_helper(c.builder, typ)
    jbky__rgh.data = fpe__zqwm
    dttm__fest = dnknf__dfq.is_error
    return NativeValue(jbky__rgh._getvalue(), is_error=dttm__fest)


@box(TupleArrayType)
def box_tuple_arr(typ, val, c):
    data_typ = StructArrayType(typ.data)
    jbky__rgh = c.context.make_helper(c.builder, typ, val)
    arr = box_struct_arr(data_typ, jbky__rgh.data, c, is_tuple_array=True)
    return arr
...
def tuple_arr_getitem(arr, ind):
    ...
    if isinstance(ind, types.Integer):
        upjy__ksu = 'def impl(arr, ind):\n'
        jivc__rvcos = ','.join(f'get_data(arr._data)[{tacd__gjate}][ind]' for
            tacd__gjate in range(len(arr.data)))
        upjy__ksu += f'  return ({jivc__rvcos})\n'
        ncf__zzm = {}
        exec(upjy__ksu, {'get_data': bodo.libs.struct_arr_ext.get_data},
            ncf__zzm)
        kmpe__faalr = ncf__zzm['impl']
        return kmpe__faalr
...
def tuple_arr_setitem(arr, ind, val):
    ...
    if val == types.none or isinstance(val, types.optional):
        return
    if isinstance(ind, types.Integer):
        etrt__bps = len(arr.data)
        upjy__ksu = 'def impl(arr, ind, val):\n'
        upjy__ksu += '  data = get_data(arr._data)\n'
        upjy__ksu += '  null_bitmap = get_null_bitmap(arr._data)\n'
        upjy__ksu += '  set_bit_to_arr(null_bitmap, ind, 1)\n'
        for tacd__gjate in range(etrt__bps):
            upjy__ksu += f'  data[{tacd__gjate}][ind] = val[{tacd__gjate}]\n'
        ncf__zzm = {}
        exec(upjy__ksu, {'get_data': bodo.libs.struct_arr_ext.get_data,
            'get_null_bitmap': bodo.libs.struct_arr_ext.get_null_bitmap,
            'set_bit_to_arr': bodo.libs.int_arr_ext.set_bit_to_arr}, ncf__zzm)
        kmpe__faalr = ncf__zzm['impl']
        return kmpe__faalr
