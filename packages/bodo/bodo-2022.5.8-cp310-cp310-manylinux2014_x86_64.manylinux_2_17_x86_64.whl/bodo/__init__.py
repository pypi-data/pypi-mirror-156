...
def _global_except_hook(exctype, value, traceback):
    ...
    byio__izclw = MPI.COMM_WORLD
    qntn__zuhu = byio__izclw.Get_rank()
    njfmr__vpe = 3.0
    lam__gfg = True
    yznt__mdrpz = byio__izclw.Ibarrier()
    oev__howt = time.time()
    while time.time() - oev__howt < njfmr__vpe:
        time.sleep(0.1)
        if yznt__mdrpz.Test():
            lam__gfg = False
            break
    try:
        ...
        if lam__gfg:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {qntn__zuhu}. \n')
