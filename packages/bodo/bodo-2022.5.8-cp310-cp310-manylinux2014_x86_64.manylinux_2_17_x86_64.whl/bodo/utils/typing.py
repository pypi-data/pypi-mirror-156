...
def decode_if_dict_array_overload(A):
    if isinstance(A, types.BaseTuple):
        lslea__nnh = len(A.types)
        yten__oticb = 'def f(A):\n'
        pyz__gza = ','.join(f'decode_if_dict_array(A[{i}])' for i in range(
            lslea__nnh))
        yten__oticb += '  return ({}{})\n'.format(pyz__gza, ',' if 
            lslea__nnh == 1 else '')
        xpz__diptq = {}
        exec(yten__oticb, {'decode_if_dict_array': decode_if_dict_array},
            xpz__diptq)
        impl = xpz__diptq['f']
        return impl
    ...
    if isinstance(A, bodo.SeriesType):
        ...
        def impl(A):
            erv__zeo = bodo.hiframes.pd_series_ext.get_series_data(A)
            ketg__fkmoi = bodo.hiframes.pd_series_ext.get_series_index(A)
            name = bodo.hiframes.pd_series_ext.get_series_name(A)
            bqegn__vfc = decode_if_dict_array(erv__zeo)
            return bodo.hiframes.pd_series_ext.init_series(bqegn__vfc,
                ketg__fkmoi, name)
        return impl
...
def to_str_arr_if_dict_array(t):
    ...
    if isinstance(t, bodo.TableType):
        sza__kyjav = tuple(to_str_arr_if_dict_array(t) for t in t.arr_types)
        return bodo.TableType(sza__kyjav, t.has_runtime_cols)
...
class BodoError(NumbaError):
    ...
    def __init__(self, msg, loc=None, locs_in_msg=None):
        ...
        xdvl__vwn = numba.core.errors.termcolor().errmsg
        super(BodoError, self).__init__(xdvl__vwn(msg), loc)
...
def raise_bodo_error(msg, loc=None):
    if bodo.transforms.typing_pass.in_partial_typing:
        ...
        byygl__ahvo = [] if loc is None else [loc]
        raise BodoError(msg, locs_in_msg=byygl__ahvo)
...
class FileInfo:
    ...
    def get_schema(self, fname):
        jugz__gjre = self.get_full_filename(fname)
        return self._get_schema(jugz__gjre)
...
class FilenameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        whh__zgzy = dmm.lookup(bodo.typeof(fe_type.fname))
        efdo__dwdd = [(a, ddudp__fbjc) for a, ddudp__fbjc in zip(whh__zgzy.
            _fields, whh__zgzy._members)]
        super().__init__(dmm, fe_type, efdo__dwdd)
...
def is_initial_value_type(t):
    if not isinstance(t, types.InitialValue) or t.initial_value is None:
        return False
    iytk__koxq = t.initial_value
    if isinstance(iytk__koxq, dict):
        iytk__koxq = iytk__koxq.values()
    return not any(isinstance(dom__fbdq, (types.Poison, numba.core.
        interpreter._UNKNOWN_VALUE)) for dom__fbdq in iytk__koxq)
...
def is_overload_constant_dict(val):
    return isinstance(val, types.LiteralStrKeyDict) and all(is_literal_type
        (dom__fbdq) for dom__fbdq in val.types) or is_initial_value_dict_type(
...
def is_overload_bool_list(val):
    return is_overload_constant_list(val) and all(is_overload_constant_bool
        (dom__fbdq) for dom__fbdq in get_overload_const_list(val))
...
def get_overload_const(val):
    ...
    if isinstance(val, types.LiteralList):
        udf__ojp = []
        for dom__fbdq in val.literal_value:
            qle__uqgjq = get_overload_const(dom__fbdq)
            if qle__uqgjq == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                udf__ojp.append(qle__uqgjq)
        return udf__ojp
    ...
    if isinstance(val, types.BaseTuple):
        udf__ojp = []
        for dom__fbdq in val.types:
            qle__uqgjq = get_overload_const(dom__fbdq)
            if qle__uqgjq == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                udf__ojp.append(qle__uqgjq)
        return tuple(udf__ojp)
...
def ensure_constant_arg(fname, arg_name, val, const_type):
    qle__uqgjq = get_overload_const(val)
    ofpxa__gxn = _const_type_repr.get(const_type, str(const_type))
    if not isinstance(qle__uqgjq, const_type):
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant {ofpxa__gxn} not {val}"
            )


def ensure_constant_values(fname, arg_name, val, const_values):
    qle__uqgjq = get_overload_const(val)
    if qle__uqgjq not in const_values:
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant value in {const_values} not '{qle__uqgjq}'"
            )
...
def check_unsupported_args(fname, args_dict, arg_defaults_dict,
    ...
    isni__iaecr = False
    for a in args_dict:
        dzol__eie = get_overload_const(args_dict[a])
        eth__esei = arg_defaults_dict[a]
        if (dzol__eie is NOT_CONSTANT or dzol__eie is not None and 
            eth__esei is None or dzol__eie is None and eth__esei is not
            None or dzol__eie != eth__esei or dzol__eie is not _no_input and
            eth__esei is _no_input or dzol__eie is _no_input and eth__esei
             is not _no_input):
            lfxqp__wxbja = (
                f'{fn_str}: {a} parameter only supports default value {eth__esei}'
                )
            isni__iaecr = True
            break
    if isni__iaecr and package_name == 'pandas':
        if module_name == 'IO':
            lfxqp__wxbja += """
...
def get_overload_constant_dict(val):
    if isinstance(val, types.LiteralStrKeyDict):
        return {get_literal_value(mkte__iotf): get_literal_value(dom__fbdq) for
            mkte__iotf, dom__fbdq in val.literal_value.items()}
    ...
    if isinstance(val, types.DictType):
        ...
        return val.initial_value
    vco__bpkf = [get_overload_const(dom__fbdq) for dom__fbdq in val.types[1:]]
    return {vco__bpkf[2 * i]: vco__bpkf[2 * i + 1] for i in range(len(
        vco__bpkf) // 2)}
...
def get_overload_const_list(val):
    ...
    if is_initial_value_list_type(val):
        return val.initial_value
    if isinstance(val, types.LiteralList):
        return [get_literal_value(dom__fbdq) for dom__fbdq in val.literal_value
            ]
...
def parse_dtype(dtype, func_name=None):
    ...
    if isinstance(dtype, types.Number) or dtype == bodo.string_type:
        return dtype
    try:
        zpw__fwlqx = get_overload_const_str(dtype)
        if zpw__fwlqx.startswith('Int') or zpw__fwlqx.startswith('UInt'):
            return bodo.libs.int_arr_ext.typeof_pd_int_dtype(pd.api.types.
                pandas_dtype(zpw__fwlqx), None)
        if zpw__fwlqx == 'boolean':
            return bodo.libs.bool_arr_ext.boolean_dtype
        if zpw__fwlqx == 'str':
            return bodo.string_type
        return numba.np.numpy_support.from_dtype(np.dtype(zpw__fwlqx))
    except:
        pass
...
def get_index_names(t, func_name, default_name):
    ...
    wafg__pjzgv = '{}: index name should be a constant string'.format(func_name
        )
    if isinstance(t, MultiIndexType):
        nlqon__uolwb = []
        for i, kpfax__jowu in enumerate(t.names_typ):
            if kpfax__jowu == types.none:
                nlqon__uolwb.append('level_{}'.format(i))
                continue
            if not is_overload_constant_str(kpfax__jowu):
                raise BodoError(wafg__pjzgv)
            nlqon__uolwb.append(get_overload_const_str(kpfax__jowu))
        return tuple(nlqon__uolwb)
...
if types.SliceLiteral in numba.core.pythonapi._boxers.functions:
    ...
    def box_slice_literal(typ, val, c):
        ols__jeah = typ.literal_value
        cahnj__aolsh = []
        for gnuu__fnz in ('start', 'stop', 'step'):
            szbgf__boq = getattr(typ.literal_value, gnuu__fnz)
            hkzft__pnny = c.pyapi.make_none(
                ) if szbgf__boq is None else c.pyapi.from_native_value(types
                .literal(szbgf__boq), szbgf__boq, c.env_manager)
            cahnj__aolsh.append(hkzft__pnny)
        xxlen__akvfq = c.pyapi.unserialize(c.pyapi.serialize_object(slice))
        etvfs__swfi = c.pyapi.call_function_objargs(xxlen__akvfq, cahnj__aolsh)
        for a in cahnj__aolsh:
            c.pyapi.decref(a)
        c.pyapi.decref(xxlen__akvfq)
        return etvfs__swfi
...
def box_list_literal(typ, val, c):
    xxp__jrg = typ.literal_value
    cdkyw__ryqmd = [c.pyapi.from_native_value(types.literal(dom__fbdq),
        dom__fbdq, c.env_manager) for dom__fbdq in xxp__jrg]
    hgdqc__jrkp = c.pyapi.list_pack(cdkyw__ryqmd)
    for a in cdkyw__ryqmd:
        c.pyapi.decref(a)
    return hgdqc__jrkp
...
def list_literal_to_list(context, builder, fromty, toty, val):
    ...
    qkl__idkfn = types.List(toty.dtype)
...
def is_literal_type(t):
    ...
    return isinstance(t, (types.Literal, types.Omitted)) and not isinstance(t,
        ...
        is_literal_type(dom__fbdq) for dom__fbdq in t.types
...
def get_literal_value(t):
    ...
    if isinstance(t, types.Literal):
        if isinstance(t, types.LiteralStrKeyDict):
            return {get_literal_value(mkte__iotf): get_literal_value(
                dom__fbdq) for mkte__iotf, dom__fbdq in t.literal_value.items()
                }
        if isinstance(t, types.LiteralList):
            return [get_literal_value(dom__fbdq) for dom__fbdq in t.
                literal_value]
        return t.literal_value
    if isinstance(t, types.Omitted):
        return t.value
    if isinstance(t, types.BaseTuple):
        return tuple(get_literal_value(dom__fbdq) for dom__fbdq in t.types)
...
def dtype_to_array_type(dtype):
    ...
    if isinstance(dtype, (types.Number, types.Boolean, types.NPDatetime,
        types.NPTimedelta)):
        erv__zeo = types.Array(dtype, 1, 'C')
        if dgt__mgyy:
            return to_nullable_type(erv__zeo)
        return erv__zeo
...
def get_udf_out_arr_type(f_return_type, return_nullable=False):
    ...
    nnsi__omw = dtype_to_array_type(f_return_type)
    nnsi__omw = to_nullable_type(nnsi__omw) if return_nullable else nnsi__omw
    return nnsi__omw
...
def types_equality_exists(t1, t2):
    aust__ipkyb = numba.core.registry.cpu_target.typing_context
    try:
        aust__ipkyb.resolve_function_type(operator.eq, (t1, t2), {})
        return True
    except:
        return False
...
def is_hashable_type(t):
    whitelist_types = (types.UnicodeType, types.StringLiteral, types.
        ...
        PandasTimestampType)
    vyjf__gyjhb = (types.bool_, bodo.datetime64ns, bodo.timedelta64ns, bodo
        .pd_timedelta_type)
    if isinstance(t, whitelist_types) or t in vyjf__gyjhb:
        return True
    aust__ipkyb = numba.core.registry.cpu_target.typing_context
    try:
        aust__ipkyb.resolve_function_type(hash, (t,), {})
        return True
    except:
        return False
...
def to_nullable_type(t):
    ...
    if isinstance(t, DataFrameType):
        ztf__hszda = tuple(to_nullable_type(t) for t in t.data)
        return DataFrameType(ztf__hszda, t.index, t.columns, t.dist, t.
            is_table_format)
...
def is_common_scalar_dtype(scalar_types):
    rvgy__jyz, jyf__nwc = get_common_scalar_dtype(scalar_types)
    return jyf__nwc
...
def get_common_scalar_dtype(scalar_types):
    ...
    try:
        yxam__bcp = np.find_common_type([numba.np.numpy_support.as_dtype(t) for
            t in scalar_types], [])
        if yxam__bcp != object:
            return numba.np.numpy_support.from_dtype(yxam__bcp), True
    except numba.core.errors.NumbaNotImplementedError as vvucq__pyu:
        pass
    ...
    kha__dumx = itertools.groupby(scalar_types)
    if next(kha__dumx, True) and not next(kha__dumx, False):
...
def get_nullable_and_non_nullable_types(array_of_types):
    ...
    for typ in array_of_types:
        if typ == bodo.libs.bool_arr_ext.boolean_array:
            gsjm__ruph.append(types.Array(types.bool_, 1, 'C'))
        elif isinstance(typ, bodo.libs.int_arr_ext.IntegerArrayType):
            gsjm__ruph.append(types.Array(typ.dtype, 1, 'C'))
        elif isinstance(typ, types.Array):
            if typ.dtype == types.bool_:
                gsjm__ruph.append(bodo.libs.bool_arr_ext.boolean_array)
            if isinstance(typ.dtype, types.Integer):
                gsjm__ruph.append(bodo.libs.int_arr_ext.IntegerArrayType(
                    typ.dtype))
        gsjm__ruph.append(typ)
    return gsjm__ruph
...
def _gen_objmode_overload(func, output_type, attr_name=None, is_function=
    ...
    if is_function:
        uwo__msvbf = getfullargspec(func)
        assert uwo__msvbf.varargs is None, 'varargs not supported'
        assert uwo__msvbf.varkw is None, 'varkw not supported'
        defaults = [] if uwo__msvbf.defaults is None else uwo__msvbf.defaults
        jwll__hdj = len(uwo__msvbf.args) - len(defaults)
        args = uwo__msvbf.args[1:] if attr_name else uwo__msvbf.args[:]
        skrqv__rpy = []
        for i, nluqt__jkjc in enumerate(uwo__msvbf.args):
            if i < jwll__hdj:
                skrqv__rpy.append(nluqt__jkjc)
            elif str(defaults[i - jwll__hdj]) != '<deprecated parameter>':
                skrqv__rpy.append(nluqt__jkjc + '=' + str(defaults[i -
                    jwll__hdj]))
            else:
                args.remove(nluqt__jkjc)
        if uwo__msvbf.kwonlyargs is not None:
            for nluqt__jkjc in uwo__msvbf.kwonlyargs:
                args.append(f'{nluqt__jkjc}={nluqt__jkjc}')
                skrqv__rpy.append(
                    f'{nluqt__jkjc}={str(uwo__msvbf.kwonlydefaults[nluqt__jkjc])}'
                    )
        sig = ', '.join(skrqv__rpy)
        args = ', '.join(args)
    else:
    ...
    dhjz__jrk = f'self.{attr_name}' if attr_name else f'{func_name}'
    yten__oticb = f'def overload_impl({sig}):\n'
    yten__oticb += f'    def impl({sig}):\n'
    if single_rank:
        yten__oticb += f'        if bodo.get_rank() == 0:\n'
        skt__symmw = '    '
    else:
        skt__symmw = ''
    yten__oticb += (
        f"        {skt__symmw}with numba.objmode(res='{type_name}'):\n")
    if is_function:
        yten__oticb += f'            {skt__symmw}res = {dhjz__jrk}({args})\n'
    else:
        yten__oticb += f'            {skt__symmw}res = {dhjz__jrk}\n'
    yten__oticb += f'        return res\n'
    yten__oticb += f'    return impl\n'
    xpz__diptq = {}
    yzu__bsp = globals()
    if not attr_name:
        yzu__bsp[func_name] = func
    exec(yten__oticb, yzu__bsp, xpz__diptq)
    xpsez__vpqai = xpz__diptq['overload_impl']
    return xpsez__vpqai


def gen_objmode_func_overload(func, output_type=None, single_rank=False):
    try:
        xpsez__vpqai = _gen_objmode_overload(func, output_type, is_function
            =True, single_rank=single_rank)
        overload(func, no_unliteral=True)(xpsez__vpqai)
    except Exception as vvucq__pyu:
        pass
...
def gen_objmode_method_overload(obj_type, method_name, method, output_type=
    ...
    try:
        xpsez__vpqai = _gen_objmode_overload(method, output_type,
            method_name, True, single_rank)
        overload_method(obj_type, method_name, no_unliteral=True)(xpsez__vpqai)
    except Exception as vvucq__pyu:
        pass
...
def gen_objmode_attr_overload(obj_type, attr_name, attr, output_type=None,
    single_rank=False):
    try:
        xpsez__vpqai = _gen_objmode_overload(attr, output_type, attr_name, 
            False, single_rank)
        overload_attribute(obj_type, attr_name, no_unliteral=True)(xpsez__vpqai
            )
    except Exception as vvucq__pyu:
        pass
...
class NumTypeStaticGetItem(AbstractTemplate):
    ...
    def generic(self, args, kws):
        val, liyxt__oti = args
        if isinstance(liyxt__oti, slice) and (isinstance(val, types.
            NumberClass) or isinstance(val, types.TypeRef) and isinstance(
            val.instance_type, (types.NPDatetime, types.NPTimedelta))):
            return signature(types.TypeRef(val.instance_type[liyxt__oti]),
                *args)
...
def construct_pysig(arg_names, defaults):
    yten__oticb = f'def stub('
    for nluqt__jkjc in arg_names:
        yten__oticb += nluqt__jkjc
        if nluqt__jkjc in defaults:
            if isinstance(defaults[nluqt__jkjc], str):
                yten__oticb += f"='{defaults[nluqt__jkjc]}'"
            else:
                yten__oticb += f'={defaults[nluqt__jkjc]}'
        yten__oticb += ', '
    yten__oticb += '):\n'
    yten__oticb += '    pass\n'
    xpz__diptq = {}
    exec(yten__oticb, {}, xpz__diptq)
    hmjuy__ysu = xpz__diptq['stub']
    return numba.core.utils.pysignature(hmjuy__ysu)


def fold_typing_args(func_name, args, kws, arg_names, defaults,
    unsupported_arg_names=()):
    kws = dict(kws)
    ovx__fwoul = len(arg_names)
    gtulb__tfmuh = len(args) + len(kws)
    if gtulb__tfmuh > ovx__fwoul:
        xprx__sdq = 'argument' if ovx__fwoul == 1 else 'arguments'
        dnhqr__kcjab = 'was' if gtulb__tfmuh == 1 else 'were'
        raise BodoError(
            f'{func_name}(): Too many arguments specified. Function takes {ovx__fwoul} {xprx__sdq}, but {gtulb__tfmuh} {dnhqr__kcjab} provided.'
            )
    lzqtl__ukqi = bodo.utils.typing.construct_pysig(arg_names, defaults)
    try:
        duodm__asp = bodo.utils.transform.fold_argument_types(lzqtl__ukqi,
            args, kws)
    except Exception as xysj__rqp:
        raise_bodo_error(f'{func_name}(): {xysj__rqp}')
    if unsupported_arg_names:
        ...
        for i, arg_name in enumerate(arg_names):
            if arg_name in unsupported_arg_names:
                ...
                qxi__zcwco[arg_name] = duodm__asp[i]
                vcv__rcgcs[arg_name] = defaults[arg_name]
        check_unsupported_args(func_name, qxi__zcwco, vcv__rcgcs)
    return lzqtl__ukqi, duodm__asp
...
class BodoArrayIteratorModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        efdo__dwdd = [('index', types.EphemeralPointer(types.uintp)), (
            'array', fe_type.arr_type)]
        super(BodoArrayIteratorModel, self).__init__(dmm, fe_type, efdo__dwdd)


@lower_builtin('iternext', BodoArrayIterator)
@iternext_impl(RefType.NEW)
def iternext_bodo_array(context, builder, sig, args, result):
    [ihhf__garzk] = sig.args
    [zzsb__uzdal] = args
    eorir__zlg = context.make_helper(builder, ihhf__garzk, value=zzsb__uzdal)
    gpqe__zkre = signature(types.intp, ihhf__garzk.arr_type)
    hscac__pbf = context.compile_internal(builder, lambda a: len(a),
        gpqe__zkre, [eorir__zlg.array])
    ketg__fkmoi = builder.load(eorir__zlg.index)
    mhink__exolu = builder.icmp_signed('<', ketg__fkmoi, hscac__pbf)
    result.set_valid(mhink__exolu)
    with builder.if_then(mhink__exolu):
        ite__fgocy = signature(ihhf__garzk.yield_type, ihhf__garzk.arr_type,
            types.intp)
        value = context.compile_internal(builder, lambda a, i: a[i],
            ite__fgocy, [eorir__zlg.array, ketg__fkmoi])
        result.yield_(value)
        jff__lyn = cgutils.increment_index(builder, ketg__fkmoi)
        builder.store(jff__lyn, eorir__zlg.index)


def index_typ_from_dtype_name_arr(elem_dtype, name, arr_typ):
    yyb__ssjy = type(get_index_type_from_dtype(elem_dtype))
    if name is None:
        cnw__vzfto = None
    else:
        cnw__vzfto = types.StringLiteral(name)
    if yyb__ssjy == bodo.hiframes.pd_index_ext.NumericIndexType:
        hsg__bmft = yyb__ssjy(elem_dtype, cnw__vzfto, arr_typ)
    elif yyb__ssjy == bodo.hiframes.pd_index_ext.CategoricalIndexType:
        hsg__bmft = yyb__ssjy(bodo.CategoricalArrayType(elem_dtype),
            cnw__vzfto, arr_typ)
    else:
        hsg__bmft = yyb__ssjy(cnw__vzfto, arr_typ)
    return hsg__bmft
...
def check_objmode_output_type(ret_tup, ret_type):
    return tuple(_check_objmode_type(dom__fbdq, t) for dom__fbdq, t in zip(
...
def _fix_objmode_df_type(val, val_typ, typ):
    ...
    if val_typ != typ:
        vys__zyj = pd.Index(val_typ.columns)
        gckrl__vxe = pd.Index(typ.columns)
        inwyz__gwngi = vys__zyj.argsort()
        jqok__fgzyn = gckrl__vxe.argsort()
        xpfz__rwx = val_typ.copy(data=tuple(np.array(val_typ.data)[
            inwyz__gwngi]), columns=tuple(vys__zyj[inwyz__gwngi]))
        ahydl__vch = typ.copy(data=tuple(np.array(typ.data)[jqok__fgzyn]),
            columns=tuple(gckrl__vxe[jqok__fgzyn]))
        if xpfz__rwx == ahydl__vch:
            val_typ = typ
