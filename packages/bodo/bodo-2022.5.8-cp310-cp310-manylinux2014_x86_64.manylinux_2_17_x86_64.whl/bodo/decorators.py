...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        lkkq__fwckk = self.overloads[sig]
        ohula__wuek = lkkq__fwckk.metadata.get('distributed_diagnostics', None)
        if ohula__wuek is None:
            uossw__mafuf = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(uossw__mafuf)
        ohula__wuek.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        ojt__urpv = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(ojt__urpv)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        zun__femza = options.pop('distributed')
        pipeline_class = (pipeline_class if zun__femza else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        izzla__soj = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if izzla__soj else
            pipeline_class)
    qyz__csh = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(qyz__csh, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(qyz__csh)
        else:
            return master_mode_wrapper(qyz__csh)
    else:
        return qyz__csh
