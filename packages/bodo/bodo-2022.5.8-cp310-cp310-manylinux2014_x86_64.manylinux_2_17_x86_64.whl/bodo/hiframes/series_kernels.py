...
def _column_filter_impl(B, ind):
    ceo__oic = bodo.hiframes.rolling.alloc_shift(len(B), B, (-1,))
    for spnt__owolk in numba.parfors.parfor.internal_prange(len(ceo__oic)):
        if ind[spnt__owolk]:
            ceo__oic[spnt__owolk] = B[spnt__owolk]
        else:
            bodo.libs.array_kernels.setna(ceo__oic, spnt__owolk)
    return ceo__oic
...
def _series_dropna_str_alloc_impl_inner(B):
    ...
    krl__ujmx = len(B)
    jbj__txo = 0
    for spnt__owolk in range(len(B)):
        if bodo.libs.str_arr_ext.str_arr_is_na(B, spnt__owolk):
            jbj__txo += 1
    qhn__tpvo = krl__ujmx - jbj__txo
    lvfit__abliv = bodo.libs.str_arr_ext.num_total_chars(B)
    ceo__oic = bodo.libs.str_arr_ext.pre_alloc_string_array(qhn__tpvo,
        lvfit__abliv)
    bodo.libs.str_arr_ext.copy_non_null_offsets(ceo__oic, B)
    bodo.libs.str_arr_ext.copy_data(ceo__oic, B)
    bodo.libs.str_arr_ext.set_null_bits_to_value(ceo__oic, -1)
    return ceo__oic
...
def compute_skew(first_moment, second_moment, third_moment, count):
    if count < 3:
        return np.nan
    lbo__oqth = first_moment / count
    usjd__zqjiv = (third_moment - 3 * second_moment * lbo__oqth + 2 * count *
        lbo__oqth ** 3)
    krbb__ger = second_moment - lbo__oqth * first_moment
    s = count * (count - 1) ** 1.5 / (count - 2
        ) * usjd__zqjiv / krbb__ger ** 1.5
    s = s / (count - 1)
    return s
...
def compute_kurt(first_moment, second_moment, third_moment, fourth_moment,
    ...
    if count < 4:
        return np.nan
    lbo__oqth = first_moment / count
    flouf__aguz = (fourth_moment - 4 * third_moment * lbo__oqth + 6 *
        second_moment * lbo__oqth ** 2 - 3 * count * lbo__oqth ** 4)
    tjdn__fxe = second_moment - lbo__oqth * first_moment
    naed__qnlp = 3 * (count - 1) ** 2 / ((count - 2) * (count - 3))
    gwtvb__xfat = count * (count + 1) * (count - 1) * flouf__aguz
    libw__zcoh = (count - 2) * (count - 3) * tjdn__fxe ** 2
    s = (count - 1) * (gwtvb__xfat / libw__zcoh - naed__qnlp)
    s = s / (count - 1)
    return s
