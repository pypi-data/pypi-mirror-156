...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    ytdq__ohuf = pq_node.out_vars[0].name
    ffzpu__tcev = pq_node.out_vars[1].name
    if ytdq__ohuf not in lives and ffzpu__tcev not in lives:
        return None
    elif ytdq__ohuf not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.out_used_cols = []
    elif ffzpu__tcev not in lives:
