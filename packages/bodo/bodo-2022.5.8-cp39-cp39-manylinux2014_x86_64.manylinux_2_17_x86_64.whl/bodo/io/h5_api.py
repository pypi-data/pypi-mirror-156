...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        bujb__dzq = []
        nrxp__vojvd = h5g_get_num_objs(obj_id)
        for ltpb__atu in range(nrxp__vojvd):
            sel__zanje = h5g_get_objname_by_idx(obj_id, ltpb__atu)
            bujb__dzq.append(sel__zanje)
        return bujb__dzq
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    xoxm__aebep = parse_dtype(dtype)
    ilke__lrts = np.int32(numba_to_c_type(xoxm__aebep))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            xgge__eofwn = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    trp__wwgvh = tuple_to_ptr(starts)
    byz__xgczg = tuple_to_ptr(counts)
    lxupt__prh = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, trp__wwgvh, byz__xgczg,
        is_parallel, out_arr.ctypes, lxupt__prh)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    byz__xgczg = tuple_to_ptr(counts)
    lxupt__prh = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, trp__wwgvh, byz__xgczg,
        is_parallel, out_arr.ctypes, lxupt__prh)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        eejxj__lzx = types.Array(dtype, ndim, 'C')
        ahoma__bpiw = args[3]
        eejxj__lzx = self.context.resolve_function_type(operator.getitem, [
            eejxj__lzx, ahoma__bpiw], {}).return_type.copy(layout='C')
        return signature(eejxj__lzx, *args)
...
def get_filter_read_indices(bool_arr):
    gxyht__neqqm = bool_arr.nonzero()[0]
    tpm__ydwny = bodo.libs.distributed_api.get_rank()
    ecfjx__gqxxs = bodo.libs.distributed_api.get_size()
    nas__eqs = np.empty(ecfjx__gqxxs, np.int64)
    yqin__lav = len(bool_arr)
    bodo.libs.distributed_api.allgather(nas__eqs, yqin__lav)
    cqpuz__eidh = nas__eqs.cumsum()[tpm__ydwny] - yqin__lav
    gxyht__neqqm += cqpuz__eidh
    cijm__tcr = bodo.libs.distributed_api.dist_reduce(len(gxyht__neqqm), np
        .int32(sum_op))
    cin__bzn = bodo.libs.distributed_api.gatherv(gxyht__neqqm)
    if tpm__ydwny == 0:
        vrgri__xslmx = cin__bzn
    else:
        vrgri__xslmx = np.empty(cijm__tcr, gxyht__neqqm.dtype)
    bodo.libs.distributed_api.bcast(vrgri__xslmx)
    prwcm__etur = bodo.libs.distributed_api.get_start(cijm__tcr,
        ecfjx__gqxxs, tpm__ydwny)
    ymg__aikb = bodo.libs.distributed_api.get_end(cijm__tcr, ecfjx__gqxxs,
        tpm__ydwny)
    return vrgri__xslmx[prwcm__etur:ymg__aikb]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        oeyk__sigzr = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], oeyk__sigzr)
        return builder.bitcast(oeyk__sigzr, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    byz__xgczg = tuple_to_ptr(counts)
    lxupt__prh = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, trp__wwgvh,
        byz__xgczg, is_parallel, out_arr.ctypes, lxupt__prh, read_indices.
