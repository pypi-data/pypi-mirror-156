...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        jvmhu__onh = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(jvmhu__onh)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        xbyyo__wze = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, xbyyo__wze)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        yrhmd__snt, = args
        rocvl__fyg = signature.return_type
        srvi__szqn = cgutils.create_struct_proxy(rocvl__fyg)(context, builder)
        srvi__szqn.obj = yrhmd__snt
        context.nrt.incref(builder, signature.args[0], yrhmd__snt)
        return srvi__szqn._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                gifcz__toegj = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        jvmhu__onh = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(jvmhu__onh)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        xbyyo__wze = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, xbyyo__wze)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        yrhmd__snt, = args
        lknpp__ppoc = signature.return_type
        eyzf__junv = cgutils.create_struct_proxy(lknpp__ppoc)(context, builder)
        eyzf__junv.obj = yrhmd__snt
        context.nrt.incref(builder, signature.args[0], yrhmd__snt)
        return eyzf__junv._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                wrw__oza = bodo.utils.conversion.coerce_to_ndarray(idx)
                tft__zum = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    wrw__oza]
                uci__gieii = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    wrw__oza]
                jvmhu__onh = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(tft__zum,
                    uci__gieii, jvmhu__onh)
            return impl
        if isinstance(idx, types.SliceType):

            def impl_slice(I, idx):
                S = I._obj
                tft__zum = bodo.hiframes.pd_series_ext.get_series_data(S)[idx]
                uci__gieii = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    idx]
                jvmhu__onh = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(tft__zum,
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            if is_scalar_type(val):
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        gifcz__toegj = (bodo.hiframes.
                            datetime_timedelta_ext._to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(gifcz__toegj))
                        wrw__oza = bodo.utils.conversion.coerce_to_ndarray(idx)
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            wrw__oza] = s
                    return impl_dt
            ...
            def impl_arr(I, idx, val):
                wrw__oza = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[wrw__oza
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        jvmhu__onh = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(jvmhu__onh)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        xbyyo__wze = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, xbyyo__wze)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        yrhmd__snt, = args
        pjiio__bzbd = signature.return_type
        uzsc__eekxc = cgutils.create_struct_proxy(pjiio__bzbd)(context, builder
            )
        uzsc__eekxc.obj = yrhmd__snt
        context.nrt.incref(builder, signature.args[0], yrhmd__snt)
        return uzsc__eekxc._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if is_list_like_index_type(idx) and idx.dtype == types.bool_:
        ...
        def impl(I, idx):
            S = I._obj
            wrw__oza = bodo.utils.conversion.coerce_to_ndarray(idx)
            tft__zum = bodo.hiframes.pd_series_ext.get_series_data(S)[wrw__oza]
            uci__gieii = bodo.hiframes.pd_series_ext.get_series_index(S)[
                wrw__oza]
            jvmhu__onh = bodo.hiframes.pd_series_ext.get_series_name(S)
            return bodo.hiframes.pd_series_ext.init_series(tft__zum,
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            tft__zum = bodo.hiframes.pd_series_ext.get_series_data(S)
            uci__gieii = bodo.hiframes.pd_series_ext.get_series_index(S)
            zuguq__mbf = (idx - uci__gieii._start) // uci__gieii._step
            return tft__zum[zuguq__mbf]
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    ...
                    wrw__oza = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(tft__zum[wrw__oza]
                        )
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    osxm__ctu = bodo.hiframes.pd_series_ext.get_series_index(S)
                    owbyo__nfm = osxm__ctu.get_loc(idx)
                    if owbyo__nfm is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(owbyo__nfm)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                wrw__oza = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[wrw__oza
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        uzb__cjia = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(ycqi__qkm,
            int) for ycqi__qkm in uzb__cjia):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            szt__uud = get_literal_value(idx)
            if szt__uud not in uzb__cjia:
                raise_bodo_error(
                    f"Series label-based getitem: '{szt__uud}' not in {uzb__cjia}"
                    )
            arr_ind = uzb__cjia.index(szt__uud)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    srvi__szqn = cgutils.create_struct_proxy(fromty)(context, builder, val)
    jhkbi__chf = context.cast(builder, srvi__szqn.obj, fromty.stype, toty.stype
        )
    zcyi__bspq = cgutils.create_struct_proxy(toty)(context, builder)
    zcyi__bspq.obj = jhkbi__chf
    return zcyi__bspq._getvalue()
