...
def box_regex_flag(typ, val, c):
    etyhj__rsbl = c.box(types.int64, val)
    aysv__ups = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(aysv__ups, (etyhj__rsbl,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    etyhj__rsbl = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, etyhj__rsbl)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    nlhv__znefp = context.get_python_api(builder)
    dnmeg__vnp = builder.icmp_signed('==', val, nlhv__znefp.borrow_none())
    with builder.if_then(dnmeg__vnp):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    kdtqi__coo = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(kdtqi__coo)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        aaqzo__wte = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if aaqzo__wte > 1:
            typ = types.List(types.Tuple([string_type] * aaqzo__wte))
...
def overload_match_group(m, *args):
    ...
    olti__nwv = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            ubcl__wpk = args[0]
            with numba.objmode(out=olti__nwv):
                out = m.group(ubcl__wpk)
            return out
        return _match_group_impl_one
    gvk__tyhc = 'tuple_str_{}'.format(len(args))
    setattr(types, gvk__tyhc, types.Tuple([olti__nwv] * len(args)))
    hbkkh__buteb = ', '.join('group{}'.format(agnp__tysr + 1) for
        agnp__tysr in range(len(args)))
    atgf__njyhg = 'def _match_group_impl(m, *args):\n'
    atgf__njyhg += '  ({}) = args\n'.format(hbkkh__buteb)
    atgf__njyhg += "  with numba.objmode(out='{}'):\n".format(gvk__tyhc)
    atgf__njyhg += '    out = m.group({})\n'.format(hbkkh__buteb)
    atgf__njyhg += '  return out\n'
    oct__vjggs = {}
    exec(atgf__njyhg, globals(), oct__vjggs)
    ebh__rhy = oct__vjggs['_match_group_impl']
    return ebh__rhy
...
def overload_match_groups(m, default=None):
    rhy__hirs = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=rhy__hirs):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(jwryz__lhr is None for jwryz__lhr in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=olti__nwv):
