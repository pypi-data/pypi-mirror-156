...
class TupleArrayType(types.ArrayCompatible):
    ...
    @property
    def dtype(self):
        return types.BaseTuple.from_types(tuple(yxpr__kzghv.dtype for
            yxpr__kzghv in self.data))
...
class TupleArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        aogkz__cvl = [('data', StructArrayType(fe_type.data))]
        models.StructModel.__init__(self, dmm, fe_type, aogkz__cvl)
...
def init_tuple_arr(typingctx, data_typ=None):
    ...
    fyh__pdso = TupleArrayType(data_typ.data)

    def codegen(context, builder, sig, args):
        mkm__muqa, = args
        xbrkd__wxj = context.make_helper(builder, fyh__pdso)
        xbrkd__wxj.data = mkm__muqa
        context.nrt.incref(builder, data_typ, mkm__muqa)
        return xbrkd__wxj._getvalue()
    return fyh__pdso(data_typ), codegen


@unbox(TupleArrayType)
def unbox_tuple_array(typ, val, c):
    data_typ = StructArrayType(typ.data)
    vrv__orijf = unbox_struct_array(data_typ, val, c, is_tuple_array=True)
    mkm__muqa = vrv__orijf.value
    xbrkd__wxj = c.context.make_helper(c.builder, typ)
    xbrkd__wxj.data = mkm__muqa
    sgkjf__kyphx = vrv__orijf.is_error
    return NativeValue(xbrkd__wxj._getvalue(), is_error=sgkjf__kyphx)


@box(TupleArrayType)
def box_tuple_arr(typ, val, c):
    data_typ = StructArrayType(typ.data)
    xbrkd__wxj = c.context.make_helper(c.builder, typ, val)
    arr = box_struct_arr(data_typ, xbrkd__wxj.data, c, is_tuple_array=True)
    return arr
...
def tuple_arr_getitem(arr, ind):
    ...
    if isinstance(ind, types.Integer):
        pgmf__tbyrz = 'def impl(arr, ind):\n'
        qkpy__zqcx = ','.join(f'get_data(arr._data)[{slihv__bzzsq}][ind]' for
            slihv__bzzsq in range(len(arr.data)))
        pgmf__tbyrz += f'  return ({qkpy__zqcx})\n'
        yjqt__njjsc = {}
        exec(pgmf__tbyrz, {'get_data': bodo.libs.struct_arr_ext.get_data},
            yjqt__njjsc)
        enl__mfmpx = yjqt__njjsc['impl']
        return enl__mfmpx
...
def tuple_arr_setitem(arr, ind, val):
    ...
    if val == types.none or isinstance(val, types.optional):
        return
    if isinstance(ind, types.Integer):
        xcq__int = len(arr.data)
        pgmf__tbyrz = 'def impl(arr, ind, val):\n'
        pgmf__tbyrz += '  data = get_data(arr._data)\n'
        pgmf__tbyrz += '  null_bitmap = get_null_bitmap(arr._data)\n'
        pgmf__tbyrz += '  set_bit_to_arr(null_bitmap, ind, 1)\n'
        for slihv__bzzsq in range(xcq__int):
            pgmf__tbyrz += (
                f'  data[{slihv__bzzsq}][ind] = val[{slihv__bzzsq}]\n')
        yjqt__njjsc = {}
        exec(pgmf__tbyrz, {'get_data': bodo.libs.struct_arr_ext.get_data,
