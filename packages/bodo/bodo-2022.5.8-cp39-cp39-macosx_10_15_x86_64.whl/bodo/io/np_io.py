...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        jagy__ezck = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, jagy__ezck)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            bmh__ssi = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(bmh__ssi.dtype)
            file_write(unicode_to_utf8(fname), bmh__ssi.ctypes, dtype_size *
                bmh__ssi.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            bmh__ssi = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(bmh__ssi.dtype)
            hsqv__wqncw = (dtype_size * bodo.libs.distributed_api.
                get_tuple_prod(bmh__ssi.shape[1:]))
            _file_write_parallel(unicode_to_utf8(fname), bmh__ssi.ctypes,
                start, count, hsqv__wqncw)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            txaw__cegv = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                txaw__cegv = min(txaw__cegv, count * dtype_size)
            if txaw__cegv < 0:
                return -1
            return txaw__cegv
        return impl
