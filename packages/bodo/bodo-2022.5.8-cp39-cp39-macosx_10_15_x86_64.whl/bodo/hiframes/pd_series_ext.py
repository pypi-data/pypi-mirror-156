...
def series_getiter(context, builder, sig, args):
    ajli__rmv = get_series_payload(context, builder, sig.args[0], args[0])
    impl = context.get_function('getiter', sig.return_type(sig.args[0].data))
    return impl(builder, (ajli__rmv.data,))
...
class HeterSeriesAttribute(OverloadedKeyAttributeTemplate):
    ...
    def generic_resolve(self, S, attr):
        ...
        if isinstance(S.index, HeterogeneousIndexType
            ...
            jxez__eolgn = get_overload_const_tuple(S.index.data)
            if attr in jxez__eolgn:
                hnbo__nxfsu = jxez__eolgn.index(attr)
                return S.data[hnbo__nxfsu]
...
class SeriesPayloadModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        tkp__lzue = [('data', fe_type.series_type.data), ('index', fe_type.
            series_type.index), ('name', fe_type.series_type.name_typ)]
        super(SeriesPayloadModel, self).__init__(dmm, fe_type, tkp__lzue)
...
class SeriesModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ...
        tkp__lzue = [('meminfo', types.MemInfoPointer(payload_type)), (
            'parent', types.pyobject)]
        super(SeriesModel, self).__init__(dmm, fe_type, tkp__lzue)


def define_series_dtor(context, builder, series_type, payload_type):
    xdr__msw = builder.module
    nerf__hiwrt = lir.FunctionType(lir.VoidType(), [cgutils.voidptr_t])
    sjq__tqh = cgutils.get_or_insert_function(xdr__msw, nerf__hiwrt, name=
        '.dtor.series.{}'.format(series_type))
    if not sjq__tqh.is_declaration:
        return sjq__tqh
    sjq__tqh.linkage = 'linkonce_odr'
    builder = lir.IRBuilder(sjq__tqh.append_basic_block())
    kiz__bakd = sjq__tqh.args[0]
    omkf__vxsu = context.get_value_type(payload_type).as_pointer()
    mwsug__bxikg = builder.bitcast(kiz__bakd, omkf__vxsu)
    xfnyd__nrl = context.make_helper(builder, payload_type, ref=mwsug__bxikg)
    context.nrt.decref(builder, series_type.data, xfnyd__nrl.data)
    context.nrt.decref(builder, series_type.index, xfnyd__nrl.index)
    context.nrt.decref(builder, series_type.name_typ, xfnyd__nrl.name)
    builder.ret_void()
    return sjq__tqh
...
def construct_series(context, builder, series_type, data_val, index_val,
    ...
    ajli__rmv = cgutils.create_struct_proxy(payload_type)(context, builder)
    ajli__rmv.data = data_val
    ajli__rmv.index = index_val
    ajli__rmv.name = name_val
    tfq__elhjd = context.get_value_type(payload_type)
    esuh__zzdyz = context.get_abi_sizeof(tfq__elhjd)
    lhu__yguu = define_series_dtor(context, builder, series_type, payload_type)
    ykaxq__axes = context.nrt.meminfo_alloc_dtor(builder, context.
        get_constant(types.uintp, esuh__zzdyz), lhu__yguu)
    zgol__syyej = context.nrt.meminfo_data(builder, ykaxq__axes)
    haaa__ltn = builder.bitcast(zgol__syyej, tfq__elhjd.as_pointer())
    builder.store(ajli__rmv._getvalue(), haaa__ltn)
    series = cgutils.create_struct_proxy(series_type)(context, builder)
    series.meminfo = ykaxq__axes
...
def init_series(typingctx, data, index, name=None):
    ...
    def codegen(context, builder, signature, args):
        ...
        wbq__qbh = construct_series(context, builder, series_type, data_val,
            index_val, name_val)
    ...
    if is_heterogeneous_tuple_type(data):
        pva__vkawq = HeterogeneousSeriesType(data, index, name)
    else:
        dtype = data.dtype
        data = if_series_to_array_type(data)
        pva__vkawq = SeriesType(dtype, data, index, name)
    sig = signature(pva__vkawq, data, index, name)
    return sig, codegen
...
def init_series_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    data = args[0]
    index = args[1]
    looh__atkd = self.typemap[data.name]
    if is_heterogeneous_tuple_type(looh__atkd) or isinstance(looh__atkd,
        types.BaseTuple):
        return None
    jvx__epzq = self.typemap[index.name]
    if not isinstance(jvx__epzq, HeterogeneousIndexType
...
def get_series_payload(context, builder, series_type, value):
    ykaxq__axes = cgutils.create_struct_proxy(series_type)(context, builder,
        value).meminfo
    payload_type = SeriesPayloadType(series_type)
    xfnyd__nrl = context.nrt.meminfo_data(builder, ykaxq__axes)
    omkf__vxsu = context.get_value_type(payload_type).as_pointer()
    xfnyd__nrl = builder.bitcast(xfnyd__nrl, omkf__vxsu)
    return context.make_helper(builder, payload_type, ref=xfnyd__nrl)
...
def get_series_data(typingctx, series_typ=None):
    ...
    def codegen(context, builder, signature, args):
        ajli__rmv = get_series_payload(context, builder, signature.args[0],
            args[0])
        return impl_ret_borrowed(context, builder, series_typ.data,
            ajli__rmv.data)
    pva__vkawq = series_typ.data
    sig = signature(pva__vkawq, series_typ)
    return sig, codegen
...
def get_series_data_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    dwzog__wshrt = args[0]
    looh__atkd = self.typemap[dwzog__wshrt.name].data
    if is_heterogeneous_tuple_type(looh__atkd) or isinstance(looh__atkd,
        types.BaseTuple):
        return None
    if equiv_set.has_shape(dwzog__wshrt):
        return ArrayAnalysis.AnalyzeResult(shape=dwzog__wshrt, pre=[])
    return None
...
def get_series_index_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    assert len(args) == 1 and not kws
    dwzog__wshrt = args[0]
    jvx__epzq = self.typemap[dwzog__wshrt.name].index
    if isinstance(jvx__epzq, HeterogeneousIndexType):
        return None
    if equiv_set.has_shape(dwzog__wshrt):
        return ArrayAnalysis.AnalyzeResult(shape=dwzog__wshrt, pre=[])
    return None
...
def cast_series(context, builder, fromty, toty, val):
    if fromty.copy(index=toty.index) == toty and isinstance(fromty.index,
        ...
        ajli__rmv = get_series_payload(context, builder, fromty, val)
        vjpgk__szety = context.cast(builder, ajli__rmv.index, fromty.index,
            toty.index)
        context.nrt.incref(builder, fromty.data, ajli__rmv.data)
        context.nrt.incref(builder, fromty.name_typ, ajli__rmv.name)
        return construct_series(context, builder, toty, ajli__rmv.data,
            vjpgk__szety, ajli__rmv.name)
...
class SeriesAttribute(OverloadedKeyAttributeTemplate):
    ...
    def resolve_head(self, ary, args, kws):
        jxo__ntmwj = 'Series.head'
        kuexp__bng = 'n',
        kkuu__iiyt = {'n': 5}
        pysig, lpge__sqetz = bodo.utils.typing.fold_typing_args(jxo__ntmwj,
            args, kws, kuexp__bng, kkuu__iiyt)
        rwcwl__qbv = lpge__sqetz[0]
        if not is_overload_int(rwcwl__qbv):
            raise BodoError(f"{jxo__ntmwj}(): 'n' must be an Integer")
        enjbg__szmmp = ary
        return enjbg__szmmp(*lpge__sqetz).replace(pysig=pysig)
    ...
    def _resolve_map_func(self, ary, func, pysig, fname, f_args=None, kws=None
        ...
        if f_args is not None:
            lqre__iqo += tuple(f_args.types)
        if kws is None:
            kws = {}
        nay__vyn = False
        ngfb__enl = True
        if fname == 'map' and isinstance(func, types.DictType):
            vca__eavt = func.value_type
            nay__vyn = True
        else:
            try:
                if types.unliteral(func) == types.unicode_type:
                    ...
                    vca__eavt = bodo.utils.transform.get_udf_str_return_type(
                        ary, get_overload_const_str(func), self.context,
                        'Series.apply')
                    ngfb__enl = False
                elif bodo.utils.typing.is_numpy_ufunc(func):
                    vca__eavt = func.get_call_type(self.context, (ary,), {}
                        ).return_type
                    ngfb__enl = False
                else:
                    vca__eavt = get_const_func_output_type(func, lqre__iqo,
                        kws, self.context, numba.core.registry.cpu_target.
                        target_context)
            except Exception as wsdgd__zvn:
                raise BodoError(get_udf_error_msg(f'Series.{fname}()',
                    wsdgd__zvn))
        if ngfb__enl:
            if isinstance(vca__eavt, (SeriesType, HeterogeneousSeriesType)
                ) and vca__eavt.const_info is None:
                raise BodoError(
                    'Invalid Series output in UDF (Series with constant length and constant Index value expected)'
                    )
            if isinstance(vca__eavt, HeterogeneousSeriesType):
                fean__aebzp, bfs__teg = vca__eavt.const_info
                if isinstance(vca__eavt.data, bodo.libs.nullable_tuple_ext.
                    NullableTupleType):
                    jhb__szx = vca__eavt.data.tuple_typ.types
                elif isinstance(vca__eavt.data, types.Tuple):
                    jhb__szx = vca__eavt.data.types
                plb__bkgsh = tuple(to_nullable_type(dtype_to_array_type(t)) for
                    t in jhb__szx)
                jtsqz__wpzla = bodo.DataFrameType(plb__bkgsh, ary.index,
                    bfs__teg)
            elif isinstance(vca__eavt, SeriesType):
                lehbh__diryj, bfs__teg = vca__eavt.const_info
                plb__bkgsh = tuple(to_nullable_type(dtype_to_array_type(
                    vca__eavt.dtype)) for fean__aebzp in range(lehbh__diryj))
                jtsqz__wpzla = bodo.DataFrameType(plb__bkgsh, ary.index,
                    bfs__teg)
            else:
                tnh__mxu = get_udf_out_arr_type(vca__eavt, nay__vyn)
                jtsqz__wpzla = SeriesType(tnh__mxu.dtype, tnh__mxu, ary.
                    index, ary.name_typ)
        else:
            jtsqz__wpzla = vca__eavt
        return signature(jtsqz__wpzla, (func,)).replace(pysig=pysig)
    ...
    def resolve_map(self, ary, args, kws):
        ...
        na_action = args[1] if len(args) > 1 else kws.pop('na_action',
            types.none)
        uyy__uwg = dict(na_action=na_action)
        jcqo__hrvk = dict(na_action=None)
        check_unsupported_args('Series.map', uyy__uwg, jcqo__hrvk,
    ...
    def resolve_apply(self, ary, args, kws):
        ...
        kws.pop('func', None)
        fkf__tfqk = args[1] if len(args) > 1 else kws.pop('convert_dtype',
            types.literal(True))
        f_args = args[2] if len(args) > 2 else kws.pop('args', None)
        uyy__uwg = dict(convert_dtype=fkf__tfqk)
        aqhn__ltudq = dict(convert_dtype=True)
        check_unsupported_args('Series.apply', uyy__uwg, aqhn__ltudq,
            package_name='pandas', module_name='Series')
        mspi__qmp = ', '.join("{} = ''".format(ydmat__qkb) for ydmat__qkb in
            kws.keys())
        fqsfc__gng = (
            f'def apply_stub(func, convert_dtype=True, args=(), {mspi__qmp}):\n'
            )
        fqsfc__gng += '    pass\n'
        aksow__wcxe = {}
        exec(fqsfc__gng, {}, aksow__wcxe)
        stcme__rmls = aksow__wcxe['apply_stub']
        pysig = numba.core.utils.pysignature(stcme__rmls)
    ...
    def _resolve_combine_func(self, ary, args, kws):
        ...
        cpe__mojyj = ary.dtype
        bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(ary,
            'Series.combine()')
        bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(other,
            'Series.combine()')
        if cpe__mojyj == types.NPDatetime('ns'):
            cpe__mojyj = pd_timestamp_type
        ijcl__cbph = other.dtype
        if ijcl__cbph == types.NPDatetime('ns'):
            ijcl__cbph = pd_timestamp_type
        vca__eavt = get_const_func_output_type(func, (cpe__mojyj,
            ijcl__cbph), {}, self.context, numba.core.registry.cpu_target.
            target_context)
        sig = signature(SeriesType(vca__eavt, index=ary.index, name_typ=
...
def pd_series_overload(data=None, index=None, dtype=None, name=None, copy=
    ...
    ranu__fpei = is_overload_none(data)
    nqeq__nkpj = is_overload_none(index)
    hrr__nuei = is_overload_none(dtype)
    if ranu__fpei and nqeq__nkpj and hrr__nuei:
        raise BodoError(
            'pd.Series() requires at least 1 of data, index, and dtype to not be none'
            )
    if is_series_type(data) and not nqeq__nkpj:
        raise BodoError(
    ...
    if is_heterogeneous_tuple_type(data) and is_overload_none(dtype):
        nrz__ufhi = tuple(len(data) * [False])

        def impl_heter(data=None, index=None, dtype=None, name=None, copy=
            False, fastpath=False):
            efi__dsg = bodo.utils.conversion.extract_index_if_none(data, index)
            tmgg__azu = bodo.utils.conversion.to_tuple(data)
            data_val = bodo.libs.nullable_tuple_ext.build_nullable_tuple(
                tmgg__azu, nrz__ufhi)
            return bodo.hiframes.pd_series_ext.init_series(data_val, bodo.
                utils.conversion.convert_to_index(efi__dsg), name)
        return impl_heter
    if ranu__fpei:
        if hrr__nuei:
            ...
            def impl(data=None, index=None, dtype=None, name=None, copy=
                False, fastpath=False):
                krzf__jumck = bodo.utils.conversion.extract_name_if_none(data,
                    name)
                efi__dsg = bodo.utils.conversion.extract_index_if_none(data,
                    index)
                numba.parfors.parfor.init_prange()
                zpo__bdcw = len(efi__dsg)
                tmgg__azu = np.empty(zpo__bdcw, np.float64)
                for dvt__wssx in numba.parfors.parfor.internal_prange(zpo__bdcw
                    ):
                    bodo.libs.array_kernels.setna(tmgg__azu, dvt__wssx)
                return bodo.hiframes.pd_series_ext.init_series(tmgg__azu,
                    bodo.utils.conversion.convert_to_index(efi__dsg),
                    krzf__jumck)
            return impl
        if bodo.utils.conversion._is_str_dtype(dtype):
            ccb__outuk = bodo.string_array_type
        else:
            sqax__qzi = bodo.utils.typing.parse_dtype(dtype, 'pandas.Series')
            if isinstance(sqax__qzi, bodo.libs.int_arr_ext.IntDtype):
                ccb__outuk = bodo.IntegerArrayType(sqax__qzi.dtype)
            elif sqax__qzi == bodo.libs.bool_arr_ext.boolean_dtype:
                ccb__outuk = bodo.boolean_array
            elif isinstance(sqax__qzi, types.Number) or sqax__qzi in [bodo.
                datetime64ns, bodo.timedelta64ns]:
                ccb__outuk = types.Array(sqax__qzi, 1, 'C')
            else:
                raise BodoError(
        ...
        if nqeq__nkpj:
            ...
            def impl(data=None, index=None, dtype=None, name=None, copy=
                ...
                krzf__jumck = bodo.utils.conversion.extract_name_if_none(data,
                    name)
                efi__dsg = bodo.hiframes.pd_index_ext.init_range_index(0, 0,
                    1, None)
                numba.parfors.parfor.init_prange()
                zpo__bdcw = len(efi__dsg)
                tmgg__azu = bodo.utils.utils.alloc_type(zpo__bdcw,
                    ccb__outuk, (-1,))
                return bodo.hiframes.pd_series_ext.init_series(tmgg__azu,
                    efi__dsg, krzf__jumck)
            return impl
        else:
    ...
    def impl(data=None, index=None, dtype=None, name=None, copy=False,
        fastpath=False):
        krzf__jumck = bodo.utils.conversion.extract_name_if_none(data, name)
        efi__dsg = bodo.utils.conversion.extract_index_if_none(data, index)
        rsd__ceei = bodo.utils.conversion.coerce_to_array(data, True,
            scalar_to_arr_len=len(efi__dsg))
        qlip__djxxi = bodo.utils.conversion.fix_arr_dtype(rsd__ceei, dtype,
            None, False)
        return bodo.hiframes.pd_series_ext.init_series(qlip__djxxi, bodo.
            utils.conversion.convert_to_index(efi__dsg), krzf__jumck)
    return impl
...
def lower_constant_series(context, builder, series_type, pyval):
    if isinstance(series_type.data, bodo.DatetimeArrayType):
        rkr__hnh = pyval.array
    else:
        rkr__hnh = pyval.values
    data_val = context.get_constant_generic(builder, series_type.data, rkr__hnh
        )
    ...
    name_val = context.get_constant_generic(builder, series_type.name_typ,
        pyval.name)
    xfnyd__nrl = lir.Constant.literal_struct([data_val, index_val, name_val])
    xfnyd__nrl = cgutils.global_constant(builder, '.const.payload', xfnyd__nrl
        ).bitcast(cgutils.voidptr_t)
    ieyij__yrl = context.get_constant(types.int64, -1)
    eqql__zxsrg = context.get_constant_null(types.voidptr)
    ykaxq__axes = lir.Constant.literal_struct([ieyij__yrl, eqql__zxsrg,
        eqql__zxsrg, xfnyd__nrl, ieyij__yrl])
    ykaxq__axes = cgutils.global_constant(builder, '.const.meminfo',
        ykaxq__axes).bitcast(cgutils.voidptr_t)
    wbq__qbh = lir.Constant.literal_struct([ykaxq__axes, eqql__zxsrg])
    return wbq__qbh
...
def _install_series_unsupported():
    for skjn__wwkur in series_unsupported_attrs:
        zzcgy__qtpn = 'Series.' + skjn__wwkur
        overload_attribute(SeriesType, skjn__wwkur)(create_unsupported_overload
            (zzcgy__qtpn))
    for fname in series_unsupported_methods:
        zzcgy__qtpn = 'Series.' + fname
        overload_method(SeriesType, fname, no_unliteral=True)(
            create_unsupported_overload(zzcgy__qtpn))
...
def _install_heter_series_unsupported():
    for skjn__wwkur in heter_series_unsupported_attrs:
        zzcgy__qtpn = 'HeterogeneousSeries.' + skjn__wwkur
        overload_attribute(HeterogeneousSeriesType, skjn__wwkur)(
            create_unsupported_overload(zzcgy__qtpn))
    for fname in heter_series_unsupported_methods:
        zzcgy__qtpn = 'HeterogeneousSeries.' + fname
