...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        xkius__diery = self.overloads[sig]
        hdr__wejqx = xkius__diery.metadata.get('distributed_diagnostics', None)
        if hdr__wejqx is None:
            hmmkw__wbp = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(hmmkw__wbp)
        hdr__wejqx.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        sqh__anz = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(sqh__anz)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        mlq__kzbe = options.pop('distributed')
        pipeline_class = (pipeline_class if mlq__kzbe else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        fnggv__vafzt = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if fnggv__vafzt else
            pipeline_class)
    xhvj__lkv = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(xhvj__lkv, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(xhvj__lkv)
        else:
            return master_mode_wrapper(xhvj__lkv)
    else:
        return xhvj__lkv
