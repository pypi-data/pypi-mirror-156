...
def box_regex_flag(typ, val, c):
    dey__nww = c.box(types.int64, val)
    cpz__ifj = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(cpz__ifj, (dey__nww,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    dey__nww = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, dey__nww)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    hlmov__ndgc = context.get_python_api(builder)
    klm__ejwu = builder.icmp_signed('==', val, hlmov__ndgc.borrow_none())
    with builder.if_then(klm__ejwu):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    fxxc__ylmig = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(fxxc__ylmig)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        lbj__junzc = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if lbj__junzc > 1:
            typ = types.List(types.Tuple([string_type] * lbj__junzc))
...
def overload_match_group(m, *args):
    ...
    acio__fhlnx = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            tdxln__qjs = args[0]
            with numba.objmode(out=acio__fhlnx):
                out = m.group(tdxln__qjs)
            return out
        return _match_group_impl_one
    qpfw__vft = 'tuple_str_{}'.format(len(args))
    setattr(types, qpfw__vft, types.Tuple([acio__fhlnx] * len(args)))
    rhrg__ufdd = ', '.join('group{}'.format(dll__bay + 1) for dll__bay in
        range(len(args)))
    ppdpy__gtgpr = 'def _match_group_impl(m, *args):\n'
    ppdpy__gtgpr += '  ({}) = args\n'.format(rhrg__ufdd)
    ppdpy__gtgpr += "  with numba.objmode(out='{}'):\n".format(qpfw__vft)
    ppdpy__gtgpr += '    out = m.group({})\n'.format(rhrg__ufdd)
    ppdpy__gtgpr += '  return out\n'
    ikyd__zxcx = {}
    exec(ppdpy__gtgpr, globals(), ikyd__zxcx)
    ipmae__mrw = ikyd__zxcx['_match_group_impl']
    return ipmae__mrw
...
def overload_match_groups(m, default=None):
    mrt__bgs = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=mrt__bgs):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(okvtx__rby is None for okvtx__rby in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=acio__fhlnx):
