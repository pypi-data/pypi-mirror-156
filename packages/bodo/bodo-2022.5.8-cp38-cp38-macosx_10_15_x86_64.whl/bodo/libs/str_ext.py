...
def unicode_to_utf8_and_len(typingctx, str_typ):
    ...
    hjc__ngual = types.Tuple([utf8_str_type, types.int64])

    def codegen(context, builder, sig, args):
        hos__bjse, = args
        gnv__xfpe = cgutils.create_struct_proxy(string_type)(context,
            builder, value=hos__bjse)
        hjiiv__ntc = cgutils.create_struct_proxy(utf8_str_type)(context,
            builder)
        znwry__aivj = cgutils.create_struct_proxy(hjc__ngual)(context, builder)
        is_ascii = builder.icmp_unsigned('==', gnv__xfpe.is_ascii, lir.
            Constant(gnv__xfpe.is_ascii.type, 1))
        with builder.if_else(is_ascii) as (tzdx__wjnkx, ssxf__kfyrb):
            with tzdx__wjnkx:
                context.nrt.incref(builder, string_type, hos__bjse)
                hjiiv__ntc.data = gnv__xfpe.data
                hjiiv__ntc.meminfo = gnv__xfpe.meminfo
                znwry__aivj.f1 = gnv__xfpe.length
            with ssxf__kfyrb:
                ypuc__iikn = lir.FunctionType(lir.IntType(64), [lir.IntType
                    (8).as_pointer(), lir.IntType(8).as_pointer(), lir.
                    IntType(64), lir.IntType(32)])
                hyr__hyqc = cgutils.get_or_insert_function(builder.module,
                    ypuc__iikn, name='unicode_to_utf8')
                dcokk__vmlpp = context.get_constant_null(types.voidptr)
                ltldw__izwi = builder.call(hyr__hyqc, [dcokk__vmlpp,
                    gnv__xfpe.data, gnv__xfpe.length, gnv__xfpe.kind])
                znwry__aivj.f1 = ltldw__izwi
                knmnd__ugs = builder.add(ltldw__izwi, lir.Constant(lir.
                    IntType(64), 1))
                hjiiv__ntc.meminfo = context.nrt.meminfo_alloc_aligned(builder,
                    size=knmnd__ugs, align=32)
                hjiiv__ntc.data = context.nrt.meminfo_data(builder,
                    hjiiv__ntc.meminfo)
                builder.call(hyr__hyqc, [hjiiv__ntc.data, gnv__xfpe.data,
                    gnv__xfpe.length, gnv__xfpe.kind])
                builder.store(lir.Constant(lir.IntType(8), 0), builder.gep(
                    hjiiv__ntc.data, [ltldw__izwi]))
        znwry__aivj.f0 = hjiiv__ntc._getvalue()
        return znwry__aivj._getvalue()
    return hjc__ngual(string_type), codegen
...
def memcmp(typingctx, dest_t, src_t, count_t=None):
    ...
    def codegen(context, builder, sig, args):
        ypuc__iikn = lir.FunctionType(lir.IntType(32), [lir.IntType(8).
            as_pointer(), lir.IntType(8).as_pointer(), lir.IntType(64)])
        ozohd__orne = cgutils.get_or_insert_function(builder.module,
            ypuc__iikn, name='memcmp')
        return builder.call(ozohd__orne, args)
...
class StrToFloat(AbstractTemplate):
    ...
    def generic(self, args, kws):
        assert not kws
        [vcmge__dhxwq] = args
        if isinstance(vcmge__dhxwq, StdStringType):
            return signature(types.float64, vcmge__dhxwq)
        if vcmge__dhxwq == string_type:
            return signature(types.float64, vcmge__dhxwq)
...
def gen_unicode_to_std_str(context, builder, unicode_val):
    gnv__xfpe = cgutils.create_struct_proxy(string_type)(context, builder,
        value=unicode_val)
    ypuc__iikn = lir.FunctionType(lir.IntType(8).as_pointer(), [lir.IntType
        (8).as_pointer(), lir.IntType(64)])
    fzi__del = cgutils.get_or_insert_function(builder.module, ypuc__iikn,
        name='init_string_const')
    return builder.call(fzi__del, [gnv__xfpe.data, gnv__xfpe.length])
...
def gen_std_str_to_unicode(context, builder, std_str_val, del_str=False):
    ...
    def _std_str_to_unicode(std_str):
        ...
        ykxye__vmfh = numba.cpython.unicode._empty_string(kind, length, 1)
        bodo.libs.str_arr_ext._memcpy(ykxye__vmfh._data, bodo.libs.str_ext.
...
class RandomAccessStringArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        qhh__anj = [('data', types.List(string_type))]
        models.StructModel.__init__(self, dmm, fe_type, qhh__anj)
...
def alloc_random_access_string_array(typingctx, n_t=None):
    ...
    def codegen(context, builder, sig, args):
        zifyg__xjwpo, = args
        vuvy__nyy = types.List(string_type)
        zlfn__cfg = numba.cpython.listobj.ListInstance.allocate(context,
            builder, vuvy__nyy, zifyg__xjwpo)
        zlfn__cfg.size = zifyg__xjwpo
        ifd__hupuw = cgutils.create_struct_proxy(sig.return_type)(context,
            builder)
        ifd__hupuw.data = zlfn__cfg.value
        return ifd__hupuw._getvalue()
...
def float_str_overload(v):
    if isinstance(v, types.Float):
        ...
        def impl(v):
            ...
            if tnjdq__tctg < 0:
                pnxpr__xwhny = 1
                tnjdq__tctg = -tnjdq__tctg
            if tnjdq__tctg < 1:
                bmmtk__xldvq = 1
            else:
                bmmtk__xldvq = 1 + int(np.floor(np.log10(tnjdq__tctg)))
            length = pnxpr__xwhny + bmmtk__xldvq + 1 + 6
...
def cast_str_to_float64(context, builder, fromty, toty, val):
    ypuc__iikn = lir.FunctionType(lir.DoubleType(), [lir.IntType(8).
        as_pointer()])
    fzi__del = cgutils.get_or_insert_function(builder.module, ypuc__iikn,
        name='str_to_float64')
    res = builder.call(fzi__del, (val,))
...
def cast_str_to_float32(context, builder, fromty, toty, val):
    ypuc__iikn = lir.FunctionType(lir.FloatType(), [lir.IntType(8).
        as_pointer()])
    fzi__del = cgutils.get_or_insert_function(builder.module, ypuc__iikn,
        name='str_to_float32')
    res = builder.call(fzi__del, (val,))
...
def cast_unicode_str_to_int64(context, builder, fromty, toty, val):
    gnv__xfpe = cgutils.create_struct_proxy(string_type)(context, builder,
        value=val)
    ypuc__iikn = lir.FunctionType(lir.IntType(toty.bitwidth), [lir.IntType(
        8).as_pointer(), lir.IntType(64)])
    fzi__del = cgutils.get_or_insert_function(builder.module, ypuc__iikn,
        name='str_to_int64')
    res = builder.call(fzi__del, (gnv__xfpe.data, gnv__xfpe.length))
    bodo.utils.utils.inlined_check_and_propagate_cpp_exception(context, builder
        )
    return res
...
class StringAttribute(AttributeTemplate):
    ...
    def resolve_format(self, string_typ, args, kws):
        kws = dict(kws)
        zxxx__hnkp = ', '.join('e{}'.format(koy__pncr) for koy__pncr in
            range(len(args)))
        if zxxx__hnkp:
            zxxx__hnkp += ', '
        ssf__inzyc = ', '.join("{} = ''".format(a) for a in kws.keys())
        awv__kyd = f'def format_stub(string, {zxxx__hnkp} {ssf__inzyc}):\n'
        awv__kyd += '    pass\n'
        yjqr__apezx = {}
        exec(awv__kyd, {}, yjqr__apezx)
        rene__gsmj = yjqr__apezx['format_stub']
        lfq__tzshc = numba.core.utils.pysignature(rene__gsmj)
        tsbid__xyze = (string_typ,) + args + tuple(kws.values())
        return signature(string_typ, tsbid__xyze).replace(pysig=lfq__tzshc)


@numba.njit(cache=True)
def str_split(arr, pat, n):
    veaam__rye = pat is not None and len(pat) > 1
    if veaam__rye:
        cawkd__edvek = re.compile(pat)
        if n == -1:
            n = 0
    elif n == 0:
        n = -1
    ...
    fcde__motwv = 0
    kpzd__lbjp = 0
    for koy__pncr in numba.parfors.parfor.internal_prange(zlfn__cfg):
        if bodo.libs.array_kernels.isna(arr, koy__pncr):
            continue
        if veaam__rye:
            uyx__ddun = cawkd__edvek.split(arr[koy__pncr], maxsplit=n)
        elif pat == '':
            uyx__ddun = [''] + list(arr[koy__pncr]) + ['']
        else:
            uyx__ddun = arr[koy__pncr].split(pat, n)
        fcde__motwv += len(uyx__ddun)
        for s in uyx__ddun:
            kpzd__lbjp += bodo.libs.str_arr_ext.get_utf8_size(s)
    qquej__qojk = bodo.libs.array_item_arr_ext.pre_alloc_array_item_array(
        zlfn__cfg, (fcde__motwv, kpzd__lbjp), bodo.libs.str_arr_ext.
        string_array_type)
    zbol__kmn = bodo.libs.array_item_arr_ext.get_offsets(qquej__qojk)
    rgra__qjtu = bodo.libs.array_item_arr_ext.get_null_bitmap(qquej__qojk)
    ifyj__ntvt = bodo.libs.array_item_arr_ext.get_data(qquej__qojk)
    obc__qwcc = 0
    for muw__kckma in numba.parfors.parfor.internal_prange(zlfn__cfg):
        zbol__kmn[muw__kckma] = obc__qwcc
        if bodo.libs.array_kernels.isna(arr, muw__kckma):
            bodo.libs.int_arr_ext.set_bit_to_arr(rgra__qjtu, muw__kckma, 0)
            continue
        bodo.libs.int_arr_ext.set_bit_to_arr(rgra__qjtu, muw__kckma, 1)
        if veaam__rye:
            uyx__ddun = cawkd__edvek.split(arr[muw__kckma], maxsplit=n)
        elif pat == '':
            uyx__ddun = [''] + list(arr[muw__kckma]) + ['']
        else:
            uyx__ddun = arr[muw__kckma].split(pat, n)
        itcsz__dtyh = len(uyx__ddun)
        for kgnpp__bjzlt in range(itcsz__dtyh):
            s = uyx__ddun[kgnpp__bjzlt]
            ifyj__ntvt[obc__qwcc] = s
            obc__qwcc += 1
    zbol__kmn[zlfn__cfg] = obc__qwcc
    return qquej__qojk
    ...
    if isinstance(x, types.Integer):
        ...
        def impl(x):
            ...
            if x == 0:
                ...
                onokw__iao = fast_ceil_log2(x + 1)
                onokw__iao = (onokw__iao + 3) // 4
            length = len(lcx__viq) + onokw__iao
            output = numba.cpython.unicode._empty_string(kind, length, 1)
            bodo.libs.str_arr_ext._memcpy(output._data, lcx__viq._data, len
                (lcx__viq), 1)
            int_to_hex(output, onokw__iao, len(lcx__viq), x)
            return output
        return impl
...
def fast_ceil_log2(x):
    ieiyt__zvlw = 0 if x & x - 1 == 0 else 1
    cxlt__yist = [np.uint64(18446744069414584320), np.uint64(4294901760),
        np.uint64(65280), np.uint64(240), np.uint64(12), np.uint64(2)]
    mjma__cdq = 32
    for koy__pncr in range(len(cxlt__yist)):
        yiuut__jxp = 0 if x & cxlt__yist[koy__pncr] == 0 else mjma__cdq
        ieiyt__zvlw = ieiyt__zvlw + yiuut__jxp
        x = x >> yiuut__jxp
        mjma__cdq = mjma__cdq >> 1
    return ieiyt__zvlw
...
def int_to_hex(typingctx, output, out_len, header_len, int_val):
    ...
    def codegen(context, builder, sig, args):
        ...
        fnx__wfj = cgutils.create_struct_proxy(sig.args[0])(context,
            builder, value=output)
        ypuc__iikn = lir.FunctionType(lir.IntType(8).as_pointer(), [lir.
            IntType(8).as_pointer(), lir.IntType(64), lir.IntType(64)])
        mqy__mncxf = cgutils.get_or_insert_function(builder.module,
            ypuc__iikn, name='int_to_hex')
        alzie__gjniu = builder.inttoptr(builder.add(builder.ptrtoint(
            fnx__wfj.data, lir.IntType(64)), header_len), lir.IntType(8).
            as_pointer())
        builder.call(mqy__mncxf, (alzie__gjniu, out_len, int_val))
