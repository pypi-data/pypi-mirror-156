...
def _column_filter_impl(B, ind):
    uke__efdtb = bodo.hiframes.rolling.alloc_shift(len(B), B, (-1,))
    for ynpmk__tfx in numba.parfors.parfor.internal_prange(len(uke__efdtb)):
        if ind[ynpmk__tfx]:
            uke__efdtb[ynpmk__tfx] = B[ynpmk__tfx]
        else:
            bodo.libs.array_kernels.setna(uke__efdtb, ynpmk__tfx)
    return uke__efdtb
...
def _series_dropna_str_alloc_impl_inner(B):
    ...
    jlvf__vodqt = len(B)
    mpajv__uti = 0
    for ynpmk__tfx in range(len(B)):
        if bodo.libs.str_arr_ext.str_arr_is_na(B, ynpmk__tfx):
            mpajv__uti += 1
    ngx__tjll = jlvf__vodqt - mpajv__uti
    gtpuk__dgvbx = bodo.libs.str_arr_ext.num_total_chars(B)
    uke__efdtb = bodo.libs.str_arr_ext.pre_alloc_string_array(ngx__tjll,
        gtpuk__dgvbx)
    bodo.libs.str_arr_ext.copy_non_null_offsets(uke__efdtb, B)
    bodo.libs.str_arr_ext.copy_data(uke__efdtb, B)
    bodo.libs.str_arr_ext.set_null_bits_to_value(uke__efdtb, -1)
    return uke__efdtb
...
def compute_skew(first_moment, second_moment, third_moment, count):
    if count < 3:
        return np.nan
    oknz__ftgr = first_moment / count
    xjaqp__wkq = (third_moment - 3 * second_moment * oknz__ftgr + 2 * count *
        oknz__ftgr ** 3)
    hah__rgc = second_moment - oknz__ftgr * first_moment
    s = count * (count - 1) ** 1.5 / (count - 2) * xjaqp__wkq / hah__rgc ** 1.5
    s = s / (count - 1)
    return s
...
def compute_kurt(first_moment, second_moment, third_moment, fourth_moment,
    ...
    if count < 4:
        return np.nan
    oknz__ftgr = first_moment / count
    xozr__roycy = (fourth_moment - 4 * third_moment * oknz__ftgr + 6 *
        second_moment * oknz__ftgr ** 2 - 3 * count * oknz__ftgr ** 4)
    kydi__fom = second_moment - oknz__ftgr * first_moment
    enhv__lio = 3 * (count - 1) ** 2 / ((count - 2) * (count - 3))
    ita__imfi = count * (count + 1) * (count - 1) * xozr__roycy
    rpv__fiu = (count - 2) * (count - 3) * kydi__fom ** 2
    s = (count - 1) * (ita__imfi / rpv__fiu - enhv__lio)
    s = s / (count - 1)
    return s
