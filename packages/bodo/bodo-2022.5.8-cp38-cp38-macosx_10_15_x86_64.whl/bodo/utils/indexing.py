...
def get_new_null_mask_bool_index(old_mask, ind, n):
    bhk__rxzb = n + 7 >> 3
    jhs__yvo = np.empty(bhk__rxzb, np.uint8)
    adfrd__csede = 0
    for tbteo__lxf in range(len(ind)):
        if ind[tbteo__lxf]:
            jwz__aaq = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
                tbteo__lxf)
            bodo.libs.int_arr_ext.set_bit_to_arr(jhs__yvo, adfrd__csede,
                jwz__aaq)
            adfrd__csede += 1
    return jhs__yvo
...
def array_getitem_bool_index(A, ind):
    ...
    sul__kzzt = A._data[ind]
    n = len(sul__kzzt)
    jhs__yvo = get_new_null_mask_bool_index(old_mask, ind, n)
    return sul__kzzt, jhs__yvo
...
def get_new_null_mask_int_index(old_mask, ind, n):
    ...
    adfrd__csede = 0
    for tbteo__lxf in range(len(ind)):
        jwz__aaq = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask, ind[
            tbteo__lxf])
        bodo.libs.int_arr_ext.set_bit_to_arr(jhs__yvo, adfrd__csede, jwz__aaq)
        adfrd__csede += 1
    return jhs__yvo
...
def array_getitem_int_index(A, ind):
    fli__hhdmm = bodo.utils.conversion.coerce_to_ndarray(ind)
    old_mask = A._null_bitmap
    sul__kzzt = A._data[fli__hhdmm]
    n = len(sul__kzzt)
    jhs__yvo = get_new_null_mask_int_index(old_mask, fli__hhdmm, n)
    return sul__kzzt, jhs__yvo


@register_jitable
def get_new_null_mask_slice_index(old_mask, ind, n):
    dlsbn__hxly = numba.cpython.unicode._normalize_slice(ind, n)
    mtg__cjicg = numba.cpython.unicode._slice_span(dlsbn__hxly)
    bhk__rxzb = mtg__cjicg + 7 >> 3
    jhs__yvo = np.empty(bhk__rxzb, np.uint8)
    adfrd__csede = 0
    for tbteo__lxf in range(dlsbn__hxly.start, dlsbn__hxly.stop,
        dlsbn__hxly.step):
        jwz__aaq = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
            tbteo__lxf)
        bodo.libs.int_arr_ext.set_bit_to_arr(jhs__yvo, adfrd__csede, jwz__aaq)
...
def array_getitem_slice_index(A, ind):
    n = len(A._data)
    old_mask = A._null_bitmap
    sul__kzzt = np.ascontiguousarray(A._data[ind])
    jhs__yvo = get_new_null_mask_slice_index(old_mask, ind, n)
    return sul__kzzt, jhs__yvo
...
def array_setitem_int_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            n = len(val._data)
            for tbteo__lxf in range(n):
                A._data[idx[tbteo__lxf]] = val._data[tbteo__lxf]
                jwz__aaq = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val.
                    _null_bitmap, tbteo__lxf)
    ...
    if bodo.utils.typing.is_scalar_type(val):
        ...
        def impl_scalar(A, idx, val):
            for tbteo__lxf in idx:
                A._data[tbteo__lxf] = val
...
def array_setitem_bool_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            bivn__agpi = 0
            for tbteo__lxf in range(n):
                if not bodo.libs.array_kernels.isna(idx, tbteo__lxf) and idx[
                    tbteo__lxf]:
                    A._data[tbteo__lxf] = val._data[bivn__agpi]
                    jwz__aaq = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val
                        ._null_bitmap, bivn__agpi)
                    bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                        tbteo__lxf, jwz__aaq)
                    bivn__agpi += 1
        return impl_arr
...
def setitem_slice_index_null_bits(dst_bitmap, src_bitmap, idx, n):
    dlsbn__hxly = numba.cpython.unicode._normalize_slice(idx, n)
    bivn__agpi = 0
    for tbteo__lxf in range(dlsbn__hxly.start, dlsbn__hxly.stop,
        dlsbn__hxly.step):
        jwz__aaq = bodo.libs.int_arr_ext.get_bit_bitmap_arr(src_bitmap,
            bivn__agpi)
        bodo.libs.int_arr_ext.set_bit_to_arr(dst_bitmap, tbteo__lxf, jwz__aaq)
        bivn__agpi += 1
...
def array_setitem_slice_index_overload(A, idx, val):
    ...
    if bodo.utils.typing.is_scalar_type(val):
        ...
        def impl_scalar(A, idx, val):
            dlsbn__hxly = numba.cpython.unicode._normalize_slice(idx, len(A))
            for tbteo__lxf in range(dlsbn__hxly.start, dlsbn__hxly.stop,
                dlsbn__hxly.step):
                A._data[tbteo__lxf] = val
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
    ...
    if not bodo.utils.utils.is_array_typ(A, False):
        ...
        if isinstance(idx, types.Integer):
            ...
            def setitem_none_int_arr(A, idx, val):
                ...
                for tbteo__lxf in idx:
                    bodo.libs.array_kernels.setna(A, tbteo__lxf)
            ...
            if A == bodo.string_array_type:
                ...
                def string_arr_impl(A, idx, val):
                    n = len(A)
                    idx = bodo.utils.conversion.coerce_to_ndarray(idx)
                    smi__nvnjq = bodo.libs.str_arr_ext.pre_alloc_string_array(n
                        , -1)
                    for tbteo__lxf in numba.parfors.parfor.internal_prange(n):
                        if idx[tbteo__lxf] or bodo.libs.array_kernels.isna(A,
                            tbteo__lxf):
                            smi__nvnjq[tbteo__lxf] = ''
                            bodo.libs.str_arr_ext.str_arr_set_na(smi__nvnjq,
                                tbteo__lxf)
                        else:
                            smi__nvnjq[tbteo__lxf] = A[tbteo__lxf]
            ...
            def setitem_none_bool_arr(A, idx, val):
                ...
                n = len(idx)
                for tbteo__lxf in range(n):
                    if not bodo.libs.array_kernels.isna(idx, tbteo__lxf
                        ) and idx[tbteo__lxf]:
                        bodo.libs.array_kernels.setna(A, tbteo__lxf)
            ...
            def setitem_optional_int_arr(A, idx, val):
                ...
                for tbteo__lxf in idx:
                    if val is None:
                        bodo.libs.array_kernels.setna(A, tbteo__lxf)
                        continue
                    A[tbteo__lxf] = bodo.utils.indexing.unoptional(val)
...
def unoptional(typingctx, val_t=None):
    ...
    def codegen(context, builder, signature, args):
        tbtb__wnog = context.make_helper(builder, val_t, args[0])
        wqou__jgxi = tbtb__wnog.data
        context.nrt.incref(builder, val_t.type, wqou__jgxi)
        return wqou__jgxi
