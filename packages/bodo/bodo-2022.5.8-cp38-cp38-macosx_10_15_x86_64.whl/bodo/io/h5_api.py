...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        jpnw__esu = []
        ece__eef = h5g_get_num_objs(obj_id)
        for pogxa__gepzz in range(ece__eef):
            aboq__bgks = h5g_get_objname_by_idx(obj_id, pogxa__gepzz)
            jpnw__esu.append(aboq__bgks)
        return jpnw__esu
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    bcuo__jkx = parse_dtype(dtype)
    fdt__rbkk = np.int32(numba_to_c_type(bcuo__jkx))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            rmquw__sqghm = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    pec__pre = tuple_to_ptr(starts)
    virb__wzsea = tuple_to_ptr(counts)
    cac__thl = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, pec__pre, virb__wzsea,
        is_parallel, out_arr.ctypes, cac__thl)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    virb__wzsea = tuple_to_ptr(counts)
    cac__thl = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, pec__pre, virb__wzsea,
        is_parallel, out_arr.ctypes, cac__thl)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        hczr__ziw = types.Array(dtype, ndim, 'C')
        zie__edlt = args[3]
        hczr__ziw = self.context.resolve_function_type(operator.getitem, [
            hczr__ziw, zie__edlt], {}).return_type.copy(layout='C')
        return signature(hczr__ziw, *args)
...
def get_filter_read_indices(bool_arr):
    kmjpp__dmwo = bool_arr.nonzero()[0]
    mkug__axck = bodo.libs.distributed_api.get_rank()
    dof__jtqwf = bodo.libs.distributed_api.get_size()
    wan__rwdhh = np.empty(dof__jtqwf, np.int64)
    kbk__yzb = len(bool_arr)
    bodo.libs.distributed_api.allgather(wan__rwdhh, kbk__yzb)
    vjjyp__ipgu = wan__rwdhh.cumsum()[mkug__axck] - kbk__yzb
    kmjpp__dmwo += vjjyp__ipgu
    ndd__bbruu = bodo.libs.distributed_api.dist_reduce(len(kmjpp__dmwo), np
        .int32(sum_op))
    kre__wjbo = bodo.libs.distributed_api.gatherv(kmjpp__dmwo)
    if mkug__axck == 0:
        urpp__luxyp = kre__wjbo
    else:
        urpp__luxyp = np.empty(ndd__bbruu, kmjpp__dmwo.dtype)
    bodo.libs.distributed_api.bcast(urpp__luxyp)
    fljq__tzp = bodo.libs.distributed_api.get_start(ndd__bbruu, dof__jtqwf,
        mkug__axck)
    nkvr__vbo = bodo.libs.distributed_api.get_end(ndd__bbruu, dof__jtqwf,
        mkug__axck)
    return urpp__luxyp[fljq__tzp:nkvr__vbo]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        khn__hrvv = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], khn__hrvv)
        return builder.bitcast(khn__hrvv, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    virb__wzsea = tuple_to_ptr(counts)
    cac__thl = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, pec__pre,
        virb__wzsea, is_parallel, out_arr.ctypes, cac__thl, read_indices.
