...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        cxhu__wceyv = context.get_abi_sizeof(context.get_data_type(dtype.dtype)
            )
        return context.get_constant(types.intp, cxhu__wceyv)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            xrbb__vpa = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(xrbb__vpa.dtype)
            file_write(unicode_to_utf8(fname), xrbb__vpa.ctypes, dtype_size *
                xrbb__vpa.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            xrbb__vpa = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(xrbb__vpa.dtype)
            jeq__ouy = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                xrbb__vpa.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), xrbb__vpa.ctypes,
                start, count, jeq__ouy)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            yirt__pbel = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                yirt__pbel = min(yirt__pbel, count * dtype_size)
            if yirt__pbel < 0:
                return -1
            return yirt__pbel
        return impl
