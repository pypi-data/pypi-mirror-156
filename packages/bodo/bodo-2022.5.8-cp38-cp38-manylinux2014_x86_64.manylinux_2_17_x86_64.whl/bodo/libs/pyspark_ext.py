...
def box_spark_session(typ, val, c):
    rrozm__jov = c.context.insert_const_string(c.builder.module, 'pyspark')
    dii__dpc = c.pyapi.import_module_noblock(rrozm__jov)
    iijq__hwf = c.pyapi.object_getattr_string(dii__dpc, 'sql')
    dnio__khqgl = c.pyapi.object_getattr_string(iijq__hwf, 'SparkSession')
    piupg__ysxfl = c.pyapi.object_getattr_string(dnio__khqgl, 'builder')
    xtqk__nsi = c.pyapi.call_method(piupg__ysxfl, 'getOrCreate', ())
    c.pyapi.decref(dii__dpc)
    c.pyapi.decref(iijq__hwf)
    c.pyapi.decref(dnio__khqgl)
    c.pyapi.decref(piupg__ysxfl)
    return xtqk__nsi
...
class RowType(types.BaseNamedTuple):
    ...
    def __init__(self, types, fields):
        ...
        bhhti__sgc = 'Row({})'.format(', '.join(
            f'{qmzb__jkwmo}:{dxu__ufqno}' for qmzb__jkwmo, dxu__ufqno in
            zip(self.fields, self.types)))
        super(RowType, self).__init__(bhhti__sgc)
...
class RowModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        qltv__uwdq = [(qmzb__jkwmo, dxu__ufqno) for qmzb__jkwmo, dxu__ufqno in
            zip(fe_type.fields, fe_type.types)]
        super(RowModel, self).__init__(dmm, fe_type, qltv__uwdq)
...
def typeof_row(val, c):
    ...
    return RowType(tuple(numba.typeof(din__mwhyj) for din__mwhyj in val),
        fields)


@box(RowType)
def box_row(typ, val, c):
    jhsb__auj = c.pyapi.unserialize(c.pyapi.serialize_object(pyspark.sql.
        types.Row))
    if all(qmzb__jkwmo.startswith(ANON_SENTINEL) for qmzb__jkwmo in typ.fields
        ):
        xwam__ond = [c.box(dxu__ufqno, c.builder.extract_value(val, i)) for
            i, dxu__ufqno in enumerate(typ.types)]
        jirqt__hitg = c.pyapi.call_function_objargs(jhsb__auj, xwam__ond)
        for obj in xwam__ond:
            c.pyapi.decref(obj)
        c.pyapi.decref(jhsb__auj)
        return jirqt__hitg
    args = c.pyapi.tuple_pack([])
    xwam__ond = []
    uqg__ctnbz = []
    for i, dxu__ufqno in enumerate(typ.types):
        hot__tyu = c.builder.extract_value(val, i)
        obj = c.box(dxu__ufqno, hot__tyu)
        uqg__ctnbz.append((typ.fields[i], obj))
        xwam__ond.append(obj)
    kws = c.pyapi.dict_pack(uqg__ctnbz)
    jirqt__hitg = c.pyapi.call(jhsb__auj, args, kws)
    for obj in xwam__ond:
        c.pyapi.decref(obj)
    c.pyapi.decref(jhsb__auj)
    c.pyapi.decref(args)
...
class RowConstructor(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        mijke__lfht = ', '.join(f'arg{i}' for i in range(len(args)))
        pevlb__vqqi = ', '.join(f"{tkmcl__eflmj} = ''" for tkmcl__eflmj in kws)
        func_text = f'def row_stub({mijke__lfht}{pevlb__vqqi}):\n'
        func_text += '    pass\n'
        buzbn__moyxw = {}
        exec(func_text, {}, buzbn__moyxw)
        awvz__urrn = buzbn__moyxw['row_stub']
        pns__angnl = numba.core.utils.pysignature(awvz__urrn)
        if args:
            segqx__ouf = RowType(args, tuple(f'{ANON_SENTINEL}{i}' for i in
                range(len(args))))
            return signature(segqx__ouf, *args).replace(pysig=pns__angnl)
        kws = dict(kws)
        segqx__ouf = RowType(tuple(kws.values()), tuple(kws.keys()))
        return signature(segqx__ouf, *kws.values()).replace(pysig=pns__angnl)
...
class SparkDataFrameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        qltv__uwdq = [('df', fe_type.df)]
        super(SparkDataFrameModel, self).__init__(dmm, fe_type, qltv__uwdq)
...
def overload_create_df(sp_session, data, schema=None, samplingRatio=None,
    ...
    uzl__razn = data.dtype.fields
    blv__zmp = len(data.dtype.types)
    func_text = (
        'def impl(sp_session, data, schema=None, samplingRatio=None, verifySchema=True):\n'
        )
    func_text += f'  n = len(data)\n'
    jvbw__ulmw = []
    for i, dxu__ufqno in enumerate(data.dtype.types):
        rta__deh = dtype_to_array_type(dxu__ufqno)
        func_text += (
            f'  A{i} = bodo.utils.utils.alloc_type(n, arr_typ{i}, (-1,))\n')
        jvbw__ulmw.append(rta__deh)
    func_text += f'  for i in range(n):\n'
    func_text += f'    r = data[i]\n'
    for i in range(blv__zmp):
        func_text += (
            f'    A{i}[i] = bodo.utils.conversion.unbox_if_timestamp(r[{i}])\n'
            )
    szplt__ixti = '({}{})'.format(', '.join(f'A{i}' for i in range(blv__zmp
        )), ',' if len(uzl__razn) == 1 else '')
    func_text += (
        '  index = bodo.hiframes.pd_index_ext.init_range_index(0, n, 1, None)\n'
        )
    func_text += f"""  pdf = bodo.hiframes.pd_dataframe_ext.init_dataframe({szplt__ixti}, index, __col_name_meta_value_create_df)
"""
    func_text += f'  pdf = bodo.scatterv(pdf)\n'
    func_text += f'  return bodo.libs.pyspark_ext.init_spark_df(pdf)\n'
    buzbn__moyxw = {}
    xgmhf__makoh = {'bodo': bodo, '__col_name_meta_value_create_df':
        ColNamesMetaType(tuple(uzl__razn))}
    for i in range(blv__zmp):
        xgmhf__makoh[f'arr_typ{i}'] = jvbw__ulmw[i]
    exec(func_text, xgmhf__makoh, buzbn__moyxw)
    impl = buzbn__moyxw['impl']
    return impl
...
def overload_df_to_rows(df):
    ...
    ltepi__lzaq = ', '.join(f'{c}=A{i}[i]' for i, c in enumerate(df.columns))
    func_text += f'    out.append(Row({ltepi__lzaq}))\n'
    func_text += '  return out\n'
    buzbn__moyxw = {}
    xgmhf__makoh = {'bodo': bodo, 'Row': pyspark.sql.types.Row}
    exec(func_text, xgmhf__makoh, buzbn__moyxw)
    impl = buzbn__moyxw['impl']
    return impl
...
def _gen_df_select(spark_df, cols, avoid_stararg=False):
    ...
    for col in cols:
        ...
        data, dqnd__bjwmy = _gen_col_code(col, df_type)
        func_text += dqnd__bjwmy
        out_data.append(data)
    return _gen_init_spark_df(func_text, out_data, out_col_names)


def _gen_init_spark_df(func_text, out_data, out_col_names):
    szplt__ixti = '({}{})'.format(', '.join(out_data), ',' if len(out_data) ==
        1 else '')
    fiyvx__hoe = '0' if not out_data else f'len({out_data[0]})'
    func_text += f'  n = {fiyvx__hoe}\n'
    func_text += (
        '  index = bodo.hiframes.pd_index_ext.init_range_index(0, n, 1, None)\n'
        )
    func_text += f"""  pdf = bodo.hiframes.pd_dataframe_ext.init_dataframe({szplt__ixti}, index, __col_name_meta_value_init_spark_df)
"""
    func_text += f'  return bodo.libs.pyspark_ext.init_spark_df(pdf)\n'
    buzbn__moyxw = {}
    xgmhf__makoh = {'bodo': bodo, 'np': np,
...
def overload_show(spark_df, n=20, truncate=True, vertical=False):
    soqir__eii = dict(truncate=truncate, vertical=vertical)
    mbd__ercy = dict(truncate=True, vertical=False)
    check_unsupported_args('SparkDataFrameType.show', soqir__eii, mbd__ercy)
...
def overload_with_column(spark_df, colName, col):
    ...
    sjo__mlvq = spark_df.df.columns
    mgbb__qviq = sjo__mlvq if col_name in sjo__mlvq else sjo__mlvq + (col_name,
        )
    vgmw__otnis, sto__fivwh = _gen_col_code(col, spark_df.df)
    out_data = [(vgmw__otnis if c == col_name else
        f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {sjo__mlvq.index(c)})'
        ) for c in mgbb__qviq]
    func_text = 'def impl(spark_df, colName, col):\n'
    func_text += '  df = spark_df._df\n'
    func_text += sto__fivwh
    return _gen_init_spark_df(func_text, out_data, mgbb__qviq)
...
def overload_with_column_renamed(spark_df, existing, new):
    ...
    jwzd__uhhz = get_overload_const_str(existing)
    ufpuf__ycaxy = get_overload_const_str(new)
    sjo__mlvq = spark_df.df.columns
    mgbb__qviq = tuple(ufpuf__ycaxy if c == jwzd__uhhz else c for c in
        sjo__mlvq)
    out_data = [f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i})'
         for i in range(len(sjo__mlvq))]
...
def overload_dataframe_columns(spark_df):
    tgtmo__zkvls = list(str(tkmcl__eflmj) for tkmcl__eflmj in spark_df.df.
        columns)
    func_text = 'def impl(spark_df):\n'
    func_text += f'  return {tgtmo__zkvls}\n'
    buzbn__moyxw = {}
    exec(func_text, {}, buzbn__moyxw)
    impl = buzbn__moyxw['impl']
    return impl
...
def init_col_from_name(typingctx, col=None):
    ...
    ieezz__sxtv = get_overload_const_str(col)
    swt__cxgg = ColumnType(ExprType('col', (ieezz__sxtv,)))

    def codegen(context, builder, signature, args):
        return context.get_constant_null(swt__cxgg)
    return swt__cxgg(col), codegen
...
def init_f_sum(typingctx, col=None):
    swt__cxgg = ColumnType(ExprType('sum', (col.expr,)))
...
def _gen_col_code_expr(expr, df_type):
    ...
    if expr.op == 'sum':
        gzozo__hquqa, qucmd__rnlc = _gen_col_code_expr(expr.children[0],
            df_type)
        i = ir_utils.next_label()
        func_text = f"""  A{i} = np.asarray([bodo.libs.array_ops.array_op_sum({gzozo__hquqa}, True, 0)])
"""
        return f'A{i}', qucmd__rnlc + func_text


def _gen_col_code_colname(col_name, df_type):
    sspk__kfay = df_type.columns.index(col_name)
    i = ir_utils.next_label()
    func_text = (
        f'  A{i} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {sspk__kfay})\n'
        )
