"""
Support for Series.str methods
"""
import operator
import re
import numba
import numpy as np
from numba.core import cgutils, types
from numba.extending import intrinsic, make_attribute_wrapper, models, overload, overload_attribute, overload_method, register_model
import bodo
from bodo.hiframes.pd_dataframe_ext import DataFrameType
from bodo.hiframes.pd_index_ext import StringIndexType
from bodo.hiframes.pd_series_ext import SeriesType
from bodo.hiframes.split_impl import get_split_view_data_ptr, get_split_view_index, string_array_split_view_type
from bodo.libs.array import get_search_regex
from bodo.libs.array_item_arr_ext import ArrayItemArrayType
from bodo.libs.str_arr_ext import get_utf8_size, pre_alloc_string_array, string_array_type
from bodo.libs.str_ext import str_findall_count
from bodo.utils.typing import BodoError, create_unsupported_overload, get_overload_const_int, get_overload_const_list, get_overload_const_str, get_overload_const_str_len, is_list_like_index_type, is_overload_constant_bool, is_overload_constant_int, is_overload_constant_list, is_overload_constant_str, is_overload_false, is_overload_none, is_overload_true, is_str_arr_type, raise_bodo_error


class SeriesStrMethodType(types.Type):

    def __init__(self, stype):
        self.stype = stype
        mno__blkk = 'SeriesStrMethodType({})'.format(stype)
        super(SeriesStrMethodType, self).__init__(mno__blkk)


@register_model(SeriesStrMethodType)
class SeriesStrModel(models.StructModel):

    def __init__(self, dmm, fe_type):
        mlu__qpt = [('obj', fe_type.stype)]
        super(SeriesStrModel, self).__init__(dmm, fe_type, mlu__qpt)


make_attribute_wrapper(SeriesStrMethodType, 'obj', '_obj')


@intrinsic
def init_series_str_method(typingctx, obj=None):

    def codegen(context, builder, signature, args):
        xnqlj__abwaz, = args
        opbte__ugv = signature.return_type
        car__kdewi = cgutils.create_struct_proxy(opbte__ugv)(context, builder)
        car__kdewi.obj = xnqlj__abwaz
        context.nrt.incref(builder, signature.args[0], xnqlj__abwaz)
        return car__kdewi._getvalue()
    return SeriesStrMethodType(obj)(obj), codegen


def str_arg_check(func_name, arg_name, arg):
    if not isinstance(arg, types.UnicodeType) and not is_overload_constant_str(
        arg):
        raise_bodo_error(
            "Series.str.{}(): parameter '{}' expected a string object, not {}"
            .format(func_name, arg_name, arg))


def int_arg_check(func_name, arg_name, arg):
    if not isinstance(arg, types.Integer) and not is_overload_constant_int(arg
        ):
        raise BodoError(
            "Series.str.{}(): parameter '{}' expected an int object, not {}"
            .format(func_name, arg_name, arg))


def not_supported_arg_check(func_name, arg_name, arg, defval):
    if arg_name == 'na':
        if not isinstance(arg, types.Omitted) and (not isinstance(arg,
            float) or not np.isnan(arg)):
            raise BodoError(
                "Series.str.{}(): parameter '{}' is not supported, default: np.nan"
                .format(func_name, arg_name))
    elif not isinstance(arg, types.Omitted) and arg != defval:
        raise BodoError(
            "Series.str.{}(): parameter '{}' is not supported, default: {}"
            .format(func_name, arg_name, defval))


def common_validate_padding(func_name, width, fillchar):
    if is_overload_constant_str(fillchar):
        if get_overload_const_str_len(fillchar) != 1:
            raise BodoError(
                'Series.str.{}(): fillchar must be a character, not str'.
                format(func_name))
    elif not isinstance(fillchar, types.UnicodeType):
        raise BodoError('Series.str.{}(): fillchar must be a character, not {}'
            .format(func_name, fillchar))
    int_arg_check(func_name, 'width', width)


@overload_attribute(SeriesType, 'str')
def overload_series_str(S):
    if not (is_str_arr_type(S.data) or S.data ==
        string_array_split_view_type or isinstance(S.data, ArrayItemArrayType)
        ):
        raise_bodo_error(
            'Series.str: input should be a series of string or arrays')
    return lambda S: bodo.hiframes.series_str_impl.init_series_str_method(S)


@overload_method(SeriesStrMethodType, 'len', inline='always', no_unliteral=True
    )
def overload_str_method_len(S_str):
    if S_str.stype.data == bodo.dict_str_arr_type:

        def _str_len_dict_impl(S_str):
            S = S_str._obj
            fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
            xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
            mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
            out_arr = bodo.libs.dict_arr_ext.str_len(fuhzm__qes)
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                xaszs__rjqi, mno__blkk)
        return _str_len_dict_impl

    def impl(S_str):
        S = S_str._obj
        fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
        xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
        mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
        out_arr = bodo.libs.array_kernels.get_arr_lens(fuhzm__qes, False)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, xaszs__rjqi,
            mno__blkk)
    return impl


@overload_method(SeriesStrMethodType, 'split', inline='always',
    no_unliteral=True)
def overload_str_method_split(S_str, pat=None, n=-1, expand=False):
    if not is_overload_none(pat):
        str_arg_check('split', 'pat', pat)
    int_arg_check('split', 'n', n)
    not_supported_arg_check('split', 'expand', expand, False)
    if is_overload_constant_str(pat) and len(get_overload_const_str(pat)
        ) == 1 and get_overload_const_str(pat).isascii(
        ) and is_overload_constant_int(n) and get_overload_const_int(n
        ) == -1 and S_str.stype.data == string_array_type:

        def _str_split_view_impl(S_str, pat=None, n=-1, expand=False):
            S = S_str._obj
            fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
            xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
            mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
            out_arr = bodo.hiframes.split_impl.compute_split_view(fuhzm__qes,
                pat)
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                xaszs__rjqi, mno__blkk)
        return _str_split_view_impl

    def _str_split_impl(S_str, pat=None, n=-1, expand=False):
        S = S_str._obj
        fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
        xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
        mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
        out_arr = bodo.libs.str_ext.str_split(fuhzm__qes, pat, n)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, xaszs__rjqi,
            mno__blkk)
    return _str_split_impl


@overload_method(SeriesStrMethodType, 'get', no_unliteral=True)
def overload_str_method_get(S_str, i):
    epcg__mrj = S_str.stype.data
    if (epcg__mrj != string_array_split_view_type and not is_str_arr_type(
        epcg__mrj)) and not isinstance(epcg__mrj, ArrayItemArrayType):
        raise_bodo_error(
            'Series.str.get(): only supports input type of Series(array(item)) and Series(str)'
            )
    int_arg_check('get', 'i', i)
    if isinstance(epcg__mrj, ArrayItemArrayType):

        def _str_get_array_impl(S_str, i):
            S = S_str._obj
            fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
            xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
            mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
            out_arr = bodo.libs.array_kernels.get(fuhzm__qes, i)
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                xaszs__rjqi, mno__blkk)
        return _str_get_array_impl
    if epcg__mrj == string_array_split_view_type:

        def _str_get_split_impl(S_str, i):
            S = S_str._obj
            fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
            xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
            mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
            numba.parfors.parfor.init_prange()
            n = len(fuhzm__qes)
            tavaz__zfs = 0
            for yeh__qadef in numba.parfors.parfor.internal_prange(n):
                gxw__waaca, gxw__waaca, zjk__cpt = get_split_view_index(
                    fuhzm__qes, yeh__qadef, i)
                tavaz__zfs += zjk__cpt
            numba.parfors.parfor.init_prange()
            out_arr = pre_alloc_string_array(n, tavaz__zfs)
            for smnk__grq in numba.parfors.parfor.internal_prange(n):
                hmo__vjil, odwzd__wffa, zjk__cpt = get_split_view_index(
                    fuhzm__qes, smnk__grq, i)
                if hmo__vjil == 0:
                    bodo.libs.array_kernels.setna(out_arr, smnk__grq)
                    vfi__rtq = get_split_view_data_ptr(fuhzm__qes, 0)
                else:
                    bodo.libs.str_arr_ext.str_arr_set_not_na(out_arr, smnk__grq
                        )
                    vfi__rtq = get_split_view_data_ptr(fuhzm__qes, odwzd__wffa)
                bodo.libs.str_arr_ext.setitem_str_arr_ptr(out_arr,
                    smnk__grq, vfi__rtq, zjk__cpt)
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                xaszs__rjqi, mno__blkk)
        return _str_get_split_impl
    if S_str.stype.data == bodo.dict_str_arr_type:

        def _str_get_dict_impl(S_str, i):
            S = S_str._obj
            fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
            xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
            mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
            out_arr = bodo.libs.dict_arr_ext.str_get(fuhzm__qes, i)
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                xaszs__rjqi, mno__blkk)
        return _str_get_dict_impl

    def _str_get_impl(S_str, i):
        S = S_str._obj
        fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
        xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
        mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
        numba.parfors.parfor.init_prange()
        n = len(fuhzm__qes)
        numba.parfors.parfor.init_prange()
        out_arr = pre_alloc_string_array(n, -1)
        for smnk__grq in numba.parfors.parfor.internal_prange(n):
            if bodo.libs.array_kernels.isna(fuhzm__qes, smnk__grq) or not len(
                fuhzm__qes[smnk__grq]) > i >= -len(fuhzm__qes[smnk__grq]):
                out_arr[smnk__grq] = ''
                bodo.libs.array_kernels.setna(out_arr, smnk__grq)
            else:
                out_arr[smnk__grq] = fuhzm__qes[smnk__grq][i]
        return bodo.hiframes.pd_series_ext.init_series(out_arr, xaszs__rjqi,
            mno__blkk)
    return _str_get_impl


@overload_method(SeriesStrMethodType, 'join', inline='always', no_unliteral
    =True)
def overload_str_method_join(S_str, sep):
    epcg__mrj = S_str.stype.data
    if (epcg__mrj != string_array_split_view_type and epcg__mrj !=
        ArrayItemArrayType(string_array_type) and not is_str_arr_type(
        epcg__mrj)):
        raise_bodo_error(
            'Series.str.join(): only supports input type of Series(list(str)) and Series(str)'
            )
    str_arg_check('join', 'sep', sep)

    def impl(S_str, sep):
        S = S_str._obj
        bxv__fraek = bodo.hiframes.pd_series_ext.get_series_data(S)
        mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
        xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
        numba.parfors.parfor.init_prange()
        n = len(bxv__fraek)
        out_arr = bodo.libs.str_arr_ext.pre_alloc_string_array(n, -1)
        for smnk__grq in numba.parfors.parfor.internal_prange(n):
            if bodo.libs.array_kernels.isna(bxv__fraek, smnk__grq):
                out_arr[smnk__grq] = ''
                bodo.libs.array_kernels.setna(out_arr, smnk__grq)
            else:
                ybwj__nzc = bxv__fraek[smnk__grq]
                out_arr[smnk__grq] = sep.join(ybwj__nzc)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, xaszs__rjqi,
            mno__blkk)
    return impl


@overload_method(SeriesStrMethodType, 'replace', inline='always',
    no_unliteral=True)
def overload_str_method_replace(S_str, pat, repl, n=-1, case=None, flags=0,
    regex=True):
    not_supported_arg_check('replace', 'n', n, -1)
    not_supported_arg_check('replace', 'case', case, None)
    str_arg_check('replace', 'pat', pat)
    str_arg_check('replace', 'repl', repl)
    int_arg_check('replace', 'flags', flags)
    if S_str.stype.data == bodo.dict_str_arr_type:

        def _str_replace_dict_impl(S_str, pat, repl, n=-1, case=None, flags
            =0, regex=True):
            S = S_str._obj
            fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
            xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
            mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
            out_arr = bodo.libs.dict_arr_ext.str_replace(fuhzm__qes, pat,
                repl, flags, regex)
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                xaszs__rjqi, mno__blkk)
        return _str_replace_dict_impl
    if is_overload_true(regex):

        def _str_replace_regex_impl(S_str, pat, repl, n=-1, case=None,
            flags=0, regex=True):
            S = S_str._obj
            fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
            xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
            mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
            numba.parfors.parfor.init_prange()
            gfsmn__tcuv = re.compile(pat, flags)
            qtoiu__prxxr = len(fuhzm__qes)
            out_arr = pre_alloc_string_array(qtoiu__prxxr, -1)
            for smnk__grq in numba.parfors.parfor.internal_prange(qtoiu__prxxr
                ):
                if bodo.libs.array_kernels.isna(fuhzm__qes, smnk__grq):
                    out_arr[smnk__grq] = ''
                    bodo.libs.array_kernels.setna(out_arr, smnk__grq)
                    continue
                out_arr[smnk__grq] = gfsmn__tcuv.sub(repl, fuhzm__qes[
                    smnk__grq])
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                xaszs__rjqi, mno__blkk)
        return _str_replace_regex_impl
    if not is_overload_false(regex):
        raise BodoError('Series.str.replace(): regex argument should be bool')

    def _str_replace_noregex_impl(S_str, pat, repl, n=-1, case=None, flags=
        0, regex=True):
        S = S_str._obj
        fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
        xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
        mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
        numba.parfors.parfor.init_prange()
        qtoiu__prxxr = len(fuhzm__qes)
        numba.parfors.parfor.init_prange()
        out_arr = pre_alloc_string_array(qtoiu__prxxr, -1)
        for smnk__grq in numba.parfors.parfor.internal_prange(qtoiu__prxxr):
            if bodo.libs.array_kernels.isna(fuhzm__qes, smnk__grq):
                out_arr[smnk__grq] = ''
                bodo.libs.array_kernels.setna(out_arr, smnk__grq)
                continue
            out_arr[smnk__grq] = fuhzm__qes[smnk__grq].replace(pat, repl)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, xaszs__rjqi,
            mno__blkk)
    return _str_replace_noregex_impl


@numba.njit
def series_contains_regex(S, pat, case, flags, na, regex):
    with numba.objmode(out_arr=bodo.boolean_array):
        out_arr = S.array._str_contains(pat, case, flags, na, regex)
    return out_arr


def is_regex_unsupported(pat):
    hegow__fdnns = ['(?a', '(?i', '(?L', '(?m', '(?s', '(?u', '(?x', '(?#']
    if is_overload_constant_str(pat):
        if isinstance(pat, types.StringLiteral):
            pat = pat.literal_value
        return any([(omk__qeudi in pat) for omk__qeudi in hegow__fdnns])
    else:
        return True


@overload_method(SeriesStrMethodType, 'contains', no_unliteral=True)
def overload_str_method_contains(S_str, pat, case=True, flags=0, na=np.nan,
    regex=True):
    not_supported_arg_check('contains', 'na', na, np.nan)
    str_arg_check('contains', 'pat', pat)
    int_arg_check('contains', 'flags', flags)
    if not is_overload_constant_bool(regex):
        raise BodoError(
            "Series.str.contains(): 'regex' argument should be a constant boolean"
            )
    if not is_overload_constant_bool(case):
        raise BodoError(
            "Series.str.contains(): 'case' argument should be a constant boolean"
            )
    zqc__azwtm = re.IGNORECASE.value
    twc__wont = 'def impl(\n'
    twc__wont += '    S_str, pat, case=True, flags=0, na=np.nan, regex=True\n'
    twc__wont += '):\n'
    twc__wont += '  S = S_str._obj\n'
    twc__wont += '  arr = bodo.hiframes.pd_series_ext.get_series_data(S)\n'
    twc__wont += '  index = bodo.hiframes.pd_series_ext.get_series_index(S)\n'
    twc__wont += '  name = bodo.hiframes.pd_series_ext.get_series_name(S)\n'
    twc__wont += '  l = len(arr)\n'
    twc__wont += '  out_arr = bodo.libs.bool_arr_ext.alloc_bool_array(l)\n'
    if is_overload_true(regex):
        if is_regex_unsupported(pat) or flags:
            if S_str.stype.data == bodo.dict_str_arr_type:
                twc__wont += """  out_arr = bodo.libs.dict_arr_ext.str_series_contains_regex(arr, pat, case, flags, na, regex)
"""
            else:
                twc__wont += """  out_arr = bodo.hiframes.series_str_impl.series_contains_regex(S, pat, case, flags, na, regex)
"""
        else:
            twc__wont += """  get_search_regex(arr, case, bodo.libs.str_ext.unicode_to_utf8(pat), out_arr)
"""
    elif S_str.stype.data == bodo.dict_str_arr_type:
        twc__wont += (
            '  out_arr = bodo.libs.dict_arr_ext.str_contains_non_regex(arr, pat, case)\n'
            )
    else:
        twc__wont += '  numba.parfors.parfor.init_prange()\n'
        if is_overload_false(case):
            twc__wont += '  upper_pat = pat.upper()\n'
        twc__wont += '  for i in numba.parfors.parfor.internal_prange(l):\n'
        twc__wont += '      if bodo.libs.array_kernels.isna(arr, i):\n'
        twc__wont += '          bodo.libs.array_kernels.setna(out_arr, i)\n'
        twc__wont += '      else: \n'
        if is_overload_true(case):
            twc__wont += '          out_arr[i] = pat in arr[i]\n'
        else:
            twc__wont += '          out_arr[i] = upper_pat in arr[i].upper()\n'
    twc__wont += (
        '  return bodo.hiframes.pd_series_ext.init_series(out_arr, index, name)\n'
        )
    zisv__eea = {}
    exec(twc__wont, {'re': re, 'bodo': bodo, 'numba': numba, 'np': np,
        're_ignorecase_value': zqc__azwtm, 'get_search_regex':
        get_search_regex}, zisv__eea)
    impl = zisv__eea['impl']
    return impl


@overload_method(SeriesStrMethodType, 'cat', no_unliteral=True)
def overload_str_method_cat(S_str, others=None, sep=None, na_rep=None, join
    ='left'):
    if not isinstance(others, DataFrameType):
        raise_bodo_error(
            "Series.str.cat(): 'others' must be a DataFrame currently")
    if not is_overload_none(sep):
        str_arg_check('cat', 'sep', sep)
    if not is_overload_constant_str(join) or get_overload_const_str(join
        ) != 'left':
        raise_bodo_error("Series.str.cat(): 'join' not supported yet")
    twc__wont = (
        "def impl(S_str, others=None, sep=None, na_rep=None, join='left'):\n")
    twc__wont += '  S = S_str._obj\n'
    twc__wont += '  arr = bodo.hiframes.pd_series_ext.get_series_data(S)\n'
    twc__wont += '  index = bodo.hiframes.pd_series_ext.get_series_index(S)\n'
    twc__wont += '  name = bodo.hiframes.pd_series_ext.get_series_name(S)\n'
    twc__wont += '  l = len(arr)\n'
    for i in range(len(others.columns)):
        twc__wont += (
            f'  data{i} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(others, {i})\n'
            )
    if S_str.stype.data == bodo.dict_str_arr_type and all(otjmb__nzxd ==
        bodo.dict_str_arr_type for otjmb__nzxd in others.data):
        kvom__ychy = ', '.join(f'data{i}' for i in range(len(others.columns)))
        twc__wont += (
            f'  out_arr = bodo.libs.dict_arr_ext.cat_dict_str((arr, {kvom__ychy}), sep)\n'
            )
    else:
        xanjr__xex = ' or '.join(['bodo.libs.array_kernels.isna(arr, i)'] +
            [f'bodo.libs.array_kernels.isna(data{i}, i)' for i in range(len
            (others.columns))])
        twc__wont += (
            '  out_arr = bodo.libs.str_arr_ext.pre_alloc_string_array(l, -1)\n'
            )
        twc__wont += '  numba.parfors.parfor.init_prange()\n'
        twc__wont += '  for i in numba.parfors.parfor.internal_prange(l):\n'
        twc__wont += f'      if {xanjr__xex}:\n'
        twc__wont += '          bodo.libs.array_kernels.setna(out_arr, i)\n'
        twc__wont += '          continue\n'
        yxnz__ymns = ', '.join(['arr[i]'] + [f'data{i}[i]' for i in range(
            len(others.columns))])
        pgwp__lka = "''" if is_overload_none(sep) else 'sep'
        twc__wont += f'      out_arr[i] = {pgwp__lka}.join([{yxnz__ymns}])\n'
    twc__wont += (
        '  return bodo.hiframes.pd_series_ext.init_series(out_arr, index, name)\n'
        )
    zisv__eea = {}
    exec(twc__wont, {'bodo': bodo, 'numba': numba}, zisv__eea)
    impl = zisv__eea['impl']
    return impl


@overload_method(SeriesStrMethodType, 'count', inline='always',
    no_unliteral=True)
def overload_str_method_count(S_str, pat, flags=0):
    str_arg_check('count', 'pat', pat)
    int_arg_check('count', 'flags', flags)
    if S_str.stype.data == bodo.dict_str_arr_type:

        def _str_count_dict_impl(S_str, pat, flags=0):
            S = S_str._obj
            fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
            xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
            mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
            out_arr = bodo.libs.dict_arr_ext.str_count(fuhzm__qes, pat, flags)
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                xaszs__rjqi, mno__blkk)
        return _str_count_dict_impl

    def impl(S_str, pat, flags=0):
        S = S_str._obj
        bxv__fraek = bodo.hiframes.pd_series_ext.get_series_data(S)
        mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
        xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
        gfsmn__tcuv = re.compile(pat, flags)
        numba.parfors.parfor.init_prange()
        qtoiu__prxxr = len(bxv__fraek)
        out_arr = bodo.libs.int_arr_ext.alloc_int_array(qtoiu__prxxr, np.int64)
        for i in numba.parfors.parfor.internal_prange(qtoiu__prxxr):
            if bodo.libs.array_kernels.isna(bxv__fraek, i):
                bodo.libs.array_kernels.setna(out_arr, i)
            else:
                out_arr[i] = str_findall_count(gfsmn__tcuv, bxv__fraek[i])
        return bodo.hiframes.pd_series_ext.init_series(out_arr, xaszs__rjqi,
            mno__blkk)
    return impl


@overload_method(SeriesStrMethodType, 'find', inline='always', no_unliteral
    =True)
def overload_str_method_find(S_str, sub, start=0, end=None):
    str_arg_check('find', 'sub', sub)
    int_arg_check('find', 'start', start)
    if not is_overload_none(end):
        int_arg_check('find', 'end', end)
    if S_str.stype.data == bodo.dict_str_arr_type:

        def _str_find_dict_impl(S_str, sub, start=0, end=None):
            S = S_str._obj
            fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
            xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
            mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
            out_arr = bodo.libs.dict_arr_ext.str_find(fuhzm__qes, sub,
                start, end)
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                xaszs__rjqi, mno__blkk)
        return _str_find_dict_impl

    def impl(S_str, sub, start=0, end=None):
        S = S_str._obj
        bxv__fraek = bodo.hiframes.pd_series_ext.get_series_data(S)
        mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
        xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
        numba.parfors.parfor.init_prange()
        qtoiu__prxxr = len(bxv__fraek)
        out_arr = bodo.libs.int_arr_ext.alloc_int_array(qtoiu__prxxr, np.int64)
        for i in numba.parfors.parfor.internal_prange(qtoiu__prxxr):
            if bodo.libs.array_kernels.isna(bxv__fraek, i):
                bodo.libs.array_kernels.setna(out_arr, i)
            else:
                out_arr[i] = bxv__fraek[i].find(sub, start, end)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, xaszs__rjqi,
            mno__blkk)
    return impl


@overload_method(SeriesStrMethodType, 'rfind', inline='always',
    no_unliteral=True)
def overload_str_method_rfind(S_str, sub, start=0, end=None):
    str_arg_check('rfind', 'sub', sub)
    if start != 0:
        int_arg_check('rfind', 'start', start)
    if not is_overload_none(end):
        int_arg_check('rfind', 'end', end)
    if S_str.stype.data == bodo.dict_str_arr_type:

        def _str_rfind_dict_impl(S_str, sub, start=0, end=None):
            S = S_str._obj
            fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
            xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
            mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
            out_arr = bodo.libs.dict_arr_ext.str_rfind(fuhzm__qes, sub,
                start, end)
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                xaszs__rjqi, mno__blkk)
        return _str_rfind_dict_impl

    def impl(S_str, sub, start=0, end=None):
        S = S_str._obj
        bxv__fraek = bodo.hiframes.pd_series_ext.get_series_data(S)
        mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
        xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
        numba.parfors.parfor.init_prange()
        qtoiu__prxxr = len(bxv__fraek)
        out_arr = bodo.libs.int_arr_ext.alloc_int_array(qtoiu__prxxr, np.int64)
        for i in numba.parfors.parfor.internal_prange(qtoiu__prxxr):
            if bodo.libs.array_kernels.isna(bxv__fraek, i):
                bodo.libs.array_kernels.setna(out_arr, i)
            else:
                out_arr[i] = bxv__fraek[i].rfind(sub, start, end)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, xaszs__rjqi,
            mno__blkk)
    return impl


@overload_method(SeriesStrMethodType, 'slice_replace', inline='always',
    no_unliteral=True)
def overload_str_method_slice_replace(S_str, start=0, stop=None, repl=''):
    int_arg_check('slice_replace', 'start', start)
    if not is_overload_none(stop):
        int_arg_check('slice_replace', 'stop', stop)
    str_arg_check('slice_replace', 'repl', repl)

    def impl(S_str, start=0, stop=None, repl=''):
        S = S_str._obj
        bxv__fraek = bodo.hiframes.pd_series_ext.get_series_data(S)
        mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
        xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
        numba.parfors.parfor.init_prange()
        qtoiu__prxxr = len(bxv__fraek)
        out_arr = bodo.libs.str_arr_ext.pre_alloc_string_array(qtoiu__prxxr, -1
            )
        for smnk__grq in numba.parfors.parfor.internal_prange(qtoiu__prxxr):
            if bodo.libs.array_kernels.isna(bxv__fraek, smnk__grq):
                bodo.libs.array_kernels.setna(out_arr, smnk__grq)
            else:
                if stop is not None:
                    vlgv__kjhn = bxv__fraek[smnk__grq][stop:]
                else:
                    vlgv__kjhn = ''
                out_arr[smnk__grq] = bxv__fraek[smnk__grq][:start
                    ] + repl + vlgv__kjhn
        return bodo.hiframes.pd_series_ext.init_series(out_arr, xaszs__rjqi,
            mno__blkk)
    return impl


@overload_method(SeriesStrMethodType, 'repeat', inline='always',
    no_unliteral=True)
def overload_str_method_repeat(S_str, repeats):
    if isinstance(repeats, types.Integer) or is_overload_constant_int(repeats):
        if S_str.stype.data == bodo.dict_str_arr_type:

            def _str_repeat_int_dict_impl(S_str, repeats):
                S = S_str._obj
                fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
                xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
                mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
                out_arr = bodo.libs.dict_arr_ext.str_repeat_int(fuhzm__qes,
                    repeats)
                return bodo.hiframes.pd_series_ext.init_series(out_arr,
                    xaszs__rjqi, mno__blkk)
            return _str_repeat_int_dict_impl

        def impl(S_str, repeats):
            S = S_str._obj
            bxv__fraek = bodo.hiframes.pd_series_ext.get_series_data(S)
            mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
            xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
            numba.parfors.parfor.init_prange()
            qtoiu__prxxr = len(bxv__fraek)
            out_arr = bodo.libs.str_arr_ext.pre_alloc_string_array(qtoiu__prxxr
                , -1)
            for smnk__grq in numba.parfors.parfor.internal_prange(qtoiu__prxxr
                ):
                if bodo.libs.array_kernels.isna(bxv__fraek, smnk__grq):
                    bodo.libs.array_kernels.setna(out_arr, smnk__grq)
                else:
                    out_arr[smnk__grq] = bxv__fraek[smnk__grq] * repeats
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                xaszs__rjqi, mno__blkk)
        return impl
    elif is_overload_constant_list(repeats):
        nivse__onze = get_overload_const_list(repeats)
        cwh__adfk = all([isinstance(reqp__mdn, int) for reqp__mdn in
            nivse__onze])
    elif is_list_like_index_type(repeats) and isinstance(repeats.dtype,
        types.Integer):
        cwh__adfk = True
    else:
        cwh__adfk = False
    if cwh__adfk:

        def impl(S_str, repeats):
            S = S_str._obj
            bxv__fraek = bodo.hiframes.pd_series_ext.get_series_data(S)
            mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
            xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
            vvxuq__dgxdp = bodo.utils.conversion.coerce_to_array(repeats)
            numba.parfors.parfor.init_prange()
            qtoiu__prxxr = len(bxv__fraek)
            out_arr = bodo.libs.str_arr_ext.pre_alloc_string_array(qtoiu__prxxr
                , -1)
            for smnk__grq in numba.parfors.parfor.internal_prange(qtoiu__prxxr
                ):
                if bodo.libs.array_kernels.isna(bxv__fraek, smnk__grq):
                    bodo.libs.array_kernels.setna(out_arr, smnk__grq)
                else:
                    out_arr[smnk__grq] = bxv__fraek[smnk__grq] * vvxuq__dgxdp[
                        smnk__grq]
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                xaszs__rjqi, mno__blkk)
        return impl
    else:
        raise BodoError(
            'Series.str.repeat(): repeats argument must either be an integer or a sequence of integers'
            )


def create_ljust_rjust_center_overload(func_name):
    twc__wont = f"""def dict_impl(S_str, width, fillchar=' '):
    S = S_str._obj
    arr = bodo.hiframes.pd_series_ext.get_series_data(S)
    index = bodo.hiframes.pd_series_ext.get_series_index(S)
    name = bodo.hiframes.pd_series_ext.get_series_name(S)
    out_arr = bodo.libs.dict_arr_ext.str_{func_name}(arr, width, fillchar)
    return bodo.hiframes.pd_series_ext.init_series(out_arr, index, name)
def impl(S_str, width, fillchar=' '):
    S = S_str._obj
    str_arr = bodo.hiframes.pd_series_ext.get_series_data(S)
    name = bodo.hiframes.pd_series_ext.get_series_name(S)
    index = bodo.hiframes.pd_series_ext.get_series_index(S)
    numba.parfors.parfor.init_prange()
    l = len(str_arr)
    out_arr = bodo.libs.str_arr_ext.pre_alloc_string_array(l, -1)
    for j in numba.parfors.parfor.internal_prange(l):
        if bodo.libs.array_kernels.isna(str_arr, j):
            bodo.libs.array_kernels.setna(out_arr, j)
        else:
            out_arr[j] = str_arr[j].{func_name}(width, fillchar)
    return bodo.hiframes.pd_series_ext.init_series(out_arr, index, name)
"""
    zisv__eea = {}
    saxm__wqfak = {'bodo': bodo, 'numba': numba}
    exec(twc__wont, saxm__wqfak, zisv__eea)
    impl = zisv__eea['impl']
    fkmd__mllgx = zisv__eea['dict_impl']

    def overload_ljust_rjust_center_method(S_str, width, fillchar=' '):
        common_validate_padding(func_name, width, fillchar)
        if S_str.stype.data == bodo.dict_str_arr_type:
            return fkmd__mllgx
        return impl
    return overload_ljust_rjust_center_method


def _install_ljust_rjust_center():
    for nxaa__mphxz in ['ljust', 'rjust', 'center']:
        impl = create_ljust_rjust_center_overload(nxaa__mphxz)
        overload_method(SeriesStrMethodType, nxaa__mphxz, inline='always',
            no_unliteral=True)(impl)


_install_ljust_rjust_center()


@overload_method(SeriesStrMethodType, 'pad', no_unliteral=True)
def overload_str_method_pad(S_str, width, side='left', fillchar=' '):
    common_validate_padding('pad', width, fillchar)
    if is_overload_constant_str(side):
        if get_overload_const_str(side) not in ['left', 'right', 'both']:
            raise BodoError('Series.str.pad(): Invalid Side')
    else:
        raise BodoError('Series.str.pad(): Invalid Side')
    if S_str.stype.data == bodo.dict_str_arr_type:

        def _str_pad_dict_impl(S_str, width, side='left', fillchar=' '):
            S = S_str._obj
            fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
            xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
            mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
            if side == 'left':
                out_arr = bodo.libs.dict_arr_ext.str_rjust(fuhzm__qes,
                    width, fillchar)
            elif side == 'right':
                out_arr = bodo.libs.dict_arr_ext.str_ljust(fuhzm__qes,
                    width, fillchar)
            elif side == 'both':
                out_arr = bodo.libs.dict_arr_ext.str_center(fuhzm__qes,
                    width, fillchar)
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                xaszs__rjqi, mno__blkk)
        return _str_pad_dict_impl

    def impl(S_str, width, side='left', fillchar=' '):
        S = S_str._obj
        bxv__fraek = bodo.hiframes.pd_series_ext.get_series_data(S)
        mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
        xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
        numba.parfors.parfor.init_prange()
        qtoiu__prxxr = len(bxv__fraek)
        out_arr = bodo.libs.str_arr_ext.pre_alloc_string_array(qtoiu__prxxr, -1
            )
        for smnk__grq in numba.parfors.parfor.internal_prange(qtoiu__prxxr):
            if bodo.libs.array_kernels.isna(bxv__fraek, smnk__grq):
                out_arr[smnk__grq] = ''
                bodo.libs.array_kernels.setna(out_arr, smnk__grq)
            elif side == 'left':
                out_arr[smnk__grq] = bxv__fraek[smnk__grq].rjust(width,
                    fillchar)
            elif side == 'right':
                out_arr[smnk__grq] = bxv__fraek[smnk__grq].ljust(width,
                    fillchar)
            elif side == 'both':
                out_arr[smnk__grq] = bxv__fraek[smnk__grq].center(width,
                    fillchar)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, xaszs__rjqi,
            mno__blkk)
    return impl


@overload_method(SeriesStrMethodType, 'zfill', inline='always',
    no_unliteral=True)
def overload_str_method_zfill(S_str, width):
    int_arg_check('zfill', 'width', width)
    if S_str.stype.data == bodo.dict_str_arr_type:

        def _str_zfill_dict_impl(S_str, width):
            S = S_str._obj
            fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
            xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
            mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
            out_arr = bodo.libs.dict_arr_ext.str_zfill(fuhzm__qes, width)
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                xaszs__rjqi, mno__blkk)
        return _str_zfill_dict_impl

    def impl(S_str, width):
        S = S_str._obj
        bxv__fraek = bodo.hiframes.pd_series_ext.get_series_data(S)
        mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
        xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
        numba.parfors.parfor.init_prange()
        qtoiu__prxxr = len(bxv__fraek)
        out_arr = bodo.libs.str_arr_ext.pre_alloc_string_array(qtoiu__prxxr, -1
            )
        for smnk__grq in numba.parfors.parfor.internal_prange(qtoiu__prxxr):
            if bodo.libs.array_kernels.isna(bxv__fraek, smnk__grq):
                out_arr[smnk__grq] = ''
                bodo.libs.array_kernels.setna(out_arr, smnk__grq)
            else:
                out_arr[smnk__grq] = bxv__fraek[smnk__grq].zfill(width)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, xaszs__rjqi,
            mno__blkk)
    return impl


@overload_method(SeriesStrMethodType, 'slice', no_unliteral=True)
def overload_str_method_slice(S_str, start=None, stop=None, step=None):
    if not is_overload_none(start):
        int_arg_check('slice', 'start', start)
    if not is_overload_none(stop):
        int_arg_check('slice', 'stop', stop)
    if not is_overload_none(step):
        int_arg_check('slice', 'step', step)
    if S_str.stype.data == bodo.dict_str_arr_type:

        def _str_slice_dict_impl(S_str, start=None, stop=None, step=None):
            S = S_str._obj
            fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
            xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
            mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
            out_arr = bodo.libs.dict_arr_ext.str_slice(fuhzm__qes, start,
                stop, step)
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                xaszs__rjqi, mno__blkk)
        return _str_slice_dict_impl

    def impl(S_str, start=None, stop=None, step=None):
        S = S_str._obj
        bxv__fraek = bodo.hiframes.pd_series_ext.get_series_data(S)
        mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
        xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
        numba.parfors.parfor.init_prange()
        qtoiu__prxxr = len(bxv__fraek)
        out_arr = bodo.libs.str_arr_ext.pre_alloc_string_array(qtoiu__prxxr, -1
            )
        for smnk__grq in numba.parfors.parfor.internal_prange(qtoiu__prxxr):
            if bodo.libs.array_kernels.isna(bxv__fraek, smnk__grq):
                out_arr[smnk__grq] = ''
                bodo.libs.array_kernels.setna(out_arr, smnk__grq)
            else:
                out_arr[smnk__grq] = bxv__fraek[smnk__grq][start:stop:step]
        return bodo.hiframes.pd_series_ext.init_series(out_arr, xaszs__rjqi,
            mno__blkk)
    return impl


@overload_method(SeriesStrMethodType, 'startswith', inline='always',
    no_unliteral=True)
def overload_str_method_startswith(S_str, pat, na=np.nan):
    not_supported_arg_check('startswith', 'na', na, np.nan)
    str_arg_check('startswith', 'pat', pat)
    if S_str.stype.data == bodo.dict_str_arr_type:

        def _str_startswith_dict_impl(S_str, pat, na=np.nan):
            S = S_str._obj
            fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
            xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
            mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
            out_arr = bodo.libs.dict_arr_ext.str_startswith(fuhzm__qes, pat, na
                )
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                xaszs__rjqi, mno__blkk)
        return _str_startswith_dict_impl

    def impl(S_str, pat, na=np.nan):
        S = S_str._obj
        bxv__fraek = bodo.hiframes.pd_series_ext.get_series_data(S)
        mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
        xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
        numba.parfors.parfor.init_prange()
        qtoiu__prxxr = len(bxv__fraek)
        out_arr = bodo.libs.bool_arr_ext.alloc_bool_array(qtoiu__prxxr)
        for i in numba.parfors.parfor.internal_prange(qtoiu__prxxr):
            if bodo.libs.array_kernels.isna(bxv__fraek, i):
                bodo.libs.array_kernels.setna(out_arr, i)
            else:
                out_arr[i] = bxv__fraek[i].startswith(pat)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, xaszs__rjqi,
            mno__blkk)
    return impl


@overload_method(SeriesStrMethodType, 'endswith', inline='always',
    no_unliteral=True)
def overload_str_method_endswith(S_str, pat, na=np.nan):
    not_supported_arg_check('endswith', 'na', na, np.nan)
    str_arg_check('endswith', 'pat', pat)
    if S_str.stype.data == bodo.dict_str_arr_type:

        def _str_endswith_dict_impl(S_str, pat, na=np.nan):
            S = S_str._obj
            fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
            xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
            mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
            out_arr = bodo.libs.dict_arr_ext.str_endswith(fuhzm__qes, pat, na)
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                xaszs__rjqi, mno__blkk)
        return _str_endswith_dict_impl

    def impl(S_str, pat, na=np.nan):
        S = S_str._obj
        bxv__fraek = bodo.hiframes.pd_series_ext.get_series_data(S)
        mno__blkk = bodo.hiframes.pd_series_ext.get_series_name(S)
        xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
        numba.parfors.parfor.init_prange()
        qtoiu__prxxr = len(bxv__fraek)
        out_arr = bodo.libs.bool_arr_ext.alloc_bool_array(qtoiu__prxxr)
        for i in numba.parfors.parfor.internal_prange(qtoiu__prxxr):
            if bodo.libs.array_kernels.isna(bxv__fraek, i):
                bodo.libs.array_kernels.setna(out_arr, i)
            else:
                out_arr[i] = bxv__fraek[i].endswith(pat)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, xaszs__rjqi,
            mno__blkk)
    return impl


@overload(operator.getitem, no_unliteral=True)
def overload_str_method_getitem(S_str, ind):
    if not isinstance(S_str, SeriesStrMethodType):
        return
    if not isinstance(types.unliteral(ind), (types.SliceType, types.Integer)):
        raise BodoError(
            'index input to Series.str[] should be a slice or an integer')
    if isinstance(ind, types.SliceType):
        return lambda S_str, ind: S_str.slice(ind.start, ind.stop, ind.step)
    if isinstance(types.unliteral(ind), types.Integer):
        return lambda S_str, ind: S_str.get(ind)


@overload_method(SeriesStrMethodType, 'extract', inline='always',
    no_unliteral=True)
def overload_str_method_extract(S_str, pat, flags=0, expand=True):
    if not is_overload_constant_bool(expand):
        raise BodoError(
            "Series.str.extract(): 'expand' argument should be a constant bool"
            )
    djy__dqh, regex = _get_column_names_from_regex(pat, flags, 'extract')
    ouvi__azue = len(djy__dqh)
    if S_str.stype.data == bodo.dict_str_arr_type:
        twc__wont = 'def impl(S_str, pat, flags=0, expand=True):\n'
        twc__wont += '  S = S_str._obj\n'
        twc__wont += '  arr = bodo.hiframes.pd_series_ext.get_series_data(S)\n'
        twc__wont += (
            '  index = bodo.hiframes.pd_series_ext.get_series_index(S)\n')
        twc__wont += (
            '  name = bodo.hiframes.pd_series_ext.get_series_name(S)\n')
        twc__wont += f"""  out_arr_list = bodo.libs.dict_arr_ext.str_extract(arr, pat, flags, {ouvi__azue})
"""
        for i in range(ouvi__azue):
            twc__wont += f'  out_arr_{i} = out_arr_list[{i}]\n'
    else:
        twc__wont = 'def impl(S_str, pat, flags=0, expand=True):\n'
        twc__wont += '  regex = re.compile(pat, flags=flags)\n'
        twc__wont += '  S = S_str._obj\n'
        twc__wont += (
            '  str_arr = bodo.hiframes.pd_series_ext.get_series_data(S)\n')
        twc__wont += (
            '  index = bodo.hiframes.pd_series_ext.get_series_index(S)\n')
        twc__wont += (
            '  name = bodo.hiframes.pd_series_ext.get_series_name(S)\n')
        twc__wont += '  numba.parfors.parfor.init_prange()\n'
        twc__wont += '  n = len(str_arr)\n'
        for i in range(ouvi__azue):
            twc__wont += (
                '  out_arr_{0} = bodo.libs.str_arr_ext.pre_alloc_string_array(n, -1)\n'
                .format(i))
        twc__wont += '  for j in numba.parfors.parfor.internal_prange(n):\n'
        twc__wont += '      if bodo.libs.array_kernels.isna(str_arr, j):\n'
        for i in range(ouvi__azue):
            twc__wont += "          out_arr_{}[j] = ''\n".format(i)
            twc__wont += (
                '          bodo.libs.array_kernels.setna(out_arr_{}, j)\n'.
                format(i))
        twc__wont += '      else:\n'
        twc__wont += '          m = regex.search(str_arr[j])\n'
        twc__wont += '          if m:\n'
        twc__wont += '            g = m.groups()\n'
        for i in range(ouvi__azue):
            twc__wont += '            out_arr_{0}[j] = g[{0}]\n'.format(i)
        twc__wont += '          else:\n'
        for i in range(ouvi__azue):
            twc__wont += "            out_arr_{}[j] = ''\n".format(i)
            twc__wont += (
                '            bodo.libs.array_kernels.setna(out_arr_{}, j)\n'
                .format(i))
    if is_overload_false(expand) and regex.groups == 1:
        mno__blkk = "'{}'".format(list(regex.groupindex.keys()).pop()) if len(
            regex.groupindex.keys()) > 0 else 'name'
        twc__wont += (
            '  return bodo.hiframes.pd_series_ext.init_series(out_arr_0, index, {})\n'
            .format(mno__blkk))
        zisv__eea = {}
        exec(twc__wont, {'re': re, 'bodo': bodo, 'numba': numba,
            'get_utf8_size': get_utf8_size}, zisv__eea)
        impl = zisv__eea['impl']
        return impl
    eraas__noprr = ', '.join('out_arr_{}'.format(i) for i in range(ouvi__azue))
    impl = bodo.hiframes.dataframe_impl._gen_init_df(twc__wont, djy__dqh,
        eraas__noprr, 'index', extra_globals={'get_utf8_size':
        get_utf8_size, 're': re})
    return impl


@overload_method(SeriesStrMethodType, 'extractall', inline='always',
    no_unliteral=True)
def overload_str_method_extractall(S_str, pat, flags=0):
    djy__dqh, gxw__waaca = _get_column_names_from_regex(pat, flags,
        'extractall')
    ouvi__azue = len(djy__dqh)
    hatbs__mkxk = isinstance(S_str.stype.index, StringIndexType)
    ovzq__rbu = ouvi__azue > 1
    sazsv__zqvt = '_multi' if ovzq__rbu else ''
    if S_str.stype.data == bodo.dict_str_arr_type:
        twc__wont = 'def impl(S_str, pat, flags=0):\n'
        twc__wont += '  S = S_str._obj\n'
        twc__wont += '  arr = bodo.hiframes.pd_series_ext.get_series_data(S)\n'
        twc__wont += (
            '  index = bodo.hiframes.pd_series_ext.get_series_index(S)\n')
        twc__wont += (
            '  name = bodo.hiframes.pd_series_ext.get_series_name(S)\n')
        twc__wont += (
            '  index_arr = bodo.utils.conversion.index_to_array(index)\n')
        twc__wont += (
            '  index_name = bodo.hiframes.pd_index_ext.get_index_name(index)\n'
            )
        twc__wont += '  regex = re.compile(pat, flags=flags)\n'
        twc__wont += '  out_ind_arr, out_match_arr, out_arr_list = '
        twc__wont += f'bodo.libs.dict_arr_ext.str_extractall{sazsv__zqvt}(\n'
        twc__wont += f'arr, regex, {ouvi__azue}, index_arr)\n'
        for i in range(ouvi__azue):
            twc__wont += f'  out_arr_{i} = out_arr_list[{i}]\n'
        twc__wont += (
            '  out_index = bodo.hiframes.pd_multi_index_ext.init_multi_index(\n'
            )
        twc__wont += (
            "    (out_ind_arr, out_match_arr), (index_name, 'match'))\n")
    else:
        twc__wont = 'def impl(S_str, pat, flags=0):\n'
        twc__wont += '  regex = re.compile(pat, flags=flags)\n'
        twc__wont += '  S = S_str._obj\n'
        twc__wont += (
            '  str_arr = bodo.hiframes.pd_series_ext.get_series_data(S)\n')
        twc__wont += (
            '  index = bodo.hiframes.pd_series_ext.get_series_index(S)\n')
        twc__wont += (
            '  name = bodo.hiframes.pd_series_ext.get_series_name(S)\n')
        twc__wont += (
            '  index_arr = bodo.utils.conversion.index_to_array(index)\n')
        twc__wont += (
            '  index_name = bodo.hiframes.pd_index_ext.get_index_name(index)\n'
            )
        twc__wont += '  numba.parfors.parfor.init_prange()\n'
        twc__wont += '  n = len(str_arr)\n'
        twc__wont += '  out_n_l = [0]\n'
        for i in range(ouvi__azue):
            twc__wont += '  num_chars_{} = 0\n'.format(i)
        if hatbs__mkxk:
            twc__wont += '  index_num_chars = 0\n'
        twc__wont += '  for i in numba.parfors.parfor.internal_prange(n):\n'
        if hatbs__mkxk:
            twc__wont += (
                '      index_num_chars += get_utf8_size(index_arr[i])\n')
        twc__wont += '      if bodo.libs.array_kernels.isna(str_arr, i):\n'
        twc__wont += '          continue\n'
        twc__wont += '      m = regex.findall(str_arr[i])\n'
        twc__wont += '      out_n_l[0] += len(m)\n'
        for i in range(ouvi__azue):
            twc__wont += '      l_{} = 0\n'.format(i)
        twc__wont += '      for s in m:\n'
        for i in range(ouvi__azue):
            twc__wont += '        l_{} += get_utf8_size(s{})\n'.format(i, 
                '[{}]'.format(i) if ouvi__azue > 1 else '')
        for i in range(ouvi__azue):
            twc__wont += '      num_chars_{0} += l_{0}\n'.format(i)
        twc__wont += (
            '  out_n = bodo.libs.distributed_api.local_alloc_size(out_n_l[0], str_arr)\n'
            )
        for i in range(ouvi__azue):
            twc__wont += (
                """  out_arr_{0} = bodo.libs.str_arr_ext.pre_alloc_string_array(out_n, num_chars_{0})
"""
                .format(i))
        if hatbs__mkxk:
            twc__wont += """  out_ind_arr = bodo.libs.str_arr_ext.pre_alloc_string_array(out_n, index_num_chars)
"""
        else:
            twc__wont += '  out_ind_arr = np.empty(out_n, index_arr.dtype)\n'
        twc__wont += '  out_match_arr = np.empty(out_n, np.int64)\n'
        twc__wont += '  out_ind = 0\n'
        twc__wont += '  for j in numba.parfors.parfor.internal_prange(n):\n'
        twc__wont += '      if bodo.libs.array_kernels.isna(str_arr, j):\n'
        twc__wont += '          continue\n'
        twc__wont += '      m = regex.findall(str_arr[j])\n'
        twc__wont += '      for k, s in enumerate(m):\n'
        for i in range(ouvi__azue):
            twc__wont += (
                """        bodo.libs.distributed_api.set_arr_local(out_arr_{}, out_ind, s{})
"""
                .format(i, '[{}]'.format(i) if ouvi__azue > 1 else ''))
        twc__wont += """        bodo.libs.distributed_api.set_arr_local(out_ind_arr, out_ind, index_arr[j])
"""
        twc__wont += (
            '        bodo.libs.distributed_api.set_arr_local(out_match_arr, out_ind, k)\n'
            )
        twc__wont += '        out_ind += 1\n'
        twc__wont += (
            '  out_index = bodo.hiframes.pd_multi_index_ext.init_multi_index(\n'
            )
        twc__wont += (
            "    (out_ind_arr, out_match_arr), (index_name, 'match'))\n")
    eraas__noprr = ', '.join('out_arr_{}'.format(i) for i in range(ouvi__azue))
    impl = bodo.hiframes.dataframe_impl._gen_init_df(twc__wont, djy__dqh,
        eraas__noprr, 'out_index', extra_globals={'get_utf8_size':
        get_utf8_size, 're': re})
    return impl


def _get_column_names_from_regex(pat, flags, func_name):
    if not is_overload_constant_str(pat):
        raise BodoError(
            "Series.str.{}(): 'pat' argument should be a constant string".
            format(func_name))
    if not is_overload_constant_int(flags):
        raise BodoError(
            "Series.str.{}(): 'flags' argument should be a constant int".
            format(func_name))
    pat = get_overload_const_str(pat)
    flags = get_overload_const_int(flags)
    regex = re.compile(pat, flags=flags)
    if regex.groups == 0:
        raise BodoError(
            'Series.str.{}(): pattern {} contains no capture groups'.format
            (func_name, pat))
    tqsu__mvzhq = dict(zip(regex.groupindex.values(), regex.groupindex.keys()))
    djy__dqh = [tqsu__mvzhq.get(1 + i, i) for i in range(regex.groups)]
    return djy__dqh, regex


def create_str2str_methods_overload(func_name):
    lpa__vbca = func_name in ['lstrip', 'rstrip', 'strip']
    twc__wont = f"""def f({'S_str, to_strip=None' if lpa__vbca else 'S_str'}):
    S = S_str._obj
    str_arr = bodo.hiframes.pd_series_ext.get_series_data(S)
    str_arr = decode_if_dict_array(str_arr)
    index = bodo.hiframes.pd_series_ext.get_series_index(S)
    name = bodo.hiframes.pd_series_ext.get_series_name(S)
    numba.parfors.parfor.init_prange()
    n = len(str_arr)
    num_chars = {'-1' if lpa__vbca else 'num_total_chars(str_arr)'}
    out_arr = bodo.libs.str_arr_ext.pre_alloc_string_array(n, num_chars)
    for j in numba.parfors.parfor.internal_prange(n):
        if bodo.libs.array_kernels.isna(str_arr, j):
            out_arr[j] = ""
            bodo.libs.array_kernels.setna(out_arr, j)
        else:
            out_arr[j] = str_arr[j].{func_name}({'to_strip' if lpa__vbca else ''})
    return bodo.hiframes.pd_series_ext.init_series(out_arr, index, name)
"""
    twc__wont += f"""def _dict_impl({'S_str, to_strip=None' if lpa__vbca else 'S_str'}):
    S = S_str._obj
    arr = bodo.hiframes.pd_series_ext.get_series_data(S)
    index = bodo.hiframes.pd_series_ext.get_series_index(S)
    name = bodo.hiframes.pd_series_ext.get_series_name(S)
    out_arr = bodo.libs.dict_arr_ext.str_{func_name}({'arr, to_strip' if lpa__vbca else 'arr'})
    return bodo.hiframes.pd_series_ext.init_series(out_arr, index, name)
"""
    zisv__eea = {}
    exec(twc__wont, {'bodo': bodo, 'numba': numba, 'num_total_chars': bodo.
        libs.str_arr_ext.num_total_chars, 'get_utf8_size': bodo.libs.
        str_arr_ext.get_utf8_size, 'decode_if_dict_array': bodo.utils.
        typing.decode_if_dict_array}, zisv__eea)
    pof__qxywu = zisv__eea['f']
    nwl__nyw = zisv__eea['_dict_impl']
    if lpa__vbca:

        def overload_strip_method(S_str, to_strip=None):
            if not is_overload_none(to_strip):
                str_arg_check(func_name, 'to_strip', to_strip)
            if S_str.stype.data == bodo.dict_str_arr_type:
                return nwl__nyw
            return pof__qxywu
        return overload_strip_method
    else:

        def overload_str_method_dict_supported(S_str):
            if S_str.stype.data == bodo.dict_str_arr_type:
                return nwl__nyw
            return pof__qxywu
        return overload_str_method_dict_supported


def create_str2bool_methods_overload(func_name):
    twc__wont = 'def dict_impl(S_str):\n'
    twc__wont += '    S = S_str._obj\n'
    twc__wont += '    arr = bodo.hiframes.pd_series_ext.get_series_data(S)\n'
    twc__wont += (
        '    index = bodo.hiframes.pd_series_ext.get_series_index(S)\n')
    twc__wont += '    name = bodo.hiframes.pd_series_ext.get_series_name(S)\n'
    twc__wont += f'    out_arr = bodo.libs.dict_arr_ext.str_{func_name}(arr)\n'
    twc__wont += (
        '    return bodo.hiframes.pd_series_ext.init_series(out_arr, index, name)\n'
        )
    twc__wont += 'def impl(S_str):\n'
    twc__wont += '    S = S_str._obj\n'
    twc__wont += (
        '    str_arr = bodo.hiframes.pd_series_ext.get_series_data(S)\n')
    twc__wont += (
        '    index = bodo.hiframes.pd_series_ext.get_series_index(S)\n')
    twc__wont += '    name = bodo.hiframes.pd_series_ext.get_series_name(S)\n'
    twc__wont += '    numba.parfors.parfor.init_prange()\n'
    twc__wont += '    l = len(str_arr)\n'
    twc__wont += '    out_arr = bodo.libs.bool_arr_ext.alloc_bool_array(l)\n'
    twc__wont += '    for i in numba.parfors.parfor.internal_prange(l):\n'
    twc__wont += '        if bodo.libs.array_kernels.isna(str_arr, i):\n'
    twc__wont += '            bodo.libs.array_kernels.setna(out_arr, i)\n'
    twc__wont += '        else:\n'
    twc__wont += '            out_arr[i] = np.bool_(str_arr[i].{}())\n'.format(
        func_name)
    twc__wont += '    return bodo.hiframes.pd_series_ext.init_series(\n'
    twc__wont += '      out_arr,index, name)\n'
    zisv__eea = {}
    exec(twc__wont, {'bodo': bodo, 'numba': numba, 'np': np}, zisv__eea)
    impl = zisv__eea['impl']
    fkmd__mllgx = zisv__eea['dict_impl']

    def overload_str2bool_methods(S_str):
        if S_str.stype.data == bodo.dict_str_arr_type:
            return fkmd__mllgx
        return impl
    return overload_str2bool_methods


def _install_str2str_methods():
    for cxq__uemim in bodo.hiframes.pd_series_ext.str2str_methods:
        zgzmr__qgcwm = create_str2str_methods_overload(cxq__uemim)
        overload_method(SeriesStrMethodType, cxq__uemim, inline='always',
            no_unliteral=True)(zgzmr__qgcwm)


def _install_str2bool_methods():
    for cxq__uemim in bodo.hiframes.pd_series_ext.str2bool_methods:
        zgzmr__qgcwm = create_str2bool_methods_overload(cxq__uemim)
        overload_method(SeriesStrMethodType, cxq__uemim, inline='always',
            no_unliteral=True)(zgzmr__qgcwm)


_install_str2str_methods()
_install_str2bool_methods()


@overload_attribute(SeriesType, 'cat')
def overload_series_cat(s):
    if not isinstance(s.dtype, bodo.hiframes.pd_categorical_ext.
        PDCategoricalDtype):
        raise BodoError('Can only use .cat accessor with categorical values.')
    return lambda s: bodo.hiframes.series_str_impl.init_series_cat_method(s)


class SeriesCatMethodType(types.Type):

    def __init__(self, stype):
        self.stype = stype
        mno__blkk = 'SeriesCatMethodType({})'.format(stype)
        super(SeriesCatMethodType, self).__init__(mno__blkk)

    @property
    def mangling_args(self):
        return self.__class__.__name__, (self._code,)


@register_model(SeriesCatMethodType)
class SeriesCatModel(models.StructModel):

    def __init__(self, dmm, fe_type):
        mlu__qpt = [('obj', fe_type.stype)]
        super(SeriesCatModel, self).__init__(dmm, fe_type, mlu__qpt)


make_attribute_wrapper(SeriesCatMethodType, 'obj', '_obj')


@intrinsic
def init_series_cat_method(typingctx, obj=None):

    def codegen(context, builder, signature, args):
        xnqlj__abwaz, = args
        esb__wqg = signature.return_type
        nuxvt__pdvys = cgutils.create_struct_proxy(esb__wqg)(context, builder)
        nuxvt__pdvys.obj = xnqlj__abwaz
        context.nrt.incref(builder, signature.args[0], xnqlj__abwaz)
        return nuxvt__pdvys._getvalue()
    return SeriesCatMethodType(obj)(obj), codegen


@overload_attribute(SeriesCatMethodType, 'codes')
def series_cat_codes_overload(S_dt):

    def impl(S_dt):
        S = S_dt._obj
        fuhzm__qes = bodo.hiframes.pd_series_ext.get_series_data(S)
        xaszs__rjqi = bodo.hiframes.pd_series_ext.get_series_index(S)
        mno__blkk = None
        return bodo.hiframes.pd_series_ext.init_series(bodo.hiframes.
            pd_categorical_ext.get_categorical_arr_codes(fuhzm__qes),
            xaszs__rjqi, mno__blkk)
    return impl


unsupported_cat_attrs = {'categories', 'ordered'}
unsupported_cat_methods = {'rename_categories', 'reorder_categories',
    'add_categories', 'remove_categories', 'remove_unused_categories',
    'set_categories', 'as_ordered', 'as_unordered'}


def _install_catseries_unsupported():
    for ttjb__ayzt in unsupported_cat_attrs:
        mzfu__sys = 'Series.cat.' + ttjb__ayzt
        overload_attribute(SeriesCatMethodType, ttjb__ayzt)(
            create_unsupported_overload(mzfu__sys))
    for izo__wxc in unsupported_cat_methods:
        mzfu__sys = 'Series.cat.' + izo__wxc
        overload_method(SeriesCatMethodType, izo__wxc)(
            create_unsupported_overload(mzfu__sys))


_install_catseries_unsupported()
unsupported_str_methods = {'casefold', 'decode', 'encode', 'findall',
    'fullmatch', 'index', 'match', 'normalize', 'partition', 'rindex',
    'rpartition', 'slice_replace', 'rsplit', 'translate', 'wrap', 'get_dummies'
    }


def _install_strseries_unsupported():
    for izo__wxc in unsupported_str_methods:
        mzfu__sys = 'Series.str.' + izo__wxc
        overload_method(SeriesStrMethodType, izo__wxc)(
            create_unsupported_overload(mzfu__sys))


_install_strseries_unsupported()
