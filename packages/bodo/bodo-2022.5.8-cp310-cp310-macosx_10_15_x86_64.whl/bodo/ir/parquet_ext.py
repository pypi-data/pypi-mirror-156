...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    kmal__icp = pq_node.out_vars[0].name
    xdm__yzw = pq_node.out_vars[1].name
    if kmal__icp not in lives and xdm__yzw not in lives:
        return None
    elif kmal__icp not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.out_used_cols = []
    elif xdm__yzw not in lives:
