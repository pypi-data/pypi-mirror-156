...
def _global_except_hook(exctype, value, traceback):
    ...
    jbna__gypbj = MPI.COMM_WORLD
    noyy__johcv = jbna__gypbj.Get_rank()
    qdoff__hemjd = 3.0
    bgn__yya = True
    kzuj__dbzw = jbna__gypbj.Ibarrier()
    hmisu__jgg = time.time()
    while time.time() - hmisu__jgg < qdoff__hemjd:
        time.sleep(0.1)
        if kzuj__dbzw.Test():
            bgn__yya = False
            break
    try:
        ...
        if bgn__yya:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {noyy__johcv}. \n')
