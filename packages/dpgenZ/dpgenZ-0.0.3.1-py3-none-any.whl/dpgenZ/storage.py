...
class Storage:
    ...
    def readorc(self, BUCKET_NAME, OBJECT_NAME):

        e=self.client.presigned_get_object(BUCKET_NAME,OBJECT_NAME+'.orc')
        df = pd.read_orc(e)
        return df

    def writeorc(self,BUCKET_NAME_IN,OBJECT_NAME_IN,BUCKET_NAME_OUT,OBJECT_NAME_OUT):

        b = self.client.presigned_get_object(BUCKET_NAME_IN,OBJECT_NAME_IN+'.orc')
        d = pd.read_orc(b)
        csv = d.to_csv().encode('utf-8')
        self.client.put_object(
        BUCKET_NAME_OUT,
...
class Postgres:
    def __init__(self, USER=None, PASSWORD=None, HOST=None, PORT=None, DATABASE=None):
        ...
        self.psycopg=psycopg2.connect(user=USER,
                 password=PASSWORD,
                 host=HOST,
                 port=PORT,
                 database=DATABASE)          

    def readpostgres(self,execute):
        df = pd.read_sql(execute,self.psycopg)
        return df

    def writepostgres(self,execute,ACCESS_KEY,PRIVATE_KEY,BUCKET_NAME_OUT,OBJECT_NAME_OUT):
        client = Minio(
            "apilakedpa.apps.xplat.fis.com.vn",
            access_key=ACCESS_KEY,
            secret_key=PRIVATE_KEY,
