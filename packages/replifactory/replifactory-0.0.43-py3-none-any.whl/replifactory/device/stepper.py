...
class Stepper:
    ...
    def print_status(self):
        s = self.read_register(self.REGISTER_STATUS, 2)
        # print(bin(s[0]), bin(s[1]))

        bits = [s[0] >> i & 1 for i in range(8)[::-1]]
        names = ["SCK_MOD", "STEP_LOSS_B", "STEP_LOSS_A", "OCD", "TH_SD", "TH_WRN", "UVLO", "WRONG_CMD"]
        byte2 = dict(zip(names, bits))

        bits = [s[1] >> i & 1 for i in range(8)[::-1]]
        names = ["NOTPERF_CMD", "MOT_STATUS1", "MOT_STATUS2", "DIR", "SW_EVN", "SW_F", "BUSY", "HiZ"]
        byte1 = dict(zip(names, bits))

        status_dict = {(False, False): 'stopped',
