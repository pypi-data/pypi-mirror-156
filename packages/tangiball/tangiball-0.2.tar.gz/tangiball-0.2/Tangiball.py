...
class peakfinder:
    ...
    def analyse_multi(self,filedir="",outdir=""):
        ...
        for filename in os.listdir(localdir):
            ...
            self.analyse(filedir=indir,outdir=outdir)
        return
    
    # Plot a voltage series
    def plot_series(self,filedir=""):
        localdir = self.dir_check(filedir)
        # Get the raw data and the peaks
        data = convert_rawdata(localdir)
        peaks = find_allpeaks(data)
        # Plot the data
        for i in range(1,4):
...
# Plot a voltage series
def plot_series(ts,vs,peaks=[],graph_title=""):
    # Get the baseline
    baseline,dev = find_baseline(vs)
    plt.plot(ts,vs)
...
def find_allpeaks(df):
    peaks = {}
    # Cycle through V1, V2 and V3
    for i in range(1,4):
        peaks[f"V{i}"] = find_peaks(df["Time (ms)"],df[f"V{i}"])
    return peaks
...
def find_peaks(ts,vs):
    ...
    peaks = process_peaks(ts,vs,peaks)
    return peaks

# Get the baseline of a voltage series
def find_baseline(vs,mode="deviation"):
...
def process_peaks(ts,vs,unprocessed_peaks):
