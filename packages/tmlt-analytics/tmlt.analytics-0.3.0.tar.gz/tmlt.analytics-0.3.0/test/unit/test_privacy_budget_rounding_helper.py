...
class TestGetAdjustedBudget(TestCase):
    ...
    def test_int_request(self):
        ...
        """
        adjusted = get_adjusted_budget(99, self.REMAINING_EXACT)
        self.assertEqual(adjusted, 99)

        adjusted = get_adjusted_budget(101, self.REMAINING_EXACT)
        self.assertEqual(adjusted, 101)
    ...
    def test_float_request(self):
        ...
        adjusted = get_adjusted_budget(99.1, self.REMAINING_EXACT)
        self.assertEqual(adjusted, ExactNumber.from_float(99.1, False))

        fudge_factor = 1 / 1e9

        # Even if request is only slightly less, we still shouldn't round up.
        requested = self.REMAINING_INT - fudge_factor
        adjusted = get_adjusted_budget(requested, self.REMAINING_EXACT)
        self.assertEqual(adjusted, ExactNumber.from_float(requested, False))

        # Slightly greater than the remaining budget means we should round down.
        requested = self.REMAINING_INT + fudge_factor
        adjusted = get_adjusted_budget(requested, self.REMAINING_EXACT)
        self.assertEqual(adjusted, self.REMAINING_EXACT)

        # Up to the threshold, we should still round down.
        requested = self.REMAINING_INT + (self.REMAINING_INT * fudge_factor)
        adjusted = get_adjusted_budget(requested, self.REMAINING_EXACT)
        self.assertEqual(adjusted, self.REMAINING_EXACT)
        ...
        requested = self.REMAINING_INT + (self.REMAINING_INT * fudge_factor * 2)
        adjusted = get_adjusted_budget(requested, self.REMAINING_EXACT)
        self.assertEqual(adjusted, ExactNumber.from_float(requested, False))


class TestRequestedBudgetIsSlightlyHigherThanRemaining(TestCase):
    """Tests that our 'slightly higher' check works as intended."""

    def test_requested_budget_much_higher(self):
        ...
        self._compare_budgets(requested, remaining, False)

    def test_requested_budget_much_lower(self):
    ...
    def test_requested_budget_equals_remaining(self):
        """No need to perform any rounding if request and response are equal."""
        requested = ExactNumber(50)
        remaining = ExactNumber(50)
        self._compare_budgets(requested, remaining, False)

    def test_requested_budget_slightly_lower(self):
    ...
    def test_requested_budget_slightly_higher(self):
        ...
        requested = remaining + ((remaining - 1) * fudge_factor)
        self._compare_budgets(requested, remaining, True)

        # Exactly equal to the tolerance threshold
        requested = remaining + (remaining * fudge_factor)
        self._compare_budgets(requested, remaining, True)

    def test_infinite_request_and_remaining(self):
        """Confirm that the comparison of infinite budgets works correctly."""
        self._compare_budgets(
            ExactNumber(float("inf")), ExactNumber(float("inf")), False
        )

    @typechecked
    def _compare_budgets(
        self, requested: ExactNumber, remaining: ExactNumber, expected_comparison: bool
    ):
