...
class Positions(NumDataTimeStamped):
	...
	def interpolateAtTime(self,utc:int,sorted_time_points:list = [] ,startIndex:int = 0):
		...
		# lat
		sorted_time_points_none_cleanded = [t for t in sorted_time_points if not self.positions[t].lat is None] 
		alpha,pre_utc,post_utc,past_nearest_index = self.getAlpha(sorted_time_points_none_cleanded,utc,startIndex)
		if alpha is None:
			new_lat = None #self.positions[pre_utc].lat
		else:
			new_lat = self.positions[pre_utc].lat * (1-alpha) + self.positions[post_utc].lat * alpha

		# lon
		sorted_time_points_none_cleanded = [t for t in sorted_time_points if not self.positions[t].lon is None] 
		alpha,pre_utc,post_utc,past_nearest_index = self.getAlpha(sorted_time_points_none_cleanded,utc,past_nearest_index)
		if alpha is None:
			new_lon = None #self.positions[pre_utc].lon
		else:	
			new_lon = self.positions[pre_utc].lon * (1-alpha) + self.positions[post_utc].lon * alpha

		# dep
		sorted_time_points_none_cleanded = [t for t in sorted_time_points if not self.positions[t].dep is None] 
		alpha,pre_utc,post_utc,past_nearest_index = self.getAlpha(sorted_time_points_none_cleanded,utc,past_nearest_index)
		if alpha is None:
			new_dep = None #self.positions[pre_utc].dep
		else:
			new_dep = self.positions[pre_utc].dep * (1-alpha) + self.positions[post_utc].dep * alpha
		#print("pre_utc",pre_utc)


		# hgt
		sorted_time_points_none_cleanded = [t for t in sorted_time_points if not self.positions[t].hgt is None] 
		alpha,pre_utc,post_utc,past_nearest_index = self.getAlpha(sorted_time_points_none_cleanded,utc,past_nearest_index)
		if alpha is None:
			new_hgt = None #self.positions[pre_utc].hgt
		else:
			new_hgt = self.positions[pre_utc].hgt * (1-alpha) + self.positions[post_utc].hgt * alpha

		# uncert
		sorted_time_points_none_cleanded = [t for t in sorted_time_points if not self.positions[t].uncert is None] 
		alpha,pre_utc,post_utc,past_nearest_index = self.getAlpha(sorted_time_points_none_cleanded,utc,past_nearest_index)
		if alpha is None:
			new_uncert = None #self.positions[pre_utc].uncert
		else:
...
class Attitudes(NumDataTimeStamped):
	...
	def interpolateAtTime(self,utc:int,sorted_time_points:list = None ,startIndex:int = 0):
		...
		sorted_time_points_none_cleanded = [t for t in sorted_time_points if not self.attitudes[t].containsNoneValue()]

		prev_nearest_index, past_nearest_index = findNearestNeighbors(sorted_time_points_none_cleanded,utc,startIndex)
		pre_utc = sorted_time_points_none_cleanded[prev_nearest_index]
		post_utc = sorted_time_points_none_cleanded[past_nearest_index]
...
def median2D(x_:list,y_:list,maxIteratinos = 1000,TerminationTolerance = 0.000001):
	...
	for i in range(maxIteratinos):
		...
		divisor = 0
		R = np.array([0,0])
		vardi = 0; # marker for vardi modification
		...
		for j in range(len(x)):
			...
			if not (estimation == current).all():
				...
				divisor = divisor + (1 / magnDistance)
		...
		if vardi == 0:
			...
			compare1 = [0, (1 - (vardi / magnR)) ]
			compare2 = [ 1, (vardi / magnR) ]
