...
try:
    ...
    from ._schemas import JWT
    from ._schemas.wdms import (
        Company,
        CompanyList,
        Area,
        AreaList,
...
class Client(OAuthClient):
    ...
    @validate_arguments
    def terminals_upload_transaction(
        self,
        terminals: List[int],
        **kwargs
    ) -> Any:
        ...
        **kwargs
        ...
        if not id:
            return DepartmentList(
                **self.get(path=path, params=params, **kwargs)
            )
    ...
    def delete_transaction(self, id: int, **kwargs) -> bool:
        ...
        return self.delete(
            path=f'{self.device_api_url}transactions/{id}/',
            **kwargs
        )
