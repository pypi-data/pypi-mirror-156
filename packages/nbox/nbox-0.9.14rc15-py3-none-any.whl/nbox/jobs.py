...
def _nbx_job(folder_name: str):
  ...
  job_is_valid = re.findall("^[a-zA-Z0-9_]+$", job_id_or_name)
  if not job_is_valid:
    raise ValueError("Job ID or name can only contain letters and underscore")
  ...
  logger.info(f"Creating a folder: {folder_name}")
...
def new_model(project_name):
  ...
  open(".nboxignore", "w").close() # touch .nboxignore
