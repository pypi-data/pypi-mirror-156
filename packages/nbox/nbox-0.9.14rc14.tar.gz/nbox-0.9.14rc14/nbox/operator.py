...
from .hyperloop.dag_pb2 import DAG, Flowchart, Node, RunStatus
from .hyperloop.serve_pb2 import Serving
from .nbxlib.tracer import Tracer
from .messages import get_current_timestamp, write_string_to_file
...
class Operator():
  ...
  def __call__(self, *args, **kwargs):
    ...
    with log_latency(f"{self.__class__.__name__}-prestep"):
      input_dict = {}
    ...
    # ---- USER SEPERATION BOUNDARY ---- #
    with log_latency(f"{self.__class__.__name__}-poststep"):
      outputs = {}
      if out is None:
  ...
  def deploy(
    ...
    *,
    ...
    write_string_to_file(U.join(init_folder, "job_proto.pbtxt"), job_proto)
