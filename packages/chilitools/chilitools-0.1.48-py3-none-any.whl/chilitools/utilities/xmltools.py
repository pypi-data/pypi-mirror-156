...
def getIllegalChars(xmlString) -> list:
...
def removeTimelineTags(docXML: str, verbose: bool = False):
  ...
  doc = str(_stripTimelineTag(doc=doc, verbose=verbose))
  if verbose: print(f"Size of doc after: {len(doc)}")
  return doc
...
def _stripTimelineTag(doc, verbose: bool = False):
  ...
  for snippet in snippets:
    ...
    snippet['snippetDocXML'] = str(doc=_stripTimelineTag(BeautifulSoup(snippet['snippetDocXML'], 'xml'), verbose=verbose))
