...
from utilities.backoffice import backofficeURLInput
from api.connector import ChiliConnector
from api.mycp import generateLoginTokenForURL, generateOAuthTokenFromCredentials, setUserType
from utilities.defaults import STAFF_TYPE
...
def menuLoop(app: dict):
  while (True):
    ...
    menuLength = len(app['menuItems'])
    menuItems = list(app['menuItems'].keys())
    print(f"\nCHILI Publish Testing Tools\t>>\tCopy Output: {app['clipboard']}")
    print("---------------------------------------------------------\n")
    ...
    if re.search('[1-' + str(menuLength) + ']', option):
      option = int(option) - 1
