...
from user_defined_alias_1310 import (
    Si,
    SiO2,
    fdtd_config,
    source_config,
...
def simulation(
    ...
    if extract:
        ...
        me_result = fdtd.run_mode_expansion(
            fdtd_path=result.workspace
        )  # for mode monitor
        ...
        df = passive_fdtd_mode_expansion_fde_result_chart(
            ...
            dict(x=0, y="plotX", z="plotY", mode="1"),
        )
        tf = TaskResult(df, "intensity")
        x, y, Z = (
            tf.raw_data["horizontal"],
        ...
        df = passive_fdtd_fd_result_chart(
            ...
            dict(x="plotX", y="plotY", z=0, wavelength=0),
        )
        tf = TaskResult(df, "intensity")
        x, y, Z = (
            tf.raw_data["horizontal"],
        ...
        # --------
        lineplot(10 * np.log10(tf.DataFrame))
        ...
        df = passive_fdtd_mode_expansion_chart(
            ...
            dict(mode="plotX", wavelength="1.31"),
        )
        tf = TaskResult(df, "line")
        lineplot(tf.DataFrame)
        plt.xlabel("mode num")
