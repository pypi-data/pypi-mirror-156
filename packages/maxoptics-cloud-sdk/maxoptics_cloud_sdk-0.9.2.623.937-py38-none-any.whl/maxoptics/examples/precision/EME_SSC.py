...
def simulation(gds_file, gds_topcell, run=True, wavelength=1.5, grid=0.2):
    ...
    struct_sio2 = EME_SSC.gds_import(
        gds_file, gds_topcell, "1/0", SiO2_SSC, -3, 0
    )
    struct_sio2["mesh_order"] = 1
    struct_si = EME_SSC.gds_import(
        gds_file, gds_topcell, "2/0", Si_SSC, 0, 0.2
    )
    struct_si["mesh_order"] = 2
    struct_sion = EME_SSC.gds_import(
        gds_file, gds_topcell, "3/0", SiON_SSC, 0, 3
    )
    ...
    if run:
        ...
        x, y, Z = (
            data.raw_data["horizontal"],
            data.raw_data["vertical"],
            data.raw_data["data"],
        )
        fig = heatmap(x, y, Z)
        plt.xlabel("x (μm)")
        ...
        emePropagationData = t2.passive_eme_sweep_chart(
            target="line", attribute="S", operation="ABS^2"
        )
        ...
        #             )
        json_save(
            os.path.join(plot_path, "01" + "_abs^2(" + "S21" + ")_line.json"),
            emePropagationData.raw_data,
        )
