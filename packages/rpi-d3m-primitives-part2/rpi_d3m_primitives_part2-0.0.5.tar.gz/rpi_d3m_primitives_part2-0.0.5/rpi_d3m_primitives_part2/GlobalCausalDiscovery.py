...
from rpi_d3m_primitives_part2.causalDiscovery.causaldiscovery_BayesEst import causaldiscovery_BayesEst
from sklearn.impute import SimpleImputer
from rpi_d3m_primitives_part2.featSelect.RelationSet import RelationSet


Inputs = container.DataFrame
Outputs = container.ndarray # output the DAG

__all__ = ('GlobalCausalDiscovery',)
...
class Params(params.Params):
    ...
    dag_: Optional[Union[np.ndarray, List[np.ndarray]]]
...
class GlobalCausalDiscovery(SupervisedLearnerPrimitiveBase[Inputs, Outputs, Params, Hyperparams]):
    """
     A primitive which does global causal discovery.
    """
    __git_commit__ = utils.current_git_commit(os.path.dirname(__file__))
    metadata = metadata_base.PrimitiveMetadata({
        'id': '1f4ab9c5-de28-428e-9261-7c8cc039283f',
        'version': rpi_d3m_primitives_part2.__coreversion__,
        'name': 'Global Causal Discovery',
        'keywords': ['Causal Discovery','Structure Learning','Classification', 'Bayesian Inference'],
        'description': 'This algorithm is an implementation of Global Causal Discovery using Indepedence test based DAG learning method.',
        'source': {
            'name': rpi_d3m_primitives_part2.__author__,
            'contact': 'mailto:yinn2@rpi.edu',
            'uris': [
                'https://gitlab.com/N.Yin/rpi-d3m-part2/-/blob/{git_commit}/rpi_d3m_primitives_part2/GlobalCausalDiscovery.py'.format(
        ...
        'python_path': 'd3m.primitives.classification.global_causal_discovery.RPI',
        'algorithm_types': [
            metadata_base.PrimitiveAlgorithmType.CAUSAL_ANALYSIS],
    ...
    def fit(self, *, timeout: float = None, iterations: int = None) -> None:
        ...
        ## perform causal discovery using training data
        D, dag_learned = causaldiscovery_BayesEst(X_train, Y_train)
        
        self._fitted = True
        self._dag = dag_learned
