import numpy as np
from rpi_d3m_primitives_part2.featSelect.discretization import HC_discretization
...
class RelationSet:
	...
	def remove(self):
		self.index_list = np.setdiff1d(np.arange(self.num_features), np.array(self.removeIdx))
		self.NUM_STATES = self.NUM_STATES[self.index_list]
		self.data = self.data[:,self.index_list]
