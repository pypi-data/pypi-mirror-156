...
from rpi_d3m_primitives_part2.structuredClassifier.BN_Classifier_model import Model as BNC
from sklearn.impute import SimpleImputer
...
from rpi_d3m_primitives_part2.structuredClassifier.BayesianClassifier import BayesianClassifier as BC
...
__all__ = ('GCD_Classification',)

class Params(params.Params):
    n_bins_: Optional[int]
    strategy_: Optional[str]
    learning_method_: Optional[str]
    depth_: Optional[int]
    alpha_: Optional[float]
    L_: Optional[int]
    S_: Optional[int]
    burnin_: Optional[int]
    ...
    pseudo_count_: Optional[Union[int, float]]
...
class Hyperparams(hyperparams.Hyperparams):
    ...
    depth = hyperparams.UniformInt(
            ...
            upper = 10000,
            default = -1,
            description = 'maximum size of conditioning set',
            semantic_types = ['https://metadata.datadrivendiscovery.org/types/TuningParameter']
            )
    alpha = hyperparams.Uniform(
            ...
            upper = 1.0,
            default = 0.05,
            description = 'significant level of chi-square independence test',
            semantic_types = ['https://metadata.datadrivendiscovery.org/types/TuningParameter']
            )
    pseudo_count = hyperparams.Uniform(
            lower = 1e-10,
            upper = 1e10,
            default = 1,
    ...
    learning_method = hyperparams.Enumeration[str](
            values=['MLE', 'BayesianLearning'],
            default='MLE',
            description='whether to perform bayesian learning.',
            semantic_types = ['https://metadata.datadrivendiscovery.org/types/TuningParameter']
            )
...
class GCD_Classification(SupervisedLearnerPrimitiveBase[Inputs, Outputs, Params, Hyperparams]):
    """
     A primitive which does global causal discovery. It reveals the causal relations between features which encoded in the given data. To demonstrate its application, the algorithm also implement a structured classifier use the results of the global causal discovery.
    """
    __git_commit__ = utils.current_git_commit(os.path.dirname(__file__))
    metadata = metadata_base.PrimitiveMetadata({
        'id': '4e74a859-fe9f-4520-a9d7-a023bb2a805f',
        'version': rpi_d3m_primitives_part2.__coreversion__,
        'name': 'Global Causal Discovery',
        'keywords': ['Global Causal Discovery','Causal Graphical Graph','Classification'],
        'description': 'This algorithm is an implementation of widely used global causal discovery. It reveals the causal relations between features which encoded in the given data. To demonstrate its application, the algorithm also implement a structured classifier use the results of the global causal discovery. ',
        'source': {
            'name': rpi_d3m_primitives_part2.__author__,
            'contact': 'mailto:yinn2@rpi.edu',
            'uris': [
                'https://gitlab.com/N.Yin/rpi-d3m-part2/-/blob/{git_commit}/rpi_d3m_primitives_part2/GCD_Classification.py'.format(
        ...
        'python_path': 'd3m.primitives.classification.global_causal_discovery.ClassifierRPI',
        'algorithm_types': [
    ...
    def __init__(self, *, hyperparams: Hyperparams, random_seed: int = 0, docker_containers: typing.Union[typing.Dict[str, base.DockerContainer]] = None) -> None:
        ...
        self._alpha = self.hyperparams['alpha']
        self._pseudo_count = self.hyperparams['pseudo_count']
        ...
        self._learning_method = self.hyperparams['learning_method']
    ...
    def fit(self, *, timeout: float = None, iterations: int = None) -> None:
        ...
        ## fit the classifier
        if self._learning_method == "MLE":
            self._D, self._structure = CD(X_train, Y_train, depth = self._depth, alpha = self._alpha)
            self._structure_fit = True
            self._fitted = True

            np.savetxt('Causal_Structure.txt', self._structure, fmt = '%d', delimiter = ',')
            g = draw_graph(self._structure, self._names)
            g.render('Causal_Graph', view = False, cleanup = True)
        elif self._learning_method == "BayesianLearning":
            clf = BC(S = self._S, L = self._L, burnin = self._burnin, random_seed = self._rs,equivalent_sample_size = self._pseudo_count)
    ...
    def produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]:  # inputs: m x n numpy array
        if self._fitted:
            ...
            ## fit the classifier
            if self._learning_method == "MLE":
                clf = BNC(structure = self._structure, structure_fit = self._structure_fit, D = self._D, pseudo_count = self._pseudo_count)
                clf.fit(X_train, Y_train, state_names)
            elif self._learning_method == "BayesianLearning":
                clf = BC(DAG = self._structure, L = self._L, S = self._S, burnin = self._burnin, random_seed = self._rs, equivalent_sample_size = self._pseudo_count)
            ...
            ## prediction
            if self._learning_method == "MLE":
                output = clf.predict(disc_X_test)
            elif self._learning_method == "BayesianLearning":
    ...
    def set_params(self, *, params: Params) -> None:
        ...
        self._D = params['D_']
        self._depth = params['depth_']
        self._alpha = params['alpha_']
        self._pseudo_count = params['pseudo_count_']
        self._L = params['L_']
        self._S = params['S_']
        self._burnin = params['burnin_']
        # self._equivalent_sample_size = params['equivalent_sample_size_']
        self._learning_method = params['learning_method_']
