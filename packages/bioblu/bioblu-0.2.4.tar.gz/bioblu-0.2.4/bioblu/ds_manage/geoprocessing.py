...
def get_real_world_latlong_shift_from_px_shift(px_shift_yx: Tuple[int, int], gsd_cm: float,
    ...
    logging.debug(f"px shift yx: {px_shift_yx}")
    logging.debug(f"px angle: {px_shift_angle}")
    logging.debug(f"World shift angle: {abs_shift_angle}")
    logging.debug(f"{px_shift_angle == abs_shift_angle =}")
    logging.debug(f"euclidean shift distance: {shift_distance_px}")
    logging.debug(f"Real world shift (px): {actual_px_shift_yx}")
    logging.debug(f"Real world shift (m): {actual_meter_yx_shift}")
    logging.debug(f"Latitude shift: {lat_shift}")
    logging.debug(f"Lognitude shift: {lon_shift}")
...
def get_M_N(latitude: float):
    ...
    # M = (a * (1 - e ** 2)) / ((1 - (e ** 2) * ((math.sin(math.radians(latitude))) ** 2)) ** (3/2))
...
def get_all_img_coordinates(fdir_img, save_csv=False) -> pd.DataFrame:
    ...
    img_list = ds_annotations.get_all_fpaths_by_extension(fdir_img, YOLO_IMG_FORMATS)
    ...
    with Image.open(fpath_img) as im:
        ...
        if exif_raw is not None:
            ...
            if geoinfo:
                ...
                coords_found = True
    # Image.open(fpath_img).close()  # Because the file could not be closed in  in get_raw_ecif()
    return (lat, lon), coords_found
...
def get_raw_exif(fpath: str) -> PIL.Image.Exif:
    ...
    # Running this with the context manager leads to ValueError: seek of closed file
    # The same happens when the file gets closed.
    img = Image.open(fpath)
    exif = img.getexif()
    # img.close()
    return exif
...
def collect_clicks_on_all_images(fdir, altitude_m, save_csv=True):
    ...
    fpaths = [os.path.join(fdir, fname) for fname in sorted(os.listdir(fdir)) if fname.lower().endswith((".tif", ".tiff", ".jpg", ".jpeg"))]
    points = None
    for fpath in fpaths:
...
if __name__ == "__main__":
    ...
    # print(get_M_N(50))
    # print(get_meridian_parallel_radii(50))
    #
    # print(get_M_N(75))
    # print(get_meridian_parallel_radii(75))

    # fpath_img = "/media/findux/DATA/Documents/Malta_II/surveys/2022-01-28_Gnejna_beach/Test_Gnejna_bay 28-01-2022/RGB images/5m altitude/DJI_0697.JPG"
    # geolocate_point_on_img((200, 200), fpath_img, 5)

    # gcp_49 = "/media/findux/DATA/Documents/Malta_II/surveys/GCPs/paradise_bay_gcps/49/"
    # collect_clicks_on_all_images(gcp_49, 7)

    # gcp_59 = "/media/findux/DATA/Documents/Malta_II/surveys/GCPs/paradise_bay_gcps/59/"
    # collect_clicks_on_all_images(gcp_59, 7)

    # folder = "/media/findux/DATA/Documents/Malta_II/surveys/GCPs/paradise_bay_gcps/59/"
    # get_all_img_coordinates(folder, True)

    folder = "/media/findux/DATA/Documents/Malta_II/surveys/2021-12-16_paradise_bay_5m/RGB images and video/"
    get_all_img_coordinates(folder, True)

    folder = "/media/findux/DATA/Documents/Malta_II/surveys/2022-01-28_Gnejna_beach/Test_Gnejna_bay 28-01-2022/RGB images/5m altitude/"
    get_all_img_coordinates(folder, True)

    folder = "/media/findux/DATA/Documents/Malta_II/surveys/2022-01-28_Gnejna_beach/Test_Gnejna_bay 28-01-2022/RGB images/10m-30m altitude/"
    get_all_img_coordinates(folder, True)
