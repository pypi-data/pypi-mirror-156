...
def hist(hist_function, *, options={}, layouts={}, **interact_params):
    """
    ...
    if 'plot_height' not in layouts:
        layouts['plot_height'] = default_layouts['plot_height']

    if 'plot_width' not in layouts:
        layouts['plot_width'] = default_layouts['plot_width']
    ...
    fig.layout.height = layouts['plot_height']
    fig.layout.width = layouts['plot_width']
...
#    'height', 'width'
#])
def bar(x_fn, y_fn, *, options={}, layouts={}, **interact_params):
    """
    ...
    fig = options.get('_fig', False) or _create_fig(
        x_sc=bq.OrdinalScale, options=options, layouts=layouts
    )
    [bar] = _create_marks(
...
def scatter(x_fn, y_fn, *, options={}, layouts={}, **interact_params):
    """
...
def line(x_fn, y_fn, *, options={}, layouts={}, **interact_params):
    """
...
class Figure(object):
    ...
    def __init__(self, options={}, layouts={}):
    ...
    def bar(self, x_fn, y_fn, *, options={}, layouts={}, **interact_params):
        options = tz.assoc(options, '_fig', self.figure)
        layouts=layouts
...
def _create_fig(
    *,
    ...
    if 'x_grid_lines' not in layouts:
        layouts['x_grid_lines'] = default_layouts['x_grid_lines']

    if 'y_grid_lines' not in layouts:
        layouts['y_grid_lines'] = default_layouts['y_grid_lines']

    if 'x_grid_color' not in layouts:
        layouts['x_grid_color'] = default_layouts['x_grid_color']

    if 'y_grid_color' not in layouts:
        layouts['y_grid_color'] = default_layouts['y_grid_color']
    ...
    x_ax.grid_lines = layouts['x_grid_lines']
    y_ax.grid_lines = layouts['y_grid_lines']

    x_ax.grid_color = layouts['x_grid_color']
    y_ax.grid_color = layouts['y_grid_color']
