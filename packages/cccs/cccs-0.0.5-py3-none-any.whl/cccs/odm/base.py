"""
CCCS's built in Object Document Model tool.
...
from cccs.common import loader
from cccs.common.dict_utils import recursive_update
from cccs.common.isotime import now_as_iso
from cccs.common.net import is_valid_domain, is_valid_ip
from cccs.common.uid import get_random_id
...
logger = logging.getLogger(f'{loader.APP_NAME}.odm')
...
class Model:
    ...
    def markdown(cls, toc_depth=1, defaults=None) -> Union[str, Dict]:
        markdown_content = "[comment]: # (AUTOGENERATED MARKDOWN CONTENT)\n"
        ...
        def get_type(field_class: _Field) -> Tuple(str, Model):
            if field_class.__class__ == Optional:
                ...
                if '/' in module_path:
                    ...
                    module_path = module_path.split(loader.APP_NAME)[1].replace('.', '/')
