...
class Sublist(list):
    ...
    def group_multi(in_list):
        ...
        for chunk in rest_chunks:
            if len(set(first_chunk) & set(chunk)) > 0:
                first_chunk = set(first_chunk) | set(chunk)
            else:
                other.append(chunk)
        rest_chunks = other

        if len(rest_chunks) > 0:
            return [list(first_chunk)] + Sublist.group_multi(rest_chunks)
        else:
            return [first_chunk]
    ...
    def hist(var_list,
        ...
        :param var_list: List of numbers
