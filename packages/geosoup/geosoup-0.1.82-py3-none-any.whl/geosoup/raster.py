...
class Raster(object):
    ...
    def band_histogram(self,
                       ...
                       bin_range=None):
        """
        Method to compute histogram for a raster band. Uses numpy histogram function.
        :param band: Band index, starts from 0
        :param bins: If bins is an int, it defines the number of equal-width bins
        ...
        :param bin_range: The lower and upper range of the bins. If not provided,
                    range is simply (a.min(), a.max()). Values outside the range are ignored.
                    The first element of the range must be less than or equal to the second.
                    range affects the automatic bin computation as well. While bin width
                    is computed to be optimal based on the actual data within range,
                    the bin count will fill the entire range including portions containing no data.
        """
        self.read_array(band_order=[band],
                        nodatavalue=self.nodatavalue)

        array = self.array if self.nodatavalue is None else self.array[self.array != self.nodatavalue]

        histogram, bin_edges = np.histogram(array[~np.isnan(array)],
                                            bins=bins,
                                            range=bin_range)
    ...
    def add_overviews(self,
        ...
        :param resampling: nearest (default),average,rms,gauss,cubic,cubicspline,lanczos,average_magphase,mode
        :param overviews: A list of integral overview levels to build
        :param kwargs: arguments for making overviews: e.g.: bigtiff='yes', compress='lzw', etc.
        """
