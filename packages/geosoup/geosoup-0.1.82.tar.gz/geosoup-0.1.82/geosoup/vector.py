...
class Vector(object):
    ...
    def write_geoms_as_files(self,
                             name_attribute=None,
                             out_folder=None,
                             verbose=False):
        """
        Method to divide a multi geometry shapefile into multiple shapefiles
        :param name_attribute: Attribute to use for naming the output files
        :param out_folder: Output file folder
        :param verbose: If some steps whole be verbose
        """

        if out_folder is None:
            out_folder = Handler(self.filename).dirname

        counter = None

        for i, wkt_geom in enumerate(self.wktlist):

            file_suffix = '_geom_{}.shp'.format(str(i + 1)) if (name_attribute is None
                                                                or name_attribute not in self.attr_def) \
                else '_{}.shp'.format(str(self.attributes[i][name_attribute].replace(' ', '_')))

            filename = out_folder + Handler().sep + Handler(self.filename).basename.replace('.shp', '') + \
                file_suffix

            if verbose:
                sys.stdout.write('Writing {}...'.format(filename))

            new_vec = Vector(filename=filename,
                             name=Handler(filename).basename,
                             spref=self.spref,
                             geom_type=OGR_GEOM_DEF[self.type],
                             verbose=verbose)

            new_vec.wktlist = [self.wktlist[i]]
            new_vec.type = self.type
            new_vec.attr_def = self.attr_def
            new_vec.attributes = [self.attributes[i]]
            new_vec.nfeat = 1
            new_vec.fields = self.fields
            new_vec.datasource = None

            new_vec.write_vector(outfile=filename)
            counter = i

        if verbose:
            sys.stdout.write('\nWritten {} files from {}.\n'.format(str(counter + 1),
                                                                    Handler(self.filename).basename))
    ...
    def reproject(self,
        ...
        vector = Vector(geom_type=OGR_GEOM_DEF[self.type],
                        spref=destination_spatial_ref,
                        name=Handler(self.name).add_to_filename('_reproj'))

        vector.filename = Handler(self.filename).add_to_filename('_reproj')
