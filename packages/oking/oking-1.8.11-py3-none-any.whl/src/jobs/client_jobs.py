...
def job_send_representative(job_config: dict) -> None:
    """
        Job para enviar os clientes para api okvendas
    ...
    try:
        ...
        if db_config.sql is None:
            send_log(job_config.get('job_name'), job_config.get('enviar_logs'), False, f'Comando sql para criar produtos nao encontrado', 'warning', 'REPRESENTANTE')
            return



    except Exception as e:
        send_log(job_config.get('job_name'), job_config.get('enviar_logs'), False, f'Erro nao esperado na integracao de clientes: {str(e)}', 'error', 'REPRESENTANTE')
