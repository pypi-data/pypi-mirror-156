...
class LspCompiler:
    ...
    __errors: Dict[Path, List[SolcOutputError]]
    ...
    @property
    def errors(self) -> Dict[Path, List[SolcOutputError]]:
        ...
        for cu, solc_output in out:
            ...
            for error in solc_output.errors:
                if error.source_location is not None:
                    ...
                    self.__errors[path].append(error)
                #if error.secondary_source_locations is not None:
                #for secondary_location in error.secondary_source_locations:
                #source_unit_name = secondary_location.file
                #path = cu.source_unit_name_to_path(source_unit_name)
                #self.__errors[path].append(error)
            errored = any(
