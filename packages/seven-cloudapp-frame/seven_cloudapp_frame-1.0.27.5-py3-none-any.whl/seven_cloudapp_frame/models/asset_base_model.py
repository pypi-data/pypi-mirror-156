...
@LastEditTime: 2022-06-27 15:47:41
...
class AssetBaseModel():
    ...
    def update_store_asset(self, app_id,store_id, store_name, asset_type, asset_value, asset_object_id, source_type, source_object_id, source_object_name, log_title,handler_name="",request_code="", info_json={}):
        """
        :description: 变更商家资产
        :param app_id：应用标识
        :param store_id：商家ID
        ...
        db_transaction = DbTransaction(db_config_dict=config.get_value("db_cloudapp"),context=self.context)
        store_asset_model = StoreAssetModel(db_transaction=db_transaction, context=self.context)
        store_asset_log_model = StoreAssetLogModel(db_transaction=db_transaction, context=self.context)
        ...
        try:
            ...
            store_asset_log.info_json = info_json if info_json else {}
            ...
            if result == False:
                ...
                invoke_result_data.error_message = "系统繁忙,请稍后再试"
                return invoke_result_data
            store_asset_log_model.delete_dependency_key(f"store_asset_log_list:appid_{app_id}")
    ...
    def get_store_asset(self, app_id, asset_type, asset_object_id):
        """
        ...
        if not asset_type:
            return None
        store_asset_model = StoreAssetModel(context=self.context)
        store_asset_id_md5 = CryptoHelper.md5_encrypt_int(f"{app_id}_{asset_type}_{asset_object_id}")
    ...
    def get_store_asset_list(self, app_ids, asset_type=0):
        """
    ...
    def get_store_asset_log_list(self, app_id, asset_type, page_size=20, page_index=0, store_id=0,store_name="", asset_object_id="", start_date="", end_date="", source_type=0, source_object_id=None, field="*", is_cache=True, operate_type=-1):
        """
        ...
        if source_object_id:
            if type(source_object_id) == str:
                ...
                condition += " AND source_object_id=%s"
                params.append(source_object_id)
        store_asset_log_model = StoreAssetLogModel(context=self.context)
        if is_cache:
            page_list, total = store_asset_log_model.get_cache_dict_page_list(field, page_index, page_size, condition, order_by="create_date desc", params=params,dependency_key=f"store_asset_log_list:appid_{app_id}")
        else:
            page_list, total = store_asset_log_model.get_dict_page_list(field, page_index, page_size, condition, order_by="create_date desc", params=params)
        if len(page_list) > 0:
            for item in page_list:
                if item["app_id"] != app_id:
                    return [],0
