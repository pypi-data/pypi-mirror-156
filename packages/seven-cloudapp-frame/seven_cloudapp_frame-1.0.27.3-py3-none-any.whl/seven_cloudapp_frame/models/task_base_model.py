...
@LastEditTime: 2022-06-22 15:31:11
...
class TaskBaseModel(FrameBaseModel):
    ...
    def process_receive_reward(self,app_id,act_id,module_id,user_id,login_token,task_id,task_sub_type,handler_name,request_code,task_type=0,check_new_user=False,check_user_nick=True,continue_request_expire=5,is_stat=True,info_json=None):
        """
        ...
        try:
            invoke_result_data = self.business_process_executing(app_id, act_id, module_id, user_id, login_token, handler_name, check_new_user, check_user_nick, continue_request_expire, acquire_lock_name, source_object_id=source_object_id)
                ...
                if task_invoke_result_data.success == True:
                    ...
                    if reward_value <= 0 or task_count.now_count <=0:
                        ...
                        if task_type == int(TaskType.invite_new_user.value) or task_type == int(TaskType.invite_join_member.value):
                            ...
                            if task_type == int(TaskType.invite_new_user.value):
                                invite_sub_table = self.get_business_sub_table("invite_log_tb",{"act_id":act_id,"user_id":user_id})
                                invite_sub_table = "" if not invite_sub_table else str(invite_sub_table)
                            else:
    ...
    def process_invite_user(self,app_id,act_id,module_id,user_id,login_token,from_user_id,handler_name,check_user_nick=True,check_new_user=True,continue_request_expire=5,close_invite_limit=False):
        """
    ...
    def process_routine_task(self,task_type,app_id,act_id,module_id,user_id,login_token,handler_name,check_new_user=False,check_user_nick=True,continue_request_expire=5):
        """
        :description: 处理常规任务 只限制领取数量和领取时间时可以使用,并且只计数不领取奖励
        :param task_type:任务类型
        :param app_id:应用标识
        :param act_id:活动标识
        :param module_id:活动模块标识
    ...
    def process_routine_invite_task(self,task_type,sub_prefix,app_id,act_id,module_id,user_id,login_token,from_user_id,handler_name,check_user_nick=True,check_new_user=True,continue_request_expire=5,close_invite_limit=False):
        """
        :description: 处理常规邀请任务(当一个活动有多个邀请任务时，可以使用当前方法)
        :param task_type:任务类型
        :param sub_prefix:分表前缀名
        :param app_id:应用标识
        :param act_id:活动标识
        :param module_id:活动模块标识
        :param user_id:当前用户标识
        ...
        try:
            ...
            if invoke_result_data.success == True:
                ...
                user_base_model = UserBaseModel(context=self.context)
                from_user_info_dict = user_base_model.get_user_info_dict(app_id,act_id,from_user_id)   
                invite_sub_table = self.get_business_sub_table(f"invite_log_{sub_prefix}_tb",{"act_id":act_id,"user_id":user_id})
                invite_sub_table = sub_prefix if not invite_sub_table else f"{sub_prefix}_"+str(invite_sub_table)
                invite_log_model = InviteLogModel(context=self.context,sub_table=invite_sub_table)
                is_invite = invite_log_model.get_total("act_id=%s and invite_user_id=%s", params=[act_id, user_id])
                if is_invite > 0:
                    invoke_result_data.success = False
