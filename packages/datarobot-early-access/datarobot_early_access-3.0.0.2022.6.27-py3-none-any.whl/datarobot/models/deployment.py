...
from ..utils import deprecated, from_api, get_id_from_location
...
class Deployment(APIObject, DeploymentQueryBuilderMixin):
    ...
    _prediction_usage = t.Dict(
        {
            ...
            t.Key("last_timestamp", optional=True): String
            >> (lambda s: dateutil.parser.parse(s)),  # pylint: disable=unnecessary-lambda
        }
    ).allow_extra("*")
    _health = t.Dict(
        {
            ...
            t.Key("start_date", optional=True): String
            >> (lambda s: dateutil.parser.parse(s)),  # pylint: disable=unnecessary-lambda
            t.Key("end_date", optional=True): String
            >> (lambda s: dateutil.parser.parse(s)),  # pylint: disable=unnecessary-lambda
        }
    ).allow_extra("*")
    _converter = t.Dict(
        {
            ...
            t.Key("status", optional=True): String() | t.Null(),
    ...
    def __repr__(self):
        return "{}({})".format(self.__class__.__name__, self.label or self.id)
    ...
    def activate(self, max_wait=600):
        """
    ...
    def deactivate(self, max_wait=600):
        """
    ...
    def _change_status(self, status, max_wait):  # pylint: disable=missing-function-docstring
    ...
    def get_features(self):
        ...
        return from_api(list(data), keep_null_keys=True)
    ...
    def update_segment_analysis_settings(
        ...
        segment_analysis_attributes=None,
        max_wait=DEFAULT_MAX_WAIT,
    ):
    ...
    def update_prediction_intervals_settings(
        ...
        if percentiles:
            ...
            from .model import DatetimeModel  # pylint: disable=import-outside-toplevel
...
class DeploymentListFilters(object):  # pylint: disable=missing-class-docstring
    def __init__(
        self,
        role=None,
        service_health=None,
        ...
        **Performing OR logic**
        ...
        self.service_health = _check(
            t.List[String()], service_health  # pylint: disable=unsubscriptable-object
        )
        self.model_health = _check(
            t.List[String()], model_health  # pylint: disable=unsubscriptable-object
        )
        self.accuracy_health = _check(
            t.List[String()], accuracy_health  # pylint: disable=unsubscriptable-object
        )
        self.execution_environment_type = _check(
            t.List[String()], execution_environment_type  # pylint: disable=unsubscriptable-object
        )
        self.importance = _check(
            t.List[String()], importance  # pylint: disable=unsubscriptable-object
        )

    def construct_query_args(self):  # pylint: disable=missing-function-docstring
        query_args = {}
