...
from ..utils import deprecation_warning, from_api


class HasHistogram(object):  # pylint: disable=missing-class-docstring
...
class Feature(APIObject, HasHistogram):
    ...
    def __init__(
        ...
        if isinstance(id, dict):
            ...
            self.__init__(**from_api(id))  # pylint: disable=non-parent-init-called
        else:
            self.id = id
...
class ModelingFeature(APIObject, HasHistogram):
    ...
    def __repr__(self):
        return "ModelingFeature({})".format(self.name)
...
class DatasetFeature(APIObject):
    ...
    def __repr__(self):
        return "Feature({}, dataset_id={})".format(self.name, self.dataset_id)
