...
import re
from typing import Any, cast, Dict, List, Match, Optional, Tuple, TYPE_CHECKING, Union
...
def _pivot_multilabel_predictions(
    ...
    values_df = pd.DataFrame(
        values, columns=["{}{}".format(class_prefix, label) for label in labels]
    )
    thresholds_df = pd.DataFrame(
        thresholds, columns=["threshold_{}".format(label) for label in labels]
    )
    predictions_df = pd.DataFrame(
        predictions, columns=["prediction_{}".format(label) for label in labels]
    )
    return pd.concat(
        [
