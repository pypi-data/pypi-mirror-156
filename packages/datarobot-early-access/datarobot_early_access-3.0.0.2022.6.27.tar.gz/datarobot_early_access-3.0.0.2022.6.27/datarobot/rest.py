...
from .errors import ClientError, JobAlreadyRequested, ServerError
...
class RESTClientObject(requests.Session):
    ...
    def __init__(
        ...
        if max_retries is None:
            retry_kwargs: Dict[str, Union[int, float, Dict[str, Any]]] = {
                "connect": 5,
                "read": 0,
                "backoff_factor": 0.1,
            }
    ...
    def build_request_with_file(
        ...
        if file_path:
            if not os.path.exists(file_path):
                raise ValueError("Provided file does not exist {}".format(file_path))
