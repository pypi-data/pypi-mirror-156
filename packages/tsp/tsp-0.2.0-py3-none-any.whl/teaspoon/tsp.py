...
class TSP:
    """ a teaspoon / Time Series Profile / thermal state of permafrost 
    ...
    def monthly(self) -> "TSP":
        """ Return data as monthly averages 
    ...
    def plot_trumpet(self, year: Optional[int]=None, begin: Optional[datetime]=None, end: Optional[datetime]=None, **kwargs) -> 'matplotlib.figure.Figure':
        ...
        **kwargs : dict, optional
        ...
        max_t = df.groupby('depth').max().get('temperature_in_ground').values
        min_t = df.groupby('depth').min().get('temperature_in_ground').values
        mean_t = df.groupby('depth').mean(skipna=True).get('temperature_in_ground').values
        
        fig = trumpet_curve(depth=self.depths, t_max=max_t, t_min=min_t, t_mean=mean_t, **kwargs)
        fig.show()
