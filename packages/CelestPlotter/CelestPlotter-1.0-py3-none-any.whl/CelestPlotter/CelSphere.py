#To plot converted RA and Declination values on the Celestial Sphere 
...
import CelestPlotter.InNCon as IN
import math


def Plot():#method to plot the RA and Declination values
    fig = plt.figure()
    ax = fig.gca(projection='3d')
    #Polar co-ordinates for sphere
    ...
    ax.add_artist(a)
    #Setting the legends for Celestial Sphere and Equatorial Plane
    ...
    #Setting X,Y,Z Cartesian axes' labels
    ...
    error=None
    try:
        RA,Dec=IN.In() # Inputting RA and Declination values from User
        RA_deg,Dec_deg=IN.Convert(RA,Dec)# Converting RA, Dec values to degrees
        class ValError(Exception):
            pass
        if( math.isnan(RA_deg)==True and math.isnan(Dec_deg)==True): # Exception raised if input is given in wrong format
            raise ValError
        #RA and Declination values converted to Cartesian co-ords.
        ...
        #Scatter plot for obtained values of x,y,z
