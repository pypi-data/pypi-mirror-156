...
class InteractiveHistogram(AsyncEmbeddable):
    ...
    def init_embeddable(self, df, column, **kwargs):
        ...
        series_has_more_than_100k_rows = len(original_series) > 100_000
        min_max_diff_is_larger_than_1billion = (
            abs(default_max - default_min) > 1_000_000_000
        )
        use_seaborn = (
            series_has_more_than_100k_rows or min_max_diff_is_larger_than_1billion
        )
        ...
        if plotting_backend == "seaborn":
            if series_has_more_than_100k_rows:
                note = """
                The histogram currently only supports up to 100,000 rows of data.<br>
                <br>
                However, your table has more rows than that.<br>
                <br>
                In the near future, we will support this kind of data.<br>
                In the meantime, you might want to sample or filter your data.
                """
