...
class RowsSampler(AsyncEmbeddable):
    ...
    def init_embeddable(
        self, df=None, max_rows=100, notification_text=None, on_render=None, **kwargs
    ):
        self.df = df
        self.max_rows = max_rows
        self.on_render = on_render
        if notification_text is None:
            notification_text = f"Your dataframe has more than {self.max_rows:,} rows which leads to long loading times. Therefore, we randomly sampled {self.max_rows:,} rows."
            #  If you want you can remove the sampling. Alternatively, you might want to manually filter or aggregate the data manually before plotting it.
