...
class PackageVersion():
    def __init__(self, package_name, package_type, version, license, publish_time, ):
        self.version = version
        self.license = license
...
class SqlTool(object):
        ...
        if data:
            if version_id:
                update_version = f"update package_version set `version` = '%s', `license` = '%s', `publish_time` = '%s'," \
                                 f" `package_name` = '%s', `package_id` = %d, `package_type` = '%s' where " \
                                 f"package_name = '%s' and package_type='%s'" % (
                                     version.version, version.license, version.publish_time, version.package_name,
                                     int(data['id']),
                                     data['package_type'], version.package_name, version.package_type
                                 )
    ...
    def package_deps(self, deps: PackageDeps):
        ...
        for require in deps.requirement:
            ...
            if deps_id and deps_version:
                sql = f"update package_dependencies set `dependency_version_expression` = %s, " \
                      f"`lj_dependency_version_expression`=%s, `dependency_version`= %s, `dependency_type`= %s, `package_name`= %s, `package_version`= %s, " \
                      f"`dependency_package_name`= %s, `dependency_package_id`= %s, `package_id`= %s, " \
                      f"`package_type`= %s"
            else:
            ...
            args_list.append((require['requirement'], deps.lj_dependency_version_expression,
                              deps_version['version'], deps.dependency_type, _data['package_name'],
