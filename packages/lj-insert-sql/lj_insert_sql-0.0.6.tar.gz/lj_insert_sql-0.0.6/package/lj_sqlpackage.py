...
class PackageDeps():
    def __init__(self, package_name, package_type, lj_dependency_version_expression, dependency_type, requirement:list):
            ...
            if deps.requirement:
                for require in deps.requirement:
                    ...
                    sql = f"insert into package_dependencies(`dependency_version_expression`, " \
                          ...
                          f"`package_type`) values('%s', '%s', '%s', '%s', '%s', '%s', '%s', %d, %d, '%s')" % (
                              require['requirement'], deps.lj_dependency_version_expression,
                ...
                logging.info("版本表达式不存在，不入库" + deps.requirement)
        else:
            logging.info("查询信息不存在", info_sql, version_sql)
