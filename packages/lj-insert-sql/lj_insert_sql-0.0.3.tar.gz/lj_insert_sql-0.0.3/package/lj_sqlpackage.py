...
class SqlTool(object):
    ...
    def package_file(self, file: PackageFile):
        ...
        if file_id:
            sql = f"update `package_file`(`version`, `file_name`, `file_type`, `file_md5`, `package_id`, `package_type`" \
                  f") set version='%s', file_name='%s', file_type='%s', file_md5='%s', package_id=%d, package_type='%s'" % (
            ...
            if deps.requirement:
                for require in deps.requirement:
                    ...
                    sql = f"insert into package_dependencies(`dependency_version_expression`, " \
                          ...
                          f"`dependency_package_name`, `dependency_package_id`, `dependency_version`, `package_id`, " \
                          f"`package_type`, `version`) values('%s', '%s', '%s', '%s', '%s', '%s', %d, '%s', %d, '%s', %d)"%(
