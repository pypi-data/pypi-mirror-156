...
class ServiceActionCommands(ServiceAction):
    ...
    def parse(self, payload) -> bool:
        if self.has_separator(payload) == True:
            ...
            if payload.find(" ") == 0 or payload.find("{") == 0 or payload.find("[") == 0:
                return False
            else:
                return True
    ...
    def handle_echo(self, payload, service:ServiceContext) -> bool:
        lv_message = f"echo_request{ self.separator }{ service.client_id }{ self.separator}{ payload }"
        service.send_text(lv_message)
        return True

    def handle_ping(self, payload, service:ServiceContext) -> bool:
        service.send_text(f"ping_request{ self.separator }{ service.client_id } is alive")
        return True
    ...
    def handle_info(self, payload, service:ServiceContext) -> bool:
        ...
        service.send_text(f"info_request{ self.separator }{ service.client_id }{ self.separator }{answer}")
        return True
    ...
    def handle(self, payload, service:ServiceContext) -> bool:
        ...
        #pos_sep = payload.find(self.separator)
        ...
        if my_command == "connect" or my_command == "ping_request" or my_command == "echo_request" or my_command == "info_request":
            return True
...
class Service(ServiceContext):
    ...
    def init_command_mode(self, echo=True) -> bool:
