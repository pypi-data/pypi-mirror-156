...
def annotate_match(state, timeout, run_async, delete_if_exists, keep_ref_unknown_calls):
    """Get annotations for stream of calls."""
    if run_async:
        create_function = kms.create_async_processor
...
@options.build_option
def sv_match(
    state, delete_if_exists, region_match_tolerance, cnv_match_tolerance
):
