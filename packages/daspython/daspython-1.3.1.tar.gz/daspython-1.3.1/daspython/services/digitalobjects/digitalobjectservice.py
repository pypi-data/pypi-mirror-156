...
class DigitalObjectService(ApiMethods):
    ...
    def download_request(self, entry_code: str, digital_object_code_list: list[str] = None):
        ...
        if intersection:
            for digital_object_code in intersection:
                item = self.__get_download_request_items(
    ...
    def __get_download_request_items(self, digital_object_code: str, json_do_list: any) -> any:
    ...
    def get_files_from_download_request(self):
        ...
        requests_ready = list(filter(lambda x: x.is_ready == True, response.items))
