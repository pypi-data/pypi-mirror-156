...
@click.option(
    "--iam-user",
    default="coiled",
    help="Name for the IAM User we'll create in your AWS account",
)
@click.option(
    "--setup-policy",
    default=None,
...
PROMPTS = {
    ...
    "replace_access_key": (
        ...
        "It's not possible to retrieve secret for an existing key. "
        "Should we delete key [green]{key_id}[/green] and create a new key?"
    ),
}
...
def update_policy(iam, account, name, doc):
    ...
    try:
        ...
        print(
            f"The updated document for Policy [green]{policy_arn}[/green] is [bold]{new_version}[/bold]"
        )
        print()
    except Exception as e:
        coiled.add_interaction(
            action="CreatePolicyArn",
            success=False,
            arn=policy_arn,
            error_message=str(e),
        )
        # TODO check if we need to delete old policy version
        print("[red]We were unable to update the existing policy[/red]:")
        print(f"  [red]{e}[/red]")
        print()
...
def make_access_key(iam, user, retry=0):
    try:
...
def wait_till_key_works(iam, key, secret):
    ...
    print(
        "\nAccess key is still not ready, you'll need to manually complete setting up your Coiled account with AWS."
    )
    return False


def do_intro(sts, iam):
    print(
        "This script will use your local AWS credentials to setup the IAM you'll need to use Coiled.\n"
    )
    ...
    try:
        ...
        print(
            f"You're currently using credentials for the AWS identity [green]{identity_as}[/green] "
            f"and will be setting up Coiled for account [green]{account}[/green]{alias_string}."
        )
        print(
            "If this isn't the correct AWS identity or account, you can stop now and specify a different profile "
            "from your AWS credentials file using the `--profile` argument.\n"
        )
        ...
        # FIXME track
        print(
            "[red]Your local AWS credentials are not configured.[/red]"
            "\nUsually these credentials will be saved in `~/.aws/credentials`. "
            "Please see the AWS docs if you need to\n"
            "- install the AWS CLI: "
            ...
            "[link]https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-quickstart.html[/link]\n"
        )
    except Exception as e:
        print("We got this error when attempting to determine your AWS account:")
        print(f"    [red]{e}[/red]")

    return None


def do_coiled_setup(iam, user_name, region, yes):
    ...
    if key and secret:
        print(
            "[bold]You can now setup your Coiled account to the AWS credentials you just created."
        )
        print(
            "This means that your AWS access key will be sent to Coiled, where it will be stored securely and "
            "used so that we can create clusters in your AWS account (on your behalf)."
        )
        print(
            "We will also initially create some infrastructure in your account (VPC, subnets, etc), "
        ...
        if yes or Confirm.ask(
            ...
            if wait_till_key_works(iam, key, secret):
                ...
                print(
                    "[red]Your access key is still not ready, you'll need to complete Coiled setup manually."
                )
                print()
        else:
...
def do_full_setup(iam, user_name, account, region, setup_name, ongoing_name, yes):
    ...
    if user_exists:
        ...
        if len(user_keys) > 1:
            print(
                "We will not be able to create a new access key for this user unless we delete an existing key."
            )
            # print("If you proceed, you'll be able to select which existing access key to delete. (TODO)")
        print(
            "If you would like to create a different user, you can stop now and specify the a different name "
            "for the IAM user we create with the `--iam-user` argument."
        )
        print()
    ...
    print(
        "We'll attempt to create or update the following resources in your AWS account:"
    )
    ...
    if setup_diff:
        print(
            f"We would make the following changes to the existing [green]{setup_name}[/green] IAM Policy:"
        )
    ...
    if ongoing_diff:
        print(
            f"We would make the following changes to the existing [green]{ongoing_diff}[/green] IAM Policy:"
        )
    ...
    if not setup_arn or not ongoing_arn:
        print(
            "You can see the documents for the IAM Policies we'll create at "
    ...
    if create_arns:
        ...
        for arn in create_arns:
            print(f"  {arn}")
    print(
        f"IAM User [green]{user_name}[/green] is now setup with the required IAM Policies attached."
    )
    print()

    do_coiled_setup(iam, user_name, region, yes)


def do_just_update_policies(iam, account, setup_name, ongoing_name, yes):
    ...
    if not setup_arn:
        print(
            f"[red]WARNING[/red]: We did not find an IAM Policy named [green]{setup_name}[/green]."
        )
        print(
            "You can use `--setup-name` to specify a different name for the existing setup policy."
        )
        print()

    if not ongoing_arn:
        print(
            f"[red]WARNING[/red]: We did not find an IAM Policy named [green]{ongoing_name}[/green]."
        )
        print(
            "You can use `--ongoing-name` to specify a different name for the existing setup policy."
        )
        print()
    ...
    if setup_arn and ongoing_arn and not setup_diff and not ongoing_diff:
        ...
        print("You may still need to update your [bold]ongoing[/bold] IAM Policy.")
    elif ongoing_arn and not ongoing_diff:
        print("Your [bold]ongoing[/bold] IAM Policy is up-to-date!")
        print("You may still need to update your [bold]setup[/bold] IAM Policy.")
...
def do_setup(
    aws_profile, slug, setup_name, ongoing_name, just_update_policies, region, yes=False
):
    ...
    try:
        try:
            ...
            coiled.add_interaction(
                action="BotoSession", success=True, profile=aws_profile
            )
            ...
            print()
            print(
                f"[red]It appears that the profile `{aws_profile}` is not configured in your local AWS credentials."
            )
            print(
        ...
        try:
            ...
            if not (account := do_intro(sts, iam)):
                return False

            if just_update_policies:
                do_just_update_policies(iam, account, setup_name, ongoing_name, yes=yes)
            else:
                do_full_setup(
            ...
            if "AccessDenied" in str(e):
                print()
                print(
                    f"You don't have enough permissions to [green]{error_op}[/green] using your "
                    "current AWS profile/user."
                )
                print(
                ...
                print()
                print(
                    "In order to run this setup script you'll need the following IAM permissions:"
                )
                ...
                print(
                    ...
                    "and IAM Policies described in our docs: "
