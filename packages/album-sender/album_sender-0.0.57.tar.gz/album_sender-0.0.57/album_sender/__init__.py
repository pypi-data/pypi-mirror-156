...
def waitSend(chat_id, media_group_size):
	...
	count_by_chat_id[chat_id] = count_by_chat_id.get(chat_id, 0) + media_group_size + 1
	wait = next_send_time.get(chat_id, 0) + media_group_size ** 2 + media_group_size * 10 - time.time() + media_group_size * 10 + min(30, count_by_chat_id[chat_id]) - 15
...
def send_v2_imp(chat, result, rotate=0, send_all=False, no_cut=False, size_factor = None, rotate_detail = {}):
...
def send_v2(chat, result, rotate=0, send_all=False, no_cut=False, size_factor = None, rotate_detail = {}):
	try:
		return send_v2_imp(chat, result, rotate=rotate, send_all=send_all, no_cut=no_cut, size_factor = size_factor, rotate_detail = rotate_detail)
	except Exception as e:
