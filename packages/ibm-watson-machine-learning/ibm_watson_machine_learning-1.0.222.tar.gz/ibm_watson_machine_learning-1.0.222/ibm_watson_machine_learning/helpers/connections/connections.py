...
class DataConnection(BaseDataConnection):
    ...
    def _recreate_holdout(
        ...
        if self.auto_pipeline_params.get('prediction_columns') is not None:
            ...
            X_train = DataFrame(X_train, columns=self.auto_pipeline_params.get('feature_columns', self.auto_pipeline_params['prediction_columns']))
            X_holdout = DataFrame(X_holdout, columns=self.auto_pipeline_params.get('feature_columns', self.auto_pipeline_params['prediction_columns']))
    ...
    def read(self,
        ...
        sampling_type = kwargs.get('sampling_type', DEFAULT_SAMPLING_TYPE)
        sample_size_limit = kwargs.get('sample_size_limit', DEFAULT_SAMPLE_SIZE_LIMIT)
        enable_sampling = kwargs.get('enable_sampling', True)
        read_normal_data_asset_without_flight = kwargs.get('read_normal_data_asset_without_flight', False)
        ...
        if self.type == DataConnectionTypes.S3:
            ...
            # workaround for reading data assets on CPD 4.5: issue #28332
            data = None
            if read_normal_data_asset_without_flight and self._wml_client and self._is_data_asset_normal():
                data = self._download_training_data_from_data_asset_storage()

            if data is None:  # read data in normal scenario with flight
                if use_flight and not self._obm:
                    ...
                    try:
                        ...
                        if self._wml_client and (self._is_data_asset_normal() or self._is_data_asset_nfs()):
                            import warnings
        ...
        if raw or (self.auto_pipeline_params.get('prediction_column') is None
            ...
            if with_holdout_split:  # when this connection is training one
                ...
                if _return_subsampling_stats:
                    ...
                    return X_train, X_holdout, y_train, y_holdout, data[1], data[2]
                else:
