...
from ibm_watson_machine_learning.utils.deployment.errors import ModelPromotionFailed
...
class Models(WMLResource):
    ...
    def _publish_from_object(self, model, meta_props, training_data=None, training_target=None, pipeline=None, feature_names=None, label_column_names=None, project_id=None):
        ...
        if self._client.ICP_45 is None and self._client.ICP_40 is None and self._client.ICP_35 is None and self._client.ICP_30 is None and self._client.WSD_20 is None:
        ...
        try:
            if 'pyspark.ml.pipeline.PipelineModel' in str(type(model)):
                ...
                if self._client.CLOUD_PLATFORM_SPACES or self._client.ICP_35 or self._client.ICP_40 or self._client.ICP_45:
                    if self._client.default_space_id is not None:
                ...
                if self.ConfigurationMetaNames.RUNTIME_UID in meta_props:
                    ...
                    meta_props[self._client.repository.ModelMetaNames.RUNTIME_UID] = {"href": API_VERSION + RUNTIMES + "/" + meta_props[self._client.repository.ModelMetaNames.RUNTIME_UID]}
                if self.ConfigurationMetaNames.SOFTWARE_SPEC_UID in meta_props:
                    if self._client.ICP_30 or self._client.ICP_35 or self._client.ICP_40 or self._client.ICP_45:
                        self._validate_meta_prop(meta_props, self.ConfigurationMetaNames.SOFTWARE_SPEC_UID, STR_TYPE, False)
                        meta_props[self.ConfigurationMetaNames.SOFTWARE_SPEC_UID] = \
    ...
    def _publish_from_training(self, model_uid, meta_props, training_data=None, training_target=None,version=False,artifactId=None,subtrainingId=None, feature_names=None, label_column_names=None):
        """
        ...
        if "pipeline" in details["entity"]:
            if any([self._client.ICP_30, self._client.CLOUD_PLATFORM_SPACES, self._client.ICP_35, self._client.ICP_40, self._client.ICP_45]):
        ...
        if "experiment" in details["entity"]:
            ...
            if "pipeline" in subtraining_object["entity"]:
                if any([self._client.ICP_30, self._client.ICP_35, self._client.ICP_40, self._client.ICP_45, self._client.CLOUD_PLATFORM_SPACES]):
        ...
        if not self._ICP:
            ...
            if self._client.ICP_35 or self._client.ICP_40 or self._client.ICP_45:
                params = {}
                params.update({'version': self._client.version_param})
                creation_response = requests.post(
                    self._wml_credentials['url'] + '/ml/v4/models',
    ...
    def _store_autoAI_model(self,model_path, meta_props, training_data=None, training_target=None,version=False,artifactId=None,subtrainingId=None, feature_names=None, label_column_names=None):
        """
                Store trained model from object storage into Watson Machine Learning repository on IBM Cloud
        """
        model_meta = self.ConfigurationMetaNames._generate_resource_metadata(
    ...
    def _publish_from_file(self, model, meta_props=None, training_data=None, training_target=None, ver=False,
        ...
        if os.path.isdir(model):
            ...
            try:
                if not self._client.CLOUD_PLATFORM_SPACES and not self._client.ICP_35 and not self._client.ICP_40 and not self._client.ICP_45:
                    if self.ConfigurationMetaNames.SPACE_UID in meta_props:
                        self._validate_meta_prop(meta_props, self.ConfigurationMetaNames.SPACE_UID, STR_TYPE, False)
                        meta_props[self._client.repository.ModelMetaNames.SPACE_UID] = {"href": API_VERSION + SPACES + "/" + meta_props[self._client.repository.ModelMetaNames.SPACE_UID]}
    ...
    def store(self, model, meta_props=None, training_data=None, training_target=None, pipeline=None,version=False,artifactid=None, feature_names=None, label_column_names=None,subtrainingId=None):
        ...
        if (self._client.CAMS and self._client.CLOUD_PLATFORM_SPACES or self._client.ICP_35 or self._client.ICP_40
            or self._client.ICP_45) and self._client.default_space_id is None and self._client.default_project_id is None:
        ...
        if not isinstance(model, STR_TYPE):
            ...
            if ("tensorflow.python.keras.engine.training.Model" in str(type(model)) or "tf." in str(type(model))) and \
                ...
                save_model = self._store_tf_model(model, meta_props, feature_names=feature_names, label_column_names=label_column_names)
                ...
                saved_model = self._store_autoAI_model(model_path=model, meta_props=meta_props, training_data=training_data, training_target=training_target, version=version, artifactId=artifactid, subtrainingId=subtrainingId, feature_names=feature_names, label_column_names=label_column_names)
                ...
                saved_model = self._publish_from_file(model=model, meta_props=meta_props, training_data=training_data, training_target=training_target,ver=version,artifactid=artifactid, feature_names=feature_names, label_column_names=label_column_names)
            else:
                 saved_model = self._publish_from_training(model_uid=model, meta_props=meta_props, training_data=training_data, training_target=training_target,version=version,artifactId=artifactid, subtrainingId=subtrainingId, feature_names=feature_names, label_column_names=label_column_names)
    ...
    def update(self, model_uid, meta_props=None, update_model=None):
        ...
        if meta_props is not None: # TODO
            ...
            if (self._client.ICP_30 or self._client.ICP_35 or self._client.ICP_40 or self._client.ICP_45 or self._client.CLOUD_PLATFORM_SPACES ) and update_model is not None:
    ...
    def load(self, artifact_uid):
        ...
        if self._client.CAMS or self._client.CLOUD_PLATFORM_SPACES or self._client.ICP_35 or self._client.ICP_40 or self._client.ICP_45:
    ...
    def download(self, model_uid, filename='downloaded_model.tar.gz', rev_uid=None, format=None):
        ...
        try:
            if self._client.WSD:
                import urllib
                #model_details = self.get_details(model_uid)
                model_get_response = requests.get(self._client.service_instance._href_definitions.get_data_asset_href(model_uid),
                                              params=self._client._params(),
                                              headers=self._client._get_headers())
    ...
    @docstring_parameter({'str_type': STR_TYPE_NAME})
    def get_details(self, model_uid=None, limit=None, asynchronous=False, get_all=False):
        """
           ...
           :param asynchronous: if True, it will work as a generator (optional)
           :type asynchronous: int

           :param get_all: if True, it will get all entries in 'limited' chunks (optional)
           :type get_all: int
    ...
    def list(self, limit=None, asynchronous=False, get_all=False):
        """
        ...
        """
        ##For CP4D, check if either spce or project ID is set
        def process_resources(self, model_resources: dict):
        ...
        if self._client.WSD:
            ...
            self._client._check_if_either_is_set()
            if asynchronous:
                return (process_resources(self, model_resources) for model_resources in
    ...
    def list_revisions(self, model_uid, limit=None):
        ...
        if self._client.ICP_30 is None and not self._client.CLOUD_PLATFORM_SPACES and not self._client.ICP_35 \
                and not self._client.ICP_40 and not self._client.ICP_45:
            raise WMLClientError(
    ...
    def promote(self, model_id: str, source_project_id: str, target_space_id: str):
        """
           ...
           :param model_id: stored model
           :type model_id: {str_type}

           :param source_project_id: stored model
           :type source_project_id: {str_type}

           :param target_space_id: stored model
           :type target_space_id: {str_type}

           :returns: promoted model id
           :rtype: str
        ...
        promote_payload = {"spaceId": target_space_id,
                           ...
                           "assetType": "wml_model",
                           "assetDescription": "Model promoted by ibm_wml client"}
        promote_href = self._client.service_instance._href_definitions.promote_asset_href(model_id)
        response = requests.post(
            promote_href,
            headers=self._client._get_headers(),
            json=promote_payload
        )
        promotion_details = self._client.repository._handle_response(200, u'promote model asset', response)

        try:
            promoted_model_id = promotion_details['promotedAsset']['asset_id']
        except KeyError as key_err:
            raise ModelPromotionFailed(source_project_id, target_space_id, promotion_details, reason=key_err)

        return promoted_model_id
        ...
        if 'scikit-learn_' in model_type or 'mllib_' in model_type:
            ...
            with open(path_to_archive, 'rb') as f:
                if is_xml(path_to_archive):
                    ...
                    if (self._client.CLOUD_PLATFORM_SPACES or self._client.ICP_35 or self._client.ICP_40 or self._client.ICP_45) and \
    ...
    def _publish_from_object_cloud(self, model, meta_props, training_data=None, training_target=None, pipeline=None,
        ...
        try:
            ...
            tf_model_types = [f'tensorflow_{version}' for version in ('2.1', '2.4', '2.7')]
