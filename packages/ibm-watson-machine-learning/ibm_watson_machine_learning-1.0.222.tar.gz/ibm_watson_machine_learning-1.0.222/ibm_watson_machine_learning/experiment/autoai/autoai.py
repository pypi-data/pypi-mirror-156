...
from ibm_watson_machine_learning.wml_client_error import ForbiddenActionForGitBasedProject
...
class AutoAI(BaseExperiment):
    ...
    def optimizer(self,
        ...
        sample_size_limit: int, optional
            The size of sample upper bound (in bytes). The default value is 1GB.
        ...
        # TS exogenous, to be removed
        feature_columns = None
        pipeline_types = None
        supporting_features_at_forecast = None

        if 'feature_columns' in kwargs:
            feature_columns = kwargs.pop('feature_columns')

        if 'pipeline_types' in kwargs:
            pipeline_types = kwargs.pop('pipeline_types')

        if 'supporting_features_at_forecast' in kwargs:
            supporting_features_at_forecast = kwargs.pop('supporting_features_at_forecast')
