...
DEFAULT_SAMPLING_TYPE = SamplingTypes.FIRST_N_RECORDS
...
class ExperimentIterableDataset(IterableDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self._set_sample_size_limit(sample_size_limit)
        ...
        if self.authorized:
            ...
            self.connection = FlightConnection(
                ...
                data_size_limit=self.sample_size_limit,
    ...
    def _set_sample_size_limit(self, sample_size_limit):
        """If non-default value of sample_size_limit was not passed,
        set Sample Size Limit based on T-Shirt size if code is run on training pod:
        For memory < 32 (T-Shirts: XS,S, M) default is 100MB,
        For memory >= 32 or runs outside pod default is 1GB
        """
        if sample_size_limit == DEFAULT_SAMPLE_SIZE_LIMIT:
            size_mem = int(findall(r'[0-9]+', os.environ.get('MEM', '32'))[0])
            if size_mem >= 32:
                self.sample_size_limit = DEFAULT_SAMPLE_SIZE_LIMIT     # 1GB in bytes
            else:
                self.sample_size_limit = DEFAULT_REDUCED_SAMPLE_SIZE_LIMIT      # 100MB in bytes
        else:
