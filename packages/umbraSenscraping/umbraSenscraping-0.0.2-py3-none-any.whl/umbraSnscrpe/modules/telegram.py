...
@dataclasses.dataclass
class TelegramPost(src.base.Item):
	url: str
	...
	outlinksss = src.base._DeprecatedProperty('outlinksss', lambda self: ' '.join(self.outlinks), 'outlinks')
...
class Channel(src.base.Entity):
	...
	videos: typing.Optional[src.base.IntWithGranularity] = None
	links: typing.Optional[src.base.IntWithGranularity] = None
	files: typing.Optional[src.base.IntWithGranularity] = None

	photosGranularity = src.base._DeprecatedProperty('photosGranularity', lambda self: self.photos.granularity, 'photos.granularity')
	videosGranularity = src.base._DeprecatedProperty('videosGranularity', lambda self: self.videos.granularity, 'videos.granularity')
	linksGranularity = src.base._DeprecatedProperty('linksGranularity', lambda self: self.links.granularity, 'links.granularity')
	filesGranularity = src.base._DeprecatedProperty('filesGranularity', lambda self: self.files.granularity, 'files.granularity')

	def __str__(self):
		return f'https://t.me/s/{self.username}'


class TelegramChannelScraper(src.base.Scraper):
	...
	def _get_entity(self):
		...
		for div in channelInfoDiv.find_all('div', class_ = 'tgme_channel_info_counter'):
			...
			if type_ == 'members':
				...
				kwargs[type_] = src.base.IntWithGranularity(value, granularity)

		return Channel(**kwargs)

	@classmethod
	def _cli_setup_parser(cls, subparser):
		subparser.add_argument('channel', type = src.base.nonempty_string('channel'), help ='A channel name')
