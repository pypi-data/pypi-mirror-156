...
class InstagramPost(src.base.Item):
	url: str
...
class User(src.base.Entity):
	...
	followers: src.base.IntWithGranularity
	following: src.base.IntWithGranularity
	posts: src.base.IntWithGranularity
...
class _InstagramCommonScraper(src.base.Scraper):
	...
	def _initial_page(self):
		if self._initialPage is None:
			...
			if r.status_code not in (200, 404):
				...
				raise src.base.ScraperException('Redirected to login page')
			self._initialPage = r
...
class InstagramUserScraper(_InstagramCommonScraper):
	...
	def _get_entity(self):
		...
		followers = src.base.IntWithGranularity(*parse_num(m.group(1)))
		following = src.base.IntWithGranularity(*parse_num(m.group(2)))
		posts = src.base.IntWithGranularity(*parse_num(m.group(3)))
		return User(
	...
	def _cli_setup_parser(cls, subparser):
		subparser.add_argument('username', type = src.base.nonempty_string('username'), help ='An Instagram username (no leading @)')
...
class InstagramHashtagScraper(_InstagramCommonScraper):
	...
	@classmethod
	def _cli_setup_parser(cls, subparser):
		subparser.add_argument('hashtag', type = src.base.nonempty_string('hashtag'), help ='An Instagram hashtag (no leading #)')
