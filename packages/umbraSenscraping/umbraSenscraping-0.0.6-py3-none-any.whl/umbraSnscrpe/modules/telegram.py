...
@dataclasses.dataclass
class TelegramPost(umbraSnscrpe.base.Item):
	url: str
	...
	outlinksss = umbraSnscrpe.base._DeprecatedProperty('outlinksss', lambda self: ' '.join(self.outlinks), 'outlinks')
...
@dataclasses.dataclass
class Channel(umbraSnscrpe.base.Entity):
	username: str
	title: str
	verified: bool
	photo: str
	...
	photos: typing.Optional[umbraSnscrpe.base.IntWithGranularity] = None
	videos: typing.Optional[umbraSnscrpe.base.IntWithGranularity] = None
	links: typing.Optional[umbraSnscrpe.base.IntWithGranularity] = None
	files: typing.Optional[umbraSnscrpe.base.IntWithGranularity] = None

	photosGranularity = umbraSnscrpe.base._DeprecatedProperty('photosGranularity', lambda self: self.photos.granularity, 'photos.granularity')
	videosGranularity = umbraSnscrpe.base._DeprecatedProperty('videosGranularity', lambda self: self.videos.granularity, 'videos.granularity')
	linksGranularity = umbraSnscrpe.base._DeprecatedProperty('linksGranularity', lambda self: self.links.granularity, 'links.granularity')
	filesGranularity = umbraSnscrpe.base._DeprecatedProperty('filesGranularity', lambda self: self.files.granularity, 'files.granularity')

	def __str__(self):
		return f'https://t.me/s/{self.username}'


class TelegramChannelScraper(umbraSnscrpe.base.Scraper):
	...
	def _initial_page(self):
		if self._initialPage is None:
			...
			if r.status_code != 200:
				raise umbraSnscrpe.base.ScraperException(f'Got status code {r.status_code}')
	...
	def _get_entity(self):
		...
		kwargs['photo'] = soup.find('img', class_ = 'tgme_page_photo_image').attrs['umbraSnscrpe']
		...
		for div in channelInfoDiv.find_all('div', class_ = 'tgme_channel_info_counter'):
			...
			if type_ == 'members':
				...
				kwargs[type_] = umbraSnscrpe.base.IntWithGranularity(value, granularity)

		return Channel(**kwargs)

	@classmethod
	def _cli_setup_parser(cls, subparser):
		subparser.add_argument('channel', type = umbraSnscrpe.base.nonempty_string('channel'), help ='A channel name')
