...
class VKontaktePost(src.base.Item):
	url: str
...
@dataclasses.dataclass
class User(src.base.Entity):
	username: str
	name: str
	verified: bool
	description: typing.Optional[str] = None
	websites: typing.Optional[typing.List[str]] = None
	followers: typing.Optional[src.base.IntWithGranularity] = None
	posts: typing.Optional[src.base.IntWithGranularity] = None
	photos: typing.Optional[src.base.IntWithGranularity] = None
	tags: typing.Optional[src.base.IntWithGranularity] = None
	following: typing.Optional[src.base.IntWithGranularity] = None

	followersGranularity = src.base._DeprecatedProperty('followersGranularity', lambda self: self.followers.granularity, 'followers.granularity')
	postsGranularity = src.base._DeprecatedProperty('postsGranularity', lambda self: self.posts.granularity, 'posts.granularity')
	photosGranularity = src.base._DeprecatedProperty('photosGranularity', lambda self: self.photos.granularity, 'photos.granularity')
	tagsGranularity = src.base._DeprecatedProperty('tagsGranularity', lambda self: self.tags.granularity, 'tags.granularity')
	followingGranularity = src.base._DeprecatedProperty('followingGranularity', lambda self: self.following.granularity, 'following.granularity')

	def __str__(self):
		return f'https://vk.com/{self.username}'


class VKontakteUserScraper(src.base.Scraper):
	name = 'vkontakte-user'
	...
	def _initial_page(self):
		if self._initialPage is None:
			...
			if r.status_code not in (200, 404):
				raise src.base.ScraperException(f'Got status code {r.status_code}')
					...
					for geoblockOffset in range(lastWorkingOffset + 1, offset + 10):
						...
						if not geoPosts.startswith('<div id="post'):
							if geoPosts == '"\\/blank.php?block=119910902"':
								continue
							raise src.base.ScraperException(f'Got an unknown response: {geoPosts[:200]!r}...')
						yield from _process_soup(soup = bs4.BeautifulSoup(geoPosts, 'lxml'))
					continue
				raise src.base.ScraperException(f'Got an unknown response: {posts[:200]!r}...')
	...
	def _get_entity(self):
		...
		if (countsDiv := soup.find('div', class_ = 'counts_module')):
			for a in countsDiv.find_all('a', class_ = 'page_counter'):
				...
				if label in ('followers', 'posts', 'photos', 'tags'):
					kwargs[label] = src.base.IntWithGranularity(count, granularity)
		...
		if (idolsDiv := soup.find('div', id = 'profile_idols')):
			if (topDiv := idolsDiv.find('div', class_ = 'header_top')) and topDiv.find('span', class_ = 'header_label').text == 'Following':
				kwargs['following'] = src.base.IntWithGranularity(*parse_num(topDiv.find('span', class_ ='header_count').text))
		...
		if (followersDiv := soup.find('div', id = 'public_followers')):
			if (topDiv := followersDiv.find('div', class_ = 'header_top')) and topDiv.find('span', class_ = 'header_label').text == 'Followers':
				kwargs['followers'] = src.base.IntWithGranularity(*parse_num(topDiv.find('span', class_ ='header_count').text))

		return User(**kwargs)

	@classmethod
	def _cli_setup_parser(cls, subparser):
		subparser.add_argument('username', type = src.base.nonempty_string('username'), help ='A VK username')
