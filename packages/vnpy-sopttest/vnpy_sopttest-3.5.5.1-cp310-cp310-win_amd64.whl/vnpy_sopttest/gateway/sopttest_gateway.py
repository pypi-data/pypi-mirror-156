...
class SopttestGateway(BaseGateway):
    """
    VeighNa用于对接期权CTP柜台穿透式测试环境的交易接口。
    """

    default_name: str = "SOPTTEST"
...
class SopttestMdApi(MdApi):
    """"""

    def __init__(self, gateway: SopttestGateway) -> None:
        """构造函数"""
        super().__init__()

        self.gateway: SopttestGateway = gateway
...
class SopttestTdApi(TdApi):
    ...
    def cancel_order(self, req: CancelRequest) -> None:
        ...
        self.reqid += 1
        self.reqOrderAction(sopttest_req, self.reqid)
