...
VALID_ID_CHARS = b"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789.-"
...
class CookieIdPersistence(IdPersistence):
    def __init__(
        ...
        self.set_cookie_template = f"{cookie_name}=%s;HttpOnly;SameSite={samesite}"
        self.delete_cookie_template = (
            f"{cookie_name}=;Max-Age=-1;HttpOnly;SameSite={samesite}"
        )
        if path:
            self.set_cookie_template += f";Path={path}"
            self.delete_cookie_template += f";Path={path}"
        if domain:
            self.set_cookie_template += f";Domain={domain}"
            self.delete_cookie_template += f";Domain={domain}"
        if max_age:
            self.set_cookie_template += f";Max-Age={max_age}"
        if secure:
            self.set_cookie_template += ";Secure"
            self.delete_cookie_template += ";Secure"
        self._extract_cookie = re.compile(
            rf"\b{re.escape(cookie_name)}=({ID_PATTERN})"
        ).search
        self.localtime = _localtime
        self.mktime = _mktime
    ...
    def set_id_middleware(self, session, app, environ, start_response):
        def start_response(
            ...
            id = session.id
            if session.orig_id != session.id:
                if id:
                    headers += [("Set-Cookie", self.set_cookie_template % (id,))]
                else:
                    headers += [("Set-Cookie", self.delete_cookie_template)]
            elif id and self.max_age:
                modified_at = self.mktime(session.backend.modified_at(id))
                age = self.mktime(self.localtime()) - modified_at
                if age > self.max_age // 8:
                    headers += [("Set-Cookie", self.set_cookie_template % (id,))]
                    session.backend.touch(id)
    ...
    def load(self):
        ...
        if id is not None:
            try:
                ...
                logger.warning("Could not load session %r from %r", id, self.backend)
                self.orig_id = None
                self.id = _generate_id()
                self.data = {}
        else:
...
class Backend(object):
    ...
    def touch(self, id):
        """
        Update the modified time for the given id without changing the stored
        data
        """
        raise NotImplementedError()

    def modified_at(self, session_id) -> time.struct_time:
        """
...
class MemoryBackend(Backend):
    ...
    def touch(self, id, timestamp=time.localtime):
        _, data = self.store[id]
        self.store[id] = (timestamp(), data)

    def modified_at(self, id):
        try:
            return self.store[id][0]
        except KeyError:
            return EPOCH
...
class FileBackend(Backend):
    def __init__(self, directory=None, prefix=None, serializer=PickleSerializer()):
    ...
    def save(self, id, data, rename=os.rename, mktmp=tempfile.NamedTemporaryFile):
        path = self.getpath(id)
    ...
    def touch(self, id):
        os.utime(self.getpath(id))

    def modified_at(self, id, t=time.localtime):
        try:
            return t(os.stat(self.getpath(id)).st_mtime)
        except OSError:
            return EPOCH

    def keys(self):
        p = re.compile("^{}{}$".format(re.escape(self.prefix), ID_PATTERN)).match
        prefix_len = len(self.prefix)
        return (s[prefix_len:] for s in os.listdir(self.directory) if p(s))


def SessionMiddleware(app, id_persister=CookieIdPersistence(), backend=FileBackend()):
