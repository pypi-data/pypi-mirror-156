...
if 'sdist' not in dist.commands and compiler.find_library_file([library_path],
    ...
    # the Python module.
    if os.path.exists('jsbsim.cxx'):
        source_file = 'jsbsim.cxx'
    else:
...
data_files = [
                ('share/JSBSim', ['LICENSE.txt']),
                ('share/JSBSim/aircraft', list(XML_files('aircraft'))),
                ('share/JSBSim/engine', list(XML_files('engine'))),
                ('share/JSBSim/systems', list(XML_files('systems'))),
                ('share/JSBSim/scripts', list(XML_files('scripts'))),
            ]
...
for d in os.scandir('data/aircraft'):
    if d.is_dir():
        ...
        data_files.append(('share/JSBSim/'+dir_name,
