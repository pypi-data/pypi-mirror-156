...
def hcpsoboa(objf, func_opter_parms):
    '''
    粒子群-柯西变异-蝴蝶混合优化算法
    ...
    def cauchy_pdf(x, a=0, b=1):
        return (b / ((x-a)**2 + b**2)) / np.pi
    ...
    for t in range(1, max_iter+1):
        ...
        for i in range(1, popsize+1):
            ...
            # 自适应权重
            w = np.sin((np.pi*t) / (2*max_iter)+ np.pi) + 1

            # 全局|局部搜索
            r1, r2 = np.random.rand(), np.random.rand()
            p_ = p - 0.1 * (max_iter-t) / max_iter # 动态切换概率
            if np.random.rand() < p_: # 在最优解附件搜索
            ...
            if fval <= best_y:
                ...
                # 柯西变异
                best_x_c = best_x * cauchy_pdf(best_x)
                fval_c = objf(best_x_c, **kwargs)
                if fval_c <= best_y:
                    best_y = fval_c
                    best_x = best_x_c
...
if __name__ == '__main__':
    ...
    logger = get_logger('./test/log/hcpsoboa_test.txt', screen_show=True)
    ...
    func_opter_parms = hcpsoboa(objf, func_opter_parms)
