...
from dramkit.iotools import pickle_file, unpickle_file
...
def check_parms_mdl(parms_mdl, objective, num_class, logger=None):
    ...
    if objective is None:
        ...
        if isinstance(parms_mdl, dict) and 'objective' in parms_mdl.keys() \
            ...
            logger_show('objective与parms_mdl设置不一样，以前者为准！', logger, 'warn')
    ...
    if objective in ['multiclass']:
        if not isinstance(num_class, int):
            ...
            if isinstance(parms_mdl, dict) and 'num_class' in \
                ...
                logger_show('num_class与parms_mdl设置不一样，以前者为准！', logger, 'warn')
...
def get_parms_train_or_cv(parms_train_or_cv=None):
    '''
    | 获取lgb.train或lgb.cv参数
    | parms_train_or_cv为以字典形式设置的train或cv参数，
      若关键参数没设置，则会补充设置默认值
    '''
    ...
    if parms_train_or_cv is None:
        parms_train_or_cv = parms_default
    parms_loss = {x: parms_default[x] \
                                for x in parms_default.keys() if \
                                    x not in parms_train_or_cv.keys()}
    parms_train_or_cv.update(parms_loss)
...
def lgb_cv(X_train, y_train, objective=None, parms_mdl=None, parms_cv=None,
    ...
    parms_mdl = get_parms_mdl(parms_mdl=parms_mdl, objective=objective,
                              num_class=num_class, logger=logger)
    parms_cv = get_parms_train_or_cv(parms_train_or_cv=parms_cv)
...
def lgb_cv_grid_search(X_train, y_train, objective=None, parms_mdl=None,
                       parms_to_opt=None, parms_cv=None, logger=None):
    '''lgb交叉验证网格搜索调参'''

    if not isinstance(parms_to_opt, dict) or len(parms_to_opt) == 0:
        logger_show('检测到待优化参数parms_to_opt为空！', logger, 'error')
        return None, None
    ...
    if len(parms_mdl['metric']) > 1 and \
        ...
        logger_show('发现多个metric，将以{}为优化目标！'.format(metric),
                    logger, 'warn')
    ...
    for grid_parm in grid_parms:
        logger_show('交叉验证网格搜索中：{} / {} ...'.format(k, len(grid_parms)),
                    logger, 'info')
        logger_show('当前参数：{}'.format(grid_parm), logger, 'info')
        k += 1
...
def lgb_cv_hoo(X_train, y_train, objective=None,
    ...
    for Ikf, (trnIdxs, valIdxs) in enumerate(folds.split(X_train, y_train)):
        logger_show('{}/{}折交叉验证训练中...'.format(Ikf+1, n_fold),
                    logger, 'info')
