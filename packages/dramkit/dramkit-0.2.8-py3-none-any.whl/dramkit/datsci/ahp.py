...
#%%
def score2JM(df_score, max_important_level=9, judge_type='big_score'):
    '''
    将指标重要性评分转化为判断矩阵
    ...
    def _importance_cols(colA, colB, judge_type='big_score'):
        '''
    ...
    for colA in df_score.columns:
        for colB in df_score.columns:
            if colA == colB:
                ...
                a_b, b_a = _importance_cols(colA, colB, judge_type=judge_type)
                if a_b >= b_a:
...
#%%
def check_JM(judge_mat, tol=0.001, logger=None):
    '''
    ...
    for i in range(JM.shape[0]):
        for j in range(JM.shape[1]):
            if abs(JM[i, j] * JM[j, i] - 1) > tol:
                logger_show('判断矩阵可能错误的行和列位置: {}, {}'.format(i, j),
                            logger, 'warn')
                return False
    return True

#%%
def cal_weights(judge_mat, RI_dict=None, check_JM_tol=0.001, logger=None):
    '''
    ...
    JMOK = check_JM(judge_mat, tol=check_JM_tol, logger=logger)
    if not JMOK:
        logger_show('判断矩阵可能有误，请检查！', logger, 'warn')

    if RI_dict is None:
        RI_dict = RI_DICT.copy()
    ...
    if logger != False and not isnull(logger):
        logger_show('判断矩阵：\n{}'.format(judge_mat), logger, 'info')
        logger_show('特征值：\n{}'.format(lambdas.real), logger, 'info')
        logger_show('特征向量：\n{}'.format(vectors.real), logger, 'info')
        logger_show('最大特征值：{}'.format(lmdmax.real), logger, 'info')
        logger_show('最大特征值对应特向量：\n{}'.format(max_vector.real), logger, 'info')
        logger_show('标准化权重：\n{}'.format(w), logger, 'info')
        logger_show('RI: {}'.format(RI), logger, 'info')
        logger_show('CI: {}'.format(CI), logger, 'info')
        logger_show('CR: {}'.format(CR), logger, 'info')

    return w, isOK, (CR, CI, lmdmax, RI)


def cal_weights_sim(judge_mat, RI_dict=None, check_JM_tol=0.001, logger=None):
    '''
    ...
    if logger != False and not isnull(logger):
        logger_show('判断矩阵：\n{}'.format(judge_mat), logger, 'info')
        logger_show('最大特征值：{}'.format(lmdmax))
        logger_show('最大特征值对应特向量：\n{}'.format(max_vector), logger, 'info')
        logger_show('标准化权重：\n{}'.format(w), logger, 'info')
        logger_show('RI: {}'.format(RI), logger, 'info')
        logger_show('CI: {}'.format(CI), logger, 'info')
        logger_show('CR: {}'.format(CR), logger, 'info')
...
#%%
def cal_weights_mats(judge_mats, RI_dict=None, w_func=None,
                     skip_bad_JM=True, check_JM_tol=0.001, logger=None):
    '''
    ...
    for k in range(len(judge_mats)):
        w, isOK, (CR, CI, lmdmax, RI) = w_func(judge_mats[k], RI_dict=RI_dict,
                                        check_JM_tol=check_JM_tol, logger=None)
        CIs.append(CI)
        CRs.append(CR)
        OKs.append(isOK)
        if isOK:
            ...
            if skip_bad_JM:
                logger_show('第{}个判断矩阵不能通过一致性检验，将被舍弃！'.format(k+1),
                            logger, 'info')
            else:
                logger_show('第{}个判断矩阵不能通过一致性检验！'.format(k+1),
                            logger, 'warn')
                us.append(CI)
                JMs.append(judge_mats[k])
    ...
    w, isOK, (CR, CI, lmdmax, RI) = w_func(JM, RI_dict=RI_dict,
...
if __name__ == '__main__':
    ...
    judge_mat = score2JM(df_score, max_important_level=9,
                            judge_type='score')
    ...
    w1, isOK1, (CR1, CI1, lmdmax1, RI1), (JM1, P1, CIs1, CRs1) = \
        cal_weights_mats(judge_mats, RI_dict=None, w_func=w_func1,
                         skip_bad_JM=skip_bad_JM, check_JM_tol=check_JM_tol,
                         logger=logger)
    print('\n')
    print('isOK1: {}'.format(isOK1))
    print('w1: \n{}'.format(w1))

    w2, isOK2, (CR2, CI2, lmdmax2, RI2), (JM2, P2, CIs2, CRs2) = \
        cal_weights_mats(judge_mats, RI_dict=None, w_func=w_func2,
                         skip_bad_JM=skip_bad_JM, check_JM_tol=check_JM_tol,
                         logger=logger)
    print('\n')
    print('isOK2: {}'.format(isOK2))
    print('w2: \n{}'.format(w2))
    ...
    w, isOK, (CR, CI, lmdmax, RI), (JM, P, CIs, CRs) = cal_weights_mats(
                    judge_mats, RI_dict=None, w_func=w_func1,
                    skip_bad_JM=skip_bad_JM, check_JM_tol=check_JM_tol,logger=logger)
    print('\n')
