...
class DalleSelfAttention(torch.nn.Module):
    ...
    def __init__(self, hidden_size, num_attention_heads,
                 ...
                 cogview_pb_relax=False, custom_relax=False, cogview_sandwich_layernorm=False,
                 cogview_layernorm_prescale=False):
    ...
    def forward(self, hidden_states, ltor_mask, use_cache=False, cache=None,):
        ...
        if self.cogview_sandwich_layernorm:
            output = rescale_max(output, self.cogview_layernorm_prescale)
            output = self.before_first_addition_layernorm(output)
