    ...
    **Autogenerated function**
    ...
    Args:
        ...
        tags(List or Dict, optional): list of tags in the format of [{"Key": tag-key, "Value": tag-value}] or dict in the format of
                {tag-key: tag-value} The tags to assign to the VPC. Defaults to None.
...
async def search(
    ...
    """
    hub.log.warning(
        f"aws.ec2.vpc.search '{name}' state has been deprecated. Please use exec.run with aws.ec2.vpc.get instead."
    )
    result = dict(comment=[], old_state=None, new_state=None, name=name, result=True)
    ret = await hub.exec.aws.ec2.vpc.get(
        ctx=ctx, name=name, filters=filters, resource_id=resource_id, default=default
    )
    result["result"] = ret["result"]
    result["comment"] = ret["comment"]
    if result["result"]:
