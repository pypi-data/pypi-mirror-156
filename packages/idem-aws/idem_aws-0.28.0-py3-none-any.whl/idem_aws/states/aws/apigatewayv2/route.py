...
async def present(
    ...
    Args:
        ...
        request_models(Dict, optional): The request models for the route. Supported only for WebSocket APIs.
        request_parameters(Dict, optional): The request parameters for the route. Supported only for WebSocket APIs.\
    ...
    if before_ret:
        ...
        update_route_ret = await hub.exec.aws.apigatewayv2.route.update(
            ctx,
            api_id=api_id,
...
async def search(hub, ctx, name, api_id: str, resource_id: str) -> Dict[str, Any]:
    ...
    search_ret = await hub.exec.boto3.client.apigatewayv2.get_route(
        ctx, ApiId=api_id, RouteId=resource_id
    )
    if not search_ret["result"]:
        ...
        if "NotFoundException" not in str(search_ret["comment"][0]):
            ...
            result["comment"] = (
                f"Unable to find aws.apigatewayv2.route resource with resource id '{resource_id}'",
            )
        return result

    result["old_state"] = hub.tool.aws.apigatewayv2.route.convert_raw_route_to_present(
        api_id=api_id, raw_resource=search_ret["ret"], idem_resource_name=name
    )
    result["new_state"] = copy.deepcopy(result["old_state"])
