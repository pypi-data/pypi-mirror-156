...
hub.exec.boto3.client.apigatewayv2.get_authorizer
...
async def present(
    ...
    if resource_id:
        before_ret = await hub.exec.boto3.client.apigatewayv2.get_authorizer(
    ...
    if before_ret:
        ...
        update_authorizer_ret = await hub.exec.aws.apigatewayv2.authorizer.update(
            ctx,
            api_id=api_id,
    ...
    if (not result["old_state"]) or resource_updated:
        after_ret = await hub.exec.boto3.client.apigatewayv2.get_authorizer(
            ctx, ApiId=api_id, AuthorizerId=resource_id
        )
...
async def search(hub, ctx, name, api_id: str, resource_id: str) -> Dict[str, Any]:
    ...
    search_ret = await hub.exec.boto3.client.apigatewayv2.get_authorizer(
        ctx, ApiId=api_id, AuthorizerId=resource_id
    )
    if not search_ret["result"]:
        ...
        if "NotFoundException" not in str(search_ret["comment"][0]):
            ...
            result["comment"] = (
                f"Unable to find aws.apigatewayv2.authorizer resource with resource id '{resource_id}'",
            )
        return result
