...
async def present(
    ...
    Args:
        ...
        cors_configuration(Dict, optional): A CORS configuration. Supported only for HTTP APIs.
    ...
    if resource_id:
        before_ret = await hub.exec.boto3.client.apigatewayv2.get_api(
    ...
    if before_ret:
        ...
        result["old_state"] = hub.tool.aws.apigatewayv2.api.convert_raw_api_to_present(
        ...
        update_api_ret = await hub.exec.aws.apigatewayv2.api.update(
            ctx,
    ...
    if (not result["old_state"]) or resource_updated:
        after_ret = await hub.exec.boto3.client.apigatewayv2.get_api(
            ctx, ApiId=resource_id
        )
        ...
        result["new_state"] = hub.tool.aws.apigatewayv2.api.convert_raw_api_to_present(
...
async def search(hub, ctx, name, resource_id: str) -> Dict[str, Any]:
    ...
    search_ret = await hub.exec.boto3.client.apigatewayv2.get_api(
        ctx, ApiId=resource_id
    )
    if not search_ret["result"]:
        ...
        if "NotFoundException" not in str(search_ret["comment"][0]):
            ...
            result["comment"] = (
                f"Unable to find aws.apigatewayv2.api resource with resource id '{resource_id}'",
            )
        return result
