...
async def present(
    ...
    Args:
        ...
        access_log_settings(Dict, optional): Settings for logging access in this stage.
        ...
        default_route_settings(Dict, optional): The default route settings for the stage
        ...
        route_settings(Dict, optional): Route settings for the stage, by routeKey.
    ...
    if before_ret:
        ...
        update_stage_ret = await hub.exec.aws.apigatewayv2.stage.update(
            ctx,
            api_id=api_id,
...
async def search(hub, ctx, name, api_id: str, resource_id: str) -> Dict[str, Any]:
    r"""
    ...
    search_ret = await hub.exec.boto3.client.apigatewayv2.get_stage(
        ctx, ApiId=api_id, StageName=resource_id
    )
    if not search_ret["result"]:
        ...
        if "NotFoundException" not in str(search_ret["comment"][0]):
            ...
            result["comment"] = (
                f"Unable to find aws.apigatewayv2.stage resource with resource id '{resource_id}'",
            )
        return result

    result["old_state"] = hub.tool.aws.apigatewayv2.stage.convert_raw_stage_to_present(
        api_id=api_id, raw_resource=search_ret["ret"]
    )
    result["new_state"] = copy.deepcopy(result["old_state"])
