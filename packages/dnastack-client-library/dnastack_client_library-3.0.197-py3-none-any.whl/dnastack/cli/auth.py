...
def status(context: Optional[str], output: Optional[str] = None):
    """ Check the status of all authenticators. """
    handler = AuthCommandHandler(context_name=context)
    show_iterator(output or RESOURCE_OUTPUT_SPEC.default, handler.get_states())
...
class AuthCommandHandler:
    ...
    def __init__(self, context_name: Optional[str] = None):
        ...
        self._auth_manager = AuthManager(self._config_manager.load(), self._context_name)
    ...
    def get_states(self, endpoint_ids: List[str] = None) -> Iterator[Dict[str, Any]]:
        auth_manager = self._auth_manager
        return auth_manager.get_states()
