...
def list_endpoints(context: Optional[str]):
    ...
    click.echo(to_json([
        ...
        for endpoint in EndpointCommandHandler(context_name=context).list_endpoints()
    ]))
...
def add_endpoint(context: Optional[str], id: str, short_type: str):
    """ Add a new endpoint """
    EndpointCommandHandler(context_name=context).add_endpoint(id, short_type)
...
class EndpointCommandHandler:
    ...
    def add_endpoint(self, id: str, short_type: str):
        ...
        if context is None:
            context = self.__config.contexts[target_context_name] = Context(auto_sync=False)
