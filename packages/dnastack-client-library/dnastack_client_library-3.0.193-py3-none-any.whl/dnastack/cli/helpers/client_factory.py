...
class ConfigurationBasedClientFactory:
    ...
    def get(self,
            ...
            endpoint_id: Optional[str] = None) -> SERVICE_CLIENT_CLASS:
        return cls.make(
            self.get_endpoint(
    ...
    def get_endpoint(self,
        ...
        assert config.current_context is not None, (
            f'The current context ({config.current_context}) is not set.'
        )
        assert config.current_context in config.contexts, (
            f'The current context ({config.current_context}) is set but the context is not in the configuration file.'
        )

        context = config.contexts[config.current_context]
        endpoints = {
        ...
        assert endpoints, (
            f'The current context ({config.current_context}) does not have any endpoints for '
            f'{supported_service_type_list_in_string}.'
        )
        ...
        if endpoint_id:
            self._assert_with_alternative_ids(
                ...
                (
                    ...
                    f'in this context ({config.current_context}).'
                ),
                endpoints.keys()
            )
            ...
            self._assert_with_alternative_ids(
                ...
                (
                    ...
                    f'{supported_service_type_list_in_string} in this context ({config.current_context}).'
                ),
                endpoints.keys()
            )
