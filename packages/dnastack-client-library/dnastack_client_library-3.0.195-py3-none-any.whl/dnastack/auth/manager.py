...
from dnastack.http.authenticators.abstract import Authenticator, AuthStateStatus
...
class AuthManager:
    ...
    def revoke(self, endpoint_ids: List[str], confirmation_operation: Optional[Callable[[], bool]] = None) -> List[str]:
        ...
        for authenticator in authenticators:
            ...
            if state.get('grants') and state.get('grants').get('scope'):
                granted_scopes.extend(sorted(str(state.get('grants').get('scope')).split(r' ')))
    ...
    def get_states(self, endpoint_ids: List[str] = None) -> Iterator[Dict[str, Any]]:
        ...
        for authenticator in self.get_authenticators(endpoint_ids):
            ...
            current_hash = JsonModelMixin.hash(state['auth_info'])
    ...
    def get_filtered_endpoints(self, endpoint_ids: List[str] = None) -> List[ServiceEndpoint]:
        wrapper = ConfigurationWrapper(self._config)
        return [
            endpoint
