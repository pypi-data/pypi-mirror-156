...
from rime_sdk.protos.test_run_results.test_run_results_pb2 import ListTestRunsRequest
    ...
    def list_test_runs(self) -> List[TestRun]:
        ...
        with self._backend.get_test_run_results_stub() as test_run_results:
            ...
            while True:
                ...
                res = test_run_results.ListTestRuns(request)
                new_test_run_ids = [run.metadata.id for run in res.test_runs]
                test_run_ids.extend(new_test_run_ids)
                # Advance to the next page of test cases.
                page_token = res.next_page_token
                # we've reached the last page of test cases.
                if not res.has_more:
                    break
        return [TestRun(self._backend, _id) for _id in test_run_ids]
    ...
    def get_firewall(self) -> Firewall:
        ...
        try:
            with self._backend.get_firewall_stub() as firewall_tester:
                ...
                return Firewall(self._backend, mapping[self._project_id])
        except grpc.RpcError as e:
            raise ValueError(e)

    def delete_firewall(self) -> None:
        """Delete firewall for this project if exists."""
        req = DeleteFirewallRequest(firewall_id=self._project_id)
        try:
            with self._backend.get_firewall_stub() as firewall_tester:
                firewall_tester.DeleteFirewall(req)
        except grpc.RpcError as rpc_error:
            if rpc_error.code() == grpc.StatusCode.NOT_FOUND:
                raise ValueError("This project does not have a firewall.") from None
            raise rpc_error
