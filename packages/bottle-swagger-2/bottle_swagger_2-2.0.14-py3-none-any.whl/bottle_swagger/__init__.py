...
import re
import logging
from bottle import request, response, HTTPResponse, json_dumps, static_file, FormsDict
...
def convert_bottle_rule_to_swagger_path(path):
    return re.sub(r'/<([^:>]+?)(:[^>]+)?>', r'/{\1}', path)


def forms_dict_fixer(fd: FormsDict):
    result = {}
    for k, v in fd.dict.items():
...
class SwaggerPlugin(object):
    ...
    * ``extra_bravado_config`` -- (object) Any additional Bravado configuration items you may want.
    ...
    def _swagger_op(self, route):
        ...
        path = convert_bottle_rule_to_swagger_path(route.rule)
...
class BottleIncomingRequest(IncomingRequest):
    ...
    @property
    def form(self):
        return forms_dict_fixer(self.request.forms)
