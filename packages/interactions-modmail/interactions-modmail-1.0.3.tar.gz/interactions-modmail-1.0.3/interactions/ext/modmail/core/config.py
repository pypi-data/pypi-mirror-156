...
from interactions import Color, PresenceActivityType, StatusType

from ._color_data import ALL_COLORS
from .models import Default, DMDisabled, InvalidConfigError
...
class ConfigManager:
    ...
    public_keys = {
        ...
        "prefix": "?",
        "mention": "@here",
        "main_color": hex(Color().blurple),
        "error_color": hex(Color().red),
        "user_typing": False,
        "mod_typing": False,
        ...
        # threads
        "sent_emoji": ":heavy_check_mark:",
        "blocked_emoji": ":no_entry_sign:",
        "close_emoji": ":lock:",
        ...
        # moderation
        "recipient_color": str(0xF1C40F),
        "mod_color": hex(Color().green),
        "mod_tag": None,
        # anonymous message
        "anon_username": None,
        ...
        "confirm_thread_creation_accept": ":heavy_check_mark: ",
        "confirm_thread_creation_deny": ":heavy_check_mark:",
        # regex
    ...
    def populate_cache(self) -> dict:
        ...
        with open(f"{path}/config.json", "r", encoding="utf-8") as f:
            ...
            try:
                data.update(
                    {k.lower(): v for k, v in json.load(f).items() if k.lower() in self.all_keys}
                )
    ...
    def set(self, key: str, item: typing.Any, convert=True) -> None:
        ...
        if key in self.colors:
            try:
                ...
                try:
                    ...
                    raise InvalidConfigError("Invalid color name or hex.") from e
                ...
                if hex_ is None:
                    raise
            return self.__setitem__(key, f"#{hex_}")
        ...
        if key in self.booleans:
            ...
            if isinstance(item, self.enums.get(key)):
