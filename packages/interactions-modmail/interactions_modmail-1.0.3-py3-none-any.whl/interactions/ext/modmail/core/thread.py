...
class Thread:
    ...
    async def _format_info_embed(
        ...
        if member is not None:
            ...
            if self.manager.modmail.config["thread_show_join_age"]:
                print(member.joined_at.tzinfo == time.tzinfo)
    ...
    async def _close(
        ...
        if delete_channel:
            tasks.append(self.channel.modify(archived=True, locked=True))
...
class ThreadManager:
    ...
    async def populate_cache(self) -> None:
        for _id in self.modmail.config.get("modmail_guild_id"):
            for channel in await self.modmail.modmail_guild(_id).get_all_channels():
        ...
        if recipient is None and channel is not None:
            ...
            if thread is None:
                ...
                if thread is not None:
                    ...
                    await channel.modify(topic=f"User ID: {user_id}")
            return thread
