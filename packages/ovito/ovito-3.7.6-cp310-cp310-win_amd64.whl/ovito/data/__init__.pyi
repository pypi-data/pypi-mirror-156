...
__all__ = ['DataCollection', 'DataObject', 'TriangleMesh', 'SimulationCell', 'Property', 'PropertyContainer', 'DataTable', 'ElementType', 'SurfaceMesh', 'SurfaceMeshTopology', 'VoxelGrid', 'ParticleType', 'BondType', 'BondsEnumerator', 'CutoffNeighborFinder', 'NearestNeighborFinder', 'PTMNeighborFinder', 'Particles', 'Bonds', 'TrajectoryLines', 'DislocationNetwork', 'DislocationSegment']
from typing import NamedTuple, Tuple, Optional, Any, Union, Iterator, Mapping, MutableMapping, Sequence, MutableSequence, NamedTuple, TypeVar
import ovito.vis
...
which represents a whole *data set*. Furthermore, data objects can be nested into hierarchy. 
...
.. seealso:: 
    ...
    def identifier(self) -> str:
        """The unique identifier string of the data object. It serves as look-up key in object dictionaries, for example the :py:attr:`DataCollection.tables` collection, or as a target name in various places where a data object needs to be referenced by name, e.g. the `TimeAveragingModifier.operate_on` field. 
...
Data objects generated by modifiers in a pipeline typically have an automatically assigned identifier, as documented in the description of the respective modifier. When writing your own modifier function, you are responsible for giving new data objects created by your modifier function a meaningful identifier, so that subsequent modifiers in the pipeline can refer to these data objects."""
    ...
    @property
    def vis(self) -> ovito.vis.DataVis:
        """The :py:class:`DataVis` element currently associated with this data object, which is responsible for visually rendering the stored data. If set to ``None``, the data object remains non-visual and doesn't appear in rendered images or the viewports. Furthermore, note that the same :py:class:`DataVis` element may be assigned to multiple data objects in order to synchronize their visual appearance."""
        ...

    def make_mutable(self, subobj: DataObjectType) -> DataObjectType:
        """This helper method requests a deep copy of *subobj*, which must be a child :py:class:`DataObject` of this parent :py:class:`DataObject`. A copy will only be made in case the sub-object is currently referenced by at least one more parent object. If, however, the sub-object is exclusive owned by this :py:class:`DataObject`, no copy is made and the original sub-object is returned as is. The returned object is safe to modify without unexpected side effects, because any shared ownership is converted an exclusive ownership by the method. 

Please see the section :ref:`underscore_notation` for a discussion of object ownership and typical use-cases for this method. 

:param DataObject subobj: A existing sub-object of this parent data object, for which exclusive ownership is requested.
:return: A copy of *subobj* if its ownership was previously shared with some other parent. Otherwise the original object is returned."""
        ...

class DataCollection(DataObject):
    """Base: :py:class:`ovito.data.DataObject`

A :py:class:`DataCollection` is a container class holding together individual *data objects*, each representing different fragments of a dataset. For example, a dataset loaded from a simulation data file may consist of particles, the simulation cell information and additional auxiliary data such as the current timestep number of the snapshots, etc. All this information is contained in one :py:class:`DataCollection`, which exposes the individual pieces of information as sub-objects, for example via the :py:attr:`DataCollection.particles`, :py:attr:`DataCollection.cell` and :py:attr:`DataCollection.attributes` fields. 

Data collections are the elementary entities that get processed within a data :py:class:`Pipeline`. Each modifier receives a data collection from the preceding modifier, alters it in some way, and passes it on to the next modifier. The output data collection of the last modifier in the pipeline is returned by the `Pipeline.compute()` method. 

A data collection essentially consists of a bunch of `DataObjects`, which are all stored in the :py:attr:`DataCollection.objects` list. Typically, you don't access the data objects through this list directly but rather use on of the special accessor fields provided by the :py:class:`DataCollection` class, which give more convenient access to data objects of a particular kind. For example, the :py:attr:`surfaces` dictionary provides key-based access to all the :py:class:`SurfaceMesh` instances currently in the data collection. 

You can programmatically add or remove data objects from a data collection by manipulating its :py:attr:`objects` list. For instance, to populate a new data collection instance that is initially empty with a new :py:class:`SimulationCell` object: 
...
:py:meth:`create_cell` method or simply assign a new instance of the :py:class:`SimulationCell` class to the :py:attr:`cell` attribute."""
    ...
    @property
    def dislocations(self) -> DislocationNetwork:
...
Typically you don't have to work with this list directly, because the :py:class:`DataCollection` class provides several convenience accessor attributes for the different flavors of data objects. For example, the :py:attr:`DataCollection.particles` field returns the :py:class:`Particles` object from this data objects list. Dictionary views such as :py:attr:`DataCollection.tables` and :py:attr:`DataCollection.surfaces` provide key-based access to a particular class of data objects from this list."""
        ...

    @property
    def grids(self) -> Mapping[str, VoxelGrid]:
...
```"""
    ...
    @property
    def surfaces(self) -> Mapping[str, SurfaceMesh]:
...
```"""
    ...
    @property
    def tables(self) -> Mapping[str, DataTable]:
...
```"""
    ...
    @property
    def triangle_meshes(self) -> Mapping[str, TriangleMesh]:
...
which can be used to look it up in the dictionary."""
    ...
    def apply(self, modifier: ovito.pipeline.Modifier, frame: Optional[int]=None) -> None:
...
The method allows modifying a data collection with one of Ovito's built-in modifiers directly without the need to build up a complete
...
gets replaced by the output produced by the invoked modifier function. Note that it is possible to first create a copy of
the original data using the :py:meth:`.clone` method if needed. The following code example
demonstrates how to use the :py:meth:`!apply()` method to successively modify a dataset:
...
Furthermore, it is possible to use the :py:meth:`!apply()` in the implementation of a :ref:`user-defined modifier function <writing_custom_modifiers>`
to invoke the built-in modifiers of OVITO as sub-routine:
...
```"""
    ...
    def create_cell(self, matrix: numpy.typing.ArrayLike, pbc: Tuple[bool, bool, bool]) -> SimulationCell:
...
:param Mapping[str, Any] vis_params: Optional dictionary to initialize attributes of the attached :py:class:`SimulationCellVis` element (only used if the cell object is newly created by the method).
:rtype: ovito.data.SimulationCell
...
The logic of this method is roughly equivalent to the following code::
    ...
    def create_particles(self, **kwargs) -> Particles:
...
:param Mapping[str, Any] vis_params: Optional dictionary to initialize attributes of the attached :py:class:`ParticlesVis` element (only used if the particles object is newly created by the method).
...
This class represents a single type of data elements, for example a particular atom or bond type. 
It serves as common base class for the :py:class:`ParticleType` and :py:class:`BondType` classes, 
which represent these more specific types. 

Each type has a unique numeric :py:attr:`.id`, which is used when looking up types given some 
numeric value in a *typed* :py:class:`Property` array. An example for a typed property is the particle property ``'Particle Type'``. 
The property object manages all its associated types in its `Property.types` list. 

The `Property.type_by_id()` and `Property.type_by_name()` 
methods allow looking up a certain :py:class:`ElementType` based on its numeric identifier or name string."""
...
the :py:class:`ParticleType` class. The property fields of the class control how the particles of that type get visualized in terms of e.g. color, particle radius, shape, etc.

The :py:class:`ParticleType` class inherits several general data fields from its base class :py:class:`ElementType`, e.g. the :py:attr:`~ElementType.color`, :py:attr:`~ElementType.name`
...
The :py:class:`ParticleType` instances are all stored in the :py:class:`Property` object with the name ``'Particle Type'``, which also stores for each particle what
its type is. The association between particles and particle types is established via a unique numeric type :py:attr:`~ElementType.id`. The following code shows how to iterate over
all particle types in a dataset, which are listed in the :py:attr:`Property.types` field of the :py:attr:`particle_types` property:
...
The order in which the particle types are stored in the :py:attr:`Property.types` list is arbitrary, and the unique numeric IDs of particle types have no specific meaning in general.
A common operation is to find the :py:class:`~ParticleType` in the list corresponding to a given numeric ID. For this look up operation, the :py:class:`Property` class provides the :py:meth:`type_by_id` method:
...
For this kind of look up operation, the :py:meth:`type_by_name` method may be used, which assumes that each type has a unique :py:attr:`name` (which may not always be true):
...
```"""
    ...
    def backface_culling(self) -> bool:
        """Activates back-face culling for the user-defined particle shape mesh to speed up rendering. If turned on, polygonal sides of the shape mesh facing away from the viewer will not be rendered. You can turn this option off if the particle's shape is not closed and two-sided rendering is required. This option only has an effect if a user-defined shape has been assigned to the particle type using the :py:meth:`.load_shape` method. 
...
:Default: ``True``"""
    ...
    def highlight_edges(self) -> bool:
        """Activates the highlighting of the polygonal edges of the user-defined particle shape during rendering. This option only has an effect if a user-defined shape has been assigned to the particle type using the :py:meth:`.load_shape` method. 
...
```"""
    ...
    @property
    def shape(self) -> ovito.vis.ParticlesVis.Shape:
...
The :py:meth:`load_shape` method lets you specify a user-defined mesh geometry for this particle type. Calling this method automatically switches the shape parameter to mode ``Mesh``. 

Setting the shapes of particle types permanently, i.e., for all frames of a loaded simulation trajectory, typically requires a user-defined modifier function. This function is inserted into the :py:class:`Pipeline` to make the necessary changes to the :py:class:`ParticleType` objects associated with the :py:class:`Property` named ``Particle Type``: 
...
```"""
    ...
    @property
    def use_mesh_color(self) -> bool:
        """Use the intrinsic mesh color(s) instead of the particle color when rendering particles of this type. This option only has an effect if a user-defined shape has been assigned to the particle type using the :py:meth:`.load_shape` method. 
...
:Default: ``False``"""
    ...
    def vdw_radius(self) -> float:
        """The van der Waals radius of the particle type. This value is used by the :py:class:`CreateBondsModifier` to decide which pairs of particles are close enough to be connected by a bond. In contrast to the :py:attr:`radius` parameter, the van der Waals radius does not affect the visual appearance of the particles of this type. 

:Default: ``0.0``"""
        ...

    def load_defaults(self) -> None:
        """Given the type's chemical :py:attr:`name`, which must have been set before calling this method, initializes the type's :py:attr:`color` and :py:attr:`radius` fields with default values from OVITO's internal database of chemical elements. This method is useful when creating new atom types while building up a molecule structure."""
        ...
...
The shape loaded from the geometry file will be scaled with the :py:attr:`radius` value set for this particle type or the per-particle value stored in the ``Radius`` particle property if present. The shape of each particle will be rendered such that its origin is located at the coordinates of the particle. 
...
See also the :py:attr:`is_infinite_line` property."""
    ...
    @property
    def points(self) -> numpy.typing.ArrayLike:
...
Note that the element types may be stored in arbitrary order in this list. Thus, it is not valid to use a numeric type ID as an index into this list."""
    ...
    def type_by_id(self, id: int, raise_error: bool=True) -> ElementType:
        """Looks up and returns the :py:class:`ElementType` with the given unique numeric ID in this property's :py:attr:`types` list.
Depending on *raise_error*, raises a ``KeyError`` or returns ``None`` if no type with the numeric ID exists.
...
An :ref:`"underscore" version <underscore_notation>` of the method exists, which should be used whenever you intend to modify the returned type object. :py:meth:`!type_by_id_()` implicitly calls :py:meth:`make_mutable` on the :py:class:`ElementType` to make sure it can be changed without unexpected side effects:
...
```"""
    ...
    def type_by_id_(self, id: int, raise_error: bool=True) -> ElementType:
        ...

    def type_by_name(self, name: str, raise_error: bool=True) -> ElementType:
        """Looks up and returns the :py:class:`ElementType` with the given name in this property's :py:attr:`types` list.
If multiple types exists with the same name, the first type is returned. 
Depending on *raise_error*, raises a ``KeyError`` or returns ``None`` if there isn't a type with that name. 
...
An :ref:`"underscore" version <underscore_notation>` of the method exists, which should be used whenever you intend to modify the returned type object. :py:meth:`!type_by_name_()` implicitly calls :py:meth:`make_mutable` on the :py:class:`ElementType` to make sure it can be changed without unexpected side effects:
...
```"""
    ...
    def type_by_name_(self, name: str, raise_error: bool=True) -> ElementType:
        ...

    def __len__(self) -> int:
        ...

    def __iter__(self) -> Iterator[numpy.typing.NDArray]:
        ...

    def __getitem__(self, idx: Any) -> numpy.typing.NDArray:
        ...
...
OVITO has several concrete implementations of the abstract :py:class:`PropertyContainer` interface: 
    ...
    * :py:attr:`SurfaceMesh.regions`"""
    ...
    def create_property(self, name: str, dtype: Optional[numpy.typing.DTypeLike]=None, components: Optional[int]=None, data: Optional[numpy.typing.ArrayLike]=None) -> Property:
...
To create a *standard* property, one of the standard property names must be provided as *name* argument:
...
If the property to be created already exists in the container, it is replaced with a modifiable copy if necessary.
...
:param name: Either a standard property type constant or a name string.
...
:returns: The newly created :py:class:`Property` object."""
    ...
    def delete_elements(self, mask: numpy.typing.ArrayLike) -> None:
...
The effect of this statement is the same as for applying the :py:class:`DeleteSelectedModifier` to the particles list."""
    ...
    def delete_indices(self, indices: numpy.typing.ArrayLike) -> None:
        """Deletes a subset of the elements from this container. The elements to be deleted must be specified in terms
...
class SimulationCell(DataObject):
    """Base: :py:class:`ovito.data.DataObject`
...
Every :py:class:`SimulationCell` has an attached :py:class:`SimulationCellVis` element,
...
```"""
    ...
    @property
    def vis(self) -> ovito.vis.SimulationCellVis:
        """The :py:class:`DataVis` element currently associated with this data object, which is responsible for visually rendering the stored data. If set to ``None``, the data object remains non-visual and doesn't appear in rendered images or the viewports. Furthermore, note that the same :py:class:`DataVis` element may be assigned to multiple data objects in order to synchronize their visual appearance."""
        ...

    def __getitem__(self, idx: Any) -> numpy.typing.ArrayLike:
        ...

    def delta_vector(self, ra: numpy.typing.ArrayLike, rb: numpy.typing.ArrayLike, return_pbcvec: bool=False) -> numpy.ndarray:
...
:param bool return_pbcvec: If True, also returns the vector :math:`n`, which specifies how often the vector :math:`(r_b' - r_a)` crosses the periodic cell boundaries.
...
that is inherited from the :py:class:`PropertyContainer` base class. 
...
:py:class:`PropertyContainer` base class. 
...
:guilabel:`Nucleobase`                              int        
:guilabel:`Nucleotide Axis`                         float      X, Y, Z
:guilabel:`Nucleotide Normal`                       float      X, Y, Z
...
:guilabel:`Superquadric Roundness`                  float      Phi, Theta
...
=================================================== ========== ==================================================="""
    ...
    def identifiers(self) -> Property:
        """The :py:class:`Property` data array for the ``Particle Identifier`` standard particle property; or ``None`` if that property is undefined."""
        ...
    ...
    def positions(self) -> Property:
        """The :py:class:`Property` data array for the ``Position`` standard particle property; or ``None`` if that property is undefined."""
        ...
    ...
    @property
    def vis(self) -> ovito.vis.ParticlesVis:
        """The :py:class:`DataVis` element currently associated with this data object, which is responsible for visually rendering the stored data. If set to ``None``, the data object remains non-visual and doesn't appear in rendered images or the viewports. Furthermore, note that the same :py:class:`DataVis` element may be assigned to multiple data objects in order to synchronize their visual appearance."""
        ...

    def add_particle(self, position: numpy.typing.ArrayLike) -> int:
...
:returns: The index of the newly created particle, i.e. :py:attr:`(Particles.count-1) <ovito.data.PropertyContainer.count>`."""
    ...
    def delta_vector(self, a: numpy.typing.ArrayLike, b: numpy.typing.ArrayLike, cell: SimulationCell, return_pbcvec: bool=False) -> numpy.ndarray:
...
:param SimulationCell cell: The periodic domain. Typically, `DataCollection.cell` is used as argument here.
...
:returns: The delta vector and, optionally, the vector :math:`n`."""
    ...
    def create_bonds(self, **kwargs) -> Bonds:
...
:param Mapping[str, Any] vis_params: Optional dictionary to initialize attributes of the attached :py:class:`BondsVis` element (only used if the bonds object is newly created by the method).
...
```"""
    ...
    def create_angles(self, **kwargs) -> Angles:
        ...

    def create_dihedrals(self, **kwargs) -> Dihedrals:
        ...

    def create_impropers(self, **kwargs) -> Impropers:
        ...

    def remap_indices(self, particles: Particles, indices: Sequence[int]=None) -> Union[numpy.ndarray, slice]:
...
Given two data collections *A* and *B* containing the same set of particles but in different order, :py:meth:`remap_indices` determines for each particle in *B* the zero-based index at which the same particle is found in *A*. For instance::
...
The index mapping generated by :py:meth:`remap_indices` allows you to retrieve property values of particles in *A* in the same order in which they appear in *B*, making it easy to perform computations involving property values at different trajectory timesteps, e.g.:: 

  mapping = A.particles.remap_indices(B.particles)
  displacements = B.particles.positions - A.particles.positions[mapping]


:py:meth:`remap_indices` compares the unique IDs stored in the ``Particle Identifier`` property arrays of both snapshots to compute the index permutation map. If this property is not defined, which may be the case if the imported trajectory file did not contain atom IDs, the :py:meth:`remap_indices` method simply assumes that both snapshots use the same constant storage order and returns the identity mapping - as a Python :py:class:`slice` object for optimal performance when being used for NumPy indexing. A slice object is also returned in case the ordering of particle IDs turns out to be the same in both snapshots and no remapping is necessary. 
...
================================ ======================== ========== =======================
:guilabel:`Bond Type`            :py:attr:`bond_types`    int        
:guilabel:`Color`                :py:attr:`colors`        float      R, G, B
:guilabel:`Length`                                        float       
:guilabel:`Particle Identifiers`                          int64      A, B\\
:guilabel:`Periodic Image`       :py:attr:`pbc_vectors`   int        X, Y, Z 
:guilabel:`Selection`            :py:attr:`selection`     int         
:guilabel:`Topology`             :py:attr:`topology`      int64      1, 2\\
:guilabel:`Transparency`                                  float       
:guilabel:`Width`                                         float       
================================ ======================== ========== ======================="""
    ...
    def add_bond(self, a: int, b: int, type: Optional[int]=None, pbcvec: Optional[numpy.typing.ArrayLike]=None) -> int:
...
This data structure holds the connectivity information of a :py:class:`SurfaceMesh`, which
consists of *vertices*, *faces* and *halfedges*. It is stored in the :py:attr:`SurfaceMesh.topology` field.

All entities (vertices, faces, and halfedges) of the mesh are identified by numeric indices ranging from 
0 to (:py:attr:`vertex_count`-1), (:py:attr:`face_count`-1), and (:py:attr:`edge_count`-1), respectively. 
Vertices and faces of the mesh may be associated with auxiliary properties, which are stored separately from 
the topology in the :py:attr:`SurfaceMesh.vertices` and :py:attr:`SurfaceMesh.faces` property containers.
In particular, the coordinates of the mesh's vertices are stored in the ``Position`` property array of :py:attr:`SurfaceMesh.vertices`.
...
Thus, halfedge pairs are essential for forming a connected, two-dimensional surface manifold. The surface is said to to be *closed*, 
i.e. it has no open boundaries, if all halfedges of the mesh are associated with corresponding opposite halfedges.
...
For each face the topology object maintains a circular linked-list of directed halfedges bounding that face.
It can be accessed through the :py:meth:`first_face_edge` and :py:meth:`next_face_edge`/:py:meth:`prev_face_edge` methods."""

    @property
    def vertex_count(self) -> int:
        """Number of vertices of the :py:class:`SurfaceMesh`. This is always equal to the :py:attr:`~PropertyContainer.count` of the :py:attr:`SurfaceMesh.vertices` property container."""
        ...

    @property
    def face_count(self) -> int:
        """Number of faces of the :py:class:`SurfaceMesh`. This is always equal to the :py:attr:`~PropertyContainer.count` of the :py:attr:`SurfaceMesh.faces` property container."""
        ...

    @property
    def edge_count(self) -> int:
        """Number of halfedges in the :py:class:`SurfaceMesh`."""
        ...
    ...
    def has_opposite_face(self, face: int) -> bool:
    ...
    def first_vertex_edge(self, vertex: int) -> int:
    ...
    def next_vertex_edge(self, edge: int) -> int:
        """Returns another halfedge leaving from the same vertex as *edge*. Together with :py:meth:`first_vertex_edge` this method allows you to iterate over all halfedges connected to a given vertex. Once the end of the linked list has been reached, the method returns -1. 
...
```"""
    ...
    def first_face_edge(self, face: int) -> int:
    ...
    def next_face_edge(self, edge: int) -> int:
        """Given the halfedge *edge* bounding some face, this method returns the following halfedge when going around the face in forward direction (counter-clockwise - when looking on the face from the front). All halfedges of the face form a circular sequence - without a particular beginning or end. You can loop over this circular sequence in foward or backward direction with the :py:meth:`next_face_edge` and :py:meth:`prev_face_edge` methods. Given some mesh face, you can obtain a first halfedge through the :py:meth:`first_face_edge` method. 
...
```"""
    ...
    def prev_face_edge(self, edge: int) -> int:
        """Given the halfedge *edge* bounding some face, this method returns the previous halfedge going around that face in backward direction (clockwise - when looking on the face from the front). All halfedges of a face form a circular sequence - without a specific beginning or end. You can loop over this circular sequence in foward or backward direction with the :py:meth:`next_face_edge` and :py:meth:`prev_face_edge` methods."""
        ...

    def first_edge_vertex(self, edge: int) -> int:
    ...
    def second_edge_vertex(self, edge: int) -> int:
    ...
    def adjacent_face(self, edge: int) -> int:
        """Returns the face that is bounded by the halfedge *edge*."""
        ...

    def first_face_vertex(self, face: int) -> int:
        """Given a face, this method returns some vertex of that face. This is equivalent to retrieving the vertex to which the first halfedge of the face is conneced to, i.e. ``first_edge_vertex(first_face_edge(face))``."""
        ...

    def opposite_edge(self, edge: int) -> int:
...
You may call the convenience method :py:meth:`has_opposite_edge` to determine whether a halfedge is associated with a corresponding opposite halfedge. If the surface mesh does not form a closed manifold, the halfedges at the boundary of the manifold do not have opposite halfedges, because there are no adjacent faces where the surface terminates."""
    ...
    def has_opposite_edge(self, edge: int) -> int:
    ...
    def next_manifold_edge(self, edge: int) -> int:
        """"""
        ...

    def count_vertex_edges(self, vertex: int) -> int:
    ...
    def count_face_edges(self, face: int) -> int:
    ...
    def count_manifolds(self, edge: int) -> int:
        """"""
        ...

    def find_edge(self, face: int, vertex1: int, vertex2: int) -> int:
...
A triangle mesh representation of a surface or, more precisely, a two-dimensional manifold that is closed and orientable. 
Typically, surface meshes are produced by OVITO's modifiers such as :py:class:`ConstructSurfaceModifier`, 
:py:class:`CreateIsosurfaceModifier`, :py:class:`CoordinationPolyhedraModifier` or :py:class:`VoronoiAnalysisModifier`. 
See the user manual page on surface meshes for more information on this geometric data structure.
...
That means faces of the surface mesh can connect vertices on opposite sides of the simulation box and will wrap around correctly. 
...
representation of the mesh when it comes to visualization. You can explicitly request a non-periodic version of the mesh, which was clipped at the 
periodic box boundaries, by calling the :py:meth:`to_triangle_mesh` method from a script.

The spatial domain the surface mesh is embedded in is specified as a :py:class:`SimulationCell` object attached to the 
:py:class:`SurfaceMesh`. It is stored in the :py:attr:`SurfaceMesh.domain` field. Note that this :py:class:`SimulationCell` 
may, in some situations, not be identical with the global simulation :py:attr:`cell` set for 
the :py:class:`DataCollection`.

Spatial regions

Being a closed, orientable manifold a surface mesh divides space into two or more spatial regions. 
For example, if the surface mesh was constructed by the :py:class:`ConstructSurfaceModifier` from a set of input particles, 
then the space enclosed by the surface is the "filled" region and the exterior space is the "empty" region containing no particles. 
...
It may be that a surface mesh is degenerate. In such a case there is only one spatial region filling entire space. For example, 
when there are no input particles, the :py:class:`ConstructSurfaceModifier` cannot construct a regular surface mesh and
...
A surface mesh can be exported to a geometry file in the form of a triangle mesh using OVITO's :py:func:`export_file` function. 
...
to the :py:class:`SurfaceMesh`. The actual truncation of the mesh happens later on, during the final visualization step, when a non-periodic version is computed."""
    ...
    @property
    def domain(self) -> SimulationCell:
    ...
    @property
    def domain_(self) -> SimulationCell:
        ...
    ...
    def vertices(self) -> PropertyContainer:
        """The :py:class:`PropertyContainer` storing all per-vertex properties of the mesh, including the vertex coordinates (``Position`` property)."""
        ...
    ...
    @property
    def space_filling_region(self) -> int:
    ...
    def get_clipping_planes(self) -> numpy.typing.NDArray:
        """Returns a *N* x 4 array containing the definitions of the *N* clipping planes attached to this :py:class:`SurfaceMesh`. 
...
Note that the returned Numpy array is a copy of the internal data stored by the :py:class:`SurfaceMesh`."""
    ...
    def set_clipping_planes(self, planes: numpy.typing.ArrayLike) -> None:
    ...
    def get_face_adjacency(self) -> numpy.typing.NDArray:
        ...

    def get_faces(self) -> numpy.typing.NDArray:
        ...

    def locate_point(self, pos: ovito.vis.Vector3, eps: float=1e-06) -> Optional[int]:
...
:param array-like pos: The (x,y,z) coordinates of the query point
:param float eps: Numerical precision threshold for point-on-boundary test
:return: The numeric ID of the spatial region containing *pos*; or ``None`` if *pos* is exactly on the dividing boundary between two regions"""
        ...

    def to_triangle_mesh(self) -> Tuple[TriangleMesh, Optional[TriangleMesh], numpy.typing.NDArray]:
...
This data object stores the network of dislocation lines extracted by a :py:class:`DislocationAnalysisModifier`. You can access it through the :py:attr:`DataCollection.dislocations` field. 

The dislocation network is associated with a :py:class:`DislocationVis` element controlling the visual appearance of the dislocation lines. It can be accessed through the :py:attr:`~DataObject.vis` attribute of the :py:class:`~DataObject` base class.
...
A dislocation network can be written to a data file in the form of polylines using the :py:func:`ovito.io.export_file` function (select the ``vtk/disloc`` output format). During export, a non-periodic version is produced by clipping dislocation lines at the domain boundaries."""

    @property
    def vis(self) -> ovito.vis.DislocationVis:
        """The :py:class:`DataVis` element currently associated with this data object, which is responsible for visually rendering the stored data. If set to ``None``, the data object remains non-visual and doesn't appear in rendered images or the viewports. Furthermore, note that the same :py:class:`DataVis` element may be assigned to multiple data objects in order to synchronize their visual appearance."""
        ...

    @property
    def segments(self) -> Sequence[DislocationSegment]:
        """The list of dislocation segments in this dislocation network. This list-like object is read-only and contains :py:class:`DislocationSegment` objects."""
        ...

    def set_segment(self, index: int, true_burgers_vector: Optional[ovito.vis.Vector3]=None, cluster_id: Optional[int]=None, points: Optional[numpy.typing.ArrayLike]=None, custom_color: Optional[ovito.vis.Color]=None) -> None:
...
:param index: The zero-based index of the dislocation line in the :py:attr:`segments` array to be modified.
:param true_burgers_vector: The lattice-space Burgers vector (:py:attr:`true_burgers_vector`) to be assigned to the dislocation line.
:param cluster_id: The numeric ID of the crystallite cluster the dislocation line is embedded in.
:param points: A *N* x 3 NumPy array with the Cartesian coordinates of the dislocation line vertices.
:param custom_color: RGB color to be used for rendering the line instead of the automatically determined color.


Example of a :ref:`user-defined modifier function <writing_custom_modifiers>` manipulating the dislocation line data:
...
:Default: 0"""
    ...
    @property
    def vis(self) -> ovito.vis.TriangleMeshVis:
        """The :py:class:`DataVis` element currently associated with this data object, which is responsible for visually rendering the stored data. If set to ``None``, the data object remains non-visual and doesn't appear in rendered images or the viewports. Furthermore, note that the same :py:class:`DataVis` element may be assigned to multiple data objects in order to synchronize their visual appearance."""
        ...

    def get_vertices(self) -> numpy.typing.NDArray:
    ...
    def get_edge_visibility(self) -> numpy.typing.NDArray:
        ...

    def get_vertex_colors(self) -> Optional[numpy.typing.NDArray]:
        ...

    def get_vertex_pseudocolors(self) -> Optional[numpy.typing.NDArray]:
        ...

    def get_face_colors(self) -> Optional[numpy.typing.NDArray]:
        ...

    def get_face_pseudocolors(self) -> Optional[numpy.typing.NDArray]:
        ...

    def get_normals(self) -> Optional[numpy.typing.NDArray]:
        ...

    def set_vertices(self, coordinates: numpy.typing.ArrayLike) -> None:
    ...
    def set_faces(self, vertex_indices: numpy.typing.ArrayLike) -> None:
    ...
    def set_edge_visibility(self, edge_flags: numpy.typing.ArrayLike) -> None:
        ...

    def set_vertex_colors(self, colors: Optional[numpy.typing.ArrayLike]) -> None:
        ...

    def set_vertex_pseudocolors(self, pseudocolors: Optional[numpy.typing.ArrayLike]) -> None:
        ...

    def set_face_colors(self, colors: Optional[numpy.typing.ArrayLike]) -> None:
        ...

    def set_face_pseudocolors(self, pseudocolors: Optional[numpy.typing.ArrayLike]) -> None:
        ...

    def set_normals(self, normals: Optional[numpy.typing.ArrayLike]) -> None:
        ...
...
class TrajectoryLines(PropertyContainer):
    """Base: :py:class:`ovito.data.PropertyContainer`

Data object that stores the trajectory lines of a set of particles, 
which have been traced by the :py:class:`GenerateTrajectoryLinesModifier`. 
It is typically part of a pipeline's output data collection, 
from where it can be accessed via the `DataCollection.trajectories` field. 

A :py:class:`TrajectoryLines` object has an associated :py:class:`TrajectoryVis`
...
This visual element is accessible through the :py:attr:`~DataObject.vis` attribute of the base class."""
    ...
    @property
    def positions(self) -> Property:
        """The :py:class:`Property` data array storing the XYZ coordinates of the line vertices."""
        ...
    ...
    @property
    def particle_ids(self) -> Property:
        """The :py:class:`Property` data array storing the particle IDs of the line vertices."""
        ...

    @property
    def particle_ids_(self) -> Property:
        ...

    @property
    def time_stamps(self) -> Property:
        """The :py:class:`Property` data array storing the time stamps of the line vertices."""
        ...
...
However, the computed vector (``delta``) and image offset (``pbc_shift``) will be unique for each visited image of a neighbor particle."""
    ...
    def neighbor_distances(self, index: int) -> numpy.typing.NDArray[numpy.float64]:
        """Returns the list of distances between some central particle and all its neighbors within the cutoff range. 

:param int index: The 0-based index of the central particle whose neighbors should be enumerated.
...
This method is equivalent to the following code, but performance is typically a lot better::
   ...
   def neighbor_distances(index):
    ...
    def neighbor_vectors(self, index: int) -> numpy.typing.NDArray[numpy.float64]:
...
The constructor takes the requested number of nearest neighbors, *N*, and a `DataCollection`
...
system contains too few particles and is non-periodic."""
    ...
    def find_all(self, indices: Optional[Sequence[int]]=None) -> Tuple[numpy.typing.NDArray[numpy.int64], numpy.typing.NDArray[numpy.float64]]:
...
This data object type represents a series of data points and is used for generating histogram plots and other 2d data graphs. 
A data table consists of an array of y-values and, optionally, an array of corresponding x-values, one for each data point. 
Both arrays are standard :py:class:`Property` objects managed by the data table, which is a :py:class:`PropertyContainer` 
subclass.

If the :py:attr:`x` data array is not present, the x-coordinates of the data points are implicitly determined by the table's
:py:attr:`interval`, which specifies a value range along the x-axis over which the data points are evenly distributed. 
This is used, for example, for histograms having equisized bins. 

Data tables generated by modifiers such as :py:class:`CoordinationAnalysisModifier` and :py:class:`HistogramModifier`
are accessible through the `DataCollection.tables` dictionary of the data collection.
Please see the documentation of a modifier to find out what data table(s) it produces.

Examples:

The following code examples demonstrate how to create a new :py:class:`DataTable` and
fill it with data values in Python. You can use these techniques, for instance, to write custom modifier functions 
that output their results as data plots in OVITO.
...
Note how the :py:meth:`create_property` method is used here to create two :py:class:`Property`
objects holding the x- and y-coordinates of the data points. These property objects are then set as :py:attr:`x` and :py:attr:`y` arrays
of the :py:class:`DataTable`.

Generating a graph with several line plots requires creating a *vector* property object for the :py:attr:`y` array of the :py:class:`DataTable`:
...
To generate a bar chart, the :py:attr:`x` property must be filled with numeric IDs 0,1,2,3,... denoting the individual bars. 
Each bar is then assigned a text label by creating a :py:class:`ElementType` instance and adding it to the `Property.types` list:
...
The bin counts must be assigned to the table's :py:attr:`y` property. The number of elements in the :py:attr:`y` property array, together with the
:py:attr:`interval`, determine the number of histogram bins and their width:
...
If you are going to refer to the data table after it has been inserted into the `DataCollection.objects`
list, you should give it a unique :py:attr:`identifier` at construction time, as shown in the following example:
...
Here, the :ref:`user-defined modifier function <writing_custom_modifiers>` outputs a table to the data pipeline having the 
:py:attr:`identifier` ``'trig-func'``. When :py:func:`export_file` is called later with the export 
file format ``'txt/table'``, the table identifier must be specified as extra argument ``key`` to select the right data table to export.
Furthermore, the table's identifier may be used as lookup key to retrieve the table from the :py:attr:`tables` dictionary view
of a :py:class:`DataCollection`."""
...
:Default: ``DataTable.PlotMode.Line``"""
    ...
    def xy(self) -> numpy.typing.NDArray:
...
A two- or three-dimensional structured grid. Each cell (voxel) of the grid is of the same size and shape. 
The geometry of the entire grid, its :py:attr:`domain`, is defined by an attached 
:py:class:`SimulationCell` object, which specific a three-dimensional parallelepiped 
...
For two-dimensional grids, for which the :py:attr:`is2D` property of the :py:attr:`domain` is set to true, the third entry of the :py:attr:`shape` tuple is always equal to 1. 

Assigning new shape dimensions to the grid automatically resizes the one-dimensional data arrays of the :py:class:`PropertyContainer` base class if necessary and updates its :py:attr:`PropertyContainer.count` property to be equal to the product of the three shape dimensions, i.e. the total number of voxel cells."""
        ...

    @property
    def vis(self) -> ovito.vis.VoxelGridVis:
        """The :py:class:`DataVis` element currently associated with this data object, which is responsible for visually rendering the stored data. If set to ``None``, the data object remains non-visual and doesn't appear in rendered images or the viewports. Furthermore, note that the same :py:class:`DataVis` element may be assigned to multiple data objects in order to synchronize their visual appearance."""
        ...