...
def StrToArr(text: list, value_enter: str):
    """
        With this module, you can read only numbers from a text
...
def help():
    text = "-----HELP MENU-----\n" \
            "\n" \
            "If you want to see all modules, CTRL + Left Click on 'ARRMOVS', which is on (import arrmovs)\n"\
...
def StrToArrFromFile(filename: str):
    """
        With this module, you can return only numbers from a file text

        examle:
            import arrmov as arr

            print(arr.StrToArrFromFile("myfile.txt"))
