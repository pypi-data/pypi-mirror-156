...
if __name__ == '__main__':
    ...
    calendar_df = HBDB().read_cal((datetime.strptime(yesterday, '%Y%m%d') - timedelta(15)).strftime('%Y%m%d'), yesterday)
    ...
    run_date_yesterday = trade_df[trade_df['TRADE_DATE'] <= yesterday]['TRADE_DATE'].iloc[-1]
    if yesterday == run_date_yesterday:
        update_all = False
