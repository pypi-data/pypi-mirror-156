...
def get_fund_scale(file_path, fund_code, change_dates):
    """
    生成基金规模数据
    """
    scale = HBDB().read_fund_scale_given_code(fund_code)
    ...
    change_dates = [date for date in change_dates if date >= min(scale_share['报告日期']) and date <= max(scale_share['报告日期'])]
...
def get_fund_holder(file_path, fund_code, change_dates):
    """
    生成基金持有人结构数据
    """
    ...
    change_dates = [date for date in change_dates if date >= min(organ_person['报告日期']) and date <= max(organ_person['报告日期'])]
...
def get_fund_manager_info(fund_code):
    ...
    for manager_code in manager['MANAGER_CODE'].unique().tolist():
        ...
        all_funds = HBDB().read_fund_info()
        all_funds = all_funds.rename(columns={'jjdm': 'FUND_CODE', 'jjmc': 'FUND_FULL_NAME', 'jjjc': 'FUND_SHORT_NAME', 'clrq': 'BEGIN_DATE', 'zzrq': 'END_DATE', 'jjfl': 'FUND_TYPE_1ST', 'ejfl': 'FUND_TYPE_2ND', 'kffb': 'OPEN_CLOSE'})
        all_funds['FUND_TYPE_1ST'] = all_funds['FUND_TYPE_1ST'].replace({'f': 'FOF型', '1': '股票型', '2': '债券型', '3': '混合型', '4': '另类投资型', '7': '货币型', '9': 'QDII型'})
        all_funds['FUND_TYPE_2ND'] = all_funds['FUND_TYPE_2ND'].replace({'f3': '债券型FOF', 'f4': '货币型FOF', 'f1': '股票型FOF', 'f2': '混合型FOF', 'f5': '另类投资FOF',
        ...
        manager_funds = all_funds[all_funds['FUND_CODE'].isin(manager_funds['FUND_CODE'].unique().tolist())]
        funds_scale_list = []
        ...
        fund_manager_info_text += '{0}目前在管产品数量为{1}只，在管基金规模为{2}亿元，其中{3}{4}。管理期内{5}年化收益为{6}%， 最大回撤为{7}%，年化夏普比率为{8}，卡玛比率为{9}。'\
            .format(manager_name, manager_product_num, manager_product_scale, manager_scale_repr, manager_product_repr, fund_name, annual_return, max_drawdown, annual_sharpe_ratio, calmar_ratio)
...
def get_fund_manager_method(file_path, fund_code, change_dates):
    """
    生成基金经理投资理念数据
    """
    fund = HBDB().read_stock_fund_info()
    ...
    hbs_rank_history = FEDB().read_hbs_rank_history_given_code(fund_code)
    ...
    size_style_abs_history = FEDB().read_hbs_size_style_property_history_given_code(fund_code)
    theme_ind_abs_history = FEDB().read_hbs_theme_industry_property_history_given_code(fund_code)
    stock_abs_history = FEDB().read_hbs_holding_property_history_given_code(fund_code)
    ...
    change_dates = [date for date in change_dates if date >= min(shift_ts['报告日期']) and date <= max(shift_ts['报告日期'])]
    ...
    plt.tight_layout()
    plt.savefig('{0}{1}_shift.png'.format(file_path, fund_code))
    ...
    for i in range(2):
        for j in range(2):
            ...
            axij2.set_ylabel(pic_list[i][j][:5] + '（蓝）')
    plt.tight_layout()
    plt.savefig('{0}{1}_cen.png'.format(file_path, fund_code))
    ...
    plt.legend(loc=2)
    plt.tight_layout()
    plt.savefig('{0}{1}_indu_stock_cen.png'.format(file_path, fund_code))

    holding_period = FEDB().read_hbs_stock_trading_property_given_code(fund_code)
    ...
    holding_period['FUND_NAME'] = fund_name
...
def get_fund_position(file_path, fund_code, change_dates):
    """
    生成基金仓位数据
    """
    ...
    change_dates = [date for date in change_dates if date >= min(fund_position.index) and date <= max(fund_position.index)]
...
def get_fund_cr(file_path, fund_code, change_dates):
    """
    生成基金持仓集中度数据
    """
    ...
    change_dates = [date for date in change_dates if date >= min(cr5_mean['报告日期']) and date <= max(cr5_mean['报告日期'])]
...
def get_fund_finance(file_path, fund_code, change_dates):
    """
    生成基金持仓财务数据
    """
    ...
    change_dates = [date for date in change_dates if date >= min(fund_finance['报告日期']) and date <= max(fund_finance['报告日期'])]
    change_dates_loc = get_change_dates_loc(change_dates, sorted(fund_finance['报告日期'].unique().tolist()))
    fig, ax = plt.subplots(2, 2, figsize=(6, 6))
    pic_list = [['PE', 'PB'], ['ROE（%）', '股息率（%）']]
    for i in range(2):
        for j in range(2):
            axij = sns.lineplot(ax=ax[i][j], x='报告日期', y=pic_list[i][j], data=fund_finance, color='#F04950', label=fund_code)
...
def get_fund_stock_change(file_path, fund_code, change_dates):
    """
    获取基金个股变动数据
    """
    ...
    plt.ylabel('概率分布')
    plt.tight_layout()
    plt.savefig('{0}{1}_dist.png'.format(file_path, fund_code))
...
def get_fund_turnover(file_path, fund_code, change_dates):
    """
    生成基金换手数据
    """
    ...
    change_dates = [date for date in change_dates if date >= min(fund_turnover['报告日期']) and date <= max(fund_turnover['报告日期'])]
...
def get_fund_theme(file_path, fund_code, change_dates):
    """
    生成基金主题数据
    """
    fund = HBDB().read_stock_fund_info()
    ...
    fund_theme_shift_cen = FEDB().read_hbs_theme_industry_property_given_code(fund_code)
    fund_theme = FEDB().read_hbs_theme_property_given_code(fund_code)
    ...
    change_dates = [date for date in change_dates if date >= min(fund_theme_ts['报告日期']) and date <= max(fund_theme_ts['报告日期'])]
...
def get_fund_industry(file_path, fund_code, change_dates):
    """
    生成基金行业数据
    """
    fund = HBDB().read_stock_fund_info()
    ...
    fund_industry_shift_cen = FEDB().read_hbs_theme_industry_property_given_code(fund_code)
    fund_industry = FEDB().read_hbs_industry_property_given_code(fund_code)
    ...
    change_dates = [date for date in change_dates if date >= min(fund_industry_ts['报告日期']) and date <= max(fund_industry_ts['报告日期'])]
...
def get_fund_size(file_path, fund_code, change_dates):
    """
    生成基金规模数据
    """
    fund = HBDB().read_stock_fund_info()
    ...
    change_dates = [date for date in change_dates if date >= min(fund_size_ts['报告日期']) and date <= max(fund_size_ts['报告日期'])]
...
def get_fund_style(file_path, fund_code, change_dates):
    """
    生成基金风格数据
    """
    fund = HBDB().read_stock_fund_info()
    ...
    change_dates = [date for date in change_dates if date >= min(fund_style_ts['报告日期']) and date <= max(fund_style_ts['报告日期'])]
...
def get_fund_achievement(fund_code):
    ...
    fund_nav = HBDB().read_fund_cumret_given_code(fund_code, three_years_ago, datetime.today().strftime('%Y%m%d'))
...
def get_fund_performace(file_path, fund_code, change_dates):
    fund = HBDB().read_stock_fund_info()
    fund = fund.rename(columns={'jjdm': 'FUND_CODE', 'jjmc': 'FUND_FULL_NAME', 'jjjc': 'FUND_SHORT_NAME', 'clrq': 'BEGIN_DATE', 'zzrq': 'END_DATE', 'ejfl': 'FUND_TYPE', 'kffb': 'OPEN_CLOSE'})
    fund = fund[fund['FUND_CODE'] == fund_code]
    fund_name = fund['FUND_SHORT_NAME'].values[0]
    ...
    fund_nav = HBDB().read_fund_cumret_given_code(fund_code, establish_date, datetime.today().strftime('%Y%m%d'))
    fund_nav = fund_nav.rename(columns={'JJDM': 'FUND_CODE', 'JZRQ': 'TRADE_DATE', 'HBCL': 'CUM_RET'})
    fund_nav['TRADE_DATE'] = fund_nav['TRADE_DATE'].astype(str)
    fund_nav = fund_nav.sort_values('TRADE_DATE')
    fund_nav['FUND_NAME'] = fund_name
    ...
    change_dates = [date for date in change_dates if date >= min(fund_index_cumret['TRADE_DATE']) and date <= max(fund_index_cumret['TRADE_DATE'])]
    change_dates_loc = get_change_dates_loc(change_dates, sorted(fund_index_cumret['TRADE_DATE'].unique().tolist()))
    fig, ax = plt.subplots(figsize=(6, 3))
    sns.lineplot(x='TRADE_DATE', y=fund_name, data=fund_index_cumret, color='#F04950', label=fund_name)
    sns.lineplot(x='TRADE_DATE', y=index_name, data=fund_index_cumret, color='#6268A2', label=index_name)
    ax.vlines(x=change_dates_loc, ymin=0.0, ymax=max(max(fund_index_cumret[fund_name]), max(fund_index_cumret[index_name])), linestyles='dashed', color='#959595')
    ...
    plt.legend(loc=2)
    plt.tight_layout()
    plt.savefig('{0}{1}_return.png'.format(file_path, fund_code))

    fund_index_d = fund_d.merge(index_d, left_index=True, right_index=True, how='left').reset_index()
    fig, ax = plt.subplots(figsize=(6, 3))
    sns.lineplot(x='TRADE_DATE', y=fund_name, data=fund_index_d, color='#F04950', label=fund_name)
    sns.lineplot(x='TRADE_DATE', y=index_name, data=fund_index_d, color='#6268A2', label=index_name)
    ax.vlines(x=change_dates_loc, ymin=min(min(fund_index_d[fund_name]), min(fund_index_d[index_name])), ymax=0.0, linestyles='dashed', color='#959595')
    ax.xaxis.set_major_locator(ticker.MultipleLocator(90))
    ax.yaxis.set_major_formatter(FuncFormatter(to_100percent))
    plt.xlabel('')
    plt.ylabel('回撤')
    ...
    plt.legend(loc=2)
    plt.tight_layout()
    plt.savefig('{0}{1}_drawdown.png'.format(file_path, fund_code))

    fund_return_period = HBDB().read_fund_return_given_code_and_date(fund_code, (datetime.today() - timedelta(30)).strftime('%Y%m%d'))
    ...
    fund_samekind_return_period = HBDB().read_fund_samekind_return_given_code_and_date(fund_code, (datetime.today() - timedelta(30)).strftime('%Y%m%d'))
    ...
    fund_year_return = HBDB().read_fund_year_return_given_code(fund_code)
    ...
    fund_year_volatility = HBDB().read_fund_year_volatility_given_code(fund_code)
    ...
    fund_year_sharpratio = HBDB().read_fund_year_sharpratio_given_code(fund_code)
    ...
    fund_year_maxdrawdown= HBDB().read_fund_year_maxdrawdown_given_code(fund_code)
...
def get_fund_brison_attribution(file_path, fund_code, change_dates):
    fund_brison_attribution = HBDB().read_fund_brinson_attribution_given_code(fund_code)
    ...
    change_dates = [date for date in change_dates if date >= min(fund_brison_attribution_disp['报告日期']) and date <= max(fund_brison_attribution_disp['报告日期'])]
    ...
    plt.tight_layout()
    plt.savefig('{0}{1}_brison_attribution.png'.format(file_path, fund_code))
...
def get_fund_barra_attribution(file_path, fund_code):
    fund_barra_attribution = HBDB().read_fund_barra_attribution_given_code(fund_code)
    ...
    ax1 = sns.barplot(ax=ax[0], x='style_factor', y='data_value', data=fund_barra_exposure, color='#C94649', label='风格暴露')
    ax2 = sns.barplot(ax=ax[1], x='style_factor', y='data_value', data=fund_barra_return, color='#C94649', label='风格收益')
    ax1.legend(loc=2)
    ax2.legend(loc=2)
    ax1.set_xlabel('')
    ax2.set_xlabel('')
    ...
    plt.tight_layout()
    plt.savefig('{0}{1}_barra.png'.format(file_path, fund_code))
...
def get_quantitative_report(fund_code, compare_fund_code_list=[]):
    """
    生成公募基金量化分析报告
    """
    fund = HBDB().read_stock_fund_info()
    ...
    fund_name = fund['FUND_SHORT_NAME'].values[0]
    file_path = 'D:/Git/hbshare/hbshare/fe/xwq/data/quantitative_report/{}/'.format(fund_code)
    ...
    risk, statement1, statement2, statement3 = report_info()
    ...
    fund_manager_info_text, manager_all_ori, manager_all = get_fund_manager_info(fund_code)
    change_dates = [date for date in manager_all_ori['任职日期'].unique().tolist() + manager_all_ori['离任日期'].unique().tolist() if date != '-']
    change_dates = sorted(list(set(change_dates)))
    ...
    fund_manager_method_new_text, new_disp = get_fund_manager_method(file_path, fund_code, change_dates)
    ...
    document.add_paragraph(text=' ')
    document.add_picture('{0}{1}_shift.png'.format(file_path, fund_code))
    ...
    document.add_paragraph(text=' ')
    document.add_picture('{0}{1}_cen.png'.format(file_path, fund_code))
    ...
    document.add_paragraph(text=' ')
    document.add_picture('{0}{1}_indu_stock_cen.png'.format(file_path, fund_code))
    ...
    fund_position_text = get_fund_position(file_path, fund_code, change_dates)
    document.add_paragraph(text=fund_position_text)
    document.add_picture('{0}{1}_position.png'.format(file_path, fund_code))

    document.add_heading(text='2.2 规模分析', level=3)
    fund_size_disp, fund_size_text = get_fund_size(file_path, fund_code, change_dates)
    ...
    fund_size_appendix = '注：本报告大中小盘根据持仓情况进行划分，先根据巨潮大中小盘指数中成分股的划分确定大中小盘划分阈值，再根据阈值统计基金持仓中的大中小盘暴露。'
    document.add_paragraph(text=fund_size_appendix)

    document.add_heading(text='2.3 风格分析', level=3)
    fund_style_disp, fund_style_text = get_fund_style(file_path, fund_code, change_dates)
    ...
    fund_style_appendix = '注：本报告成长价值风格根据持仓情况进行划分，先根据巨潮成长价值指数中成分股的划分确定成长价值风格划分阈值，再根据阈值统计基金持仓中的成长价值风格暴露。'
    document.add_paragraph(text=fund_style_appendix)

    document.add_heading(text='2.4 主题分析', level=3)
    fund_theme_disp, fund_theme_text = get_fund_theme(file_path, fund_code, change_dates)
    ...
    fund_industry_disp, fund_industry_text = get_fund_industry(file_path, fund_code, change_dates)
    ...
    document.add_heading(text='2.6 持股分析', level=3)
    fund_cr_text = get_fund_cr(file_path, fund_code, change_dates)
    ...
    fund_finance_text = get_fund_finance(file_path, fund_code, change_dates)
    document.add_paragraph(text=fund_finance_text)
    document.add_picture('{0}{1}_finance.png'.format(file_path, fund_code))
    fund_holding_latest, fund_holding_latest_text, fund_holding_disp, fund_holding_disp_text = get_fund_stock_change(file_path, fund_code, change_dates)
    ...
    document.add_picture('{0}{1}_dist.png'.format(file_path, fund_code))
    ...
    document.add_paragraph(text=' ')
    HoldingCompare(fund_code, compare_fund_code_list).get_holding_compare()
    p = document.add_paragraph(text='该产品持仓情况以及与其他同类型产品持仓比较情况（若有）见此excel：', style=None)
    add_hyperlink(p, 'D:/Git/hbshare/hbshare/fe/xwq/data/quantitative_report/{0}/持仓情况.xlsx'.format(fund_code), '持仓情况。', '#3FAEEE', True)

    document.add_heading(text='2.7 换手率分析', level=3)
    fund_turnover_text = get_fund_turnover(file_path, fund_code, change_dates)
    ...
    fund_return_period, fund_year_performance = get_fund_performace(file_path, fund_code, change_dates)
    document.add_paragraph(text='该产品业绩收益和回撤情况统计如下：', style=None)
    document.add_picture('{0}{1}_return.png'.format(file_path, fund_code))
    document.add_picture('{0}{1}_drawdown.png'.format(file_path, fund_code))
    ...
    fund_brison_attribution_text = get_fund_brison_attribution(file_path, fund_code, change_dates)
    document.add_paragraph(text=fund_brison_attribution_text, style=None)
    document.add_picture('{0}{1}_brison_attribution.png'.format(file_path, fund_code), width=Cm(18), height=Cm(10))
    fund_barra_text = get_fund_barra_attribution(file_path, fund_code)
    document.add_paragraph(text=fund_barra_text, style=None)
    document.add_picture('{0}{1}_barra.png'.format(file_path, fund_code))

    document.add_heading(text='五. 总结', level=1)
    fund_summary = '※规模：该产品最新规模为{0}'.format(fund_scale_text.split('最新规模为')[1])
    document.add_paragraph(text=fund_summary)
    fund_summary = '※持有人结构：该产品目前{0}。'.format(fund_holder_text.split('从最新持有人结构看，')[1].split('，占比分别为')[0])
    document.add_paragraph(text=fund_summary)
    ...
    document.add_paragraph(text=fund_summary)
    fund_summary = '※持股：该产品{0}，{1}。'.format(fund_cr_text.split('该产品持股集中度情况如下，')[1].split('。')[0],
    ...
    document.add_paragraph(text=fund_summary)
    fund_summary = '※绩效归因：从Brison绩效归因看，该产品最新一期{0}；从Barra风格归因看，该产品最新一期{1}'.format(fund_brison_attribution_text.split('从最新一期看，')[1].split('。')[0], fund_barra_text.split('如下，')[1])
    document.add_paragraph(text=fund_summary)
...
if __name__ == "__main__":
    ...
    get_quantitative_report(fund_code, compare_fund_code_list)

    fund_code = '450004'
    get_quantitative_report(fund_code)

    # file_path = 'D:/Git/hbshare/hbshare/fe/xwq/data/quantitative_report/{}/'.format(fund_code)
    # get_fund_performace(file_path, fund_code, change_dates)
