...
__hdf5Available = False
try:
	import nabCompression.h5
	__hdf5Available = True
except:
	try:
		import deltaRice.h5
		__hdf5Available = True
...
class DataRun:
	...
	def __openWaveformFiles(self):
		self.__coincFiles = wf.waveformFile(self.__coincFilenames, pixelMapping = self.parameterFile().BoardChannelPixelMap['pixel'], bulkRead = self.bulkRead)
		self.__noiseFiles = wf.waveformFile(self.__noiseFilenames, pixelMapping = self.parameterFile().BoardChannelPixelMap['pixel'], bulkRead = self.bulkRead)
		self.__pulsrFiles = wf.waveformFile(self.__pulsrFilenames, pixelMapping = self.parameterFile().BoardChannelPixelMap['pixel'], bulkRead = self.bulkRead)
		self.__singlFiles = wf.waveformFile(self.__singlFilenames, pixelMapping = self.parameterFile().BoardChannelPixelMap['pixel'], bulkRead = self.bulkRead)
	...
	def runStartTime(self):
		...
		if self.singleWaves() is not None:
			return self.singleWaves().getFileStartTime()[0]
		elif self.coincWaves() is not None:
			return self.coincWaves().getFileStartTime()[0]
