    ...
    def __enter__(self):
        ...
        for key, value in self._config.items():
            ...
            _log_and_print("Path on compute is specified: '{}'.".format(
                "AdditionalOptions" in value and "PathOnCompute" in value["AdditionalOptions"]
                and value["AdditionalOptions"]["PathOnCompute"]))
    ...
    def __exit__(self, *exc_details):
        ...
        if len(exc_details) == 3 and \
            ...
            for key, value in self._config.items():
                ...
                output_location = value.get("OutputLocation", {})
                asset_type = output_location.get("Type", None)
                if asset_type is not None:
                    self._register_asset(key,
                                         value.get("OutputLocation", {}).get("Uri", {}).get("Path", None),
                                         asset_type)
        ...
        try:
            ...
            if output_type.lower() in ['urifile', 'urifolder', 'mltable']:
                ...
                if len(data_asset_response.data_versions) != 1 or name not in data_asset_response.data_versions.keys():
                    raise RuntimeError(
                        f'Data asset batch_get_or_create_unregistered_output_data api returned invalid response')
                ...
                if model_asset_response.models is None or len(
                    ...
                    raise RuntimeError(
                        f'Model asset batch_get_or_create_unregistered_output_model api returned invalid response')
