...
class FileDataset(AbstractDataset):
    ...
    def mount(self, mount_point=None, **kwargs):
        ...
        rslex_direct_volume_mount = None
        try:
            rslex_direct_volume_mount = dataprep_fuse().rslex_direct_volume_mount
            # if we are using newer version of dataprep override mount to point to clex-only implementation.
            mount = dataprep_fuse().clex_mount
        except:
            _get_logger().warning('Attempting `rslex_direct_volume_mount` with an older dataprep version. '
                                  'Falling back to calling dprepfuse.mount()')
            pass

        if rslex_direct_volume_mount:
            ...
            try:
                ...
                message = str(e)
                _get_logger().info('Failed to run rslex based mount due to exception of type {} with message {}.'
                                   'Falling back to dataflow mount.'.format(type(e).__name__, message))
                print('Not mounting as a volume: {}. \nFalling back to dataflow mount.'.format(message))
                pass
            except dprep_fuse.VolumeMountNotEnabled:
                _get_logger().info("RslexDirectVolumeMount is not enabled")
                print('Volume mount is not enabled. \nFalling back to dataflow mount.')
                pass
            except dprep_fuse.VolumeMountFailed as e:
                _get_logger().warning('Error during volume mount: {}'.format(e))
                ...
                message = str(e)
                _get_logger().warning('Unexpected error during volume mount: {}'.format(message))

        (base_path, was_data_pulled) = _find_path_prefix(dataflow, skip_validate)
        if not skip_validate and not was_data_pulled:
            ...
            _get_logger().info('skip_validate is set, will mount with CLex.')
    ...
    def as_cache(self, datacache_store):
        ...
        dc = _Datacache.create(datacache_store.workspace, datacache_store, self)
...
def _get_prefix(path, dataflow, avoid_data_pull=False):
    ...
    if isinstance(paths[0], DataPrepError):
        raise UserErrorException('DataPrepError: {0}, '
                                 'please make sure the URI/Path is accessible and in correct format'
    ...
    if 'Portable Path' in download_records[0]:
        ...
        for record in download_records:
            ...
            if isinstance(value, StreamInfo):
                ...
                raise AzureMLException('Unexpected error during file download.')
