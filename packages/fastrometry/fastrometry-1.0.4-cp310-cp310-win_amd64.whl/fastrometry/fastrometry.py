...
def getCMDargs():
    ...
    parser.add_argument('-wcsdiagnostics', action='store_true', help=formatting('If this option is provided, fastrometry will write an additional csv file beside the FITS image file which provides diagnostic information about the WCS solution. The PSE centroids, their sky coordinate values via the WCS solution, the corresponding sky coordinates from the catalogue, and then the differentials, are written to the csv diagnostic file.'))
    parser.add_argument('--save', metavar='ID', help=formatting('Save the options settings as a file to disk to recall later. User supplies an ID, for example: "--save UVIT". Useful if frequently processing different telescopic fields, allowing quick input of WCS solver settings. Saves all settings EXCEPT for filename, ra, dec, pmepoch, so that different images but taken from the same telescope can be processed at their unique individual characteristics.'))
...
def findWCS(filename=None, ra=None, dec=None, scale=None, scalebnds=None, rotation=None, rotationbnds=None, buffer=None, shape=None, catalogue=None, pmepoch=None, filter=None, npts=None, nrefinepts=None, pixsat=None, kernelrad=None, sourcesep=None, vertextol=None, nmatchpoints=None, nmatchpercent=None, wcsdiagnostics=None, save=None, load=None, verbosity=None, debug=None):
    ...
    if load is None:
        ...
        try:
            ...
            with open('{}\\cmd_args\\saveoptions_{}.json'.format(user_dir,id),'r') as f:
                ...
                wcsdiagnostics = jobject['wcsdiagnostics']
    ...
    catalog_points = np.zeros((nrefinepts,2))
    mean_catcoords = np.zeros(2)

    if verbosity >= 1:
        print("\nGetting sources from catalogue...")
    num_catsources = getIntermediateCoords(ra, dec, scale, img_xmax, img_ymax, shape, filter, catalogue, pmepoch, nrefinepts, allintrmpoints, catalog_points, mean_catcoords, user_dir, debug_report, verbosity, debug)
    if verbosity >= 1:
        print("done")
    ...
    if verbosity >= 1:
        print("\nPerforming WCS...")
    WCS(scale, scalebnds, rotation, rotationbnds, npts, nrefinepts, vertextol, allintrmpoints, catalog_points, mean_catcoords, pse_metadata, pse_metadata_inv, srcindexmap_initial, srcindexmap_refine, img_xmax, img_ymax, minmatches, kerneldiam, num_psesources, num_catsources, headervals, wcsdiagnostics, user_dir, debug_report, filename, verbosity, debug)
    if verbosity >= 1:
        print("done")
