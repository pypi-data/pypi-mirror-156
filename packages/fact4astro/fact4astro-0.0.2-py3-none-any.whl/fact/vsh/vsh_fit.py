...
def vsh_fit(dra, ddc, dra_err, ddc_err, ra, dc, ra_dc_cor=None,
    ...
    if "std_clean" in kwargs.keys() and kwargs["std_clean"]:
        ...
        if isinstance(clip_limit, (int, float)):
            print("The fit will be iterated until it converges, that is,\n"
                  "no more outliers, based on a clean sample of "
    ...
    if "bs_err" in kwargs.keys() and kwargs["bs_err"]:
        ...
        output = bootstrap_resample_4_err(mask, dra, ddc, dra_err, ddc_err,
                                          ...
                                          l_max, fit_type, num_iter, output, N_resample)
...
def vsh_fit_4_table(data_tab, l_max=1, fit_type="full", pos_in_rad=False,
    ...
    # For RA/Dec
    if "ra" in data_tab.colnames:
        print("[msg] use equatorial coordinate system (ra, dec)")
        ...
        elif "dpmra" in data_tab.colnames:
            dra = np.array(data_tab["dpmra"])
            ddec = np.array(data_tab["dpmdec"])
        else:
            print("[err] one of 'dra', 'pmra' or 'dpmra' should be specificed.")
            sys.exit(1)
        ...
        elif "dpmra_err" in data_tab.colnames:
            dra_err = np.array(data_tab["dpmra_err"])
            ddec_err = np.array(data_tab["dpmdec_err"])
        elif "dpmra_error" in data_tab.colnames:
            dra_err = np.array(data_tab["dpmra_error"])
            ddec_err = np.array(data_tab["dpmdec_error"])
        else:
            print("[msg] one of 'dra_err', 'dra_error', 'pmra_err', 'pmra_error' "
                  "'dpmra_err' or 'dpmra_error' should be specificed.")
            print("[msg] if not, use an equal weights.")
        ...
        elif "dpmra_dpmdec_corr" in data_tab.colnames:
            dra_ddc_cor = np.array(data_tab["dpmra_dpmdec_corr"])
        elif "dpmra_dpmdec_cor" in data_tab.colnames:
            dra_ddc_cor = np.array(data_tab["dpmra_dpmdec_cor"])
        elif "dpmra_dpmdec_cov" in data_tab.colnames:
            dra_ddc_cov = np.array(data_tab["dpmra_dpmdec_cov"])
            dra_ddc_cor = dra_ddc_cov / dra_err / ddec_err
        else:
            print("[msg] one of 'dra_ddec_cov', 'dra_ddec_cor', 'pmra_pmdec_corr' or "
                  "'pmra_pmdec_cor' should be specificed.")
            print("[msg] if not, consider covariance matrix as a diagonal matrix.")
            dra_ddc_cor = None

    elif "lon" in data_tab.colnames:
        print("[msg] use arbitrary coordinate system (lon, lat)")
        ra = np.array(data_tab["lon"])
        dec = np.array(data_tab["lat"])

        if "dlon" in data_tab.colnames:
            dra = np.array(data_tab["dlon"])
            ddec = np.array(data_tab["dlat"])
        elif "pmlon" in data_tab.colnames:
            dra = np.array(data_tab["pmlon"])
            ddec = np.array(data_tab["pmlat"])
        elif "dpmlon" in data_tab.colnames:
            dra = np.array(data_tab["dpmlon"])
            ddec = np.array(data_tab["dpmlat"])
        else:
            print("[err] one of 'dlon', 'pmlon' or 'dpmlon' should be specificed.")
            sys.exit(1)

        if "dlon_err" in data_tab.colnames:
            dra_err = np.array(data_tab["dlon_err"])
            ddec_err = np.array(data_tab["dlat_err"])
        elif "dlon_error" in data_tab.colnames:
            dra_err = np.array(data_tab["dlon_error"])
            ddec_err = np.array(data_tab["dlat_error"])
        elif "pmlon_err" in data_tab.colnames:
            dra_err = np.array(data_tab["pmlon_err"])
            ddec_err = np.array(data_tab["pmlat_err"])
        elif "pmlon_error" in data_tab.colnames:
            dra_err = np.array(data_tab["pmlon_error"])
            ddec_err = np.array(data_tab["pmlat_error"])
        elif "dpmlon_err" in data_tab.colnames:
            dra_err = np.array(data_tab["dpmlon_err"])
            ddec_err = np.array(data_tab["dpmlat_err"])
        elif "dpmlon_error" in data_tab.colnames:
            dra_err = np.array(data_tab["dpmlon_error"])
            ddec_err = np.array(data_tab["dpmlat_error"])
        else:
            print("[msg] one of 'dlon_err', 'dlon_error', 'pmlon_err', 'pmlon_error' "
                  "'dpmlon_err' or 'dpmlon_error' should be specificed.")
            print("[msg] if not, use an equal weights.")
            dra_err = np.ones(len(data_tab))
            ddec_err = np.ones(len(data_tab))

        if "dlon_dlat_cov" in data_tab.colnames:
            dra_ddc_cov = np.array(data_tab["dlon_dlat_cov"])
            dra_ddc_cor = dra_ddc_cov / dra_err / ddec_err
        elif "dlon_dlat_cor" in data_tab.colnames:
            dra_ddc_cor = np.array(data_tab["dlon_dlat_cor"])
        elif "pmlon_pmlat_corr" in data_tab.colnames:
            dra_ddc_cor = np.array(data_tab["pmlon_pmlat_corr"])
        elif "pmlon_pmlat_cor" in data_tab.colnames:
            dra_ddc_cor = np.array(data_tab["pmlon_pmlat_cor"])
        elif "dpmlon_dpmlat_corr" in data_tab.colnames:
            dra_ddc_cor = np.array(data_tab["dpmlon_dpmlat_corr"])
        elif "dpmlon_dpmlat_cor" in data_tab.colnames:
            dra_ddc_cor = np.array(data_tab["dpmlon_dpmlat_cor"])
        elif "dpmlon_dpmlat_cov" in data_tab.colnames:
            dra_ddc_cov = np.array(data_tab["dpmlon_dpmlat_cov"])
            dra_ddc_cor = dra_ddc_cov / dra_err / ddec_err
        else:
            print("[msg] one of 'dra_ddec_cov', 'dra_ddec_cor', 'pmlon_pmlat_corr' or "
                  "'pmlon_pmlat_cor' should be specificed.")
            print("[msg] if not, consider covariance matrix as a diagonal matrix.")
            dra_ddc_cor = None
