...
@click.option(
    ...
    default=False,
    help=(
        "(beta) Use this flag to provision a high-performance datastore backed by AWS FSx for Lustre. This "
...
@click.option(
    ...
    default="low",
    help=(
        "(beta) Throughput setting for high-perfomance datastores. Select one of [low, medium, high]. \n\n"
        ...
        "Because FSx Datastores offer elevated performance, please note that they can incur higher monthly "
...
@click.option(
    ...
    default=1200,
    help=(
        "(beta) Capacity setting for high-performance datastores in GiB. Must be 1200, 2400 or "
...
def create(
    ...
    try:
        ...
        if incomplete_id is not None:
            ...
            if should_resume is True:
                ...
                click.echo(
                    f"\n{info.datastore_currently_creating_datastore(name=dstore.name, version=str(dstore.version), cluster=cluster)}"
                )
                dstore.upload()
        else:
    ...
    click.echo(info.datastore_uploaded_successfully(name=name, version=str(dstore.version), cluster=cluster))

    if _fsx_enabled:
        click.echo(info.datastore_fsx_total_throughput(_fsx_throughput_alias, _fsx_capacity_gib))
