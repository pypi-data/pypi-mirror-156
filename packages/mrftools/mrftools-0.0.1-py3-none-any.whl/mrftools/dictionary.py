...
class Dictionary:
    ...
    def Export(self, filename, force=False):
        if ".mrf" in filename:
            ...
            try:
                outfile.create_group("dictionaries")
            except:
                pass
            if (self.name in list(outfile["dictionaries"].keys())) and not force:
                print("Dictionary '" + self.name + "' already exists in .mrf file. Specify 'force' to overwrite")
            else:
                dictionary = outfile["dictionaries"].create_group(self.name)
                dictionary.attrs.create("name", self.name)
