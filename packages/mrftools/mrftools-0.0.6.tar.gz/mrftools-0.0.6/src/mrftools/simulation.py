...
class Simulation: 
    def __init__(self,sequenceParameters, dictionaryParameters, name="", numTimepoints = -1, phaseRange=(-2*np.pi, 2*np.pi), numSpins=1, fullResults=[], truncationMatrix=[], truncatedResults=[]):
    ...
    def Execute(self, numBatches = 1):
        ...
        self.fullResults = Mxy

    def CalculateSVD(self, truncationNumber):
        dictionary = np.transpose(self.fullResults)
        ...
        (u,s,v) = np.linalg.svd(normalizedDictionary)
        vt = np.transpose(v)
        self.truncationMatrix = vt[:,0:truncationNumber]
        print(np.shape(normalizedDictionary))
        print(np.shape(self.truncationMatrix))
        ...
        if ".mrf" in filename:
            ...
            if (self.name in list(outfile["simulations"].keys())) and not force:
                ...
                if includeFullResults:
                    simulation["fullResults"] = self.fullResults
                else:
                    simulation["fullResults"] = []
                if includeSVDResults:
    ...
    def GetAverageTimecourse(self, dictionaryEntryNumbers:list):
        np.shape(np.average(self.fullResults[:,dictionaryEntryNumbers], 1))
    ...
    def Plot(self, numTimepoints=-1, dictionaryEntryNumbers=[], plotTruncated=False):
        ...
        for entry in dictionaryEntryNumbers:
            if(plotTruncated):
                ...
                plt.plot(abs(self.fullResults[:,entry]), label=str(self.dictionaryParameters.entries[entry]))
        ax.legend()
    ...
    def Import(filename, simulationName):
        if ".mrf" in filename:
            ...
            simulationFullResults = simulationGroup["fullResults"][:]
            ...
            new_simulation = Simulation(importedSequenceParameters, importedDictionaryParameters, simulationName, simulationNumTimepoints, simulationPhaseRange, simulationNumSpins, simulationFullResults, simulationTruncationMatrix, simulationTruncatedResults)
            infile.close()
