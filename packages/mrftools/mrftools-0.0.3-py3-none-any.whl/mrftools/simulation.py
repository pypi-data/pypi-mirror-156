...
class Simulation: 
    ...
    def ExecuteNumbaSimulation(numTimepoints,T1s,T2s,TRs,TEs,FAs,phaseValues):
        ...
        for dictionaryEntryNumber in numba.prange(0,numberOfDictionaryEntries):
            ...
            for iTimepoint in range(numTimepoints):
                ...
                crf = np.cos(FAs[iTimepoint])
                srf = np.sin(FAs[iTimepoint])
    ...
    def Execute(self):
        ...
        for dictionaryEntry in self.dictionaryParameters.entries:
        ...
        Mx0,My0,Mz0 = self.ExecuteNumbaSimulation(self.numTimepoints, np.asarray(T1s),np.asarray(T2s),np.asarray(TRs),np.asarray(TEs),np.asarray(FAs),phaseValues)
    ...
    def CalculateSVD(self, truncationNumber):
        ...
        (u,s,v) = np.linalg.svd(normalizedDictionary, full_matrices=False, compute_uv=True, hermitian=False)
        truncationMatrix = v[:,0:truncationNumber]
        truncatedResults = np.matmul(normalizedDictionary,truncationMatrix).transpose()
        return(truncationMatrix, truncatedResults)
    ...
    def Plot(self, numTimepoints=-1, dictionaryEntryNumbers=[]):
        if numTimepoints == -1:
        ...
        for entry in dictionaryEntryNumbers:
            plt.plot(range(numTimepoints), abs(self.results[:,entry]), label=str(self.dictionaryParameters.entries[entry]))
        ax.legend()
