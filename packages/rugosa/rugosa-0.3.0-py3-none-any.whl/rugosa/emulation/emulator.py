...
e.g.
    with dragodis.open_program(r"C:\input.exe") as dis:
        ...
        ctx = emulator.context_at(0x123456)
"""
...
from .flowchart import iter_paths
...
class Emulator:
    ...
    This is used for:
        - Initial interfacing with emulation.
        - Holding the Dragodis disassembler.
        - Managing the hooks used for function calls.
    ...
    def __init__(self, disassembler: dragodis.Disassembler, max_instructions=10000, branch_tracking=True, teleport=True):
        ...
        :param disassembler: Backend dragodis disassembler used for obtaining instructions.
        ...
        self.arch = disassembler.processor_name
        if self.arch == "x86":
    ...
    def get_opcode_hook(self, opcode) -> Optional[callable]:
        """
        Gets the opcode implementation for the given opcode mnemonic.
        ...
        def init_contexts():
            ...
            for call_ea in func.calls_to:
                if call_ea in func:
                    ...
                    continue
                try:
                    self.disassembler.get_function(call_ea)
    ...
    def create_emulated(self, func_ea, return_type=None, return_size=None, enforce_args=False) -> Callable:
        ...
        """
        func_obj = self.disassembler.get_function(func_ea)
        ...
        def emulated_function(*args, context: ProcessorContext = None):
            ...
            func_sig = context.get_function_signature(func_obj.start)
            if enforce_args and len(func_sig.arguments) != len(args):
                raise TypeError(
                    f"Function takes {len(func_sig.arguments)} positional arguments, but {len(args)} were given.")

            logger.debug(f'Emulating {func_sig.name}')
            for arg, func_arg in zip(args, func_sig.arguments):
