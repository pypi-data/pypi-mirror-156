...
def get_desertores_por_anio(anio,modalidad=None):
    ...
    if modalidad is None:
        ...
        url_ebr = path_file+"\\DESERCION\\DESERCION_{}"
        ds_ebr = pd.read_csv(url_ebr.format("{}.csv".format(anios_str)))        
        if anio >=2018:
            list_df = []
            list_df.append(ds_ebr)
            url_ebe = path_file+"\\DESERCION\\DESERCION_EBE_{}" 
            ds_ebe = pd.read_csv(url_ebe.format("{}.csv".format(anios_str)))
...
def get_shock_economico(anio,cache=False):
    ...
    if anio ==2020:
        ...
        df_merge_schok.drop(columns=['ID_NIVEL','COD_MOD','ANEXO','INGRESO_SIN_LOG', 'INGRESO_CON_LOG'],inplace=True)
...
def get_siagie_por_anio(anio,id_grado=None,id_grado_list=None,modalidad=None,modalidad_list=None,dtypes_columns=None,
                        ...
                        id_persona_df=None,id_nivel=None,id_nivel_list=None,macro_region=None,desercion=False, reduce_mem_usage=False):  
    ...
    if macro_region   is not None:        
        df_serv = get_df_servicios(anio=anio,macro_region=macro_region)
