...
class RegressionDocumenting(BinaryDocumenting):
    ...
    def analyse(self):
        ...
        # Print & Finish plot
        logger.info("Mean Absolute Error:            {:.2f} \u00B1 {:.2f}".format(np.mean(mae), np.std(mae)))
        logger.info("Mean Squared Error:             {:.2f} \u00B1 {:.2f}".format(np.mean(mse), np.std(mse)))
        logger.info("R2 Score:                       {:.2f} \u00B1 {:.2f}".format(np.mean(r2), np.std(r2)))
        logger.info("Max Error:                      {:.2f} \u00B1 {:.2f}".format(
            np.mean(max_error), np.std(max_error)))
        logger.info("Mean Absolute Percentage Error: {:.2f} \u00B1 {:.2f}".format(
    ...
    def model_performance(self):
        ...
        for k, v in self.metrics.items():
            ...
            self.cell(w=50, h=self.lh, txt=f"{v[0]:.2f} \u00B1 {v[1]:.2f}", border="L", align="C")
        self.ln(self.lh * 3)
