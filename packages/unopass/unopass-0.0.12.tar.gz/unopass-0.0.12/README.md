...
```python
from unopass import unopass, signout


"""
unopass({VAULT}, {ITEM}, {FIELD})

OPTIONAL: signout of the 1Password CLI session:
    call signout() with deauthorize=True to end the 1Password session
"""

username = unopass("Personal", "server", "username")
password = unopass("Personal", "server", "password")

signout(deauthorize=True)
