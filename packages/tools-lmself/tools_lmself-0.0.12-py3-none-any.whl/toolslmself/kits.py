...
class requests_plus():
    ...
    @staticmethod
    def get_picture(url, reqNum=1, savePath=r'.', saveName=r'', isDownload=True, **kwargs):
        ...
        for _ in range(reqNum):
            try:
                ...
                if req.status_code == 200:
                    if isDownload:
                        open(fr'{savePath}/{saveName or time.strftime("%Y%m%d-%H%M%S.png")}', 'wb').write(req.content)
                    return req
            except Exception as e:
                pass
        return False
