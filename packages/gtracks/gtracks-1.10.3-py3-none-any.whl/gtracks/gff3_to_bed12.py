...
def main():
    ...
    genes = BedTool(tuple(generate_bed(args.gff, canon=args.canon))).sort()
    exons = BedTool(tuple(generate_bed(args.gff, type='exon', canon=args.canon))).sort()
    cds = BedTool(tuple(generate_bed(args.gff, type='CDS', canon=args.canon))).sort()
    for gene in genes:
