...
class RIMDB():
    ...
    @trying
    def __init__(self, usr_db: str = None, pass_db: str = None, host_db: str= None, database: str = None, driver: str = 'mysql', base: str = 'DB') -> None:
        load_dotenv()
    ...
    def connect_db(self) -> Engine:
        self.engine = create_engine(f'{self.driver}://{self.usr_db}:{self.pass_db}@{self.host_db}/{self.database}')
        return self.engine
