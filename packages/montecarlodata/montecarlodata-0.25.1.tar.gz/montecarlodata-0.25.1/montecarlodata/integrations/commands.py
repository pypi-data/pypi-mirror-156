...
def add_databricks_metastore(ctx, **kwargs):
    ...
    ).onboard_databricks_metastore(connection_type=DATABRICKS_METASTORE_CONNECTION_TYPE, **kwargs)


@integrations.command(help=f'Setup a Databricks Delta integration. For metadata queries on delta tables when using an external metastore in databricks.')
@click.pass_obj
@add_common_options(DATABRICKS_OPTIONS)
@add_common_options(DATABRICKS_DATA_COLLECTOR_OPTIONS)
@add_common_options(ONBOARDING_CONFIGURATION_OPTIONS)
@click_config_file.configuration_option(settings.OPTION_FILE_FLAG)
def add_databricks_delta(ctx, **kwargs):
    """
    Onboard a databricks delta service
    """
    DatabricksOnboardingService(
        config=ctx['config'], mc_client=create_mc_client(ctx)
    ).onboard_databricks_metastore(connection_type=DATABRICKS_DELTA_CONNECTION_TYPE, **kwargs)
