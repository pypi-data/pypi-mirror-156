    ...
    def _data_source_node_to_value(
        ...
        if self._mock_sql_inputs is not None and data_source_node.input_name in self._mock_sql_inputs:
            ...
            time_window = get_time_window_from_data_source_node(
                raw_data_limits=pendulum.period(
            ...
            if self._has_snowpark:
                ...
                # Need to wrap the sql_str in a bracket to make sure we don't get a syntax error
                return f"({ds})"

    def _get_ds_sql_str(self, ds: VirtualDataSource, time_window: Optional[pendulum.Period]) -> str:
            ...
            if batch_data_source.HasField("snowflake"):
                ...
                if snowflake_args.HasField("table"):
                    ...
                    if time_window is not None:
                        if snowflake_args.HasField("common_args"):
                            # V5 uses snowflake_args.common_args.timestamp_field
                            common_args = snowflake_args.common_args
                            assert common_args.HasField("timestamp_field")
                            timestamp_key = common_args.timestamp_field
                        else:
                            # V3 uses snowflake_args.timestamp_key
                            assert snowflake_args.HasField("timestamp_key")
        ...
        if use_pandas:
            ...
            def udf(params: List) -> Dict:
                ...
                try:
                    ...
                    raise ValueError(
                        f"Invalid SQL: please review your SQL for this feature declaration. Received error: {type(e).__name__}: {str(e)}"
                    )
