...
class NNClassificationErrorV2:
    ...
    def generate_top1_error_data(hdfstore, objnames, ax, key_head='/original', within_category_error=False, distance: str = "correlation"):
        ...
        for obj in objnames:
            ...
            top1_excdist = g['top1_cvals'][:] #1st dim = list of imgs in series, 2nd dim = exclusion dists, vals = top1 cvals with exc dist
            top1_other = g['top1_cvals_otherobj'][:] #1st dim = list of imgs in series, vals = top1 cvals excluding the same obj
            ...
            if within_category_error:
                ...
                in_same_objcat = np.array([obj_cat == other_obj.split('_')[0] for other_obj in objnames])

                same_objcat_cvals = gather_info_same_obj_cat(g, obj, objnames[in_same_objcat]) #1st dim = different objs in same obj cat, 2nd dim = imgs, 3rd dim = exclusion dist in ax
    ...
    @staticmethod
    def plot_top1_err_per_axis(top1_error_per_obj, top1_error_mean, legend_list, title, num_obj=200):
        mStyles = [".",",","v","^","<",
                ">","1","2","3","4",
                "8","s","p","P","*",
                "h","H","+","x","X"]
        ...
        axes.errorbar(np.linspace(0,10,11), top1_error_mean.T, 
                     ...
                     linestyle='-', alpha=0.8, c='red', linewidth=2.5, marker='o', label='average', zorder=21, capsize=2)
        ...
        for idx, obj_cat in enumerate(legend_reduced):
            offset = offsets[idx]
            data = np.array(top1_error_per_obj[int(num_obj/20)*idx:int(num_obj/20)*(idx+1)])
            x = np.linspace(-1+offset,9+offset,11)
            tile_dim = data.shape[0]
            axes.scatter(np.tile(x, (tile_dim, 1)), data, marker=mStyles[idx], color=colors(idx), s=2, alpha=1)
            axes.errorbar(x, data.mean(axis=0), 
                        yerr=[data.mean(axis=0)-np.quantile(data, 0.25,axis=0), np.quantile(data, 0.75,axis=0)-data.mean(axis=0)],
                        linestyle='--',alpha=0.9, linewidth=0.8, marker=mStyles[idx], color=colors(idx), label=obj_cat, capsize=2)
        ...
        for i in range(int(len(minor_ticks)/2)):
            axes.axvspan(minor_ticks[2*i+1], minor_ticks[2*(i+1)], facecolor='b', alpha=0.05)
    ...
    def plot_top1_err_avgd(num_correct_ax, total_count_ax, legend):
        ...
        for i in range(len(top1_err_ax)):
            axes.plot(x, top1_err_ax[i], '-', alpha=1, linewidth=2, marker=mStyles[i], markersize=10)
