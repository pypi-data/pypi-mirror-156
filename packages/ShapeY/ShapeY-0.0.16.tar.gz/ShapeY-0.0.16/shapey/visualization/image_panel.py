...
class DataPrepImagePanels:
    def __init__(self, imgdir, datadir, num_series=31, num_img_single_series=11, num_obj_per_cat=10):
        self.imgnames, self.objnames, self.obj_categories = self.get_image_and_obj_names(datadir)
    ...
    def get_list_of_errors(self, obj: str, ax: str, exc_dist: int, key_head: str ='original') -> list:
        ...
        for img in error_refimg:
            ...
            candidates, candidate_cvals, dists = self.get_all_candidate_images_sameobj(obj, ax, series_num, key_head=key_head)
        ...
        list_of_errors = list(map(list, zip(error_refimg, error_matchimg, candidate_list)))
        error_cvals = list(map(list, zip(error_cval_refimg, error_cval_matchimg, candidate_cval_list)))
        return list_of_errors, error_cvals

    def get_whole_error_list(self, ax: str, exc_dist: int, key_head: str='original') -> Tuple[list, list]:
        first = True
        for obj in tqdm(self.objnames):
            if first:
                list_of_errors, error_cvals = self.get_list_of_errors(obj, ax, exc_dist, key_head=key_head)
                first = False
            else:
                new_list_of_errors, new_error_cvals = self.get_list_of_errors(obj, ax, exc_dist, key_head=key_head)
                list_of_errors += new_list_of_errors
                error_cvals += new_error_cvals
        return list_of_errors, error_cvals

    def get_coltf_sameobj_candidates(self, obj_name: str, axis: str) -> Tuple[np.ndarray, np.ndarray]:
        same_obj_tf = np.char.startswith(self.imgnames, obj_name)
        series_names = np.array([img.split('-')[1].split('.')[0] for img in self.imgnames])
    ...
    def get_all_candidate_images_sameobj(self, obj_name: str, axis: str, series_idx: int, key_head: str = 'original') -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    def get_top_matches_sameobj_with_excdist(self, candidates: np.ndarray, candidate_cvals: np.ndarray, dists: np.ndarray, excdist: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    def get_coord_corrmat(self, obj_name, ax='all'):
        ...
        if ax=='all':
            return idx*self.num_img_single_series*self.num_series, (idx+1)*self.num_img_single_series*self.num_series
        else:
            ax_idx = self.axis_of_interest.index(ax)
            return idx*self.num_img_single_series*self.num_series + ax_idx*self.num_img_single_series, idx*self.num_img_single_series*self.num_series + (ax_idx+1)*self.num_img_single_series
    ...
    def map_top_per_obj_idx_to_global_idx(self, objname, top_per_obj_idx):
        ...
        obj_idx = np.tile(np.linspace(0, top_per_obj_idx_mapped.shape[1]-1, num=top_per_obj_idx_mapped.shape[1]), (top_per_obj_idx_mapped.shape[0],1))
        obj_idx[:,np.where(self.objnames == objname)[0][0]:] = obj_idx[:,np.where(self.objnames == objname)[0][0]:]+1
    ...
    def get_best_matching_same_obj_with_exclusion(self, objname, axis, sample_row_idx, key_head='original'):
        #sampled image name
        ...
        top1cvals_idx = top1cvals_idx + np.where(self.objnames==objname)[0][0]*self.num_img_single_series*self.num_series
        top1cvals_idx[invalid] = -1
        row1_excdist_cvals = top1cvals_excdist[sample_row_idx, :]
        row1_imgnames = np.array(list(map(self.map_to_imgname, top1cvals_idx)))[sample_row_idx, :]
        return sampled_imgname, (row1_excdist_cvals, row1_imgnames)
    
    def get_top11_best_matching_any_other_obj(self, objname, axis, sample_row_idx, key_head='original'):
        #sampled image name
        sampled_imgname = '{}-{}{:02d}.png'.format(objname, axis, sample_row_idx+1)
        with h5py.File(self.datadir, 'r') as hdfstore:
            obj_ax_key = '/' + key_head + '/'+ str(objname)+'/'+axis
    ...
    def get_top11_best_matching_per_any_other_obj(self, objname, axis, sample_row_idx, key_head='original'):
        #sampled image name
        sampled_imgname = '{}-{}{:02d}.png'.format(objname, axis, sample_row_idx+1)
        with h5py.File(self.datadir, 'r') as hdfstore:
            obj_ax_key = '/' + key_head + '/'+ str(objname)+'/'+axis
            ...
            top_per_obj_idx = hdfstore[obj_ax_key+'/top1_per_obj_idxs'][:] #top per obj index are indices within the object, thus needs to be mapped to a global index
        #row3 top 11 best matching per object
        row3_top_per_obj_idx = np.argsort(-top_per_obj_cvals)[:, :11]

        top_per_obj_idx_mapped = self.map_top_per_obj_idx_to_global_idx(objname, top_per_obj_idx)
        row3_img_idx = np.take_along_axis(top_per_obj_idx_mapped, row3_top_per_obj_idx, axis=1)

        row3_top_per_obj_cvals = np.take_along_axis(top_per_obj_cvals, row3_top_per_obj_idx, axis=1)[sample_row_idx, :]
        row3_imgnames = np.array(list(map(self.map_to_imgname, row3_img_idx)))[sample_row_idx, :]
        return sampled_imgname, (row3_top_per_obj_cvals, row3_imgnames)
    
    def get_top11_best_matching_per_any_other_obj_category(self, objname, axis, sample_row_idx, key_head='original', keep_sameobj_cat=False):
        #sampled image name
        sampled_imgname = '{}-{}{:02d}.png'.format(objname, axis, sample_row_idx+1)
        with h5py.File(self.datadir, 'r') as hdfstore:
            obj_ax_key = '/' + key_head + '/'+ str(objname)+'/'+axis
        ...
        for objcat in self.obj_categories:
            if objcat == current_obj_cat:
                ...
                if keep_sameobj_cat:
                    top_per_objcat_cvals.append(top_per_obj_cvals[:,start_idx:end_idx].max(axis=1))
                    idxs = top_per_obj_cvals[:,start_idx:end_idx].argmax(axis=1)
                    top_per_objcat_idxs.append(
                        np.take_along_axis(top_per_obj_idx_mapped[:, start_idx:end_idx], 
                        np.expand_dims(idxs, axis=1), axis=1))
            else:
        ...
        top_per_objcat_idxs = np.squeeze(np.array(top_per_objcat_idxs, dtype=np.int64).T, axis=0)
        
        row4_top_per_objcat_idx =  np.argsort(-top_per_objcat_cvals)[:, :11]
        row4_img_idx = np.take_along_axis(top_per_objcat_idxs, row4_top_per_objcat_idx, axis=1)

        row4_top_per_objcat_cvals = np.take_along_axis(top_per_objcat_cvals, row4_top_per_objcat_idx, axis=1)[sample_row_idx, :]
        row4_imgnames = np.array(list(map(self.map_to_imgname, row4_img_idx)))[sample_row_idx, :]
        return sampled_imgname, (row4_top_per_objcat_cvals, row4_imgnames)

    def get_top_per_obj_row_with_exclusion(self, row_info_same_obj, row_info_top_per_obj, exclusion_dist):
        ...
        #append and sort
        top_per_obj_cvals = np.append(row_info_top_per_obj[0], best_matching_same_obj_cval)
        top_per_obj_imgnames = np.append(row_info_top_per_obj[1], best_matching_same_obj)
        sorted = np.argsort(top_per_obj_cvals)
        top_per_obj_cvals = np.take_along_axis(top_per_obj_cvals, sorted[::-1], axis=0)
        top_per_obj_imgnames = np.take_along_axis(top_per_obj_imgnames, sorted[::-1], axis=0)
    ...
    def get_top_per_objcat_with_exclusion(self, row_info_top_per_obj, row_info_top_per_objcat, obj_cat):
        ...
        if 'same_obj_cat_imname' in locals():
            top_per_objcat_cvals = np.append(row_info_top_per_objcat[0], same_obj_cat_cval)
            top_per_objcat_imgnames = np.append(row_info_top_per_objcat[1], same_obj_cat_imname)
        else:
        ...
        same_obj_cat_tf = np.array([obj_cat in s.split('_') for s in top_per_objcat_imgnames])
        ...
        top_per_objcat_cvals = np.take_along_axis(top_per_objcat_cvals, sorted[::-1], axis=0)
        top_per_objcat_imgnames = np.take_along_axis(top_per_objcat_imgnames, sorted[::-1], axis=0)
    ...
    def get_top_per_obj_whole_panel_with_exclusion(self, objname, axis, exclusion_dist):
        #grab all inputs
        blockPrint()
        row1_infos = [self.get_best_matching_same_obj_with_exclusion(objname, axis, samp_idx) for samp_idx in range(0, 11)]
        row3_infos = [self.get_top11_best_matching_per_any_other_obj(objname, axis, samp_idx) for samp_idx in range(0, 11)]
        enablePrint()
        #organize the infos
        top_per_obj_row_infos = [self.get_top_per_obj_row_with_exclusion(e[0][1], e[1][1], exclusion_dist) for e in zip(row1_infos, row3_infos)]
        sampled_imgs = [e[0] for e in row1_infos]
        return sampled_imgs, top_per_obj_row_infos

    def get_top_per_objcat_whole_panel_with_exclusion(self, objname, axis, exclusion_dist):
        #grab all inputs
        blockPrint()
        row1_infos = [self.get_best_matching_same_obj_with_exclusion(objname, axis, samp_idx) for samp_idx in range(0, 11)]
        row3_infos = [self.get_top11_best_matching_per_any_other_obj(objname, axis, samp_idx) for samp_idx in range(0, 11)]
        row4_infos = [self.get_top11_best_matching_per_any_other_obj_category(objname, axis, samp_idx, keep_sameobj_cat=True) for samp_idx in range(0, 11)]
        enablePrint()
        #organize the infos
        ...
        for i, (samp_img, (cval, imname)) in enumerate(row1_infos):
            if ~np.isnan(cval[exclusion_dist]):
                row3_infos[i] = (samp_img, (np.insert(row3_infos[i][1][0], 0, cval[exclusion_dist]), 
                                            np.insert(row3_infos[i][1][1], 0, imname[exclusion_dist])))
        top_per_obj_row_infos = [self.get_top_per_objcat_with_exclusion(e[0][1], e[1][1], obj_cat=objname.split('_')[0]) for e in zip(row3_infos, row4_infos)]
    ...
    def get_whole_panel_single_sample(self, objname, axis, sample_idx):
        blockPrint()
        sampled_img, row1_info = self.get_best_matching_same_obj_with_exclusion(objname, axis, sample_idx)
        _, row2_info = self.get_top11_best_matching_any_other_obj(objname, axis, sample_idx)
        _, row3_info = self.get_top11_best_matching_per_any_other_obj(objname, axis, sample_idx)
        _, row4_info = self.get_top11_best_matching_per_any_other_obj_category(objname, axis, sample_idx)
    ...
    def get_corr_fall_off_graph_infos_whole_panel(self, objname, axis, objcat=False):
        ...
        for obj in objnames:
            ...
            for exc_dist in range(0,11):
                if objcat:
                    sampled_imgs, top_per_obj_row_infos = self.get_top_per_objcat_whole_panel_with_exclusion(obj, axis, exc_dist)
                else:
                    sampled_imgs, top_per_obj_row_infos = self.get_top_per_obj_whole_panel_with_exclusion(obj, axis, exc_dist)
...
class ImageGridV2:
    def __init__(self, row: int, col: int, axis_pad: float = 0.05):
        ...
        self.grid = MplImageGrid(
                self.figure, 111,
                nrows_ncols=(self.row, self.col),
                axes_pad=self.axis_pad
            )
        ...
        for i, ax in enumerate(self.grid):
            ...
            if c == 0:
                ...
                if len(ylabel_separated) > 1:
                    ax.set_ylabel("{}\n{}".format(ylabel_separated[0], ylabel_separated[1]), fontsize=7, fontweight='bold')
                else:
            ...
            if len(top_line_text) > 1:
                ...
                t = top_line_text[0]
            ax.text(128, 5, t, color='yellow', fontsize=7, horizontalalignment='center', verticalalignment='top')
            if len(ann_seperate) > 1:
                ax.text(128, 250, ann_seperate[1], color='yellow', fontsize=10, horizontalalignment='center')
...
class ImagePanelErrorDisplay(ImageGridV2):
    ...
    def shorten_imgname(self, imgname):
        ...
        shortened_imgname = obj_codes[0]+'_'+obj_codes[1][0:4]+'-{}'.format(series_name)
        return shortened_imgname
    
    def fill_error_panel(self, list_of_errors: list, cval_list: list, ax: str, exc_dist: int, annotate: bool=True) -> None:
        blank_img = BLANK_IMG
        imnames = []
        ylabels = []
        annotations = []
        ...
        self.figure.suptitle('Axis: {}, Exclusion Distance: {}'.format(ax, exc_dist), fontsize=15, y=0.9)
...
class ImagePanelAllCandidates(ImageGridV2):
    ...
    def change_border_color(self, border_colors: list) -> None:
        for i, ax in enumerate(self.grid):
            ...
            c = i % self.col
            ax.set_axis_on()
            matplotlib.pyplot.setp(ax.spines.values(), color=border_colors[r][c], linewidth=3)

    def fill_all_candidate_panel(self, refimg: str, candidates: np.ndarray, candidate_cvals: np.ndarray) -> None:
        ...
        series_name = obj_codes[1].split('-')[1]
        shortened_imgname = obj_codes[0]+'_'+obj_codes[1][0:4]+'-{}'.format(series_name)
        self.figure.suptitle('All Candidate Images for {}'.format(shortened_imgname), fontsize=15, y=0.9)
        ...
        for i, ax in enumerate(self.grid):
            ...
            if r == 0:
                if c == 0:
                    ...
                    ax.set_title('Dist = {}'.format(dists[c-1]), fontsize=10, fontweight='bold')

    def fill_best_matching_panel(self, refimg: str, candidates_sorted: np.ndarray, candidate_cvals_sorted: np.ndarray, dists_sorted: np.ndarray, excdist: int, axis: str) -> None:
        ...
        self.fill_panel(candidate_imgnames, ylabels, annotations)
        self.figure.suptitle('Top {} matches with Exclusion Distance of {} in {}'.format(self.numsample-1, excdist, axis), y=1.2)
...
class ImagePanelSingleSample(ImageGridV2):
    ...
    def set_xlabel(self):
        for i, ax in enumerate(self.grid):
            ...
            c = i % self.col
            if r == 0:
                ax.set_title('Exc. Dist = {}'.format(c), fontsize=10, color='red', fontweight='bold')
    ...
    def fill_single_sample_panel(self, sampled_imgname, row_infos):
        ...
        for i, item in enumerate(row_infos):
            if i == 0:
                row = list(item[1])
                row = [blank_dir+img if img=='blank.png' else self.imgdir + img for img in row]
                reference_img = row[0]
                imnames.append(row)
                ann = []
        ...
        ylabels = ['Within Same Obj', 'Top 11 Any Other Obj', 'Top 11 Per Obj', 'Top 11 Per Obj Category']
        self.fill_panel(imnames, ylabels, annotations)
        self.set_xlabel()
...
class ImagePanelTopPer(ImageGridV2):
    ...
    def change_border_color(self, border_colors):
        for i, ax in enumerate(self.grid):
            ...
            c = i % self.col
            ax.set_axis_on()
            matplotlib.pyplot.setp(ax.spines.values(), color=border_colors[r][c], linewidth=2)

    def fill_top_per_panel(self, sampled_imgnames, row_infos, axis, exclusion_dist, obj_cat=False):
        blank_dir = PROJECT_DIR
        imnames = []
        annotations = []
        border_colors = []
        ...
        self.fill_panel(imnames, ylabels, annotations)
        if not obj_cat:
            self.figure.suptitle('Top Per Object with Exclusion Distance of {} in {}'.format(exclusion_dist, axis), fontsize=15, y=0.9)
        else:
            self.figure.suptitle('Top Per Object Category with Exclusion Distance of {} in {}'.format(exclusion_dist, axis), fontsize=15, y=0.9)
        self.change_border_color(border_colors)