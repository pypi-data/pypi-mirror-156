...
class ImgCorrelationDataProcessorV2:
    ...
    def cut_corrmat(self, hdfstore: File, num_objs: int, post_processed: bool = False, pp_exclusion: str = 'soft') -> None:
        ...
        obj_idxs = np.array([v+10*cat for cat in range(20) for v in np.sort(np.random.choice(10, obj_per_category, replace=False))])
        self.objnames = self.objnames_buffer[obj_idxs]
        
        #cut image names
        img_idxs_start = obj_idxs*11*31
        img_idxs_end = (obj_idxs+1)*11*31
        img_idxs = np.array([np.array([range(a, b)]) for (a, b) in zip(img_idxs_start, img_idxs_end)]).flatten()
        ...
        for i in obj_idxs:
            obj_mat_list = []
            for j in obj_idxs:
                obj_mat = cval_matrix[i*11*31:(i+1)*11*31, j*11*31:(j+1)*11*31]
    ...
    def excluded_to_zero(self, corr_mat_sameobj: cp.ndarray, axis: str, exc_dist: int, pure: bool = True) -> cp.ndarray:
        ...
        if exc_dist != 0:
            ...
            sampling_mask = 1-(tri(11,11,exc_dist-1, dtype=float) - tri(11,11,-exc_dist, dtype=float))
            sampling_mask[sampling_mask==0] = cp.nan
        # pure transform
        if pure:
            #cut out the axis of interest
            idx = self.axes_of_interest.index(axis)
            corr_mat_axis = cp.copy(corr_mat_sameobj[idx*11:(idx+1)*11, idx*11:(idx+1)*11])
            #sample with the sampling mask
            if exc_dist !=0:
                corr_mat_axis = cp.multiply(corr_mat_axis, sampling_mask)
            return corr_mat_axis
        elif exc_dist != 0:
            contain_ax = cp.array([[cp.array([c in a for c in axis]).all() for a in self.axes_of_interest]], dtype=int)
