...
def generic_portfolio(safety_stock_df, db, schema, logger=None):
    ...
    """
    logger.info("Keep all generics at store - Consolidation Starts")
    comp_drugs_to_keep = get_preference_drugs(db, schema, logger)

    # get compositions of all drugs in store
    ...
    final_df['correction_flag'] = final_df['correction_flag'].fillna('N')
    ...
    logger.info("Keep all generics at store - Consolidation Ends")
