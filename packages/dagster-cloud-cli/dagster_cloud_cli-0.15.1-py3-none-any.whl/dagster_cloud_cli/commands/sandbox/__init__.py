...
def code_sync(
    ...
    identifier_location_name_map = {
        directory.identifier: directory.location_name for directory in directories_to_sync.values()
    }
    try:
        ...
        for update in sync_method.sync_loop(timeout=5):
            ...
            if unavailable_time and datetime.now() > unavailable_time + timedelta(
                ...
                print_sync_state(states_map, remove_previous=verbosity_level == 0, refetching=True)
