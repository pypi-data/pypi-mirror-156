...
class Filter:
    ...
    def __init__(self, band, filter_directory=None, wave_units=q.um, flux_units=q.erg/q.s/q.cm**2/q.AA, **kwargs):
        ...
        self.filter_directory = filter_directory or resource_filename('svo_filters', 'data/filters/')
        ...
        if band.lower().replace('-', '').replace(' ', '') == 'tophat':
            ...
            bands = filters(self.filter_directory)

            # Read file if the band is in the filter directory
            if band in bands:

                # Get the file
                filepath = glob(os.path.join(self.filter_directory, band + '*'))[0]
            ...
            # Otherwise try a Web query or throw an error
                ...
                err = """No filters match {}\n\nSelect from available filters at {}:\n\n{}\n\nOr a full list of available filters from the\nSVO Filter Profile Service can be found at\nhttp: //svo2.cab.inta-csic.es/theory/fps3/\n\nTry again with the desired filter as '<facility>/<instrument>.<filter_name>', e.g. '2MASS/2MASS.J'""".format(band, self.filter_directory, self.all_filters(printed=False))
    ...
    def bin(self, n_bins=1, pixels_per_bin=None, wave_min=None, wave_max=None, old=False):
        """
    ...
    def all_filters(self, filter_directory=None, printed=True):
        """
        Lists all available filters in the given filter_directory
        """
        # Make nice list
        fdir = filter_directory or self.filter_directory
        flist = sorted(filters(fdir))
        tlist = "\n".join(['{:<20}{:<20}{:<}'.format(a, b, c) for a, b, c in zip(flist[::3], flist[1::3], flist[2::3])])

        # Print or return it
        if printed:
            print('All filters available at')
            print(fdir)
            print(tlist)
        else:
            return tlist
