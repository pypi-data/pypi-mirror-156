...
def power_spec(data,
    ...
    if plot_on:
        first_ind, last_ind = helpers.ind_limits(frequencies, view_range)
        plt.figure()
...
def spectrogram(data,
    ...
    if plot_on:
        ...
        axs[0].autoscale(enable=True, axis='x', tight=True)
        # Plot spectrogram
        first_ind, last_ind = helpers.ind_limits(freqs, view_range)
...
if __name__ == '__main__':
    ...
    # to do: figure out relative path bs (wtf)
    wav_path = r'../anaties/data/songbirds.wav'  # externally
    # wav_path = r'../data/songbirds.wav'  # from within
