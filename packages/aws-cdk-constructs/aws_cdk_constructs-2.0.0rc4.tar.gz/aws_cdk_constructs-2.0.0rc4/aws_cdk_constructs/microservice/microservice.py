...
class Microservice(Construct):
    ...
    def create_target_group(
        ...
        tg = _alb.ApplicationTargetGroup(
            ...
            vpc=self._vpc,
            target_type=_alb.TargetType.INSTANCE,
            target_group_name=(app_name + id + ec2_traffic_port).replace("_", "-"),
            stickiness_cookie_duration= cdk.Duration.hours(int(stickiness_cookie_duration_in_hours)) if stickiness_cookie_duration_in_hours else None,
        )
        ...
        cdk.Tags.of(tg).add("ApplicationName", app_name.lower().strip())
        cdk.Tags.of(tg).add("Environment", environment)
    ...
    def create_load_balancer(self, scope, id, app_name, environment, environments_parameters, security_group=None, internet_facing=False, load_balancer_name=None, use_cdn=False, main_component_name=None, load_balancer_idle_timeout_in_seconds=None):
        ...
        if use_cdn:
            cdn_security_group = self.create_security_group(
                ...
                app_name, 
                environment, 
                app_name + '_' + main_component_name + "_alb_secg_cdn",
                )
        ...
        alb = _alb.ApplicationLoadBalancer(
            ...
            idle_timeout= cdk.Duration.seconds(int(load_balancer_idle_timeout_in_seconds)),
        ...
        # Apply mandatory tags
        cdk.Tags.of(alb).add("ApplicationName", app_name.lower().strip())
    ...
    def __init__(
        ...
        **kwargs
        ...
        is_private_and_has_upstream = is_private and has_upstream
        ...
        cdk.CfnOutput(self, f"MicroserviceId{main_component_name}", export_name= f"Microservice_Id_{main_component_name}", value=str(self.id))
        cdk.CfnOutput(self, f"MicroserviceAppName{main_component_name}", export_name=f"Microservice_AppName_{main_component_name}", value=str(app_name))
        cdk.CfnOutput(self, f"MicroserviceMainComponentName{main_component_name}", export_name=f"Microservice_MainComponentName_{main_component_name}", value=str(main_component_name))
        cdk.CfnOutput(self, f"MicroserviceIsPublic{main_component_name}", export_name=f"Microservice_IsPublic_{main_component_name}", value=str(is_public))
        cdk.CfnOutput(self, f"MicroserviceUseCdn{main_component_name}", export_name=f"Microservice_UseCdn_{main_component_name}", value=str(use_cdn))
        cdk.CfnOutput(self, f"MicroserviceHasUpstream{main_component_name}", export_name=f"Microservice_HasUpstream_{main_component_name}", value=str(has_upstream))
        cdk.CfnOutput(self, f"MicroserviceIsHa{main_component_name}", export_name=f"Microservice_IsHa_{main_component_name}", value=str(is_ha))
        cdk.CfnOutput(self, f"MicroserviceUseEbs{main_component_name}", export_name=f"Microservice_UseEbs_{main_component_name}", value=str(use_ebs))
        cdk.CfnOutput(self, f"MicroserviceUseEfs{main_component_name}", export_name=f"Microservice_UseEfs_{main_component_name}", value=str(create_efs))
        cdk.CfnOutput(self, f"MicroserviceHasOidc{main_component_name}", export_name=f"Microservice_HasOidc_{main_component_name}", value=str(has_not_oidc))
        cdk.CfnOutput(self, f"MicroserviceAsgLogicalId{main_component_name}", export_name=f"Microservice_AutoscalingGroupLogicalId_{main_component_name}", value=str(AUTOSCALING_GROUP_LOGICAL_ID))
        cdk.CfnOutput(self, f"MicroserviceUseLdap{main_component_name}", export_name=f"Microservice_UseLdap_{main_component_name}",value=str(use_ldap))
        cdk.CfnOutput(self, f"MicroserviceHasBackup{main_component_name}", export_name=f"Microservice_HasBackup_{main_component_name}", value=str(to_be_backed_up))
        cdk.CfnOutput(self, f"MicroserviceIsHttps{main_component_name}", export_name=f"Microservice_IsHttps_{main_component_name}",value=str(is_https))
        ...
        if(is_application_load_balancer):
            ...
            if is_private:
                self.enable_fao_private_access(self.alb_security_group, port=self._tcp_connection_traffic_port)
            ...
            if is_private_and_has_upstream:
                self.alb_security_group.add_ingress_rule(
                    peer=upstream_security_group_as_peer,
                    connection=self._tcp_connection_traffic_port,
                    description="From upstream",
            ...
            if is_https:
                ...
                if has_oidc:
                    ...
                    oidc_listener_action = _alb.ListenerAction.authenticate_oidc(
                        ...
                        issuer=issuer,
                        client_id=client_id,
                        client_secret= cdk.SecretValue(value=client_secret),
        ...
        if create_efs:
            ...
            if to_be_backed_up:
                cdk.Tags.of(self._efs).add("RubrikBackup", "true")
            else:
                cdk.Tags.of(self._efs).add("RubrikBackup", "false")
        # ~~~~~~~~~~~~~~~~
        # User data
        # ~~~~~~~~~~~~~~~~
        ...
        asg = _asg.AutoScalingGroup(
            ...
            notifications= [
                _asg.NotificationConfiguration(
                    ...
                    scaling_events=_asg.ScalingEvents.TERMINATION_EVENTS
                ),
                _asg.NotificationConfiguration(
                    topic=asg_notifications_topic,
                    # the properties below are optional
                    scaling_events=_asg.ScalingEvents.ERRORS
                )
            ],
        ...
        if is_not_production:
            ...
            if tag_scheduler_uptime_days:
                cdk.Tags.of(ASG).add("SchedulerUptimeDays", tag_scheduler_uptime_days, apply_to_launched_instances=True,)   
        ...
        if existing_efs_security_group or existing_efs_security_group_id:
            ...
            if to_be_backed_up:
                ...
                production_aws_account_id = self._environments_parameters["accounts"]["production"]["id"]
                _ec2.CfnSecurityGroupIngress(
                    self,
                    "self.efs_access_from_rubrik_cloud_cluster",
                    ip_protocol = "tcp",
                    description="EFS access from Rubrik CDM Cloud Cluster",
                    group_id = existing_efs_security_group_resource.security_group_id,
