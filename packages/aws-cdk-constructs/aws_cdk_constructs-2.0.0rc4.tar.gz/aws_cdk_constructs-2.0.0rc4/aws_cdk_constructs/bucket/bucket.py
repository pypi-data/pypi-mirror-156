...
class Bucket(Construct):
    ...
    def __init__(
        ...
        **kwargs
        ...
        cdk.CfnOutput(self, f"BucketAppName{bucket_name}", export_name=f"Bucket_AppName_{bucket_name}" ,value=str(app_name))
        cdk.CfnOutput(self, f"BucketPublicReadAccess{bucket_name}", export_name=f"Bucket_PublicReadAccess_{bucket_name}",value=str(public_read_access))
        cdk.CfnOutput(self, f"BucketIncludesCdn{bucket_name}", export_name=f"Bucket_IncludesCdn_{bucket_name}", value=str(include_cdn))
        cdk.CfnOutput(self, f"BucketHasAuthentication{bucket_name}", export_name=f"Bucket_HasAuthentication_{bucket_name}", value=str(has_authentication))
        cdk.CfnOutput(self, f"BucketIsEncrypted{bucket_name}",  export_name=f"Bucket_IsEncrypted_{bucket_name}", value=str(is_encrypted))
        cdk.CfnOutput(self, f"BucketBucketName{bucket_name}",  export_name=f"Bucket_BucketName_{bucket_name}", value=str(bucket_name))
