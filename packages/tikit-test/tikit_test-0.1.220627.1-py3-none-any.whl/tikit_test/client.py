...
class Client(object):
    ...
    def create_batch_task(self,
        ...
        :param output_config: 输出数据源配置（cos目录，以/结尾。格式：<bucket>/<cos path>/）
        :type output_config: str
        :param input_config: 输入数据源配置（cos目录，以/结尾。格式：<bucket>/<cos path>/）
        ...
        try:
            ...
            if input_config is not None:
                input_data_config = models.DataConfig()
                input_data_config.DataSourceType = "COS"
                input_data_config.COSSource = self.parse_cos_info(input_config)
                req.DataConfigs = [input_data_config]

            output_data_config = models.DataConfig()
            output_data_config.DataSourceType = "COS"
            output_data_config.COSSource = self.parse_cos_info(output_config)
            req.Outputs = [output_data_config]
