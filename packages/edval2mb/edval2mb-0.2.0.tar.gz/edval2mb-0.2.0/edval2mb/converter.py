...
@click.option('--academic-year', 'academic_year', default='')
@click.option('--day-start-index', 'day_start_index', default=1)
@click.option('--period-start-index', 'period_start_index', default=1)
@click.option('--output-path', 'output_path', default='managebac_timetable.csv')
@click.option('--day', 'days', default=('Mon', 'Tue', 'Wed', 'Thu', 'Fri'), multiple=True)
@click.argument('fields', nargs=-1)
@click.pass_obj
def convert(obj, academic_year, day_start_index, period_start_index, output_path, days, fields):
    ...
    print(f'Saved csv of {rows+1} rows (including header) and {columns} columns to {output_path}')
