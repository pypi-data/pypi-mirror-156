def model_metrics(model, x_train, y_train, x_test, y_test, show_graphs=True, f_score = 0.50, classification = True, algo=None):
    ...
    if algo == 'rf': 
        ...
        features = result.sort_values(by=['importance'], ascending=False).head(20)
        display(features)
    ...
    try:
        ...
        warnings.warn("Decile breaks cannot be computed because there is not enough variation in model scores")
    ...
    if show_graphs:
        ...
        if computed_deciles:
            ...
            lift = pd.DataFrame()
            warnings.warn("Lift cannot be calculated because model deciles do not exist!")
    
    if classification == True:
        return metricx, lift, classification_metricx, features
    
    else:
        return metricx, lift, features