...
class DesignSpaceExplorationProblem(Problem):
    ...
    def get_pareto_frontier(self):
        ppa = torch.Tensor(self.ppa)
        return get_pareto_frontier(ppa)
...
def get_adrs_wrt_polytope(reference_pareto_frontier, predict_pareto_frontier):
    """
        reference_pareto_frontier: <torch.Tensor>
        predict_pareto_frontier: <torch.Tensor>
    """
    def calc_dist_wrt_linseg(p, p1, p2):
        # get the normalized tangent vector
        d = np.divide(p2 - p1, np.linalg.norm(p2 - p1))
        # get the signed parallel distance components
        s = np.dot(p1 - p, d)
        t = np.dot(p - p2, d)
        # get the clamped parallel distance
        h = np.maximum.reduce([s, t, 0])
        # get perpendicular distance component
        c = np.cross(p - p1, d)
    ...
    adrs = 0
    i = 0
    for p1, p2 in combinations(reference_pareto_frontier, 2):
        mini = float('inf')
        for gamma in predict_pareto_frontier:
            mini = min(mini, calc_dist_wrt_linseg(gamma.numpy(), p1.numpy(), p2.numpy()))
        adrs += mini
        i += 1
    adrs /= i
    return adrs
