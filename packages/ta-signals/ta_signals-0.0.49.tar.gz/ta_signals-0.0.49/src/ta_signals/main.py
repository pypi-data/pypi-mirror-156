...
class TaSignals:
    ...
    def ema(self, df, key):
        ...
        df['ema8_slope'] = df[key].rolling(window=self.window).apply(self.get_slope, raw=True)
        df['ema34_slope'] = df[key].rolling(window=self.window).apply(self.get_slope, raw=True)
