...
from convisoappsec.flowcli.common import DeployFormatter, project_code_option
...
def perform_sast(context) -> None:
    ...
    specific_params = {
        ...
        "start_commit": context.params.get('previous_commit'),
        "end_commit": context.params.get('current_commit'),
    }
...
def perform_deploy(context):
    ...
    try:
        ...
        if not bool(created_deploy):
            raise CreateDeployException("Something wrong creating deploy.")
...
def run(context, create_context, **kwargs):
    """AST - Application Security Testing. Unifies deploy creation, SAST and SCA runs. 

    Args:
        context (<class 'click.core.Context'>): Click context to setup and use the commands through Click framework.
        create_context (<class 'convisoappsec.flowcli.deploy.create.context.CreateContext'>): Context required to create deploy.

    Raises:
        click.ClickException: To treat some error in the process.
    """
    try:
        context.obj.deploy = perform_deploy(clone(context))
        perform_sast(clone(context))
        perform_sca(clone(context))
