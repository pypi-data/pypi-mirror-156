        ...
        with TQDM() as pbar:
            ...
            for epoch_idx in range(initial_epoch,num_epoch):
                ...
                for batch_idx in range(num_batches):
                    ...
                    batch_logs['loss'] = loss.item() #loss.data[0]
                    callback_container.on_batch_end(batch_idx, batch_logs)
...
class SingleInput_SingleTarget_Helper(object):
    ...
    def grab_batch(self, batch_idx, batch_size, inputs, targets, volatile=False):
        with th.no_grad(): #th=torch, isha added this to get rid of UserWarning: volatile was removed and now has no effect. Use `with torch.no_grad():` instead. target_batch = Variable(targets[batch_idx*batch_size:(batch_idx+1)*batch_size], volatile=volatile, requires_grad=False)
            input_batch = Variable(inputs[batch_idx*batch_size:(batch_idx+1)*batch_size]) #, volatile=volatile)
            target_batch = Variable(targets[batch_idx*batch_size:(batch_idx+1)*batch_size], requires_grad = False) #volatile=volatile, requires_grad=False)
