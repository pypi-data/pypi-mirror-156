...
def test_translate_spec122_to_214_l0_with_multiple_fits_cards_return_header(
    multiple_fits_cards_object,
):
    """
    Validates and tries to translate a fits header with multiple fits cards against the SPEC-122 schema
    Given: A valid SPEC-122 fits header with multiple fits cards
    When: Validating and translating headers with multiple fits cards
    Then: return translated 214 l0 header and do not raise an exception
    """
    multiple_fits_cards = spec122_validator.validate(multiple_fits_cards_object)

    spec122_validator.validate_and_translate_to_214_l0(
        multiple_fits_cards, return_type=fits.header.Header
    )


def test_translate_spec122_to_214_l0_with_multiple_fits_cards_return_HDUList(
    multiple_fits_cards_object,
):
    """
    Validates and tries to translate a fits header with multiple fits cards against the SPEC-122 schema
    Given: A valid SPEC-122 fits header with multiple fits cards
    When: Validating and translating headers with multiple fits cards
    Then: return translated 214 l0 HDUList and do not raise an exception
    """
    multiple_fits_cards = spec122_validator.validate(multiple_fits_cards_object)

    spec122_validator.validate_and_translate_to_214_l0(multiple_fits_cards, return_type=HDUList)
