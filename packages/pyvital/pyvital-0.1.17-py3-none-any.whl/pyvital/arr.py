...
def estimate_resp_rate(data, srate):
    ...
    extrema = np.concatenate([maxlist, minlist])
    np.sort(extrema)  # min, max, min, max
...
if __name__ == '__main__':
    ...
    caseid = 1
    tid = df_trks[(df_trks['caseid'] == caseid) & (df_trks['tname'] == 'SNUADC/ECG_II')]['tid'].values[0]
    ecg = pd.read_csv('https://api.vitaldb.net/' + tid).values
    SRATE = 500
    INTERVAL = SRATE * 10
    sig = ecg[:INTERVAL, 1]
    sig = interp_undefined(sig)
