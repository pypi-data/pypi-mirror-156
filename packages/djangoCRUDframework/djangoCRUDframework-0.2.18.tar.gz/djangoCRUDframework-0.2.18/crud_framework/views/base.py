...
class BaseView(View):
    ...
    def get(self, request, filters, initkwargs):
        raise NotImplemented('GET not Allowed!')

    def post(self, request, filters, initkwargs, **kwargs):
        raise NotImplemented('POST not Allowed!')

    def put(self, request, filters, initkwargs, **kwargs):
        raise NotImplemented('PUT not Allowed!')

    def delete(self, request, filters, initkwargs):
...
class BaseCrudView(BaseView):
    ...
    def get(self, request, filters, initkwargs):
        crud = self.schema_class(filters, **initkwargs)
        self.data = crud.get()
    ...
    def bulk_post(self, request, filters, initkwargs, **kwargs):
        crud = self.schema_class(filters, **initkwargs)
        body = unjsonize(request.body.decode())
