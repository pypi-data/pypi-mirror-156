...
class ColorUtils:
    ...
    def __color_dict(cls, gradient):
        ...
        defined later on"""
        return {
            "hex": [cls.RGB2hex(RGB) for RGB in gradient],
    ...
    @classmethod
    def labels_colors_map(cls, labels, opacity=0.25):
        n = len(labels)
        colors_dict = cls.rainbow(n)
    ...
    def plot_gradient_series(
        cls, color_dict, filename=None, pointsize=100, control_points=None
    ):
        ...
        if control_points is not None:
            ...
            ax.scatter(
                xcntl, ycntl, zcntl, c=control_points["hex"], s=pointsize, marker="s"
            )
...
if __name__ == "__main__":
    colors_dict = ColorUtils.linear_gradient("#e2f442", n=32)
    R = colors_dict["r"]
    G = colors_dict["g"]
    B = colors_dict["b"]
    ...
    colors_dict = ColorUtils.polylinear_gradient(
    ...
    colors_dict = ColorUtils.rainbow(n=32)
    R = colors_dict["r"]
    G = colors_dict["g"]
    B = colors_dict["b"]
    gradient = list(zip(B, G, R))
