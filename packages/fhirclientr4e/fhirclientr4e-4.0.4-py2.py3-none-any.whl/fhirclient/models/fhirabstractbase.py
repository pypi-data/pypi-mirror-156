    ...
    def update_with_json(self, jsondict):
        ...
        for name, jsname, typ, is_list, of_many, not_optional in self.elementProperties():
            ...
            if value is not None:
                ...
                elif testval is not None and self.attribute_enums().get(name):
                    enum_definition = self.attribute_enums().get(name)
                    if enum_definition['binding_strength'] == 'required':
                        # validate code enum
                        if enum_definition['class_name'] == 'str':
                            if testval not in enum_definition['restricted_to']:
                                err = TypeError(
                                    "Expecting property with required binding_strength \"{}\" to be member of {} is \"{}\""
                                    .format(name, enum_definition['url'], testval))
                        # validate CodeableConcept enums
                        elif enum_definition['class_name'] == 'CodeableConcept':
                            testval_list = testval if isinstance(testval, list) else [testval]
                            for codable_concept in testval_list:
                            ...
                            raise Exception("Unknown enum class_name {}".format(enum_definition['class_name']))
            ...
            if is_extension:
                ...
                for extension in value:
                    ...
                    if not simplified_value:
                        # look in the extension's extensions
                        simplified_values = []
                        if extension.extension:
                            for sub_extension in extension.extension:
                                simplified_values.append(next(iter([v for k, v in sub_extension.__dict__.items() if k.startswith('value') and v]), None))
                ...
                suffix = ''
                if value.system:
                    suffix = value.system.split('/')[-1]
                simplified_key = "{}_{}".format(jsname, suffix)

                js[simplified_key] = value.code
                _set_schema(simplified_key)
                ...
                for identifier in value:
                    ...
                    if identifier.type and identifier.type and identifier.type.coding:
                        simplified_key = "identifier_{}".format(identifier.type.coding[0].code)
                    elif identifier.type and identifier.type and identifier.type.text:
                        simplified_key = "identifier_{}".format(identifier.type.text)

                    js[simplified_key] = identifier.value
                    _set_schema(simplified_key)
                ...
                _set_schema(jsname)
                for v in value:
                    if hasattr(v, 'as_simplified_json'):
                        simple_value, simple_schema = v.as_simplified_json()
                        # we want the properties of all the members of the list, so deep merge
                        schema[jsname] = merge(schema[jsname], simple_schema)
                    else:
                        simple_value = v
