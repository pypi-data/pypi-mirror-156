...
from pack_client.client.common.errors import ServerError
from pack_client.client.common.variables import MESSAGE_TEXT, SENDER
...
class ClientMainWindow(QMainWindow):
    ...
    def set_active_user(self):
        ...
        if not self.current_chat_key:
            self.messages.warning(
                self, 'Ошибка', 'Для выбранного пользователя нет '
                                'ключа шифрования.')
            return
    ...
    def delete_contact(self, item):
        """ Метод удаляющий контакт из серверной и клиентсткой BD.
         После обновления баз данных обновляет и содержимое."""
    ...
    def message(self, message):
        ...
        if sender == self.current_chat:
            ...
            if self.database.check_contact(sender):
                ...
                print('NO')
                # Раз нет,спрашиваем добавить юзера в контакты.
                if self.messages.question(
                        self,
                        'Новое сообщение',
                        f'Получено новое сообщение от {sender}.\n '
                    ...
                    self.current_chat = sender
                    # Нужно заново сохранить сообщение, иначе оно будет потеря-
                    # но,т.к. на момент предыдущего вызова контакта не было.
                    self.database.save_message(
