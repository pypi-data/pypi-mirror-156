...
class VASPLoader:
    ...
    def __init__(self, directory):
        ...
        with BoltzTraP2.misc.dir_context(directory):
            ...
            if self.ebands.shape[0] > 1 or (r["magmom"] is not None and
                                            r["magmom"].shape[1] == 3):
                self.dosweight = 1.0
            else:
                self.dosweight = 2.0
...
class GenericWien2kLoader:
    ...
    """
    def __init__(self, w2kname, dosweight, scffn, structfn, energyfn, derfn=""):
    ...
    def __init__(self, directory):
        ...
        with BoltzTraP2.misc.dir_context(directory):
            ...
            if not os.path.isfile(energyfn):
                raise ValueError("energy file not found")
            (self.atoms, magmom,
             fict_magmom) = BoltzTraP2.io.read_CASTEP_output(structfn)
            BoltzTraP2.misc.info("lattice:", self.atoms.get_cell().T)
            (
                self.fermi,
...
_ESPRESSO_SCHEMA_KEY = "{http://www.w3.org/2001/XMLSchema-instance}schemaLocation"
...
class ESPRESSOLoader:
    ...
    def __init__(self, directory):
        ...
        with BoltzTraP2.misc.dir_context(directory):
            ...
            self.fermi = r["fermi"]
            self.ebands = r["E"]
            if self.ebands.shape[1] > 1 or (r["magmom"] is not None and
                                            r["magmom"].ndim > 1):
                self.dosweight = 1.0
            else:
                self.dosweight = 2.0
            self.ebands = self.ebands.reshape((self.ebands.shape[0], -1)
                                             ).transpose()
            self.nelect = r["nelect"]
    ...
    def __init__(self, directory, derivatives=False, *args, **kwargs):
        ...
        if derivatives:
            try:
                ...
                raise ValueError(
                    "no derivative information found in directory {}".
                    format(directory)
                )
        else:
            try:
                ...
                BoltzTraP2.misc.info("derivative information will be discarded")
            self.mommat = None
        try:
