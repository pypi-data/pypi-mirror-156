...
def _suggest_nbins(data, erange, Tmin=None):
    ...
    bytemp = 2. * (erange[1] - erange[0]) / (BOLTZMANN * Tmin)
...
def smoothen_DOS_direct(e, dos, Tsmooth, ep):
    ...
    norm = sp.integrate.quad(
        lambda x: dFDde(x, 0., BOLTZMANN * Tsmooth),
        -_FD_XMAX * BOLTZMANN * Tsmooth,
        _FD_XMAX * BOLTZMANN * Tsmooth
    )[0]
    nruter = np.zeros_like(ep)
    for i, mu in np.ndenumerate(ep):
        d = dFDde(mu - e, 0., BOLTZMANN * Tsmooth)
        nruter[i] = -np.trapz(d * dos, e)
    nruter /= norm
    return nruter
...
def solve_for_mu(
    epsilon, dos, N0, T, dosweight=2., refine=False, try_center=False
):
    ...
    if dos[pos] == 0.:
        ...
        kBT = T * BOLTZMANN
        if (try_center and
                min(hepsilon - mu, mu - lepsilon) >= _FD_XMAX_GAP * kBT / 2.):
...
def calc_cv(epsilon, dos, mur, Tr, dosweight=2.):
    ...
    for iT, kBT in enumerate(kBTr):
        for imu, mu in enumerate(mur):
            nruter[iT, imu] = -(
                dosweight * dos * dFDde(epsilon, mu, kBT) *
                (epsilon - mu)**2 / kBT
            ).sum() * de
...
def calc_Onsager_coefficients(L0, L1, L2, mur, Tr, vuc, Lm11=None):
    ...
    L12 = L1 / Tr[:, None, None,
                  None] / (Volt * Siemens / (Meter * Second)) / vuc
    L22 = L2 / Tr[:, None, None, None] / (
...
def apply_scissor(epsilon, dos, N0, eband, desired_gap, dosweight=2.):
    ...
    fermi = solve_for_mu(
        epsilon, dos, N0, 0., dosweight, refine=False, try_center=False
    )
