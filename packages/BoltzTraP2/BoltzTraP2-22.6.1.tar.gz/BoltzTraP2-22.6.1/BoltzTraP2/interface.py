...
#    Copyright (C) 2017-2022 Georg K. H. Madsen <georg.madsen@tuwien.ac.at>
#    Copyright (C) 2017-2022 Jesús Carrete <jesus.carrete.montana@tuwien.ac.at>
#    Copyright (C) 2017-2022 Matthieu J. Verstraete <matthieu.verstraete@ulg.ac.be>
...
def components_argument(arg):
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(
            "'{}' cannot be parsed as a Python literal. Try '[\"xx\"]' with all quotes"
            .format(arg)
        )
...
def parse_interpolate(args):
    ...
    basis_tensors = BoltzTraP2.sphere.calc_tensor_basis(data.atoms, data.magmom)
    info(
    ...
    logging.info("{} irreducible k points have been requested".format(nktarget))
    equivalences = BoltzTraP2.sphere.get_equivalences(
        data.atoms, data.magmom, nktarget
    )
    nkoutput = len(equivalences)
    logging.info("{} irreducible k points have been generated".format(nkoutput))
    if nkoutput < nkinput:
    ...
    # Gather the metadata
    metadata = BoltzTraP2.serialization.gen_bt2_metadata(data, args.derivatives)
    # Save the result
...
def parse_dope(args):
    ...
    for iT, T in enumerate(Tr):
        dopingmin = BoltzTraP2.bandlib.calc_N(
            epsilon, dos, mumax, T, data.dosweight
        ) + data.nelect
        dopingmin /= vuccm3
        dopingmax = BoltzTraP2.bandlib.calc_N(
            epsilon, dos, mumin, T, data.dosweight
        ) + data.nelect
        dopingmax /= vuccm3
...
def parse_plotbands(args):
    ...
    kpaths = [
        list(group) for k,
        group in itertools.groupby(kpaths, key=lambda x: x is not None) if k
    ]
    try:
...
def parse_plot(args):
    ...
    ylabel = dict(
        ...
        r"s^{{-1}}}}\right]$",
        S=r"$S^{{\left({}\right)}}\;" + r"\left[\mathrm{{V\,K^{{-1}}}}\right]$",
    ...
    if metadata["scattering_model"] == "uniform_lambda":
        ...
        conductivity_factor = 1. / (sp.constants.alpha * sp.constants.c)
    ...
    if args.quantity == "cv":
        ...
        for iT in range(y.shape[0]):
            for imu in range(y.shape[1]):
                y[iT,
                  imu] = la.solve(cond[iT, imu].T, kappa[iT, imu].T).T / Tr[iT]
...
def parse_arguments():
    ...
    dope_parser = subparsers.add_parser(
        "dope",
        help=
        "compute transport coefficients for particular carrier concentrations",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
