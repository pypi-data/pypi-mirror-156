...
ax3.plot(
    [0, lkp[-1]], [.2167 / BL.eV - Ef, .2167 / BL.eV - Ef], "k:", linewidth=.5
)
...
II = np.nonzero(
    (np.abs(data.kpoints[:, 1] - data.kpoints[:, 2]) < .00001) &
    (np.abs(data.kpoints[:, 0]) < .00001)
)[0]
kp = data.kpoints[II]
lkp = np.linalg.norm(kp, axis=1)
...
kp1 = np.outer(np.linspace(0, 1, 101), np.array([0, .5, .5]))
