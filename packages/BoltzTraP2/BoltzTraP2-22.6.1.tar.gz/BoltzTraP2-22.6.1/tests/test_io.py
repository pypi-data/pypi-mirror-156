...
def test_parse_vasprunxml():
    ...
    # no unknown pieces.
    assert keys == (
        "E", "atoms", "fermi", "kpoints", "magmom", "name", "nelect", "v"
    )
...
def test_read_GENE_eneandmat_old():
    ...
    assert np.allclose(
        r[2][20], [0.3333333333E+00, 0.4166666667E-01, 0.0000000000E+00]
    )
    assert np.allclose(
        r[2][80], [0.3750000000E+00, 0.8333333333E-01, 0.4166666667E-01]
    )
    assert np.allclose(
        r[2][391], [-0.4166666667E+00, 0.4166666667E+00, 0.3333333333E+00]
    )
...
def test_parse_ESPRESSO_structure(
    ...
    lattvec = 10.2076 / BoltzTraP2.units.Angstrom * 0.5 * np.array(
        [[-1., 0., 1.], [0., 1., 1.], [-1., 1., 0.]]
    )
    ...
    lattvec = np.array(
        [
            [2.8, 0., 0.], [4.5 * cosAB, 4.5 * np.sqrt(1. - cosAB * cosAB), 0.],
            [0., 0., 4.]
        ]
    )
    assert np.allclose(lattvec, atoms.get_cell())
    fractional_positions = np.array(
        [
            [0.9475, 0.8070, 0.25], [0.0525, 0.1930, 0.75],
            [0.5274, 0.2790, 0.25], [0.4726, 0.7210, 0.75]
        ]
    )
