    ...
    def build_extensions(self):
        ...
        for system, compile_flags in self.system_specific_flags.items():
            if platform.system() == system:
                opts += [i for i in compile_flags if has_flag(self.compiler, i)]
...
# 10.9 system or above, overriding default behaviour which is to target
...
if sys.platform == "darwin":
    if "MACOSX_DEPLOYMENT_TARGET" not in os.environ:
        ...
        if Version(current_system) >= Version("10.9") > Version(python_target):
...
""",
    ...
    entry_points={"console_scripts": ["btp2 = BoltzTraP2.interface:btp2_main"]},
