...
if platform.system() != 'Windows':
    from .timeroatingfilehandler import MultiProcessSafeHandler
else:
...
def tz_fix():
    ...
    tz = re.compile(r'([+-]\d{2})(\d{2})$').match(format_time.strftime('%z'))
    if format_time.timezone and tz:
        opsym = '+' if format_time.timezone > 0 else '-'
