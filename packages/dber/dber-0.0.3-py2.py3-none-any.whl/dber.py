...
class DB(object):
    ...
    def executeScript(self, sqlScript: str, commit=True, **variables):
        """
        执行SQL脚本
        ...
        try:
            ...
            self.__printSql(sqlScript.format_map(variables), None, st)
            cursor.close()
    ...
    def executeScriptFile(self, sqlScriptFilePath: str, commit=True, **variables):
        ...
        if os.path.exists(sqlScriptFilePath) and os.path.isfile(sqlScriptFilePath):
            ...
            if sqlScript and len(sqlScript) > 0:
                self.executeScript(sqlScript, commit, **variables)
