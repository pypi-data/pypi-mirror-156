...
__version__ = "0.9.1.47"
utillc_version = __version__
__all__ = ( 'EKOX', 'STACK', 'EKO', 'TYPE', 'EKOI', 'EKOT', 'EKOH', 'EKOF', 'test', 'Bool', 'ntimes', 'LINE', 'resizeImage', 'count_parameters', 'parse')
...
def EKON(*args,n=INFO,pref="", color=False, check=False, withprint=True, llevel=0, pattern="EKOX"):
    ...
    """
    EKOX(args, llevel=1, pattern="EKON", istuple=len(args)>1)
    ...
    if prt :
        ...
        if (ff[4] != None) :
            ...
            if (isinstance(x, tuple) and doublecommainid) or istuple:
                ...
                #print("=================", tk11)
                #print("=================", "\n".join(tk1))
                ppp = expression.parse(tk11)

                if pattern == "EKOX" and ppp[0] == "" and ppp[1] == "(" :
                    ppp = ppp[2]
                
                #print("àààààààààààà", ppp)
                #print("jjjjjjjjjjjjj", expression.dump(tk11))

                tk = list(map(expression.dump, expression.listp(ppp)))
...
def parse(configin = {}) :
    ...
    tag = args.tag + '_' + str(datetime_object).replace(':', '_').replace(' ', '_')
