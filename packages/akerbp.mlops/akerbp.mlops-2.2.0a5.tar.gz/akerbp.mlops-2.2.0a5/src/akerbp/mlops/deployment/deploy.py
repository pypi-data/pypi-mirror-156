...
def deploy_model(
    ...
    try:
        ...
        if c.test_file:
            ...
            if c.platform == "cdf" and artifact_version == latest_artifact_version:
                logging.info(
        ...
        # Schedule a call for keeping the latest function warm in prod
        if env == "prod" and c.platform == "cdf":
            logging.info(
                f"Creating a schedule for keeping the function {predictable_external_id} warm on weekdays during working hours"
            )
            _ = cdf_client.functions.schedules.create(
                name="Keep warm schedule",
                description="Keep the function warm by calling it with an empty payload every 29 minutes between 8AM and 8PM on weekdays",
                cron_expression="*/29 8-19 * * 1-5",
