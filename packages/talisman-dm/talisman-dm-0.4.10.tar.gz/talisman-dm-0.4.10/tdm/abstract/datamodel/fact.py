...
class AbstractFact(Generic[_FactValue], metaclass=ABCMeta):
    ...
    def value_with_confidence(self) -> Union[ValueWithConfidence, Tuple[ValueWithConfidence, ...], None]:
        if not self._metadata or 'confidence' not in self._metadata:
            return None

        if not isinstance(self._value, tuple):
            return ValueWithConfidence(deepcopy(self.value), self._metadata['confidence'])
        return tuple(ValueWithConfidence(v, c) for v, c in zip(deepcopy(self._value), self._metadata['confidence']))
    ...
    def with_changes(self: _AbstractFact, *, status: FactStatus = None, type_id: str = None,
                     ...
                     metadata: FactMetadata = None, value_confidence: Union[float, Tuple[float, ...]] = None) -> _AbstractFact:
        pass
    ...
    def update_metadata(f: Callable[..., _AbstractFact]):
        def check(self: _AbstractFact, *args, value: Union[_FactValue, Tuple[_FactValue, ...]] = None, metadata: FactMetadata = None,
                  value_confidence: Union[float, Tuple[float, ...]] = None, **kwargs
                  ) -> _AbstractFact:
            ...
            if value_confidence is not None and value is not None:
                if isinstance(value_confidence, tuple) and isinstance(value, tuple) and (len(value_confidence) == len(value)):
                    _metadata['confidence'] = value_confidence
            ...
            elif self._metadata and 'confidence' in self._metadata:
                _metadata['confidence'] = None if value is not None else self._metadata['confidence']
