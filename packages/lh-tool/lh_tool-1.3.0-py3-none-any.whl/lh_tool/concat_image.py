    ...
    for image_file in image_file_list:
        ...
        if image is None:
            ...
            if hstack:
                ...
                padding = np.ones(padding_shape) * color
            else:
...
def main():
    ...
    parser.add_argument('-i', '--input', type=str, default='.', help='image file or path of input image files')
    parser.add_argument('-a', '--another_input', type=str, help='another image file or path of input image files')
    ...
    parser.add_argument('-v', '--vertical', dest='hstack', action='store_false', help='vertical stack')
    parser.add_argument('-w', '--white', dest='black', action='store_false', help='padding with white')
    parser.add_argument('-r', '--recursive', action='store_true', help='concatenate images recursively')
    ...
    try:
        ...
        if recursive:
            ...
            assert len(image_file_list) == len(another_image_file_list), 'The number of image files under the two ' \
                                                                         'paths is not equal'
            output_path = opts.output
            assert cover or output_path is not None, 'Output path is \'None\''
            output_path = opts.input if cover else output_path
            os.makedirs(output_path, exist_ok=True)
            output_file_list = [os.path.join(output_path, os.path.basename(image_file)) for image_file in image_file_list]
            if nprocs == 1:
