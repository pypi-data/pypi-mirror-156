...
from .component import Component, ActionRow, _get_component_type, InputText
...
class Interaction:
    def __init__(
        ...
        *,
        ...
        if raw_data["data"].get("components") and raw_data["data"]["components"][0]["components"][0]["type"] == 4:
            self.values = [ i["components"][0].get("value") for i in raw_data["data"]["components"]]
            components = []
            for i in raw_data["data"]["components"]:
                components.append(i["components"][0])
            components = {
                "custom_id" : raw_data["data"]["custom_id"],
                "components": components
            }
            self.data_model = InputText.from_json(components)
        else:
            self.values: List[str] = raw_data["data"].get("values", [])
        self.component_type: int = raw_data["data"].get("component_type", 4)
    ...
    async def respond_model(self, components):
