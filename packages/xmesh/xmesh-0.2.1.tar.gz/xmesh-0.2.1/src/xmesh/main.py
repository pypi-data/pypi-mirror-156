...
def ask_for_port(type):
    ...
    portslist = comports()
    if type == 'sniffer':
      ports = [port.name for port in portslist if port.serial_number == 'MX0101']
    elif type == 'proxy':
      ports = [port.name for port in portslist if port.serial_number == 'MX0201']
    ...
    for n, port in enumerate(sorted(comports()), 1):
        sys.stderr.write(f'--- {n}: {port.name}\n')
