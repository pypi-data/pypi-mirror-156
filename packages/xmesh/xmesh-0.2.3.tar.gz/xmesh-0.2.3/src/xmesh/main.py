...
class Miniterm(object):
    ...
    def start(self):
        ...
        self.alive = True
        logdir = os.path.join(str(Path.home()), 'xMesh-Assistant')
        if not os.path.exists(logdir):
            os.mkdir(logdir)
        self.logpath = os.path.join(logdir, f'{time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime())}.raw')
...
def main(default_type='sniffer', default_port=None, default_baudrate=921600, default_rts=None, default_dtr=None, serial_instance=None):
    """Command line tool, entry point"""

    import argparse

    parser = argparse.ArgumentParser(
        description='xMesh - xMesh Assistant.')
    ...
    while serial_instance is None:
        ...
        if args.port is None or args.port == '-':
            try:
                args.port = ask_for_port(args.type)
            except KeyboardInterrupt:
                sys.stderr.write('\n')
    ...
    if not args.quiet:
        sys.stderr.write('--- xMesh Assistant on {p.name}  {p.baudrate},{p.bytesize},{p.parity},{p.stopbits} ---\n'.format(
            p=miniterm.serial))
    ...
    if not args.quiet:
        sys.stderr.write('\n--- exit ---\n')
        miniterm.logfile.close()
        ansi_escape = re.compile(r'\x1B(?:[@-Z\\-_]|\[[0-?]*[ -/]*[@-~])')
        with open(miniterm.logpath.replace('.raw', '.log'), 'w') as logfile:
            with open(miniterm.logpath, 'r') as rawfile:
                for line in rawfile:
                    logfile.write(ansi_escape.sub('', line))
        os.remove(miniterm.logpath)
    miniterm.join()
    miniterm.close()
