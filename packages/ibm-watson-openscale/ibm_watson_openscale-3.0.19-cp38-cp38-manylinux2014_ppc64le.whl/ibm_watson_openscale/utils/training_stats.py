    ...
    def __generate_input_data_schema(self):
        ...
        for f_name in schema:
            ...
            if "object" in data_type:
                ...
                if type(self.training_data_frame[f_name][0]) is int:
                    f_type = "long"
                else:
                    f_type = "string"
            ...
            if f_name in self.feature_columns:
                field["metadata"]["modeling_role"] = "feature"

                # Check for type and add to  categorical columns if not set by user
                if f_type == "string" and f_name not in self.categorical_columns and f_name != self.label_column:
                    self.categorical_columns.append(f_name)

                # Set categorical column in input schema
                if f_name in self.categorical_columns:
