...
class OWPropagate2DModesPythonScript(widget.OWWidget):

    name = "Propagate 2D Modes Python Script"
    description = "Propagate 2D Modes  Python Script"
...
def to_python_code(self, # self is beamline
    ...
    if isinstance(self.get_light_source(), WOLightSourceCOMSYL):

        full_text_code += "\n" + indent + 'filename = "%s"' % self.get_light_source().get_filename()
        full_text_code += "\n" + indent + "af_oasys = CompactAFReader.initialize_from_file(filename)"
        full_text_code += "\n" + indent + ""
        full_text_code += "\n" + indent + "output_wavefront = af_oasys.get_wavefront(my_mode_index,normalize_with_eigenvalue=1)"
    ...
    full_text_code += "\n" + indent + "from orangecontrib.comsyl.util.tally2d import Tally2D"
    full_text_code += "\n" + indent + ""
    full_text_code += "\n" + indent + "tally = Tally2D(do_store_wavefronts=False)"
    ...
    if graph_file_flag == 0:
        ...
        dump_file = "%s.png" % root_file_name
    elif graph_file_flag == 2:
        dump_file = "%s.pdf" % root_file_name
...
if __name__ == "__main__":
    ...
    ow = OWPropagate2DModesPythonScript()
    ow.show()
    a.exec_()
    ow.saveSettings()