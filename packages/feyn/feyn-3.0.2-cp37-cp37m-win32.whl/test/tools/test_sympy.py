...
class TestTools(unittest.TestCase):
    ...
    def test_special_characters(self):
        model = quickmodels.get_simple_binary_model(["a2+4/5-7~", "sex: reg.*(not_verified)"], "charges", stypes={"charges": "b"})
    ...
    def test_numerical_input_names(self):
        model = quickmodels.get_simple_binary_model(["245", "357"], "1337", stypes={"357": "c"})
    ...
    def test_duplicate_category_names(self):
        model = quickmodels.get_simple_binary_model(["357", "357"], "1337", stypes={"357": "c"})
...
def _predict_sympy_all(expr, samples, model, signif=15, symbolic_cat=True):
    ...
    for i in range(length):
        ...
        substitutions = get_sympy_substitutions(model, sample, symbolic_cat=symbolic_cat)
