...
from .render._truncate_inputs import truncate_input_names
...
class SVGGraphToolkit:
    ...
    def add_input_table(self, model):
        from .render._truncate_inputs import _isolate_interesting_regions
        ...
        boxw = max([Theme._get_string_length(value, size=content_size) for value in inputs])

        table = self.drawing.svg(
            insert=(0, 0), size=(boxw + self.margin, boxh)
        )
        ...
        trunc_size = 8
        regions = _isolate_interesting_regions(inputs, trunc_size=trunc_size, lb=3)
        ...
        for idx, name in enumerate(inputs):
            ...
            if len(name) <= trunc_size:
                ...
                self._add_bold_text(table, name, start, end, insert=(text_indent, entry_y), size=content_size)
    ...
    def add_summary_information(self, metrics, title="", short=False):
        ...
        content_width = max(Theme._get_string_length(title, size="large"), max_metric_width * font_size)
    ...
    def add_graph(self, model, label=None, show_loss=True, show_sources=False):
        ...
        for ix, elem in enumerate(model):
            ...
            if elem.name in model.inputs:
                ...
                if "gaussian" in elem.fname:
                    trunc_name = "gaussian"
                else:
                    trunc_name = elem.fname[:8]
                    if len(trunc_name) > 8:
