...
class FeynTheme:
    ...
    cycle = ['majorelle_blue', 'hot_magenta', 'robin_egg_blue', 'safety_orange', 'guppie_green', 'spiro_disco_ball', 'golden_yellow']
...
def _register_linear_cmaps(cmaps):
    ...
    for k, v in cmaps.items():
        # FIXME: this is not available in old versions and adviced not to use it?
        # see: https://matplotlib.org/stable/api/cm_api.html#matplotlib.cm.unregister_cmap
        if "unregister_cmap" in dir(cm):
