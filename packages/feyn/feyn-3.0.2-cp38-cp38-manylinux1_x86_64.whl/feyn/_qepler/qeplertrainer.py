...
def fit_models(
    ...
    orbits = [get_orbits(dnames=m.names, fnames=m.fnames, params=m.params, sample_count=m._sample_count, loss=loss) for m in models]
    ...
    def fitting_thread():
        ...
        while True:
            ...
            o = orbits[ix]
            new_losses[ix], new_params[ix] = o._fit(xarray=sampled_data, yarray=sampled_data[output], sample_weights=sample_weights)
...
def get_orbits(
    ...
    orbit = _qepler.Model(dnames, fnames, sample_count)

    orbit.params = params
    orbit._loss = loss
    return orbit

def _resample_data(data: DataFrame, sample_weights, n_samples):
    ...
    n_samples = max(n_samples, n_data_rows)
