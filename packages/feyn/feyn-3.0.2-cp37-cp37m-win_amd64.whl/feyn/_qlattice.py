...
class QLattice(QLatticeNotebookMixin if detect_notebook() else object):
    ...
    def registers(self):
        ...
        return list(self._context.registers)
    ...
    def sample_models(
        ...
        try:
            query = Query(self._context, query_string, max_complexity, input_names, function_names, output_name)
        ...
        programs = self._basealgo.generate_programs(
            query
        )

        for p in programs:
            model = self._context.to_model(p, output_name=output_name, stypes=stypes)
            if model is None:
    ...
    def reset(self, random_seed=-1):
        ...
        log.warning("Deprecation: The reset() function is deprecated. Instantiating a new feyn.QLattice() now achieves the same result.")
    ...
    def update_priors(self, priors: Dict, reset: bool = True):
        ...
        """
        priors = [(self._context.lookup_by_fname(key, 0), val) for key, val in priors.items()]
...
def connect_qlattice(
    ...
    log = logging.getLogger(__name__)
    log.warning("Deprecation: The connect_qlattice() is deprecated. Use the `feyn.QLattice()` constructor instead.")
