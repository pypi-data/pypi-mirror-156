...
def plot_model_response_1d(
    ...
    if by is None:
        if len(model.features) == 1:
            ...
            raise ValueError("'by' argument is required for models of more than one feature.")
    ...
    output_actuals = data[model.output].astype('float64')

    actual_axes = [data[by], output_actuals]
...
def _determine_categories(model):
    return [elem.name for elem in model if elem.fname == "in:cat"]
