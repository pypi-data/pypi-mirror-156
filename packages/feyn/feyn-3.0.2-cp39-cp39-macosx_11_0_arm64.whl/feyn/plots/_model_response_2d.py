...
def plot_model_response_2d(
    ...
    if filename:
        return _save_plot(
            plot_model_response_2d, filename, fig, model=model, data=data, fixed=fixed, resolution=resolution
        )
...
class PartialPlot2D:
    ...
    def _replace_registers(self):
        ...
        for idx, elem in enumerate(self.model):
            ...
            if "categories" in param and xy_input:
                new_params.append({"scale": 1, "scale_offset": 0, "w": 1, "bias": param["bias"]})
                new_fnames.append("in:linear")
            else:
                new_params.append(param.copy())
                new_fnames.append(fname)

        self.model = feyn.Model(self.model._program.copy(), self.model.names[:], new_fnames, new_params)
    ...
    def _numdat_for_categoricals(self):
        ...
        for name, state in self.categoricals:
            ...
            self.data[name + "_num"] = self.data[name].apply(lambda c, w=weights_dict: w[c])

    def _labels_ticks(self):
        """Figure out axes labels and ticks."""

        x_fname, x_state = self.model.fnames[self.x_idx], self.model.params[self.x_idx].copy()
        y_fname, y_state = self.model.fnames[self.y_idx], self.model.params[self.y_idx].copy()

        def _term_to_range(term_fname, term_state, term_name):
            if term_fname == "in:cat":
