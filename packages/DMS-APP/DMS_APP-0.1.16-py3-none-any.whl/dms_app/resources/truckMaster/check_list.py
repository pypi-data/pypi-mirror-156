...
class CheckListApproval(Resource):
	...
	def get(self):
		try:
			...
			if trip_id:
				data = checklist_collection.find({"trip_id": {'$regex': '^{trip_id}'.format(trip_id=trip_id),
                     '$options': 'mi'}})
				count = checklist_collection.count_documents({"trip_id": {'$regex': '^{trip_id}'.format(trip_id=trip_id),
                     '$options': 'mi'}})
	...
	def post(self):
		...
		try:
			...
			data1 = checklist_coll.find_one({"trip_id": trip_id})
			if not data1:
