...
class CheckListApproval(Resource):
	...
	def get(self, *args):
		try:
			...
			if trip_id:
				data = list(checklist_collection.find({"trip_id": {'$regex': '^{trip_id}'.format(trip_id=trip_id), '$options': 'mi'}}).skip(
				...
				data = list(checklist_collection.find().skip(int(page_limit) * (int(page_no) - 1)).limit(int(page_limit)))
				count = checklist_collection.count_documents({})
				if len(data):
					_response = get_response(200)
					_response["data"] = json.loads(json_util.dumps(data))
	...
	def get(self, *args):
		try:
			...
			if trip_id:
				...
				data = list(checklist_collection.find({"check_status": {"$ne": "failed"}}).skip(
					int(page_limit) * (int(page_no) - 1)).limit(int(page_limit)))
