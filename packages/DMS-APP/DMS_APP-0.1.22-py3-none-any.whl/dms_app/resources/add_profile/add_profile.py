...
search_employee.add_argument("person_id", type=str, help="Person Id")
...
class SearchEmployee(Resource):
    ...
    def get(self, *args):
        try:
            ...
            data = list(person_profile_col.find({
                     "$or": [{"person.person_id": {'$regex': '^{email}'.format(email=args["person_id"]), '$options': 'mi'}},
                             {"person.name": {'$regex': '^{first_name}'.format(first_name=args["person_id"]),
                                             '$options': 'mi'}}]}))
            _response = get_response(200)
            if data:
                _response["data"] = json.loads(json_util.dumps(data))
    ...
    def get(self, *args):
        try:
            ...
            if args["person_name"] and args["status"]:
                ...
                data = list(person_profile_col.find({"person.status": args["status"]}).skip(
                ...
                count = person_profile_col.count_documents({})
                data = list(person_profile_col.find({}).skip(args["page_limit"] * (args["page_no"] - 1)).limit(
                    args["page_limit"]))
                if len(data):
                    _response = get_response(200)
                    _response['count'] = json.loads(json_util.dumps(count))
