...
class AddRolePrivileges(Resource):
	...
	def get(self, *args):
		try:
			...
			data = list(rolesPrivileges_col.find(
				{"role_name": role_name}).skip(int(page_limit) * (int(page_no) - 1)).limit(int(page_limit)))
			if len(data):
...
class GetAllRolesPrivileges(Resource):
	...
	def get(self, *args):
		try:
			...
			database_connection = database_access()
			rolesPrivileges_col = database_connection["roles&privileges"]
			data = list(rolesPrivileges_col.find().skip(int(page_limit) * (int(page_no) - 1)).limit(int(page_limit)))
