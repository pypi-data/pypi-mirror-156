...
class MainArgumentParser:
    def __init__(self):
        parser = argparse.ArgumentParser(
            description="A pipeline from mutation to neoantigen prediction",
        ...
        run_whole = subparsers.add_parser(
            "whole",
            help="Run whole pipeline(Seq2Neo) with fastq/bam/sam file",
            add_help=False
        )
        ...
        run_immunoprediction = subparsers.add_parser(
            "immuno",
            help="Run immunogenicity prediction with peptides",
            add_help=False
        )
        ...
        run_download = subparsers.add_parser(
            "download",
            help="downloading reference genome",
            add_help=False
        )
