...
class Tag:
    ...
    @classmethod
    def create_tag(cls, tag_type, start, line, content, continuation):
        #print(f"Content of the new tag: {content} + continuation: {continuation}")
        continuation_len = 0
        attributes = {}
        if 'link' in tag_type:
        ...
        tag = Tag(tag_type, start, line, len(content) + continuation_len, attributes)
        return content, tag
