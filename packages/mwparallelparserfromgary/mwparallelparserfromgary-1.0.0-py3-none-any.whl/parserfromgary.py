...
class Parser:
    ...
    # Looking for templates, adding them to metatags and deleting from self.text
    ...
    # Creating a tag and appending the list of tags
    ...
    def save(self):
        with open('result_tags.txt', 'w+', errors="ignore") as f:
            for i, tag in enumerate(self.tags):
                f.write(str(tag) + '\n')
        with open('result_metatags.txt', 'w+', errors="ignore") as f:
            for i, tag in enumerate(self.tags_meta):
                f.write(str(tag) + '\n')
