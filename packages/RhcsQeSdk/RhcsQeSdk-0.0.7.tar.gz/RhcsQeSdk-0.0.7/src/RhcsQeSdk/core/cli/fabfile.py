...
from src.RhcsQeSdk.core.utilities.metadata import Metadata
...
def parse_output(result, cmd=None):
    ...
    if issubclass(type(result), Exception):
        logger.error(f"Error occured while running '{cmd}'")
        raise Exception(f"Error occured while running '{cmd}'")
    out_string = str(result)
    skip_strings = ["already"]
    if result.__dict__.get("failed"):
        for s in skip_strings:
            if s not in out_string:
                raise Exception()
    return result
