...
cdef class BitStreamFilterContext(object):
    ...
    def set_input_stream(self, Stream stream):
        cdef lib.AVCodecContext *codec_context_ptr = stream.codec_context.ptr
        cdef int res
        with nogil:
            self.ptr.time_base_in = codec_context_ptr.pkt_timebase
            res = lib.avcodec_parameters_from_context(self.ptr.par_in, codec_context_ptr)
            if res >= 0:
    ...
    def set_output_stream(self, Stream stream):
        ...
        cdef int res
        with nogil:
            stream.ptr.time_base = self.ptr.time_base_out
            res = lib.avcodec_parameters_to_context(codec_context_ptr, self.ptr.par_out)
        err_check(res)

    def send(self, Packet packet=None):

        cdef bint end_of_stream = packet is None
        ...
        with nogil:
            if end_of_stream:
                ...
                # Make another reference to the packet, as the bitstream filter
                # takes ownership of the reference.
                res = lib.av_packet_ref(pkt, packet.ptr)
            if res >= 0:
