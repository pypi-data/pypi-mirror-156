...
class CLIContext(TransformAuth):
    ...
    def remove_api_key_and_prompt_browser_login(self) -> None:
        ...
        if self.config.get_config_value(API_KEY_CONFIG_KEY) is not None:
            self.config.remove_config_value(API_KEY_CONFIG_KEY)
    ...
    def pinned_model_id(self, model_id: int) -> None:  # noqa: D
        if not model_id:
            self.config.remove_config_value(PINNED_MODEL_ID_CONFIG_KEY)
    ...
    def resolve_query_model_key(self, config_dir: Optional[str] = None) -> Optional[ModelKey]:
        ...
        * Otherwise, we default to None so that the query server can resolve the proper model key for the org.
        """
        if config_dir:
            last_config_path = self.cli_state.get_config_value(PREV_CONFIG_DIR_PATH)
            last_config_modified = self.cli_state.get_config_value(PREV_CONFIG_DIR_MODIFIED)
            last_config_model_id = self.cli_state.get_config_value(PREV_MODEL_ID)

            current_config_path = str(Path(config_dir).resolve())
            current_config_modified = directory_last_edited(config_dir)

            if (
                last_config_path is not None
                ...
                click.echo(f"Using local model id {last_config_model_id}, created {model_key.created_at}")
                return model_key
            ...
            if current_config_modified:
                ...
                self.cli_state.remove_config_value(PREV_CONFIG_DIR_MODIFIED)
