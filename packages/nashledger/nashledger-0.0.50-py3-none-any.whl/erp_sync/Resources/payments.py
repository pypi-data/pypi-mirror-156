        ...
        if operation == super().NEW or operation == super().UPDATE:
            ...
            if super().get_client_type() == super().QBO:
                ...
                if 'amount' in payload.keys():
                    data.update({
                        "TotalAmt": int(payload.get("amount", 0))
                    })
                ...
                for i in range(len(invoices)): 
                    if i == 0:
                        if 'reference' in invoices[i].keys():
                            data.update({
                                "PaymentRefNum": invoices[i].pop('reference')
                            })
                    ...
                    if 'amount' in invoices[i].keys():
                        invoice.update({
                            "Amount": int(invoices[i].get("amount", 0))
                        })
            ...
            # If client type is ZOHO
                ...
                for i in range(len(invoices)): 
                    if i == 0:
                        if 'reference' in invoices[i].keys():
                            payment.update({
                                "reference_number": invoices[i].pop('reference')
                            })
                    if 'invoice_id' in invoices[i].keys():
                ...
                for i in range(len(invoice_payments)):
                    if 'payment_date' in invoice_payments[i].keys():
                        invoice_payments[i]['PaymentDate'] = invoice_payments[i].pop(
                            'payment_date')
                    if 'reference' in invoice_payments[i].keys():
                    ...
                    if 'amount' in invoice_payments[i].keys():
                        invoice_payments[i]['Amount'] = int(invoice_payments[i].pop('amount'))
            ...
            # If client type is ZOHO
                ...
                if bool(invoice):
                    ...
                    if 'amount' in invoice[0].keys():
                        payment.update({
                            "Amount": int(invoice[0].pop('amount'))
                        })
                    if 'reference' in invoice[0].keys():
                        payment.update({
                            "Reference": invoice[0].pop('reference')
                        }) 
            ...
            # If client type is ODOO
                ...
                data = {
                    ...
                    "date": payload.get("date", ""),
                    "amount": int(payload.get("amount", 0)),
                    "partner_id": int(payload.get("customer_id", 0)),
                ...
                for i in range(len(invoices)):                    
                    if i == 0:
                        if 'reference' in invoices[i].keys():
                            data['payment_reference'] = invoices[i].pop('reference')

                    if "invoice_id" in invoices[i].keys():
                        invoices[i]["invoice_id"] = invoices[i].pop("invoice_id")
                    if "amount" in invoices[i].keys():
                        invoices[i]["amount"] = int(invoices[i].pop("amount"))
            ...
            # If client type is MS_DYNAMICS
                ...
                if bool(invoice):
                    ...
                    if 'reference' in invoice[0].keys():
                        data.update({
                            "paymentReference": invoice[0].pop('reference')
                        })
                    if 'amount' in invoice[0].keys():
                        data.update({
