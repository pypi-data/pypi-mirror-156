...
def send_metadata_to_redshift(database: Optional[str] = None, file_path: str = "table_metadata.json"):
    print(f"🎬 Reading the {file_path} ...")
    table_metadata_dict = json.load(open(file_path))
    ...
    for item in list(table_metadata_dict.items()):
        print(f"\t📌 {item[0].capitalize()}:")
        table_description_str = f"Country: {table_metadata_dict[item[0]]['description']['country']}\n\n" \
                                f"Table Name: {table_metadata_dict[item[0]]['description']['table_name']}\n\n" \
                                f"Table Schema: {table_metadata_dict[item[0]]['description']['table_schema']}\n\n" \
                                f"Description: {table_metadata_dict[item[0]]['description']['description']}\n\n" \
                                f"Source Name: {table_metadata_dict[item[0]]['description']['source_name']}\n\n" \
                                f"Geographical Coverage: {table_metadata_dict[item[0]]['description']['geographical_coverage']}\n\n" \
                                f"Geographical Granularity: {table_metadata_dict[item[0]]['description']['geographical_granularity']}\n\n" \
                                f"Update Frequency: {table_metadata_dict[item[0]]['description']['update_frequency']}\n\n" \
                                f"Last Update: {table_metadata_dict[item[0]]['description']['last_update']}\n\n" \
                                f"Time Frame Coverage: {table_metadata_dict[item[0]]['description']['time_coverage']}\n\n" \
                                f"Caveats: \n" \
                                f"- Additional Info: {table_metadata_dict[item[0]]['caveats']['additional_information']}\n\n" \
                                f"- Raw Data Link: {table_metadata_dict[item[0]]['caveats']['raw_data_link']}\n\n" \
                                f"- Related OIP Block: {table_metadata_dict[item[0]]['caveats']['related_block']}\n\n" \
                                f"- Update Process: {table_metadata_dict[item[0]]['caveats']['update_process']}\n\n" \
                                f"- Import Format: {table_metadata_dict[item[0]]['caveats']['import_format']}\n\n" \
                                f"- Import Separator: {table_metadata_dict[item[0]]['caveats']['import_separator']}\n\n" \
                                f"- Table Type: {table_metadata_dict[item[0]]['caveats']['table_type']}\n\n" \
                                f"- Tags: {table_metadata_dict[item[0]]['caveats']['tags']}"

        table_description_sql = f"COMMENT ON table {table_metadata_dict[item[0]]['description']['table_schema']}." \
                                f"{table_metadata_dict[item[0]]['description']['table_name']} IS " \
                                f"'{table_description_str}';"

        columns_description_sql_lst = []
        for key, value in table_metadata_dict[item[0]]["columns_description"].items():
            sql = f"COMMENT ON column {table_metadata_dict[item[0]]['description']['table_schema']}." \
                  f"{table_metadata_dict[item[0]]['description']['table_name']}." \
                  f"{key} IS '{value}';"
        ...
        print(f"\t\t🎉 Metadata insertion finished for {table_metadata_dict[item[0]]['description']['table_name']}")
