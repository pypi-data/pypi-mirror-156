...
"""
    ...
    def get_modified_files(self, repo, current_branch, compliant_branch) -> Set[str]:
        ...
        current_commit, previous_commit = self.get_relevant_commits(repo, current_branch, compliant_branch) 
        ...
        res = self.extract_paths_from_diff(diff, repo_working_dir = repo.working_dir, repo_working_tree_dir = repo.working_tree_dir, repo_git_dir = repo.git_dir)
        return res
    ...
    def extract_paths_from_diff(self, diff, repo_working_dir, repo_working_tree_dir, repo_git_dir):
            ...
            if len(self.config.component_validation) > 0:
                ...
                for jsonpath, regex in self.config.component_validation.items():
                    customized_validation_success = (
                        customized_validation_success
                        if self.customized_validation(jsonpath, regex, component, self.config.fail_if_pattern_not_found_in_component_validation)
                        else False
                    )
    ...
    @staticmethod
    def customized_validation(jsonpath: str, regex: str, component: str, fail_if_pattern_not_found_in_component_validation: bool) -> bool:
        """
    ...
    def add_conda_to_system_path(self) -> None:
        """
        This function will add the conda directory to the system path,
        if the `prepare` command is run from a GitHub Action or from an
        Azure DevOps build. It supports both Windows and Linux operating
        systems.
        """
        # let's figure out the OS
        operating_system  = self.determine_operating_system()
        # let's figure out the runtime environment
        runtime_environment = self.determine_runtime_environment()
        # let's use those values to add the conda directory to the path in the appropriate manner
        self.add_conda_to_path(operating_system, runtime_environment)
        
    def azure_devops_append_to_path(self, value: str):
        """
        Azure DevOps doesn't support a built-in command for appending values
        to the `PATH` environment variable. It's dangerous to _prepend_ Conda
        to the path, since the current Python interpreter may not be Conda-managed.
        For more context, see:
        - https://stackoverflow.microsoft.com/questions/311834
        - https://stackoverflow.com/a/57257864
        """
        current_path = os.environ["PATH"]
        print(f"##vso[task.setvariable variable=PATH]{current_path}:{value}")

    def add_conda_to_path(self, operating_system, runtime_environment) -> None:
        """
        This function will add the conda directory to the system path,
        based on the 'operating_system' and 'runtime_environment'.
        For more details, see the following links:
        https://docs.microsoft.com/en-us/azure/devops/pipelines/ecosystems/anaconda?view=azure-devops&tabs=ubuntu#add-conda-to-your-system-path
        and https://docs.github.com/en/actions/using-workflows/workflow-commands-for-github-actions#adding-a-system-path
        """
        success = False
        path = os.environ["PATH"]
        if operating_system == OperatingSystem.WINDOWS:
            if runtime_environment == RuntimeEnvironment.GITHUB_ACTION:
                Path(os.environ["GITHUB_PATH"]).write_text(os.path.join(os.environ["CONDA"], "condabin"))
                success = True
            elif runtime_environment == RuntimeEnvironment.AZURE_DEVOPS_BUILD:
                self.azure_devops_append_to_path(os.path.join(os.environ["CONDA"], "Scripts"))
                success = True
            else:
                log.info(f"Unsupported runtime environment '{runtime_environment}'.")
        elif operating_system == OperatingSystem.LINUX:
            if runtime_environment == RuntimeEnvironment.GITHUB_ACTION:
                Path(os.environ["GITHUB_PATH"]).write_text(os.path.join(os.environ["CONDA"], "bin"))
                success = True
            elif runtime_environment == RuntimeEnvironment.AZURE_DEVOPS_BUILD:
                self.azure_devops_append_to_path(os.path.join(os.environ["CONDA"], "bin"))
                success = True
            else:
                log.info(f"Unsupported runtime environment '{runtime_environment}'.")
        else:
            log.info(f"Unsupported operating system '{operating_system}'.")

        if success:
            log.info(f"Successfully added conda to the system path for runtime environment '{runtime_environment}' and operating system '{operating_system}'.")
        else:
            log.error(f"Failed to add conda to the system path for runtime environment '{runtime_environment}' and operating system '{operating_system}'.")
        
        
    @staticmethod
    def determine_operating_system() -> OperatingSystem:
        """
        This function will determine the operating system of the
        current machine ('Windows', 'Linux', or 'Other').
        """
    ...
    def determine_runtime_environment(self) -> RuntimeEnvironment:
        """
        This function will determine the runtime environment of the
        current machine ('GitHub Action', 'Azure DevOps Build', or 'Other').
        """
        github_ev = self.get_specific_ev(target = "github")
        azure_ev = self.get_specific_ev(target = "azure")
        runtime_environment = self.determine_runtime_environment_based_on_environment_variables(github_ev, azure_ev)
        return runtime_environment

    @staticmethod
    def get_specific_ev(target: str) -> Union[str, None]:
        """
        This function will return the contents of an environment variable
        relevant to the 'target' runtime environment.
        """
        if target == "github":
            return os.environ.get("GITHUB_REPOSITORY")
        elif target == "azure":
            return os.environ.get("BUILD_BUILDID")
        else:
            return ""

    @staticmethod
    def determine_runtime_environment_based_on_environment_variables(github_ev: Union[str, None], azure_ev: Union[str, None]) -> RuntimeEnvironment:
        """
        This function will use the values of the runtime environment-specific
        environment variables 'github_ev' and 'azure_ev' to determine the
        runtime environment of the current machine ('GitHub Action',
        'Azure DevOps Build', or 'Other').
        """
        if (github_ev and azure_ev):
            raise ValueError("It seems like both the GitHub- and Azure-specific environment variables are non-empty. This shouldn't be possible and implies shrike should be using alternate environment variables to determine the runtime environment. Please open an issue in the GitHub shrike repository: https://github.com/Azure/shrike/issues.")
        elif github_ev:
            return RuntimeEnvironment.GITHUB_ACTION
        elif azure_ev:
            return RuntimeEnvironment.AZURE_DEVOPS_BUILD
        else:
