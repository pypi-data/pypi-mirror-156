...
class AMLPipelineHelper:
    ...
    def _get_component_name_from_instance(self, component_instance):
        ...
        https://github.com/Azure/shrike/issues/411
        """
    ...
    def _update_hdi_dc_configs(self, pipeline):
        if not self.config.compute.hdi_dc_target:
            log.info("This pipeline doesn't involve HDI DC.")
            return pipeline
        ...
        for n in nodes:
            ...
            for input in n.inputs.values():
                try:
                    owner = input._get_data_owner()
                    if owner:
                        graph[n._instance_id].append(owner._instance_id)
                except AttributeError:
                    continue
        sorted_node_ids = toposort_flatten(graph)
        use_hdi_dc = defaultdict(bool)
        for id in sorted_node_ids:
            if id not in node_mapping:
                continue
            node = node_mapping[id]
            if (
                node.type != "ScopeComponent"
                and node.runsettings.target == self.config.compute.hdi_dc_target
            ):
                use_hdi_dc[id] = True
                continue
            for parent in graph[id]:
                if use_hdi_dc[parent]:
                    use_hdi_dc[id] = True
                    break

        for node in nodes:
            if use_hdi_dc[node._instance_id]:
                ...
                if node.runsettings.target == self.config.compute.hdi_prod_target:
                    ...
                    continue
                log.info(
                    f"Updating compute target for {node.name} {node._instance_id} to {node.runsettings.target}"
                )
        return pipeline
    ...
    def build_and_submit_new_pipeline(self):
        ...
        log.info("Overriding compute target if upstream steps use HDI DC...")
        pipeline = self._update_hdi_dc_configs(pipeline)
