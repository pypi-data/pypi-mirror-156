...
class KFT :
    ...
    def getOCRText(self, zone=False, onError=False, tesseract_path='C:\\Program Files (x86)\\Tesseract-OCR\\tesseract.exe') :
        ...
        try :
            ...
            if onError != False :
                return onError
            self.writeLog(text="getOCRText(pos="+zone(zone)+", onError=\""+onError+"\")", type=self.ERROR, comment=str(e))
            raise NameError(str(e))


        if self.VERBOSE_MODE == True :
            self.writeLog(text="getOCRText(pos="+zone(zone)+", onError=\""+onError+"\")", type=self.VERBOSE)
