...
class KFT :
    ...
    def getAllZone(self, img=False, timeout=DefaultTimeout, correlation=DefaultCorrelation, parentZone=DefaultParentZone, onError=False) :
        ...
        locationVisible = self.isVisibleBis(img, correlation, parentZone)
    ...
    def isVisibleBis(self, img=False, correlation=DefaultCorrelation, parentZone=DefaultParentZone) :
        if img == False:
            self.writeLog(text="isVisible(img=\""+str(img)+"\", correlation="+str(parentZone)+", parentZone="+str(parentZone)+")", type=self.ERROR, comment=self.ERROR_IMG_NOT_DEFINED + "")
            raise Exception(self.ERROR_IMG_NOT_DEFINED)
    ...
    def clickOn(self, img=False, timeout=DefaultTimeout, onError=False, ref=CENTER, at=[0,0], correlation=DefaultCorrelation, parentZone=DefaultParentZone, mouse_btn='left') :

        positions = self.getAllPosition(img, timeout, onError, ref, at, correlation, parentZone)

        for pos in positions:
            self.click(pos=pos, mouse_btn=mouse_btn)
    ...
    def getOCRText(self, zone=False, onError=False, tesseract_exe='C:\\Program Files (x86)\\Tesseract-OCR\\tesseract.exe') :
        
        screenshot = ImageGrab.grab(bbox=(zone[0][0],zone[0][1],zone[1][0],zone[1][1]))
        screenshot.save(tempfile.gettempdir() + '/m_screen_ocr.png')
        #print(tempfile.gettempdir() + '/m_screen_ocr.png')
        #pytesseract.pytesseract.tesseract_cmd=r'C:Program FilesTesseract-OCRtesseract.exe'
        try :
            pytesseract.pytesseract.tesseract_cmd = r''+tesseract_exe+''
            text = pytesseract.image_to_string(Image.open(tempfile.gettempdir() + '/m_screen_ocr.png'))

        except Exception as e :
            if onError != False :
                return onError
            self.writeLog(text="getOCRText(pos="+zone(zone)+", onError=\""+onError+"\", tesseract_exe=\""+tesseract_exe+"\")", type=self.ERROR, comment="Tesseract.exe not found")
            raise Exception(str(e))


        if self.VERBOSE_MODE == True :
            self.writeLog(text="getOCRText(pos="+zone(zone)+", onError=\""+onError+"\", tesseract_exe=\""+tesseract_exe+"\")", type=self.VERBOSE)
        return text[:-2]
    ...
    def replaceText(self, pos, text="", interval=0) :
        self.click(pos)
        pyautogui.hotkey("ctrl", "a")
