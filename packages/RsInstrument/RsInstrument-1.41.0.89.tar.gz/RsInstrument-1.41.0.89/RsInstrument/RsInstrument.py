...
class RsInstrument:
	"""Root class for remote-controlling instrument with SCPI commands."""
	_driver_version_const = '1.41.0.89'
	...
	def __init__(
		...
		Parameter options tokens examples:
			...
			- ``OpenTimeout=5000`` - sets timeout used at the session opening. Default: ``0``
	...
	def query_str_list(self, query: str) -> List[str]:
		...
		return self._core.io.query_str_list(query)
	...
	def query_str_list_with_opc(self, query: str, timeout: int = None) -> List[str]:
		...
		If you do not provide timeout, the method uses current opc_timeout."""
		return self._core.io.query_str_list_with_opc(query, timeout)
