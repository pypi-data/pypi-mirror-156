...
def vTrue(variable=None, type=None, types=None):
    '''
    check variable if True
    variable types: Boolean, Numbers, String, Tuple, List, Dictionary
    '''
    try:
        if type == 'function':
...
def stat_is(target=None, type=None, **kwargs):
    ''' 检查目标类型 '''
    try:
        _stat = Path(target)
        match True:
...
def retry(times=None, func=None, *args, **kwargs):
    ...
    '''
    try:
        if type(times) == int and times >= 0 and vTrue(func):
            _num = 0
            while True:
...
def datetime_now(*args, utc=None, **kwargs):
    ''' 获取日期和时间 '''
    try:
        return datetime.utcnow(*args, **kwargs) if utc == True else datetime.now(*args, **kwargs)
    except Exception as e:
        logger.exception(e)
        return None

def datetime_offset(date_time=None, utc=None, *args, **kwargs):
    ''' 获取 向前或向后特定日期时间 的日期和时间 '''
    try:
        if isinstance(date_time, datetime) == True:
            return date_time + timedelta(*args, **kwargs)
        else:
            return datetime.utcnow() + timedelta(*args, **kwargs) if utc == True else datetime.now() + timedelta(*args, **kwargs)
    except Exception as e:
        logger.exception(e)
        return None
...
def bash(sh='/bin/bash', cmd=None, isfile=False, **kwargs):
    ''' run bash command or script '''
    try:
        match True:
            case True if not stat_is(sh, 'file'):
                return None
...
def delete_files(files=None, **kwargs):
    ...
    try:
        ...
        if vTrue(files, str) and stat_is(files, 'file'):
            ...
            for _file in files:
                ...
                if vTrue(_file, str) and stat_is(_file, 'file'):
                    try:
                        remove(_file)
...
def delete_dirs(dirs=None, **kwargs):
    ...
    try:
        ...
        if vTrue(dirs, str) and stat_is(dirs, 'dir'):
            ...
            for _dir in dirs:
                ...
                if vTrue(_dir, str) and stat_is(_dir, 'dir'):
                    try:
                        rmtree(_dir)
