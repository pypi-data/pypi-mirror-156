...
from toolkits import plot, utils
...
class file(object):
    ...
    def unavailable_hosts(self, hosts=[]):
        ''' 异常服务器处理 '''
    ...
    def single_piece(
        ...
        # 根据 IP 排序
        if sort_by_ip == True:
            _ips = utils.list_sort_by_key([_host['interfaces'][0]['ip'] for _host in _data], key=inet_aton, deduplication=True)
        ...
        try:
            ...
            if (len(_data_idx) > 0) and (image != {}):
                ...
                with open(self._markdown_file, 'a') as _file:
                    ...
                    _plot_result = plot.bar({image['label']: _data_result}, _data_idx, _plot_image)
    ...
    def multiple_pieces(
        ...
        try:
            ...
            # 提取数据
            for d in _data:
                _num_max, _num_avg, _num_min = utils.nums_mam([history['value'] for history in d['history']], number_type)
                if (_num_max != None) and (_num_avg != None) and (_num_min != None):
                    _data_idx.append(d['name'])
            ...
            if (len(_data_idx) > 0) and (image != None):
                ...
                with open(self._markdown_file, 'a') as _file:
                    ...
                    _plot_result = plot.bar_cover(_plot_data, _data_idx, _plot_image)
    ...
    def handling_html(self):
        ...
        try:
            ...
            if utils.stat_is(self._html_file) == True:
