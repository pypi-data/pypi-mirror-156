...
class reports(object):
    ...
    def generic(
        ...
        if type(item_keys) == str:
            _history = utils.retry(10, self._zbx.get_history_by_item_key, hosts, time_from, time_till, item_keys, data_type)

        if type(item_keys) == list:
            for _item_key in item_keys:
                _history_slice = utils.retry(10, self._zbx.get_history_by_item_key, hosts, time_from, time_till, _item_key, data_type)
