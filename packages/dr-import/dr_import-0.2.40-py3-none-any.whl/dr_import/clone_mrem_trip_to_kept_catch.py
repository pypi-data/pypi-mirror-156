...
def main(
    ...
    logger.info("**** Cloning multi media objects")
    media_ids = []
    filtered_src_multis = []
    ...
    logger.info("**** Cloning video media objects")
    ...
    logger.info("**** Cloning summary media")
    try:
        ...
        logger.error("Problem cloning summary media")
        logger.error(exc)
    ...
    src_state_types = [src_haul_type, src_em_specific_type]
    dest_state_types = [dest_haul_type, dest_em_specific_type]
    ...
    if in_copy_discards:
        for src_video_id, dest_video_id in single_video_mapping.items():
            ...
            if len(locs) > 0:
                response = tator_api.create_localization_list(project=in_dest_project, localization_spec=dest_loc_specs)
                print(response)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--copy-discards', action="store_true")
...
def script_main() -> None:
    ...
    main(
        ...
        in_copy_discards=args.copy_discards,
