...
def write_flash(esp, args):
    ...
    global erase_ok
    if erase_ok is not True:
        if args.addr_filename[0][1].name.find("zhangkong") != -1:
#            print('\rWriting at 0x00000000... (2 %)')
            sys.stdout.flush()
            erase_flash(esp,args)
        else :
            args.address = 0x8000
            args.size = 0x2000
            erase_region(esp,args)
...
def esp_burn(port,baudrate,filename):
...
#    print('esptool.py v%s' % __version__)
    ...
    #0x10000 dfrobot.ino.bin
    args.addr_filename.append((0x10000,open(filename,"rb")))
    # 0x8000 dfrobot.ino.partitions.bin
    args.addr_filename.append((0x8000,open(filepath+"dfrobot.ino.partitions.bin","rb")))
...
class EspTool(Programmer):
  def __init__(self, port="/dev/ttyS1", baudrate=921600):
