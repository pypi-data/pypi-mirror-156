...
#        time.sleep(0.1)
    ...
    def GD32V_play_note(self, _freq, beat=None):
        self.buzz_flag = False
        if beat == None:
          data = [0x01, 0x08, (_freq >> 8)&0xff, _freq & 0xff, 0x00, 0x04]  
        else:
          data = [0x01, 0x01, (_freq >> 8)&0xff, _freq & 0xff, (beat >> 8)&0xff, beat & 0xff]
        self._send_sysex(0x54, data)
    
    def GD32V_set_ticks_tempo(self, ticks, tempo):
        data = [0x02, (ticks >> 8)&0xff, ticks & 0xff, (tempo >> 8)&0xff, tempo & 0xff]
        self._send_sysex(0x54, data)
