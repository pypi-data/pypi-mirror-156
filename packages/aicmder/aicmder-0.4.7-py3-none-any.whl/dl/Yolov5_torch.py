...
class Yolov5:
    ...
    def __init__(self, weights, imgsz) -> None:
        self.imgsz = imgsz
        self.conf_thres, self.iou_thres, self.classes, self.agnostic_nms, self.max_det = 0.55, 0.45, None, False, 1000
    ...
    def predict_image(self, img_bgr, save_img=False):
        ...
        if len(resp_d) > 0:
            resp_d = sorted(resp_d, key=lambda x: x["conf"], reverse=True)
            # print(resp_d)
        return resp_d
