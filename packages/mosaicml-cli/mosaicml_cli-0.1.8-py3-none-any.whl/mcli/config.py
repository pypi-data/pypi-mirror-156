...
class FeatureFlag(Enum):
    ...
    USE_MLPERF_NODES = 'USE_MLPERF_NODES'
...
class MCLIConfig(SerializableDataclass):
    ...
    def support_token(self) -> str:
        if SUPPORT_TOKEN_KEY_ENV not in os.environ:
            from slack_sdk import WebClient

            from mcli.utils.utils_interactive import simple_prompt

            support_token = simple_prompt(
                message="Please provide a support token (you'll need to contact us directly for this):",
                mandatory=True,
            )
            ...
            try:
                ...
                raise ValueError("The provided support token is invalid.")

            os.environ[SUPPORT_TOKEN_KEY_ENV] = support_token
        return os.environ[SUPPORT_TOKEN_KEY_ENV]
