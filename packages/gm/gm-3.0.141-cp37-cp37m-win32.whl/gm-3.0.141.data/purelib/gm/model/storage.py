...
import pandas as pd
from typing import Callable, Text, Dict, Any, List, Set, Sequence, Union, Deque

from gm import __version__
from gm.constant import DATA_TYPE_TICK, DATA_TYPE_L2TRANSACTION
from gm.csdk.c_sdk import BarLikeDict2, TickLikeDict2, py_gmi_get_cash, py_gmi_get_positions, \
    py_gmi_get_accounts, py_gmi_set_timer, gmi_now_plus, c_status_fail, py_gmi_get_account_status
from gm.enum import MODE_BACKTEST, MODE_LIVE
from gm.model.account import Account
from gm.pb.account_pb2 import Positions, Cashes, Accounts, Account as PbAccount, AccountStatuses
from gm.pb.common_pb2 import ConnectionStatus
...
class Context(object):
    ...
    sdk_lang = "python{}.{}".format(
        sys.version_info.major, sys.version_info.minor)  # type: Text
    ...
    def _add_bar2bar_data_cache(self, k, bar):
        ...
        if kk in self.bar_data_set:
            ...
            if k not in context.bar_data_cache:
                ...
                context.bar_data_cache[k] = collections.deque(
                    maxlen=context.max_bar_data_count)
    ...
    def data(symbol, frequency, count=1, fields='', bar_df=True):
        ...
        if frequency == DATA_TYPE_TICK:
            if symbol in context.tick_data_cache:
                ...
                symbol_tick_queue = context.tick_data_cache[symbol]
                if count > symbol_tick_queue.maxlen:
                    count = symbol_tick_queue.maxlen
                if count > len(symbol_tick_queue):
                    # 数据不够, 说明要补数据
                    delta_count = symbol_tick_queue.maxlen - \
                        len(symbol_tick_queue)
                    ...
                    adjust_end_time = context.now.strftime(
                        '%Y-%m-%d %H:%M:%S') if context.is_live_model() else context.backtest_end_time
                    tick_datas = history_n(symbol=symbol, frequency=frequency, count=delta_count,
                                           end_time=query_data_end_time, adjust=context.adjust_mode,
                    ...
                    for bd in tick_datas:
                        quotes = bd.get('quotes', [])
                        if len(quotes) < 10:
                            for _ in range(len(quotes), 10):
                                zero_val = {'bid_p': 0, 'bid_v': 0,
                                            'ask_p': 0, 'ask_v': 0}
                                quotes.append(zero_val)
                        bd['quotes'] = quotes
                        bd['receive_local_time'] = localtime
                        bd['flag'] = bd.get('flag', 0)
                        tick_datas_trans.append(TickLikeDict2(data=bd))

                    # type: List[TickLikeDict2]
                    comb_tick_datas = list(
                        symbol_tick_queue) + tick_datas_trans
                    # 排除掉重复的数据
                    tick_key_set = set()
                ...
                # 按要求取数据
                d = [v for idx, v in enumerate(
                    symbol_tick_queue) if idx < count]
                d.reverse()
                if len(fields) == 0:
                    return d
                    ...
                    if len(out_fields) > 0 and fields != '':
                        gmsdklogger.warning(
                            ','.join(out_fields)+'不在合法的字段列表中，合法字段:'+','.join(all_fields))
            ...
            if len(out_fields) > 0 and fields != '':
                gmsdklogger.warning(','.join(out_fields) +
                                    '不在合法的字段列表中，合法字段:'+','.join(all_fields))

            k = symbol + "_" + frequency
            if k in context.bar_data_cache:
                # type: collections.deque
                symbol_bar_queue = context.bar_data_cache[k]
                if count > symbol_bar_queue.maxlen:
                    count = symbol_bar_queue.maxlen
                if count > len(symbol_bar_queue):
                    # 数据不够, 说明要补数据
                    delta_count = symbol_bar_queue.maxlen - \
                        len(symbol_bar_queue) + 1
                    from gm.api.query import history_n
                    ...
                    for bd in bar_datas:
                        if frequency == '1d':  # 1d的话统一替换成 15:15:01
                            bd['eob'] = bd['eob'].replace(
                                hour=15, minute=15, second=1)
                        bd['receive_local_time'] = localtime
                        bar_datas_trans.append(BarLikeDict2(data=bd))

                    # type: List[BarLikeDict2]
                    comb_bar_datas = list(symbol_bar_queue) + bar_datas_trans
                    # 排除掉重复的数据
                    bar_key_set = set()
                    new_bar_datas = list()  # type: List[BarLikeDict2]
                    for bar_item in comb_bar_datas:
                        kk = (bar_item['symbol'],
                              bar_item['frequency'], bar_item['eob'])
    ...
    def _get_account_info(self, account_id, account_name):
        ...
        if result:
            ...
            if len(acc_statuses.data) > 0:
                # type: Union[ConnectionStatus, None]
                conn_status = acc_statuses.data[0].status

        account = Account(account_id, account_name, cash,
                          positions_infos, conn_status)
