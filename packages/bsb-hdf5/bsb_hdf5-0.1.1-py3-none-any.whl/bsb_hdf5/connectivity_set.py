    ...
    def _get_chunk_data(self, dest_chunk):
        with self._engine._write():
            with self._engine._handle("a") as h:
                ...
                src = grp["muxed_locs"][()]
                dest = grp["simple_locs"][()]
        return src_chunks, chunk_ptrs, src, dest

    def connect(self, src_chunk, dest_chunk, src_locs, dest_locs, handle=None):
        if len(src_locs) != len(dest_locs):
            raise ValueError("Location matrices must be of same length.")
        if handle is None:
            with self._engine._write():
                with self._engine._handle("a") as handle:
                    self._connect(src_chunk, dest_chunk, src_locs, dest_locs, handle)
        else:
            self._connect(src_chunk, dest_chunk, src_locs, dest_locs, handle)

    def _connect(self, src_chunk, dest_chunk, src_locs, dest_locs, handle):
        self._insert("out", dest_chunk, src_chunk, dest_locs, src_locs, handle)
        self._insert("inc", src_chunk, dest_chunk, src_locs, dest_locs, handle)

    def _insert(self, tag, muxed, simple, muxloc, simloc, handle):
        grp = h.require_group(f"{self._path}/{tag}/{simple.id}")
        src_id = str(muxed.id)
        unpack_me = [None, None]
        # require_dataset doesn't work for resizable datasets, see
        # https://github.com/h5py/h5py/issues/2018
