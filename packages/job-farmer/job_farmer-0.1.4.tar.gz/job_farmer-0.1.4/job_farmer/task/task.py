...
class Task:
    ...
    def execute(self, timeout_hour=-1) -> SimpleNamespace:
        ...
        try:
            ...
            results = subprocess.run(formatted_command, shell=True, text=True, check=True,
        ...
        self.__logger.info(f'- Exception: {exception}')
        self.__logger.info(f'Execution time: {execution_time} ({execution_time.total_seconds()} s)')

        return SimpleNamespace(exit_code=results.returncode, stdout=stdout, stderr=stderr, exception=exception)
