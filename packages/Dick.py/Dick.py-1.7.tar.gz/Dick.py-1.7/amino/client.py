...
from .lib.util import device_gen
...
class Client(Callbacks, SocketHandler):
    def __init__(self, deviceId: str = None, proxies: dict = None, certificatePath = None, socket_trace = False, socketDebugging = False):
        ...
        self.headerrs=headers.Headers(deviceId=self.device_id).headers   
    def parse_headers(self, data = None,deviceId:str=device_gen()):
        if data:
            #print(data)
            return headers.Headers(data=data, deviceId=deviceId).headers
        else:
            return headers.Headers(deviceId=deviceId).headers
    ...
    def reputation(self, comId, chatId: str,joinType: int = 1,thread:bool=True):
        """
        Start The self.send(json.dumps(data))ideo Chat
  ...
  "playlist":{"currentItemIndex":0,
              ...
              "items":[
                  {"author":"null",
                   ...
                   "isDone":False,
                   "mediaList":
                       [[100,"http://pm1.narvii.com/8304/b8619e9c47f81fbf57259a57db69fb28e9704d99r1-1920-1080v2_00.jpg","null"]],
                   "title":"dick.mp4",
                   "type":1,
                   "url": f"file://dick.py"}]},
      "id":"244360"},"t":120}
    ...
    def start_video_call(self, comId: str, chatId: str, joinType: int = 1):
        data = {
            "o": {
                "ndcId": comId,
                "threadId": chatId,
    ...
    def login(self, email: str, password: str,proxy:dict=None):
        """
        ...
        **Returns**
        ...
        if proxy:
            ...
            proxy =self.proxies
        response = requests.post(f"{self.api}/g/s/auth/login", headers=self.parse_headers(data=data), data=data, proxies=proxy, verify=self.certificatePath)
        self.run_socket()
    ...
    def register(self, nickname: str, email: str, password: str, verificationCode: str, deviceId: str = device.device_id):
        ...
        **Returns**
        ...
        headd=self.parse_headers(deviceId=deviceId,data=data)
        headd["NDCAUTH"]=None

        response = requests.post(f"{self.api}/g/s/auth/register", data=data, headers=headd)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
    ...
    def verify(self, email: str, code: str,dev:str,verifyInfoKey:bool=False,key=None):
        """
        Verify an account.
    ...
    def change_pass(self,password,new_password):
        
        data = json.dumps({"secret":f"0 {password}","deviceID": self.device_id})
 
        response = requests.post(f"{self.api}/g/s/auth/verify-password", headers=self.parse_headers(data=data),data=data)
        if response.json()['api:statuscode']==0:
            dat=json.dumps({"secret":f"0 {password}","updateSecret":f"0 {new_password}","validationContext":"","deviceID": self.device_id})
            res = requests.post(f"{self.api}/g/s/auth/change-password", headers=self.parse_headers(data=dat),data=dat)
            return res.json()
        else:
            return response.json()

    def request_verify_code(self, dev: str,email: str, resetPassword: bool = False,verifyInfoKey:bool =False,key=None):
        """
        ...
        **Returns**
        ...
        headd=self.parse_headers(deviceId=dev,data=data)
        headd["NDCAUTH"]=None
    ...
    def random_community(self,search:str,start: int = 0, size: int = 25):
        response=requests.get(f"{self.api}/g/s/community/search?q={search}&language=en&completeKeyword=1&start={start}&size={size}",headers=self.parse_headers(), proxies=self.proxies, verify=self.certificatePath)
    ...
    def check_community(self, comId: str):
        """
        Information of an Community.
        ...
        **Returns**
        ...
        """
        response = requests.get(f"{self.api}/g/s-x{comId}/community/info?withInfluencerList=1&withTopicList=true&influencerListOrderStrategy=fansCount", headers=self.parse_headers(), proxies=self.proxies, verify=self.certificatePath)
        if response.status_code == 200: return response.json()["api:statuscode"]
        else: return response.json()["api:statuscode"]
    ...
    def devicee(self):
        d=requests.get("https://ka-generator.herokuapp.com/device").text
        return d
