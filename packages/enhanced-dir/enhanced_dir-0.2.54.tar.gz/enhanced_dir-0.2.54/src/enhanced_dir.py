def enhanced_dir(arg, categorize=True, show_types=False, checks=False, interfaces_and_types=False, print_width=120,
    ...
    if show_graphs:
        ...
        import seaborn
        if categorize:
            dc_categorize = {i: len(j) for i, j in x[0]['passed'][f'{arg}'].items()}
            data = dc_categorize.keys()
            keys = dc_categorize.values()
            palette_color = seaborn.color_palette('bright')
            plt.pie(data, label=keys, colors=palette_color, autopct='%.0f%%')
            plt.show()
