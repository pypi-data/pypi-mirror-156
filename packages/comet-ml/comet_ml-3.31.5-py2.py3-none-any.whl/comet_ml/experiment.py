...
from .logging_messages import (
    ...
    COULDNT_COMPLETE_ATEXIT_UNREGISTER,
...
class BaseExperiment(CommonExperiment):
        ...
        if self.streamer is not None:
            ...
            if wait is True:
                ...
                try:
                    ...
                    LOGGER.error(COULDNT_COMPLETE_ATEXIT_UNREGISTER, exc_info=True)
    ...
    def _set_model_graph(self, graph, overwrite=False, framework=None):
        ...
        message = ModelGraphMessage(graph=graph)
        self._enqueue_message(message)
    ...
    def set_filename(self, fname):
        ...
        if not self.alive:
            return
        message = FileNameMessage.create(self.context, self.config, file_name=fname)
        self._enqueue_message(message)
    ...
    def set_os_packages(self):
        ...
        try:
            ...
            if os_packages_list is None:
                return
            message = OsPackagesMessage(os_packages=os_packages_list)
            self._enqueue_message(message)
    ...
    def set_pip_packages(self):
        """
        Get the installed pip packages using pkgresources and reports them to server as a message.
        Returns: None
        ...
        if self.alive:
            try:
                ...
                message = self._create_message()
                message.set_installed_packages(packages)
                self._enqueue_message(message)
    ...
    def _log_env_details(self):
        ...
        message = get_env_details_message(env_blacklist, log_env)
        self._enqueue_message(message)
