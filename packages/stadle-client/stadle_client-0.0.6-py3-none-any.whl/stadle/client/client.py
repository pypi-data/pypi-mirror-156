...
class Client:
    ...
    def __init__(self,
        ...
        self.logger.debug(f'--- Your IP is {self.agent_ip} ---')
    ...
    async def participate(self):
        ...
        try:
            ...
            if resp[0] == AggMsgType.welcome:
                ...
                self.logger.debug(f"--- Init Response: {resp[0]} ---")
                ...
                self.isRegistered = False
                self.logger.error(f"--- Init Response: {resp[1]}, system exits. ---")
                sys.exit()

            elif resp[0] == AggMsgType.max_agents_reached:
                self.logger.error("--- Max agents connected to aggregator - cannot connect ---")
                sys.exit()

        except Exception as e:
            self.logger.error(f"--- Cannot connect to the aggregator \n\t{e}---")
            sys.exit()
    ...
    async def send_initial_models(self):
        ...
        try:
            ...
            self.logger.error("Cannot load local models!")
            return
        ...
        try:
            ...
            self.logger.error("--- cannot connect to the aggregator ---")
        ...
        while self.exch_active:
            ...
            if state == ClientState.sending:
                ...
                try:
                    ...
                    self.logger.error(f"--- connection lost with the aggregator ---\n{e}")
            ...
            # Polling for sg model update
                ...
                try:
                    ...
                    if resp[0] == AggMsgType.update:
                        ...
                        self.logger.error(f'--- Max agents connected to aggregator, sgm poll failed ---')
                    else:
    ...
    def register_agent(self):
        ...
        if self.isRegistered == False:
            self.logger.debug("Registration unsuccessful")
            sys.exit()
...
class BasicClient(Client):
    ...
    def send_trained_model(self, trained_model, perf_values = {}):
        ...
        if client_state == ClientState.sg_ready:
            # Client received new SG weights before local training could complete
            self.logger.debug(f'--- The training was too slow. A new SG model is available. ---')
            return False
        else:
