...
class ImportCheck(object):
    def __init__(self) -> None:
        ...
        try:
            ...
            self.logger.warning(f"flask/gevent not found - only websocket servers can be spawned (\'send\' not affected)")
...
class CommunicationHandler():
    ...
    async def send_msg(self, msg, addr, resp_required = True):
        ...
        while (msg_resp == None or poll_resp_failed == True):
            ...
            for attempt in range(self.num_retry):
                try:
                    ...
                    if self.num_retry - attempt > 1:
                        if self.verbose:
                            self.logger.info(f'Send attempt {attempt + 1} failed for message {id}')
            ...
            if self.verbose:
                self.logger.info(f'Message sent: {msg}')
            ...
            if resp_required:
                for attempt in range(self.num_retry):
                    try:
                        ...
                        if self.num_retry - attempt > 1:
                            if self.verbose:
                                self.logger.info(f'Receive response attempt {attempt + 1} failed for message {id}')
                            await asyncio.sleep(self.retry_interval)
                            continue
                        else:
                        ...
                        if attempt != 0:
                            self.logger.info(f"Connection to {addr} restored")
                        if not poll_resp_failed and self.verbose:
                            self.logger.info(f'Response received: {msg_resp}')
                        break
            else:
                msg_resp = send_resp
    ...
    async def poll_msg_resp(self, addr, id):
        ...
        for attempt in range(self.num_retry):
            if self.verbose:
                self.logger.info(f"Polling for msg {id}")
    ...
    async def get_msg(self, addr):
        ...
        if self.verbose:
            self.logger.info(f"Message {msg_id} popped from queue")
    ...
    def push_msg_resp(self, resp, id):
        ...
        if self.verbose:
            self.logger.info(f"Response to message {id} added to queue")
    ...
    def _clean_resp_cache(self):
        while True:
            ...
            if self.verbose:
                self.logger.info(f"Response to message {msg_id} removed from cache")
