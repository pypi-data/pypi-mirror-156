...
from soxaikit.DataLoadSave import load_raw_data,combine, output_arrays_to_file

from soxaikit.feature_extract import feature_from_ppg, feature_from_xyz

from soxaikit.soxaiPlot import plot_time_domain, plot_time_domain_single_figure, plot_time_domain_single_figure_combine, plot_features, multipage_save

from soxaikit.PreProcessModel import pre_process_smooth, pre_process_average, pre_process_accumulate, pre_process_actigraph, merge_array, getSpo2, pre_process_get_wear

from soxaikit.SleepStageModel import sleep_stage_cal, print_sleep_info, plot_sleep_info
...
multipage_save(os.getcwd()+"/output/"+ f"{features_final[0][0].strftime('%Y%m%d')}" +"All_results.pdf")
# plt.show()