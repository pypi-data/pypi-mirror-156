...
def load_raw_data(sub_folder_name, date_format_string='%Y-%m-%d %H:%M:%S.%f'):
    print(f"loading combined data: {sub_folder_name}")
    output = []
    folder_name = os.getcwd()
    file = folder_name + "\\combine" + "\\" + sub_folder_name + ".csv"
    df = pd.read_csv(file)
    ...
    print(f"output size: ( {len(output)} , {len(output[0])} ), keys: {keys}")
    return output, keys
...
def output_arrays_to_file(array, keys, filename, out_folder_name="output"):
    dic = {}
    folder_name = os.getcwd()
...
def combine(sub_folder, date_format_string='%Y-%m-%d %H:%M:%S.%f', out_folder_name='combine'):
    folder_name = os.getcwd()
