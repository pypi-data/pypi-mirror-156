def pre_process_average(array_input, window_interval=60):
...
def pre_process_accumulate(array_input, window_interval=60):
    # array_input is a 2D list
    array_time = array_input[0]
    num_features = len(array_input) - 1
    len_feature = len(array_time)
...
def pre_process_smooth(array_input, window_num, output_mean: bool = False, output_max: bool = False,
                       output_min: bool = False):
    array_output_mean = []
    array_output_max = []
    array_output_min = []
    ...
    for k in range(window_num - 1, num_data):
        window_data = array_input[k - window_num + 1:k + 1]
    ...
    if num_data > window_num:
        if output_mean:
            array_output_mean = [array_output_mean[0]] * window_num + array_output_mean
        if output_max:
            array_output_max = [array_output_max[0]] * window_num + array_output_max
        if output_min:
            array_output_min = [array_output_min[0]] * window_num + array_output_min
...
def pre_process_actigraph(array_input):
    num_data = len(array_input)
    actigraph = []
    sleep_stage = []
...
def merge_array(array0_input, array0_keys, array1_input, array1_keys):
