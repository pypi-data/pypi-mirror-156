...
import yaml
import json
from PyWebRunner.utils import (which, Timeout, prompt, download_file)
...
class WebRunner(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.webdriver_init_timeout = kwargs.get('driver_init_timeout', 10)
        ...
        self.socks_user = kwargs.get('socks_user', os.getenv('SOCKS_USER', ''))
        self.socks_password = kwargs.get('socks_password', os.getenv('SOCKS_PASSWORD', ''))
        self.socks_proxy = kwargs.get('socks_proxy', os.getenv('SOCKS_PROXY', ''))

        xvfb = kwargs.get('xvfb', False)
        webdriver = kwargs.get('webdriver', 'chrome').lower()
        ...
        self.webdriver = os.environ.get('WR_DRIVER', webdriver).lower()
    ...
    def start(self):
        ...
        print("\nStarting browser ({})...".format(self.webdriver))

        if self.webdriver == "chrome" or self.webdriver == 'chrome-headless':
            ...
            if self.stealth:
                ...
                proxy_configured = all([self.socks_user, self.socks_password, self.socks_proxy])
                if proxy_configured:
                    print("Proxy is configured.")

                if not proxy_configured and self.proxy_enabled:
                    print("Proxy not configured but proxy enabled.")
                    print(self.socks_user, self.socks_password, self.socks_proxy)

                if proxy_configured and self.proxy_enabled:
                ...
                if self.webdriver == 'chrome-headless':
        ...
        elif self.webdriver in ('firefox', 'gecko'):
            ...
            if self.webdriver == 'gecko':
                ...
                try:
                    with Timeout(self.webdriver_init_timeout):
                        self.browser = webdriver.Firefox(firefox_profile=fp)
    ...
    def get_js_errors(self, levels=None):
        '''
        Gets a list of console errors from the browser
        '''
        if not levels:
            levels = ['SEVERE']
        logs = self.browser.get_log('browser')
        errors = [l for l in logs if l['level'] in levels]
        self.store.data['js_errors'] = errors
        return errors
    ...
    def focus_window(self, windex=None):
        ...
        self.browser.driver.switch_to_window(self.browser.window_handles[windex])
    ...
    def find_element(self, selector, by=None):
        ...
        elem = None
        if by == 'id':
            elem = self.browser.find_element(by=By.ID, value=selector)
        elif by == 'xpath':
            elem = self.browser.find_element(by=By.XPATH, value=selector)
        elif by == 'classname':
            elem = self.browser.find_element(by=By.CLASS_NAME, value=selector)
        elif by == 'name':
            elem = self.browser.find_element(by=By.NAME, value=selector)
        elif by == 'link_text':
            elem = self.browser.find_element(by=By.LINK_TEXT, value=selector)
        elif by == 'partial_link_text':
            elem = self.browser.find_element(by=By.PARTIAL_LINK_TEXT, value=selector)
        else:
            if selector.startswith('/'):
                elem = self.browser.find_element(by=By.XPATH, value=selector)
            else:
                elem = self.browser.find_element(by=By.CSS_SELECTOR, value=selector)
    ...
    def find_elements(self, selector, by=None):
        ...
        elems = None
        if by == 'id':
            elems = self.find_elements(by=By.ID, value=selector)
        elif by == 'classname':
            elems = self.browser.find_elements(by=By.CLASS_NAME, value=selector)
        elif by == 'xpath':
            elems = self.browser.find_elements(by=By.XPATH, value=selector)
        elif by == 'name':
            elems = self.browser.find_elements(by=By.NAME, value=selector)
        elif by == 'link_text':
            elems = self.browser.find_elements(by=By.LINK_TEXT, value=selector)
        elif by == 'partial_link_text':
            elems = self.browser.find_elements(by=By.PARTIAL_LINK_TEXT, value=selector)
        else:
            if selector.startswith('/'):
                elems = self.browser.find_elements(by=By.XPATH, value=selector)
            else:
                elems = self.browser.find_elements(by=By.CSS_SELECTOR, value=selector)
