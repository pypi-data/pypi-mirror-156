...
from Shared.certoraUtils import as_posix, abs_posix_path_obj, split_settings, run_local_spec_check
...
from Shared.certoraUtils import red_text
from Shared.certoraUtils import CERTORA_INTERNAL_SUBDIR, safe_create_dir, path_in_certora_internal
...
def run_certora(args: List[str], is_library: bool = False) -> None:
    ...
    safe_create_dir(CERTORA_INTERNAL_SUBDIR)
    ...
    try:
        ...
        pretty_args = [f"'{arg}'" if ' ' in arg else arg for arg in args]
        cl_args = ' '.join(pretty_args)
        run_logger.fatal(f"{CONTACT_CERTORA_MSG}\ncertoraRun {cl_args} --debug")
...
def type_settings_arg(settings: str) -> str:
    """
    Gets a string representing flags to be passed to another tool via --settings, in the form '-a,-b=2,-c=r,q,[,..]'
    @raise argparse.ArgumentTypeError
    """
...
def __cannonize_settings(args: argparse.Namespace) -> None:
    ...
    for setting_list in args.settings:
        ...
        for setting in split_settings(setting_list):
    ...
    with replay_json_filename.open() as replay_json_file:
        ...
        for file_name, file_contents in repro['certoraConfig'].items():
            ...
            if not (path_in_certora_internal(dir_containing_file).is_dir()):
                dir_containing_file.mkdir(parents=True)
