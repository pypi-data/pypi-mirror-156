...
class SimpleModel(LightningModule):
    def __init__(self, model_name: str = 'resnet18',
                 pretrained: bool = False, num_classes: Optional[int] = None, class_to_index=None):
        super().__init__()
...
def run_train(dataset_path, model_name="resnet18", outdir="results", img_size=(224,224), epochs=100, save_interval=10, batch_size=8, num_workers=12, gpu_ids=[0], seed=42):
    seed_everything(seed)
    data = SimpleData(root_dir=dataset_path, img_size=img_size,
                      batch_size=batch_size, num_workers=num_workers)
    model = SimpleModel(model_name=model_name,
                        pretrained=True,
                        num_classes=len(data.classes), class_to_index=data.class_to_idx)
