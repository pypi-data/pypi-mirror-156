...
COLOR_NEUTRAL = (229 / 255, 230 / 255, 231 / 255)
COLOR_LOCAL = (88 / 255, 89 / 255, 91 / 255)
COLOR_GLOBAL = (152 / 255, 214 / 255, 86 / 255)
...
def plot_states_timeseries(model, states, title=None, labels=True, cmap="plasma", figsize=(8, 2)):
...
def plot_state_durations(model, states, legend=True, alpha=1.0):
    ...
    # figsize = np.multiply(figsize, 0.75)
    plt.figure(figsize=figsize)
    weights = np.ones_like(flat_ups) / len(flat_ups) * 100
    plt.hist(flat_ups, color="C0", label="up", edgecolor="k", linewidth=0.75, weights=weights)

    weights = np.ones_like(flat_downs) / len(flat_downs) * 100
    plt.hist(flat_downs, color="C1", label="down", edgecolor="k", linewidth=0.75, alpha=alpha, weights=weights)
    if legend:
...
def plot_involvement_distribution(model, involvement, remove_xticks=False, color_localglobal=False):
    # figsize = (3, 2)
    figsize = (2, 1.7)
    # figsize = np.multiply(figsize, 0.75)
    plt.figure(figsize=figsize)
    weights = np.ones_like(involvement) / len(involvement) * 100
    N, bins, patches = plt.hist(involvement * 100, bins=12, weights=weights, rwidth=0.8, edgecolor="k", color="C0")
    max_y = plt.gca().get_ylim()[1] * 1.1

    if color_localglobal:
        for i in range(0, len(patches) // 4):
            patches[i].set_facecolor((229 / 255, 230 / 255, 231 / 255))
        for i in range(len(patches) // 4, len(patches) // 2):
            patches[i].set_facecolor((88 / 255, 89 / 255, 91 / 255))
        for i in range(len(patches) // 2, len(patches)):
            patches[i].set_facecolor((152 / 255, 214 / 255, 86 / 255))

        plt.text(45, max_y * 0.65, "local", fontsize=14, color=(88 / 255, 89 / 255, 91 / 255), ha="center")
        plt.text(77, max_y * 0.3, "global", fontsize=14, color=(139 / 255, 176 / 255, 92 / 255), ha="center")
...
def plot_phases_and_involvement(model, involvement, phases, states):
    ...
    # annotate vlines
    axs[0].text(7.7, 7.0, "down-to-up", fontsize=8)
    axs[0].vlines(7.6, 6.8, 9.0, lw=1, color="C0")
    axs[0].text(7.7, 4.4, "up-to-down", fontsize=8)
    ...
    axs[1].plot(model.t / 1000, involvement * 30, lw=1, c="C1", label="Involvement")
...
def plot_matrix(mat, cbarlabel, cbar=True, ylabels=True, plotlog=False):
    ...
    if cbar:
        ...
        cbar.ax.tick_params(width=0, labelsize=10)
        cbar.set_label(label=cbarlabel, labelpad=5, size=12)
    plt.tight_layout()
    set_axis_size(*figsize)
...
def plot_fcd_distribution(model, ds, bold_transient=0):
    ...
    weights = np.ones_like(triu_m1_vals) / len(triu_m1_vals) * 100
    plt.hist(triu_m1_vals, weights=weights, color="springgreen", zorder=10, alpha=0.75, edgecolor="k")
...
def plot_powerspectra(f, powerspecs, mean_power, savename=None, logx=False, logy=True, maxfr=None):
    ...
    plt.ylabel("Power [Hz$^2$/Hz]" if logy else "Power [Hz$^2$/Hz]")
    # plt.yticks([])
...
def filter_peaks(peaks, model, involvement, threshold_max, threshold_min=0):
...
def get_peaks(involvement, model, min_distance=1000):
    ...
    peaks25 = filter_peaks(peaks, model, involvement, 0.50, 0.25)
    peaks50 = filter_peaks(peaks, model, involvement, 0.75, 0.50)
    peaks75 = filter_peaks(peaks, model, involvement, 1, 0.75)

    return peaks25, peaks50, peaks75


def plot_peaks(peaks, model, involvement, ax=None, color="springgreen"):
    ax = ax or plt
    for p in peaks.copy():
        dt_min = p
        around = int(500 / model.params.dt)
        if dt_min + around < len(involvement):
            ax.scatter(dt_min / 1000 * model.params.dt, involvement[dt_min], color=color, s=5)
