...
def plot_counts(df, on):
    counts = df[on].value_counts()
    counts = counts.sort_values(ascending=False)
    plt.title(f"Counts on {on}")
    plt.scatter(np.arange(len(counts)), counts.values, s=2)
    plt.ylabel('counts')
...
def plot_multi(df, plot_func='scatter', plot_on=None, group_on=None, color_on=None, label_on=None, figsize=(10,6), alpha=1.0, color_dict=None, title='', x_axis_type=None, legend_loc='best', **kwargs):
    color_dict = color_dict or dict()

    if label_on is None:
        label_on = color_on or group_on

    if label_on:
        df = df.sort_values(label_on)

    if group_on is None:
        group_on = color_on or group_on

    def iter_on(df, col_name):
        if col_name is None:
            yield df, ''

        else:
            for col_val in sorted(df[col_name].unique()):
                df_g = df[df[col_name] == col_val].copy()
                yield df_g, col_val

    color_list = list(mcolors.TABLEAU_COLORS.keys()) + list(mcolors.XKCD_COLORS)
    color_id = 0
    color_lookup = dict()
    for df_p, plot_title in iter_on(df, plot_on):
        labels = set()
        plt.clf()
        plt.subplots(figsize=figsize)
        for df_g, _ in iter_on(df_p, group_on):
            color = None
            label = None

            if label_on:
                label = df_g.iloc[0][label_on]
            ...
            if color_on:
                color_val = df_g.iloc[0][color_on]
