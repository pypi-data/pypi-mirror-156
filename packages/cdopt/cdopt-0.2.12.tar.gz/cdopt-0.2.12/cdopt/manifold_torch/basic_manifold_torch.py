...
class basic_manifold_torch(basic_manifold):
    def __init__(self,name: str,variable_shape: Tuple[int], constraint_shape: Tuple[int], device = torch.device('cpu'), dtype = torch.float64,  regularize_value = 0.01, safegurad_value = 0) -> None:
        self.name = name
    ...
    def C(self,X: Tensor) -> Tensor:
        """Returns the expression of the constraints
        """



    def v2m(self,x: Tensor) -> Tensor:
        return torch.reshape(x, self.var_shape)



    def m2v(self,X: Tensor) -> Tensor:
        return X.flatten()
...
class complex_basic_manifold_torch(complex_basic_manifold):
    ...
    def array2tensor(self, X_array: np.ndarray) -> Tensor:
        dim = len(X_array)
    ...
    def tensor2array(self, X_tensor: Tensor, np_dtype = np.float64) -> np.ndarray:
    ...
    def C_quad_penalty(self, X: Tensor):
        CX = self.C(X)
        return self.sum( CX * CX.conj())


    def Feas_eval(self, X: Tensor) -> float:
        CX = self.C(X)
