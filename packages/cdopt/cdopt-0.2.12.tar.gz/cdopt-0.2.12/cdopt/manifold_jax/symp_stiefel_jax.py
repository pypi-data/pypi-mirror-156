...
class symp_stiefel_jax(basic_manifold_jax):
    def __init__(self, var_shape, train_mode = False) -> None:
        ...
        def JpM(X):
            return jnp.concatenate( (X[self._half_p:, :], -X[:self._half_p,:]), 0 )
    ...
    def Init_point(self, Xinit = None, seed = 0):
        ...
        if self.Feas_eval(X) > 1e-4:
            for jl in range(10):
                X = X - 0.5/( jnp.linalg.norm(X.T @ X, 2) )   * self.JC(X,self.C(X))
            ...
            for jl in range(10):
                ...
                if feas < 1e-2:
                    ...
                    X = jnp.linalg.solve(TTT.T, X.T ).T
                if feas < 1e-8:
                    break
