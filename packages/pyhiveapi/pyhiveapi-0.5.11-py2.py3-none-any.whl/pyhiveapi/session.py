...
class HiveSession:
    ...
    def updateInterval(self, new_interval: timedelta):
    ...
    def useFile(self, username: str = None):
    ...
    def updateTokens(self, tokens: dict, update_expiry_time: bool = True):
    ...
    def updateData(self, device: dict):
    ...
    def getCamera(self, device):
        ...
        if self.config.file:
            ...
            cameraImage = self.api.getCameraImage(device)
            if cameraImage["parsed"]["events"][0]["hasRecording"] is True:
                cameraRecording = self.api.getCameraRecording(
    ...
    def startSession(self, config: dict = {}):
        ...
        """
        self.useFile(config.get("username", self.config.username))
        self.updateInterval(
        ...
        if config != {}:
            if "tokens" in config and not self.config.file:
                self.updateTokens(config["tokens"], False)
