...
def train_multimodel(configini):
    ...
    def LearningCurve(features, targetFrame, shuffle_splits, dataset_splits, savefile_no):
        ...
        train_sizes, train_scores, test_scores = learning_curve(model, features, targetFrame, cv=cv, scoring='f1',shuffle=True, n_jobs=-1, train_sizes=np.linspace(0.01, 1.0, int(dataset_splits)))
