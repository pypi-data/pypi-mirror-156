...
class N19Parser(object):
    ...
    def is_valid(self):
        return (
            next(self.get_creditor_header_line(), None)
            and next(self.get_presenter_header_line(), None)
        )
    def get_name(self):
        presenter_header_line = next(self.get_creditor_header_line())
        return presenter_header_line[123:158]

    def get_creditor_header_line(self):
        return (
            line for line in self.lines if line[0:2] == '21'
        )

    def get_presenter_header_line(self):
        return (
            line for line in self.lines if line[0:2] == '22'
        )
    def parse(self):
        name = self.get_name()
    ...
    def parse_payment_return(self):
        creditor_header_line = next(self.get_presenter_header_line())
        payment_return = {
