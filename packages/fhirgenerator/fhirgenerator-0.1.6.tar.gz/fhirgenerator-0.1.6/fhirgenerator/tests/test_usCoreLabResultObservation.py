'''Testing US Core Lab Result Observation generator'''
...
def testUSCoreLabResultObservationGenerator():
    '''Test function for US Core Lab Result Observation Generator'''
    ...
    for detail in config_dict['resourceDetails']:
        if detail['fhirResource'] == 'USCoreLabResultObservation':
            observation_resource_details.append(detail)

    for i, detail in enumerate(observation_resource_details):
        created_resource = generateUSCoreLabResultObservation(detail, patient_id, config_dict['startDate'], config_dict['days'])

        assert created_resource['resourceType'] == 'Observation'
        assert created_resource['meta']['profile'] == ['http://hl7.org/fhir/us/core/StructureDefinition/us-core-observation-lab']
        ...
        with open(f'fhirgenerator/tests/output/test_usCoreLabResultObservation_{i}.json', 'wb') as outfile:
