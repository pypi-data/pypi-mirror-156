...
- `ValidatorModel`的简单的使用：<br>
    ...
    ```python
    from api_test_ez.ez.orm.fields import IntegerField, StringField, ListField
    ...
    class PhoneValidate(ValidatorModel):
        ...
        thumbnail = ListField(required=True, count_should_gte=1)
    ```
   ...
   4. `thumbnail`: 这是一个列表，且它的成员个数必须大于1。
