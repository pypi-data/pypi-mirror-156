...
class _MultiGrainForecastBase:
    ...
    def __init__(self, timeseries_param_dict: Dict[str, Any]):
        ...
        self.time_column_name = self.timeseries_param_dict[TimeSeries.TIME_COLUMN_NAME]
        self.grain_column_names = self.timeseries_param_dict.get(TimeSeries.GRAIN_COLUMN_NAMES, [])
        self.grain_column_names = (
            [TimeSeriesInternal.DUMMY_GRAIN_COLUMN]
            if self.grain_column_names is None or len(self.grain_column_names) == 0
            else self.grain_column_names
        )
        self.drop_column_names = self.timeseries_param_dict.get(TimeSeries.DROP_COLUMN_NAMES, []) or []
        self._max_cores_per_iteration = self.timeseries_param_dict.get(TimeSeries.MAX_CORES_PER_ITERATION, 1)
        self._iteration_timeout_minutes = self.timeseries_param_dict.get(
            TimeSeries.ITERATION_TIMEOUT_MINUTES, None
        )  # type: Optional[int]
        ...
        self._is_fit = False
        self._estimator_type = TimeSeriesInternal.FORECASTER_ESTIMATOR_TYPE  # type: str
        ...
        for quant in quantiles:
            if quant <= 0 or quant >= 1:
                raise AzureMLError.create(
                    QuantileRange,
                    target="quantiles",
                    reference_code=ReferenceCodes._FORECASTING_MULTI_GRAIN_FORECAST_BASE_QUANTILES_INVALID_VALUE,
                    quantile=quant,
                )
    ...
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        if not self._is_fit:
            raise AzureMLError.create(
                FitNotCalled, target="MultiGrainForecastBase", transformer_name=self.__class__.__name__
            )
        ...
        point_name = TimeSeriesInternal.DUMMY_PREDICT_COLUMN
        X_df = tsds.data
        ...
        return cast(np.ndarray, pred_df[TimeSeriesInternal.DUMMY_PREDICT_COLUMN].values)
    ...
    def _construct_tsds(self, X: pd.DataFrame, y: Optional[np.ndarray] = None) -> TimeSeriesDataSet:
        ...
        if TimeSeriesInternal.DUMMY_TARGET_COLUMN in X.columns:
            tsds_kwargs["target_column_name"] = TimeSeriesInternal.DUMMY_TARGET_COLUMN
        ...
        # if (index_name == TimeSeriesInternal.HORIZON_NAME or
        # index_name == TimeSeriesInternal.ORIGIN_TIME_COLNAME_DEFAULT):
        # tsds_no_origin = tsds_no_origin.droplevel(index_name)
        columns_to_drop = []
        for column_name in tsds_no_origin.data.columns:
            if (
                column_name == TimeSeriesInternal.HORIZON_NAME
                or column_name == TimeSeriesInternal.ORIGIN_TIME_COLNAME_DEFAULT
            ):
    ...
    def _fit_in_sample(self, X: TimeSeriesDataSet) -> pd.DataFrame:
        ...
        """
        point_name = TimeSeriesInternal.DUMMY_PREDICT_COLUMN
        origin_name = TimeSeriesInternal.ORIGIN_TIME_COLUMN_NAME
    ...
    def _get_forecast(self, X: TimeSeriesDataSet, max_horizon: Dict[Tuple[Any], int]) -> pd.DataFrame:
        ...
        point_name = TimeSeriesInternal.DUMMY_PREDICT_COLUMN
        origin_time_colname = TimeSeriesInternal.ORIGIN_TIME_COLUMN_NAME
    ...
    def _get_forecast_single_grain(
        ...
        if grain_level not in self._grain_levels or not self._models[grain_level]:
            raise AzureMLError.create(
                TimeseriesGrainAbsentNoGrainInTrain, target="MultiGrainForecastBase", grain=grain_level
            )
        # ---------------------------------------------------------------
    ...
    def _fit_single_grain(
        ...
        **Note:** this method calls _fit_single_grain_impl internally.
        ...
        last_value = float(series_frame.data[select_last][TimeSeriesInternal.DUMMY_TARGET_COLUMN].iloc[0])
    ...
    def _remove_origin_dependant_info_for_classical_forecasting_algo(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        arimax_raw_columns = set(
            timeseries_param_dict.get(TimeSeriesInternal.ARIMAX_RAW_COLUMNS, [])
        )  # type: Set[Any]
        ...
        for column in X.columns:
            if isinstance(column, str):
                if (
                    column.startswith(TimeSeriesInternal.HOLIDAY_COLUMN_NAME)
                    or column == TimeSeriesInternal.PAID_TIMEOFF_COLUMN_NAME
                ):
        ...
        for grain_column in grain_column_names:
            if grain_column == TimeSeriesInternal.DUMMY_GRAIN_COLUMN and grain_column not in X.index.names:
                X_index[grain_column] = TimeSeriesInternal.DUMMY_GRAIN_COLUMN
    ...
    def __setstate__(self, state):
        ...
        if "_estimator_type" not in state:
            state["_estimator_type"] = TimeSeriesInternal.FORECASTER_ESTIMATOR_TYPE
