...
from ..._diagnostics.error_definitions import ForecastingExpoSmoothingNoModel, TimeseriesDfInvalidArgForecastHorizon
...
class ExponentialSmoothing(_MultiGrainForecastBase):
    ...
    def _get_forecast_single_grain_impl(
        ...
        if max_horizon <= 0:
            raise AzureMLError.create(
                TimeseriesDfInvalidArgForecastHorizon, reference_code=ReferenceCodes._TSDF_INVALID_ARG_MAX_HORIZON_VAL3
            )
    ...
    def _model_selection_exponential_smoothing(self, X_pred_grain: pd.DataFrame, grain_level: GrainType) -> Any:
        ...
        series_values = X_pred_grain.get(TimeSeriesInternal.DUMMY_TARGET_COLUMN).values.astype(float)
