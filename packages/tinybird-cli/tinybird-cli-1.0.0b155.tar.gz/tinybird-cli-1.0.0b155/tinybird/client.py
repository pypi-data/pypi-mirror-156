...
class TinyB(object):
    ...
    async def create_token(self, name: str, scope: str):
        return await self._req(f"/v0/tokens?name={name}&scope={scope}", method='POST', data='')
    ...
    async def populate_node(self, pipe_name: str, node_name: str, populate_subset: bool = False, populate_condition: str = None, truncate: bool = True):
        params: Dict[str, Any] = {
