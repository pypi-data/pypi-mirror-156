...
from nexio_behave import environment
...
def default_env_step(func: Callable) -> Callable:
    """Decorator for creating a default environment from the Nexio Behave framework.

    Args:
        func: The function to be called to be run through the wrapper

    Returns:
        wrapper: Returns the original function after the wrapper is done.

    """

    def wrapper(*args: Any) -> None:
        """The wrapper for the default_env_step decorator.

        Args:
            args: Any input

        """
        logger.debug(str(sys.modules[func.__name__]))
        logger.debug(getattr(environment, str(sys.modules[func.__name__])))

        default_func = getattr(environment, str(sys.modules[func.__name__]))
        logger.debug(default_func)
        default_func(*args)  # Will run the framework's environment functions.
        func(
            *args
        )  # Will then run the new services. Which will overwrite any defaults.
