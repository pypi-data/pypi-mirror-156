...
def _simple_lattice_fwd_bwd_cu_loop(alpha, beta_T, beta_stay, beta_move, stay_scores, move_scores, S:semiring):
    if stay_scores.dtype != torch.float32:
        raise NotImplementError("Only fp32 supported")
    if S != Log:
        raise NotImplementError("Only Log Semiring supported")
...
def fwd_scores_cu_sparse(Ms, idx, v0, S:semiring=Log, K=1):
    if Ms.dtype != torch.float32:
        raise NotImplementError("Only fp32 supported")
    if K != 1:
        raise NotImplementError("Only K=1 supported")
    ...
    if S == Log:
        with torch.cuda.device(Ms.device.index):
            lib.fwd_scores_host_sparse_log(void_ptr(alphas),
                                           void_ptr(Ms),
                                           void_ptr(v0),
                                           void_ptr(idx),
                                           T,
...
def bwd_scores_cu_sparse(Ms, idx, vT, S:semiring=Log, K=1):
    ...
    if S == Log:
        with torch.cuda.device(Ms.device.index):
            lib.bwd_scores_host_sparse_log(void_ptr(betas),
                                           void_ptr(Ms),
                                           void_ptr(vT),
                                           void_ptr(idx_T),
                                           T,
