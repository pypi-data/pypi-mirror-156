...
def expand_part(app, part):
    ...
    Expands:
     ...
     - '%P' to [full path of the current folder] (only in file panel)
    """
    if part == '%n':
        if app.current_panel.selected_path is None:
    ...
    if part == '%P' and hasattr(app.current_panel, 'path'):
        return [str(app.current_panel.path)]
    return [part]
