...
class Application(cursedspace.Application):
    def __init__(self, args):
        ...
        self.keys = [
            ...
            (DocPanel.SCOPE, ('z', 'o'), ('toggle-show-offline',)),
            ...
            (EditorPanel.SCOPE, ('*',), ('add-tag-to-all',)),
                ...
                if resolved is not None:
                    ...
                    if commandname in LIST_SCROLL_COMMANDS:
                        ...
                        try:
                            ...
                            self.error(f"Could not run '{rawprefill[0]}': {exc}")
                            prefill = None
                        ...
                        if commandname.startswith('::'):
                            ...
                            try:
                                ...
                                self.error(f"Could not execute {commandname}: {exc}")
                                commandname = None
...
def run():
    ...
    if args.log_level.lower() in ['debug']:
        metaindex.logger.setup(level=logging.DEBUG,
                               filename=args.log_file.parent / 'metaindex.log')
    else:
