...
class FilePanel(ListPanel):
    ...
    def change_path(self, path):
        ...
        if self.path != path:
            if self.path is not None:
                self.cursor_cache[self.path] = (self.cursor, self.selected_path)
    ...
    def finish_change_path(self, items):
        ...
        if self.path in self.cursor_cache:
            self.cursor, to_select = self.cursor_cache[self.path]
            to_select = [idx
                         for idx, item in enumerate(self.items)
                         if item.path == to_select]
            if len(to_select) > 0:
                self.cursor = to_select[0]
    ...
    def selected_path(self):
        if self.items is None or self.selected_item is None:
            return None
...
class ChangeDirectory(command.Command):
    ...
    def execute(self, context, target=None):
        ...
        parts = [expand_env_variables(p)
                 for p in target.parts]
        if parts[0] == '/' and parts[1] == '/':
            parts.pop(0)
        target = pathlib.Path(os.sep.join(parts)).resolve()
...
def expand_env_variables(arg):
    for match_ in ENV_VAR_RE.finditer(arg):
        arg = arg[:match_.start()] \
            + os.getenv(match_[0][1:], match_[0]) \
            + arg[match_.end()+1:]
    return arg
