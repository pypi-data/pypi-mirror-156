...
class evalTuningParam:
    ...
    def __init__(self, cvmdl, perftype, X, Y, compvec, threshvec, subfold = None): 
        ...
        compvec = TPLSinputchecker.TPLSinputchecker(compvec,'compvec','colvec',cvmdl.NComp,1,0,1); compvec = np.sort(compvec); self.compval = compvec;
        threshvec = TPLSinputchecker.TPLSinputchecker(threshvec,'threshvec','colvec',1,0); threshvec = np.sort(threshvec); self.threshval = threshvec;
        if subfold is None:
            subfold = np.ones((n,1))
        else:
