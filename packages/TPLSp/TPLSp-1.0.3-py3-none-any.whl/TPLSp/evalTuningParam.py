...
from TPLSinputchecker import TPLSinputchecker
...
class evalTuningParam:
    ...
    def __init__(self, cvmdl, perftype, X, Y, compvec, threshvec, subfold = None): 
        ...
        compvec = TPLSinputchecker(compvec,'compvec','colvec',cvmdl.NComp,1,0,1); compvec = np.sort(compvec); self.compval = compvec;
        threshvec = TPLSinputchecker(threshvec,'threshvec','colvec',1,0); threshvec = np.sort(threshvec); self.threshval = threshvec;
        if subfold is None:
            subfold = np.ones((n,1))
        else:
        ...
        self.perf_best = np.nanmax(avgperfmat) # best mean performance
        row_best,col_best = np.where(avgperfmat==self.perf_best) # coordinates of best point
        self.compval_best = compvec[row_best[0]]; self.threshval_best = threshvec[col_best[0]]; # component and threshold of best point
        ...
        self.perf_1se = avgperfmat[row_1se[0],col_1se[0]]; # performance of 1SE point
        self.compval_1se = compvec[row_1se[0]]; self.threshval_1se = threshvec[col_1se[0]]
        maxroute = np.atleast_2d(np.max(avgperfmat,0)).T; maxrouteind = np.atleast_2d(np.argmax(avgperfmat,0)).T
        self.best_at_threshold = np.concatenate((maxroute,threshvec,compvec[maxrouteind]),axis = 1)
    ...
    def util_perfmetric(predmat,testY,perftype):
        if perftype == 'LLbinary':
            assert(np.all(testY == 0 or testY==1)), 'LL binary can be only calculated for binary measures'
            ...
            assert(np.all(testY == 0 or testY==1)), 'Accuracy can be only calculated for binary measures'
            Perf = np.nanmean(1*(testY==1 & (predmat>0.5)),0)
        elif perftype == 'AUC':
            assert(np.all(testY == 0 or testY==1)), 'AUC can be only calculated for binary measures'
