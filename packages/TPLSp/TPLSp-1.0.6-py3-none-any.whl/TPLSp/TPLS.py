...
class TPLS:
    ...
    def __init__(self, X, Y, NComp = 25, W = None, nmc = 0): 
        ...
        W = TPLSinputchecker.TPLSinputchecker(W,'W','colvec',None,0); W = W/sum(W); # normalize weight sum to 1
    ...
    def makePredictor(self,compval,threshval):
        ...
        """
        compval = TPLSinputchecker.TPLSinputchecker(compval,'compval','colvec',self.NComp,1,0,1)
        TPLSinputchecker.TPLSinputchecker(threshval,'threshval','scalar',1,0)
        if threshval == 0:
            betamap = self.betamap[:,compval-1] * 0
        else:
            betamap = self.betamap[:,compval-1] * (self.threshmap[:,compval-1] <= threshval)
        betamap = np.atleast_2d(betamap).T;
        print(betamap.shape[0])
        print(betamap.shape[1])
    ...
    def predict(self,compval,threshval,testX):
        ...
        """
        TPLSinputchecker.TPLSinputchecker(testX,'testX')
