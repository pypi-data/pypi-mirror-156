            ...
            while True:
                ...
                if 'material' in line:  # end of the material atom fraction part
                    ...
                    if material.is_mat_used(materials, mid):
                        midx = material.get_material_index(materials, mid)
...
def match_cells_materials(cells, materials):
    ...
    for c in cells:
        mid = material.get_material_index(materials, c.mid)
        c.mat = materials[mid]
    return cells
    ...
    if out_phtn_src:
        ...
        for p in parts:
            output_part_photon_source(
                part=p, cooling_times_cul=cooling_times_cul,
                with_bounding_box=with_bounding_box)
#            p.output_photon_source(cooling_times_cul=cooling_times_cul)
            bar.next()
        bar.finish()


def output_part_photon_source(part, cooling_times_cul, with_bounding_box=False):
    """
    ...
    """
    filename = part.generate_output_filename('phtn_src')
    fo = open(filename, 'w')
    # block 1
    ...
    part_total = 0.0  # g/s
    for i, c in enumerate(part.part_cell_list):
        c_total = sum(c.gamma_emit_rate[0])*c.vol
        part_total += c_total

    for i, c in enumerate(part.part_cell_list):
        line = f"{utils.format_single_output(c.vol)}"
        # normalization
        c_tol = sum(c.gamma_emit_rate[0][:]) * c.vol
        ...
        if c_tol > 0:
            for j, ger in enumerate(c.gamma_emit_rate[0][:]):
                ger_norm[j] = c.gamma_emit_rate[0][j]/sum(c.gamma_emit_rate[0])
        # output
        cul_ger_norm = 0.0
...
def treat_nuc_responses(cells, parts, nuc_treatment, dict_cid_idx=None):
    ...
    for i, cnt in enumerate(item_nuc_treatments):
        cidx = cell.get_cell_index(
            cells, cnt[0], dict_cid_idx=dict_cid_idx)  # find cell index
...
def cells_aabb_from_ptrac(cells, ptrac_filename, dict_cid_idx):
    ...
    bar = IfBar("Matching src site and cells",  max=len(
        table), suffix='%(percent).1f%% - %(eta)ds')
    for row in table.iterrows():
        ncl = int(row['ncl'])
        r = [row['xxx'], row['yyy'], row['zzz']]
        cidx = dict_cid_idx[ncl]
        cells[cidx].update_aabb_bounds(r)
        if cells[cidx].mid > 0:
    ...
    print(f"  cells not covered by ptrac: {cells_not_covered}")
    return cells, aabb
            ...
            with open(mcnp_inp, 'r') as fin:
                ...
                while True:
                    ...
                    if utils.is_blank_line(line) and cell_start and not surf_start:
                        ...
                        fo.write(line)
                        continue
                    if mcnp_input.is_comment(line):
                        fo.write(line)
                        continue
                    if mcnp_input.is_cell_title(line):
                        cell_start = True
...
#    work_dir, config.get('model', 'part_cell_list'))
    ...
    for i in range(len(cells)):
        ...
        if fill_info[cells[i].id] is not None:
    ...
    with open(mcnp_inp, 'r') as fin:
        while True:
            ...
            if mcnp_input.is_comment(line):
                ...
                c = cells[cidx]
                # split the cell
                tmp_cells, cc_start, cc_end = cell.split_cell(c, coords=coords, distance=distance, direction=direction,
                ...
                if need_split:
                    ...
                    for i in range(len(tmp_cells)):
                        tmp_surf_id = i + cc_start + surf_id_start
                        oline = f"{tmp_cells[i].id}"
                        oline = utils.mcnp_style_str_append(oline, f"{mid}")
                        if mid > 0:
                            oline = utils.mcnp_style_str_append(
                                oline, f"{den}")
                        oline = utils.mcnp_style_str_append(
                            oline, mcnp_input.proper_cell_geom(cells_info[cid]))
                        if i == 0:
                            oline = utils.mcnp_style_str_append(
                                oline, f"-{tmp_surf_id}")
                        elif i > 0 and i < len(tmp_cells)-1:
                            oline = utils.mcnp_style_str_append(
                                oline, f"{tmp_surf_id-1} -{tmp_surf_id}")
                        else:
                            oline = utils.mcnp_style_str_append(
                                oline, f"{tmp_surf_id-1}")
                        # write imp and comments
                        oline = utils.mcnp_style_str_append(
                            oline, f"imp:n={tmp_cells[i].imp_n}")
                        if tmp_cells[i].imp_p is not None:
                            oline = utils.mcnp_style_str_append(
                                oline, f"imp:p={tmp_cells[i].imp_p}")
                        if tmp_cells[i].u is not None:
                            oline = utils.mcnp_style_str_append(
                                oline, f"u={tmp_cells[i].u}")
                        if tmp_cells[i].fill is not None:
                            oline = utils.mcnp_style_str_append(
                                oline, f"fill={tmp_cells[i].fill}")
                        oline = utils.mcnp_style_str_append(
                            oline, f"$ splitted from cell {cid}\n")
                        fo.write(oline)
