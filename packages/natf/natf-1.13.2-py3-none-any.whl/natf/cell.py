...
class Cell(object):
    ...
    def fill(self, value):
        if not isinstance(value, int):
            raise ValueError('fill must be an integer')
        if value < 0:
            raise ValueError('fill must be a number that no smaller than 0')
        self._fill = value
    ...
    def update_aabb_bounds(self, coords):
        """
        ...
        coords: list of float
            The coordinate of a site [xxx, yyy, zzz]
        """

        self.aabb_bounds[0] = min(self.aabb_bounds[0], coords[0])
        self.aabb_bounds[2] = min(self.aabb_bounds[2], coords[1])
        self.aabb_bounds[4] = min(self.aabb_bounds[4], coords[2])
        self.aabb_bounds[1] = max(self.aabb_bounds[1], coords[0])
        self.aabb_bounds[3] = max(self.aabb_bounds[3], coords[1])
        self.aabb_bounds[5] = max(self.aabb_bounds[5], coords[2])
...
def split_cell(c, coords, distance=10, geom='xyz', direction='x',
    ...
    if len(coords) == 0:  # do not split if the cell is small
        return [c], 0, 0
    # find the first and last cutting coords of the cell
    cutting_coords = []
    start, end = len(coords) + 1, 0
    idx_dir = (ord(direction.upper()) - ord('X'))*2
    # do not cut if cell length is small in the direction
    if (c.aabb_bounds[idx_dir+1] - c.aabb_bounds[idx_dir]) <= distance:
        return [c], 0, 0

    # determine the cut planes
    for i in range(len(coords)):
        if coords[i] > c.aabb_bounds[idx_dir] and coords[i] < c.aabb_bounds[idx_dir+1]:
