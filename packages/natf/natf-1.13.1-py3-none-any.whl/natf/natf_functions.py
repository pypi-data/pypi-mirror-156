...
from natf.utils import E_GROUP_FILES, format_single_output, \
...
from natf.mcnp_output import get_cell_tally_info, \
    get_cell_basic_info, get_tbr_from_mcnp_output, ptrac_to_hdf5
...
def output_part_photon_source(part, cooling_times_cul, with_bounding_box=False):
    ...
    for i, c in enumerate(part.part_cell_list):
        ...
        part_total += tol
    line = f"{format_single_output(part_total)}\n"
    fo.write(line)
    # block 3
    line = ''
    for i, c in enumerate(part.part_cell_list):
    ...
    fo.write(line+'\n')
    # block 4
    e_groups = get_e_group(e_group_size=24, unit='MeV',
                           reverse=False, with_lowest_bin=False)
    line = ''
    for i, e in enumerate(e_groups):
        if i == 0:
            line = f"{format_single_output(e)}"
        else:
            line = f"{line} {format_single_output(e)}"
    fo.write(line+'\n')
    ...
    for i, c in enumerate(part.part_cell_list):
        ...
        for j, ger in enumerate(ger_norm):
            ...
            line = f"{line} {format_single_output(cul_ger_norm)}"
        if with_bounding_box:
            for k in range(6):
                line = f"{line} {format_single_output(c.aabb_bounds[k])}"
        fo.write(line+'\n')
    fo.close()
...
def cells_aabb_from_ptrac(cells, ptrac_filename, dict_cid_idx):
    ...
    for i, cell in enumerate(cells):
        if cell.fill:
            cells_fill.append(cell.id)
        elif cell.imp_n == 0:
            cells_zero_importance.append(cell.id)
        elif not cell.has_valid_aabb_bounds():
            cells_not_covered.append(cell.id)
        else:
            cells_to_split.append(cell.id)
...
def natf_cell_mat_evo(config):
    ...
    if stat == 'POST':
        ...
        if current_step < total_steps - 1:
            ...
            update_mcnp_input_materials(
                mcnp_input, cells_need_cal, ofname, dict_cid_idx=dict_cid_idx)
            tally_numbers = get_tally_numbers(mcnp_input)
            update_mcnp_input_tallies(
