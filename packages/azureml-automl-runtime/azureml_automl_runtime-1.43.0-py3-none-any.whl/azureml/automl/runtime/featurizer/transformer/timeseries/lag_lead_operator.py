...
from azureml.automl.runtime.frequency_fixer import str_to_offset_safe
...
from .forecasting_base_estimator import AzureMLForecastTransformerBase, _GrainBasedStatefulTransformer
...
class LagLeadOperator(AzureMLForecastTransformerBase, OriginTimeMixin, _GrainBasedStatefulTransformer):
    """
    ...
    def _construct_one_lag_by_time_with_origin(self,
        ...
        if lag_order == 1:
            ...
            temp_df['lag_time'] = (temp_df[X.origin_time_column_name] - cast(
                pd.DateOffset, self.freq) * (lag_order - 1))
        ...
        if not self._no_original_order_column_safe():
            ...
            if (TimeSeriesInternal.DUMMY_ORDER_COLUMN in X.data.columns):
                if self._in_fit_transform:
                    target_dummies_name = \
                        MissingDummiesTransformer.get_column_name(cast(str, X.target_column_name))
                    X = X.from_data_frame_and_metadata(
                        X.data[X.data[TimeSeriesInternal.DUMMY_ORDER_COLUMN].notnull() |
    ...
    def _construct_all_lags_by_occurrence_with_horizon_origin(self, X: TimeSeriesDataSet) -> TimeSeriesDataSet:
        ...
        if self._add_occurrence_origin_column_safe():
            if new_origins is None:
                ...
                lag = \
                    self._construct_one_lag_by_occurrence_with_horizon_origin(X, cast(str, X.target_column_name), 1)
                new_origins = pd.DatetimeIndex(lag[X.time_column_name])
    ...
    def _set_lag_option(self, X: TimeSeriesDataSet) -> None:
        ...
        if len(lags_to_construct_safe) > 0:
            for lag_col in self.lags_to_construct.keys():
                ...
                if target_dummies_name not in X.data.columns:
        ...
        if len(lags_to_construct_safe) > 0:
            ...
            def get_trailing_by_grain(gr, Xgr):
                ...
                tail_start = (tail_series.index
                              .get_level_values(X.time_column_name).min())
            ...
            if X.time_series_id_column_names:
                df_cache = X.groupby_time_series_id().apply(
                    lambda Xgr: get_trailing_by_grain(Xgr.name, Xgr))  # type: pd.DataFrame
            else:
            ...
            if (self.lag_option == LagLeadOperator.LAG_BY_TIME):
                if self._cache is not None and self.backfill_cache:
                    ts_imputer = TimeSeriesImputer(input_column=cast(str, self._cache.target_column_name),
                                                   option='fillna',
                                                   method='bfill',
                                                   freq=self.freq)
    ...
    def transform(self, X: TimeSeriesDataSet) -> TimeSeriesDataSet:
           ...
           self._cache is not None:
            ...
            for lag_col in lags_to_construct_safe.keys():
                ...
                if missing_dummies_name in self._cache.data.columns:
                    Contract.assert_true(missing_dummies_name in X.data.columns,
        ...
        if self.lag_option == LagLeadOperator.LAG_BY_OCCURRENCE:
            if TimeSeriesInternal.HORIZON_NAME not in X.data.columns:
                temp_output = MaxHorizonFeaturizer(
                    self.max_horizon, origin_time_colname=self.origin_time_colname).fit_transform(temp_output)
            ...
            result = output[non_feature_columns].merge(
                interim_output.data[feature_columns], how='left',
        ...
        if (self.lag_option == LagLeadOperator.LAG_BY_TIME):
            if X.origin_time_column_name is None:
                temp_output = self.create_origin_times(
                    temp_output, self.max_horizon, freq=self.freq,
        ...
        return TimeSeriesDataSet(result,
                                 X.time_column_name,
    ...
    def fit_transform(
            ...
            **fit_params: Any) -> TimeSeriesDataSet:
    ...
    def __setstate__(self, state: Dict[Any, Any]) -> None:
        ...
        AzureMLForecastTransformerBase.__setstate__(self, state)
