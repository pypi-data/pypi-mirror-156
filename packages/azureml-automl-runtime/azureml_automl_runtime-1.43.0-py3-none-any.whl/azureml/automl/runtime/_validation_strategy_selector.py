...
def get_auto_validation_strategy(
    ...
    """
    Identify a model evaluation strategy between CV or train-valid split based on the size of input data.
    ...
    :param y: Targets in the training data.
    ...
    number_of_cv = _get_cv_number(X, y)
...
def _get_cv_number(X: DataInputType, y: DataInputType) -> int:
    ...
    res = constants.RuleBasedValidation.DEFAULT_N_CROSS_VALIDATIONS
    if sparse.issparse(X):
        res = constants.RuleBasedValidation.SPARSE_N_CROSS_VALIDATIONS
    for rule in constants.RuleBasedValidation.VALIDATION_LIMITS_NO_SPARSE:
        if rule.LOWER_BOUND <= X.shape[0] < rule.UPPER_BOUND:
            res = rule.NUMBER_OF_CV
            break
    if res != constants.RuleBasedValidation.DEFAULT_N_CROSS_VALIDATIONS:
        # if res was modified, ensure that the number of samples for minority class is at least as much as the
        # chosen n_cv so that the splits produced contain a balanced distribution of class labels.
        # If not, reset it to the default (i.e. a train-valid split)
        labels, counts = np.unique(y, return_counts=True)
        if min(counts) < res:
            logger.error(f"Number of samples for the minority class label ({min(counts)}) is less than chosen "
                         f"n_cv_folds ({res}). Resetting it to the default.")
            res = constants.RuleBasedValidation.DEFAULT_N_CROSS_VALIDATIONS
    return res
