...
def pad_short_grains(X: pd.DataFrame,
    ...
    lags, window_size, max_horizon, n_cross_validations, cv_step_size = \
        forecasting_heuristic_utils.try_get_auto_parameters(
            automl_settings, X, y, freq)
    min_points = get_min_points(
        window_size, lags, max_horizon, n_cross_validations, cv_step_size)
