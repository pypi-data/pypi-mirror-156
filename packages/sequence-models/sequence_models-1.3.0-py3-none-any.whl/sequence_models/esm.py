...
from esm.modules import TransformerLayer, LearnedPositionalEmbedding, RobertaLMHead, ESM1bLayerNorm, AxialTransformerLayer, ContactPredictionHead
...
model = MSATransformer(256, 256, 4, 4)
src = torch.randint(len(PROTEIN_ALPHABET), (3, 4, 5))
out = model(src)
class MSATransformer(nn.Module):
    ...
    def __init__(self, d_model, d_hidden, n_layers, n_heads, n_tokens=len(PROTEIN_ALPHABET),
        ...
        self.embed_positions = LearnedPositionalEmbedding(max_positions, d_model, padding_idx)
        self.emb_layer_norm_before = nn.LayerNorm(d_model)  # NOT SURE WHAT LayerNorm SHOULD BE
        self.emb_layer_norm_after = nn.LayerNorm(d_model)  # NOT SURE WHAT LayerNorm SHOULD BE
        self.lm_head = RobertaLMHead(
            embed_dim=d_model,
            output_dim=n_tokens,
            weight=self.embed_tokens.weight
