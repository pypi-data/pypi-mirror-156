...
def test_pnc_not_recognized(start_pnc_device):
    with pytest.raises(dev.HeinzingerPNCDeviceNotRecognizedError):
...
def test_pnc_set_voltage(start_pnc_device, device_data):
    ...
    with pytest.raises(dev.HeinzingerPNCMaxVoltageExceededError):
...
def test_pnc_set_current(start_pnc_device, device_data):
    ...
    with pytest.raises(dev.HeinzingerPNCMaxCurrentExceededError):
