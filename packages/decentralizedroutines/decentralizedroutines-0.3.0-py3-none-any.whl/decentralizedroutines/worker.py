...
def install_repo(command):
    ...
    runroutine = False
    if (GIT_USER=='') | (GIT_TOKEN=='') | (GIT_ACRONYM==''):
        Logger.log.error('Installing repo %s ERROR missing git token'\
                % (command['repo']))
    else:
        ...
        GIT_URL='https://'+GIT_USER+':'+GIT_TOKEN+'@'+GIT_SERVER+'/'+GIT_ACRONYM+'/'+command['repo']
...
routines = []
consumer = KinesisStreamConsumer('deepportfolio-workerpool')
producer = KinesisStreamProducer('deepportfolio-workerpool')
