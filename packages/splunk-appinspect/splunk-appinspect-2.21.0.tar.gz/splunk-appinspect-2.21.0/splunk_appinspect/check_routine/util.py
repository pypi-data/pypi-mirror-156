...
def validate_imports(  # noqa: C901
    js_files, html_files, bad_imports_set, bad_imports_set_2, risky_imports_set
):
    ...
    try:
        for filepath in js_files:
            with open(filepath, "r", encoding="utf-8") as my_file:
                ...
                bad_imports_in_file = []
                bad_imports_in_file_2 = []
                risky_imports_in_file = []
                for match in matches:
                    if match in bad_imports_set:
                        bad_imports_in_file.append(match)
                    elif match in bad_imports_set_2:
                        bad_imports_in_file_2.append(match)
                ...
                if bad_imports_in_file_2:
                    identified_files["bad_2"] = bad_imports_in_file_2
        ...
        for filepath in html_files:
            with open(filepath, "r", encoding="utf-8") as my_file:
                ...
                if bad_imports_in_file_2:
                    identified_files["bad_html_2"] = bad_imports_in_file_2
                if risky_imports_in_file:
...
def communicate_bad_import_message(reporter, file_list):
    ...
    for file_object in file_list:
        ...
        if "bad_2" in identified_files:
            bad_imports_list_2 = identified_files["bad_2"]
            message = (
                "Embed all your app's front-end JS dependencies in the /appserver directory. "
                "If you import files from Splunk Web, your app might fail when Splunk Web updates "
                f"in the future. Bad imports: {bad_imports_list_2}"
            )
        ...
        if "bad_html" in identified_files:
            ...
            reporter.warn(message, source_file)
        if "bad_html_2" in identified_files:
            bad_html_imports_list_2 = identified_files["bad_html_2"]
            message = (
                "Embed all your app's front-end JS dependencies in the /appserver directory. "
                "If you import files from Splunk Web, your app might fail when Splunk Web updates "
                f"in the future. Bad imports (HTML): {bad_html_imports_list}"
