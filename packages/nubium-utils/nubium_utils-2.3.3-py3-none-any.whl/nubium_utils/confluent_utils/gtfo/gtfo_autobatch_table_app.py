...
class GtfoAutoBatchTableApp(GtfoTableApp):
            ...
            try:
                ...
                try:
                    while self._keep_consuming(table_recovery=True) and self._pending_table_recoveries and checks:
                        ...
                        if self.transaction.message_count == self._consume_max_count * 10:
                            LOGGER.info(
                                f"RECOVERY STATUS: transaction_offset - {self.transaction.offset() + 2}, watermark - {self._pending_table_recoveries[p]['watermarks'][1]}")
    ...
    def _app_run(self, *args, **kwargs):
        ...
        try:
            ...
            pass
        if self.transaction._pending_table_offset_increase or self.transaction._commit_offsets:
