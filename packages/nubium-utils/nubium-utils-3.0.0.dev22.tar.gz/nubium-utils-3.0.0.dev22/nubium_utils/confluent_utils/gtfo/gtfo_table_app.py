...
from .gtfo_app import GtfoApp, Transaction, GracefulTransactionFailure, FatalTransactionFailure
import logging
        ...
        for p in partitions:
            try:
                ...
                if table.active_lock:
                    ...
                    self.tables[p].close()
                #     if self.tables[p].lock_file_exists():
                #         LOGGER.warning(f'LOCK file still exists!')
                # del self.tables[p]
