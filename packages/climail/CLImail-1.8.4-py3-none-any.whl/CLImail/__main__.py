...
while True:
    try:
        ...
        check_mail.set_defaults(func=lambda:
                                list(map(lambda m: (print(U.mail_from_template(U.mail_from_id(m))), [print(colorama.Fore.GREEN + (i.rsplit("\\")[-1] + ' was saved at ' + i + '!')) for i in U.save_attachments(U.mail_from_id(m), args.path)] if args.save else None), U.mail_ids_as_str(args.size))))  # don't even ask
        ...
        save.set_defaults(func=lambda: print(
            *map((lambda x, y='\\': colorama.Fore.GREEN + f'{x.split(y)[-1]} was saved at {x}!'), U.save_attachments(U.mail_from_id(args.id), args.path)), sep='\n'))
