...
First step to run the package is you have to configure you aws account credentials in .aws folder in credentials file.
...
--threads
For number of threads you have to provide the number for e.g 23 , 30 90 etc
...
--s3_bucket_name
Name of S3 bucket which will be later used for storing the scan results.

--profile            
Arn of aws profile for e.g arn:aws:iam::account_id:user/user.name or you can get these details by runing (aws sts get-caller-identity) this command in cli

### How to run package:

Create the virtual environment and intall the quiet-riot pkg using pip.

First you have to install the package using pip command in command line. for e.g pip install quiet-riot

For help you can run the command quiet_riot --help
