        ...
        try:
            ...
            with open(wordlist_file) as file:
                ...
                if wordlist_type == 'roles':
                    ...
                    results_file = loadbalancer.threader(loadbalancer.getter(thread =input_args.threads,wordlist=wordlist),session=session)
                    # print(results_file)
                    return results_file
                    break
                elif wordlist_type == 'footprint':
...
def main():
    ...
    parser.add_argument('--scan_type',required=True, type=int, default=1,
    ...
    parser.add_argument('--s3_bucket_name', type=str, default="",
                        help=textwrap.dedent('''Name of S3 bucket which will be later used for storing the scan results.
    ...
    wordlist_type = str(input_args.scan_type)
    def sub_scan_type():
        print("")
        print("1. IAM Roles")
        print("")
        ...
        sub_iam_type = input("Kindly select on of the above scan types:")
        while True:
            if sub_iam_type == "1":
                wordlist_type = "4"
                                   ...
                                   "Statement": [
                                       {
                                           ...
                                           "Effect": "Allow",
                                           "Principal": {
                                               "AWS": f'{str(sts.get_caller_identity()["Arn"])}'
                                           },
                                           "Action": "*",
                                           "Resource": [
                                               f'arn:aws:s3:::{str(input_args.s3_bucket_name)}/*'
    ...
    if str(input_args.s3_bucket_name) != "":
        ...
        try:
            ...
            bucket_obj_url = s3.generate_presigned_url('get_object',
                                                        ...
                                                        ExpiresIn=604800)
            print("")
            print("Url for the result file is as following you can directly download the results file.")
            print("")
