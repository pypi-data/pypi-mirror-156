...
def bnd_docker():
    ...
    if supervised:
        supervisor = SupervisorServer(int(os.environ.get('SUPERVISOR_PORT', '8081')))
        supervisor_thread = Thread(target=supervisor.start)
...
def gbd_ecs_manager():
    ...
    if args.action == 'start':
        if args.service == 'solr':
            ECSManager.start_service(args.cluster, anti_pattern=[args.service], nb_tasks=args.nb_tasks)
        else:
            ECSManager.start_service(args.cluster, pattern=[args.service], nb_tasks=args.nb_tasks)
    if args.action == 'stop':
        if args.service == 'solr':
            ECSManager.stop_service(args.cluster, anti_pattern=[args.service])
        else:
