...
class Stocks(Config):
        ...
        def extract_forecast(self,df_prep,df_fcst,df_invopt,date_cols,frequency_,months_,location,column_forecast='ForecastCollab',weeks_=4,join_='left'):      
            """
            ...
            : param df_invopt: Inventory's Dataframe with the columns Item, Location(Optional), SalesHistory
            ...
            >>> df_extract =
                                                Item    Location  SalesHistory   SuggestedForecast
                                    idx0          85      905         23              200
                                    idx1          102     487         95              100
            """ 
            try:
        ...
        def forecast_with_leadtimes(self,df_LeadTimes ,df_Forecast,df_Prep,df_inv,column_forecast,frequency_,location,actualdate,join_='left'):
            ...
            : param df_Prep: Dataframe prepared for Forecast
            : param df_inv: Inventory's Dataframe with the columns Item, Location(Optional), SalesHistory
            : param column_forecast: name of the column where the desired forecast is located 
            : param frequency_: Target frequency to the dataset
            : param location: Boolean to enable the use of Location in the Inventory's dataframe
            ...
            try:                
                ...
                df_lead_cruce=df_lead_cruce.groupby(['Item','PurchaseDays'], as_index=False).mean()
                df_lead_cruce=df_lead_cruce[['Item','PurchaseDays']].drop_duplicates() 
                ...
                if frequency_ == 'M':
                      df_final_fcst.loc[df_final_fcst['PurchaseDays'].isnull(),'PurchaseDays'] = 29
                      # PurchaseDays <=30 -----------------------------------------------------------------
                      df_fcst_1 = df_final_fcst[df_final_fcst['PurchaseDays'] <= 30]
                      if not df_fcst_1.empty:       
                        df_fcst_1 = df_fcst_1[(df_fcst_1['Date']>df_Prep['timestamp'].max())&
                              (df_fcst_1['Date']<= (df_Prep['timestamp'].max()+relativedelta(months=1)+MonthEnd(pmonth)))] 
                        df_fcst_1a=df_fcst_1.drop_duplicates()
                        df_fcst_1b=df_fcst_1.drop(columns=['PurchaseDays'])
                        df_fcst_1b=df_fcst_1b.drop_duplicates()
                        df_extract_forecast1 = self.extract_forecast(df_Prep, df_fcst_1b, df_inv, column_forecast=column_forecast, location=location,frequency_= frequency_ ,
                        ...
                        df1=df_fcst_1a[['Item','PurchaseDays']].drop_duplicates() 
                        df_final_1= pd.merge(df_extract_forecast1,df1,on=['Item'],how='left')  
                        df_final_1[column_forecast] = (df_final_1[column_forecast] * df_final_1['PurchaseDays'])/(1*30)
                        df_final_1=df_final_1.drop(columns=['PurchaseDays']) 
                      ...
                      # 30< PurchaseDays <=60  -----------------------------------------------------------------
                      df_fcst_2 = df_final_fcst[(df_final_fcst['PurchaseDays'] > 30) & (df_final_fcst['PurchaseDays'] <= 60)]
                      if not df_fcst_2.empty:     
                        df_fcst_2 = df_fcst_2[(df_fcst_2['Date']>df_Prep['timestamp'].max())&
                                      (df_fcst_2['Date']<= (df_Prep['timestamp'].max()+relativedelta(months=2)+MonthEnd(pmonth)))]    
                        df_fcst_2a=df_fcst_2.drop_duplicates()
                        df_fcst_2b=df_fcst_2.drop(columns=['PurchaseDays'])
                        df_fcst_2b=df_fcst_2b.drop_duplicates()
                        df_extract_forecast2 = self.extract_forecast(df_Prep, df_fcst_2b, df_inv, column_forecast=column_forecast, location=location,frequency_= frequency_ ,
                        ...
                        df2=df_fcst_2a[['Item','PurchaseDays']].drop_duplicates() 
                        df_final_2= pd.merge(df_extract_forecast2,df2,on=['Item'],how='left')  
                        df_final_2[column_forecast] = (df_final_2[column_forecast] * df_final_2['PurchaseDays'])/(2*30)
                        df_final_2=df_final_2.drop(columns=['PurchaseDays']) 
                      ...
                      # 60< PurchaseDays <=90  -----------------------------------------------------------------
                      df_fcst_3 = df_final_fcst[(df_final_fcst['PurchaseDays'] > 60) & (df_final_fcst['PurchaseDays'] <= 90)]
                      if not df_fcst_3.empty:      
                        df_fcst_3 = df_fcst_3[(df_fcst_3['Date']>df_Prep['timestamp'].max())&
                                      (df_fcst_3['Date']<= (df_Prep['timestamp'].max()+relativedelta(months=3)+MonthEnd(pmonth)))]    
                        df_fcst_3a=df_fcst_3.drop_duplicates()
                        df_fcst_3b=df_fcst_3.drop(columns=['PurchaseDays'])
                        df_fcst_3b=df_fcst_3b.drop_duplicates()
                        df_extract_forecast3 = self.extract_forecast(df_Prep, df_fcst_3b, df_inv, column_forecast=column_forecast, location=location,frequency_= frequency_ ,
                        ...
                        df3=df_fcst_3a[['Item','PurchaseDays']].drop_duplicates() 
                        df_final_3= pd.merge(df_extract_forecast3,df3,on=['Item'],how='left')  
                        df_final_3[column_forecast] = (df_final_3[column_forecast] * df_final_3['PurchaseDays'])/(3*30)
                        df_final_3=df_final_3.drop(columns=['PurchaseDays'])
                      ...
                      # 90< PurchaseDays <=120  -----------------------------------------------------------------
                      df_fcst_4 = df_final_fcst[(df_final_fcst['PurchaseDays'] > 90) & (df_final_fcst['PurchaseDays'] <= 120)]
                      if not df_fcst_4.empty:     
                        df_fcst_4 = df_fcst_4[(df_fcst_4['Date']>df_Prep['timestamp'].max())&
                                      (df_fcst_4['Date']<= (df_Prep['timestamp'].max()+relativedelta(months=4)+MonthEnd(pmonth)))]  
                        df_fcst_4a=df_fcst_4.drop_duplicates()
                        df_fcst_4b=df_fcst_4.drop(columns=['PurchaseDays'])
                        df_fcst_4b=df_fcst_4b.drop_duplicates()
                        df_extract_forecast4 = self.extract_forecast(df_Prep, df_fcst_4b, df_inv, column_forecast=column_forecast, location=location,frequency_= frequency_ ,
                        ...
                        df4=df_fcst_4a[['Item','PurchaseDays']].drop_duplicates() 
                        df_final_4= pd.merge(df_extract_forecast4,df4,on=['Item'],how='left')  
                        df_final_4[column_forecast] = (df_final_4[column_forecast] * df_final_4['PurchaseDays'])/(4*30)
                        df_final_4=df_final_4.drop(columns=['PurchaseDays'])
                      ...
                      # 120< PurchaseDays <=150  -----------------------------------------------------------------
                      df_fcst_5 = df_final_fcst[(df_final_fcst['PurchaseDays'] > 120) & (df_final_fcst['PurchaseDays'] <= 150)]
                      if not df_fcst_5.empty:     
                        df_fcst_5 = df_fcst_5[(df_fcst_5['Date']>df_Prep['timestamp'].max())&
                                      (df_fcst_5['Date']<= (df_Prep['timestamp'].max()+relativedelta(months=5)+MonthEnd(pmonth)))]
                        df_fcst_5a=df_fcst_5.drop_duplicates()
                        df_fcst_5b=df_fcst_5.drop(columns=['PurchaseDays'])
                        df_fcst_5b=df_fcst_5b.drop_duplicates()
                        df_extract_forecast5 = self.extract_forecast(df_Prep, df_fcst_5b, df_inv, column_forecast=column_forecast, location=location,frequency_= frequency_ ,
                        ...
                        df5=df_fcst_5a[['Item','PurchaseDays']].drop_duplicates() 
                        df_final_5= pd.merge(df_extract_forecast5,df5,on=['Item'],how='left')  
                        df_final_5[column_forecast] = (df_final_5[column_forecast] * df_final_5['PurchaseDays'])/(5*30)
                        df_final_5=df_final_5.drop(columns=['PurchaseDays'])
                      ...
                      # 150< PurchaseDays <=180  -----------------------------------------------------------------
                      df_fcst_6 = df_final_fcst[(df_final_fcst['PurchaseDays'] > 150) & (df_final_fcst['PurchaseDays'] <= 180)]
                      if not df_fcst_6.empty:
                        df_fcst_6 = df_fcst_6[(df_fcst_6['Date']>df_Prep['timestamp'].max())&
                                      (df_fcst_6['Date']<= (df_Prep['timestamp'].max()+relativedelta(months=6)+MonthEnd(pmonth)))]   
                        df_fcst_6a=df_fcst_6.drop_duplicates()
                        df_fcst_6b=df_fcst_6.drop(columns=['PurchaseDays'])
                        df_fcst_6b=df_fcst_6b.drop_duplicates()
                        df_extract_forecast6 = self.extract_forecast(df_Prep, df_fcst_6b, df_inv, column_forecast=column_forecast, location=location,frequency_= frequency_ ,
                        ...
                        df6=df_fcst_6a[['Item','PurchaseDays']].drop_duplicates() 
                        df_final_6= pd.merge(df_extract_forecast6,df6,on=['Item'],how='left')  
                        df_final_6[column_forecast] = (df_final_6[column_forecast] * df_final_6['PurchaseDays'])/(6*30)
                        df_final_6=df_final_6.drop(columns=['PurchaseDays'])
                      ...
                      # PurchaseDays > 180  -----------------------------------------------------------------
                      df_fcst_7 = df_final_fcst[(df_final_fcst['PurchaseDays'] > 180)]
                      if not df_fcst_7.empty:
                        df_fcst_7= df_fcst_7[(df_fcst_7['Date']>df_Prep['timestamp'].max()+MonthEnd(pmonth))]       
                        df_fcst_7a=df_fcst_7.drop_duplicates()
                        df_fcst_7b=df_fcst_7.drop(columns=['PurchaseDays'])
                        df_fcst_7b=df_fcst_7b.drop_duplicates()
                        df_extract_forecast7 = self.extract_forecast(df_Prep, df_fcst_7b, df_inv, column_forecast=column_forecast, location=location,frequency_= frequency_ ,
                        ...
                        df7=df_fcst_7a[['Item','PurchaseDays']].drop_duplicates() 
                        df_final_7= pd.merge(df_extract_forecast7,df7,on=['Item'],how='left')  
                        df_final_7[column_forecast] = (df_final_7[column_forecast] * df_final_7['PurchaseDays'])/(int(len(df_fcst_7['Date'].unique()))*30)
                        df_final_7=df_final_7.drop(columns=['PurchaseDays'])
                ...
                if frequency_ == 'W':
                      df_final_fcst.loc[df_final_fcst['PurchaseDays'].isnull(),'PurchaseDays'] = 6
                      # PurchaseDays <=7 -----------------------------------------------------------------
                      df_fcst_1 = df_final_fcst[df_final_fcst['PurchaseDays'] <= 7]
                      if not df_fcst_1.empty:       
                        df_fcst_1 = df_fcst_1[(df_fcst_1['Date']>df_Prep['timestamp'].max())&
                                    (df_fcst_1['Date']<= (df_Prep['timestamp'].max()+relativedelta(weeks=1)))]
                        df_fcst_1a=df_fcst_1.drop_duplicates()
                        df_fcst_1b=df_fcst_1.drop(columns=['PurchaseDays'])
                        df_fcst_1b=df_fcst_1b.drop_duplicates()
                        df_extract_forecast1 = self.extract_forecast(df_Prep, df_fcst_1b, df_inv, column_forecast=column_forecast, location=location,frequency_= frequency_ ,
                        ...
                        df1=df_fcst_1a[['Item','PurchaseDays']].drop_duplicates() 
                        df_final_1= pd.merge(df_extract_forecast1,df1,on=['Item'],how='left')  
                        df_final_1[column_forecast] = (df_final_1[column_forecast] * df_final_1['PurchaseDays'])/(1*7)
                        df_final_1=df_final_1.drop(columns=['PurchaseDays']) 
                      ...
                      # 7< PurchaseDays <=14 -----------------------------------------------------------------
                      df_fcst_2 = df_final_fcst[(df_final_fcst['PurchaseDays'] > 7) & (df_final_fcst['PurchaseDays'] <= 14)]
                      if not df_fcst_2.empty:     
                        df_fcst_2 = df_fcst_2[(df_fcst_2['Date']>df_Prep['timestamp'].max())&
                                              (df_fcst_2['Date']<= (df_Prep['timestamp'].max()+relativedelta(weeks=2)))] 
                        df_fcst_2a=df_fcst_2.drop_duplicates()
                        df_fcst_2b=df_fcst_2.drop(columns=['PurchaseDays'])
                        df_fcst_2b=df_fcst_2b.drop_duplicates()
                        df_extract_forecast2 = self.extract_forecast(df_Prep, df_fcst_2b, df_inv, column_forecast=column_forecast, location=location,frequency_= frequency_ ,
                        ...
                        df2=df_fcst_2a[['Item','PurchaseDays']].drop_duplicates() 
                        df_final_2= pd.merge(df_extract_forecast2,df2,on=['Item'],how='left')  
                        df_final_2[column_forecast] = (df_final_2[column_forecast] * df_final_2['PurchaseDays'])/(2*14)
                        df_final_2=df_final_2.drop(columns=['PurchaseDays']) 
                      ...
                      # 14< PurchaseDays <=21  -----------------------------------------------------------------
                      df_fcst_3 = df_final_fcst[(df_final_fcst['PurchaseDays'] > 14) & (df_final_fcst['PurchaseDays'] <= 21)]
                      if not df_fcst_3.empty:      
                        df_fcst_3 = df_fcst_3[(df_fcst_3['Date']>df_Prep['timestamp'].max())&
                                              (df_fcst_3['Date']<= (df_Prep['timestamp'].max()+relativedelta(weeks=3)))] 
                        df_fcst_3a=df_fcst_3.drop_duplicates()
                        df_fcst_3b=df_fcst_3.drop(columns=['PurchaseDays'])
                        df_fcst_3b=df_fcst_3b.drop_duplicates()
                        df_extract_forecast3 = self.extract_forecast(df_Prep, df_fcst_3b, df_inv, column_forecast=column_forecast, location=location,frequency_= frequency_ ,
                        ...
                        df3=df_fcst_3a[['Item','PurchaseDays']].drop_duplicates() 
                        df_final_3= pd.merge(df_extract_forecast3,df3,on=['Item'],how='left')  
                        df_final_3[column_forecast] = (df_final_3[column_forecast] * df_final_3['PurchaseDays'])/(3*7)
                        df_final_3=df_final_3.drop(columns=['PurchaseDays'])
                      ...
                      # 21< PurchaseDays <=28  -----------------------------------------------------------------
                      df_fcst_4 = df_final_fcst[(df_final_fcst['PurchaseDays'] > 21) & (df_final_fcst['PurchaseDays'] <= 28)]
                      if not df_fcst_4.empty:     
                        df_fcst_4 = df_fcst_4[(df_fcst_4['Date']>df_Prep['timestamp'].max())&
                                              (df_fcst_4['Date']<= (df_Prep['timestamp'].max()+relativedelta(weeks=4)))]
                        df_fcst_4a=df_fcst_4.drop_duplicates()
                        df_fcst_4b=df_fcst_4.drop(columns=['PurchaseDays'])
                        df_fcst_4b=df_fcst_4b.drop_duplicates()
                        df_extract_forecast4 = self.extract_forecast(df_Prep, df_fcst_4b, df_inv, column_forecast=column_forecast, location=location,frequency_= frequency_ ,
                        ...
                        df4=df_fcst_4a[['Item','PurchaseDays']].drop_duplicates() 
                        df_final_4= pd.merge(df_extract_forecast4,df4,on=['Item'],how='left')  
                        df_final_4[column_forecast] = (df_final_4[column_forecast] * df_final_4['PurchaseDays'])/(4*7)
                        df_final_4=df_final_4.drop(columns=['PurchaseDays'])
                      ...
                      # 28< PurchaseDays <=35  -----------------------------------------------------------------
                      df_fcst_5 = df_final_fcst[(df_final_fcst['PurchaseDays'] > 28) & (df_final_fcst['PurchaseDays'] <= 35)]
                      if not df_fcst_5.empty:     
                        df_fcst_5 = df_fcst_5[(df_fcst_5['Date']>df_Prep['timestamp'].max())&
                                              (df_fcst_5['Date']<= (df_Prep['timestamp'].max()+relativedelta(weeks=5)))]
                        df_fcst_5a=df_fcst_5.drop_duplicates()
                        df_fcst_5b=df_fcst_5.drop(columns=['PurchaseDays'])
                        df_fcst_5b=df_fcst_5b.drop_duplicates()
                        df_extract_forecast5 = self.extract_forecast(df_Prep, df_fcst_5b, df_inv, column_forecast=column_forecast, location=location,frequency_= frequency_ ,
                        ...
                        df5=df_fcst_5a[['Item','PurchaseDays']].drop_duplicates() 
                        df_final_5= pd.merge(df_extract_forecast5,df5,on=['Item'],how='left')  
                        df_final_5[column_forecast] = (df_final_5[column_forecast] * df_final_5['PurchaseDays'])/(5*7)
                        df_final_5=df_final_5.drop(columns=['PurchaseDays'])
                      ...
                      # 35< PurchaseDays <=42  -----------------------------------------------------------------
                      df_fcst_6 = df_final_fcst[(df_final_fcst['PurchaseDays'] > 35) & (df_final_fcst['PurchaseDays'] <= 42)]
                      if not df_fcst_6.empty:
                        df_fcst_6 = df_fcst_6[(df_fcst_6['Date']>df_Prep['timestamp'].max())&
                                              (df_fcst_6['Date']<= (df_Prep['timestamp'].max()+relativedelta(weeks=6)))] 
                        df_fcst_6a=df_fcst_6.drop_duplicates()
                        df_fcst_6b=df_fcst_6.drop(columns=['PurchaseDays'])
                        df_fcst_6b=df_fcst_6b.drop_duplicates()
                        df_extract_forecast6 = self.extract_forecast(df_Prep, df_fcst_6b, df_inv, column_forecast=column_forecast, location=location,frequency_= frequency_ ,
                        ...
                        df6=df_fcst_6a[['Item','PurchaseDays']].drop_duplicates() 
                        df_final_6= pd.merge(df_extract_forecast6,df6,on=['Item'],how='left')  
                        df_final_6[column_forecast] = (df_final_6[column_forecast] * df_final_6['PurchaseDays'])/(6*7)
                        df_final_6=df_final_6.drop(columns=['PurchaseDays'])
                      ...
                      # PurchaseDays > 42  -----------------------------------------------------------------
                      df_fcst_7 = df_final_fcst[(df_final_fcst['PurchaseDays'] > 42)]
                      if not df_fcst_7.empty:
                        df_fcst_7 = df_fcst_7[(df_fcst_7['Date']>df_Prep['timestamp'].max())] 
                        df_fcst_7a=df_fcst_7.drop_duplicates()
                        df_fcst_7b=df_fcst_7.drop(columns=['PurchaseDays'])
                        df_fcst_7b=df_fcst_7b.drop_duplicates()
                        df_extract_forecast7 = self.extract_forecast(df_Prep, df_fcst_7b, df_inv, column_forecast=column_forecast, location=location,frequency_= frequency_ ,
                        ...
                        df7=df_fcst_7a[['Item','PurchaseDays']].drop_duplicates() 
                        df_final_7= pd.merge(df_extract_forecast7,df7,on=['Item'],how='left')  
                        df_final_7[column_forecast] = (df_final_7[column_forecast] * df_final_7['PurchaseDays'])/(7*int(len(df_fcst_7['Date'].unique())))
                        df_final_7=df_final_7.drop(columns=['PurchaseDays'])
        ...
        def forecast_with_leadtimes_location(self,df_LeadTimes,df_Forecast,df_Prep,df_inv,column_forecast,frequency_,location,actualdate,join_='left'):      
            ...
            try:
                df_lead_cruce= df_LeadTimes.copy()
                df_lead_cruce=df_lead_cruce.groupby(['Item','Location','PurchaseDays'], as_index=False).mean()
                df_lead_cruce=df_lead_cruce[['Item','Location','PurchaseDays']].drop_duplicates() 
                df_fcst_cruce = df_Forecast.copy()
                ...
                if frequency_ == 'M':
                      ...
                      if not df_fcst_1.empty:       
                        ...
                        df1=df_fcst_1a[['Item','Location','PurchaseDays']].drop_duplicates() 
                      ...
                      if not df_fcst_2.empty:     
                        ...
                        df2=df_fcst_2a[['Item','Location','PurchaseDays']].drop_duplicates() 
                      ...
                      if not df_fcst_3.empty:      
                        ...
                        df3=df_fcst_3a[['Item','Location','PurchaseDays']].drop_duplicates() 
                      ...
                      if not df_fcst_4.empty:     
                        ...
                        df4=df_fcst_4a[['Item','Location','PurchaseDays']].drop_duplicates() 
                      ...
                      if not df_fcst_5.empty:     
                        ...
                        df5=df_fcst_5a[['Item','Location','PurchaseDays']].drop_duplicates() 
                      ...
                      if not df_fcst_6.empty:
                        ...
                        df6=df_fcst_6a[['Item','Location','PurchaseDays']].drop_duplicates() 
                      ...
                      if not df_fcst_7.empty:
                        ...
                        df7=df_fcst_7a[['Item','Location','PurchaseDays']].drop_duplicates() 
        ...
        def next_forecast_with_leadtimes(self,df_LeadTimes ,df_Forecast,df_Prep,df_inv,column_forecast,frequency_,location,actualdate,join_='left'):
            """
            ...
            >>> df_forecats = next_forecast_with_leadtimes(df_LeadTimes=df_lead_time, 
                                                            df_Forecast=df_fcst,
                                                            df_Prep=df_prep,
                                                            df_inv=df_inv,
                                                            column_forecast='SuggestedForecast',
                                                            ...
                                                            actualdate=timestamp,
                                                            join_='left')
            >>> next_extract_forecast_by_leadtimes =
                                                Item    Location  SalesHistory   SuggestedForecast
                                    idx0          85      905         23              200
                                    idx1          102     487         95              100
            """ 
        ...
        def next_forecast_with_leadtimes_location(self,df_LeadTimes,df_Forecast,df_Prep,df_inv,column_forecast,frequency_,location,actualdate,join_='left'):      
            """
            ...
            >>> df_forecast = next_forecast_with_leadtimes_location(df_LeadTimes=df_lead_time, 
                                                            df_Forecast=df_fcst,
                                                            df_Prep=df_prep,
                                                            df_inv=df_inv,
                                                            column_forecast='SuggestedForecast',
                                                            ...
                                                            actualdate=timestamp,
                                                            join_='left')
            >>> next_forecast_with_leadtimes_location =
                                                Item    Location  SalesHistory   SuggestedForecast
                                    idx0          85      905         23              200
                                    idx1          102     487         95              100
            """ 
        ...
        def extract_max_sales(self,df_prep, df_invopt,date_cols,location=True,weeks_=16,months_=4,frequency_='W'):
            """
            ...
            try:      
              ...
              df_extract['MaxDailySales'] = df_extract['AvgDailyUsage'] + df_extract['MaxDailySales']
        ...
        def functions_inventory(self,df_inv,timestamp,frequency_,wd=False):

            """
            Return a dataframe with all the indicators 
          
            : param df_inv: Inventory's Dataframe with the columns Item, Location(Optional), Inventory, InTransit, SalesHistory  SuggestedForecast AvgDailyUsage MaxDailySales           
            : param timestamp: current day 
            : param frequency_: Target frequency to the dataset
            : param wd: Boolean to allow the use of working days 
            
            >>> df_inv = functions_inventory(df_inv,timestamp,frequency_='M',wd=False)  

            """
            try:
                df=df_inv.copy()
                lastdayDict={'1':'31', '2':'28', '3':'31', '4':'30', '5':'31', '6':'30', '7':'31', '8':'31', '9':'30', '10':'31', '11':'30', '12':'31'}
                DayOfMonth= int(timestamp[6:8])
                ...
                df['Availability'] = df['Inventory'] + df['InTransit']
                df['SuggestedAvailability'] = df['Availability'] - df['SuggestedForecast']
                df['LeadTimeDemand'] = df['AvgLeadTime'] * df['AvgDailyUsage']
                df['SecurityStock'] = ((df['MaxDailySales']*df['MaxLeadTime']) - (df['AvgDailyUsage']*df['AvgLeadTime']))
                ...
                df['SuggestedReorderQty'] = df['Availability'] -df['SuggestedForecast'] - df['SecurityStock']
                df['ReorderStatus']=df[['SuggestedReorderQty','ReorderPoint']].apply(lambda x: 'Purchase' if x['SuggestedReorderQty']<0 or x['SuggestedReorderQty']<= x['ReorderPoint'] else 'Hold', axis=1)
                
                if frequency_ == 'M':
                  df['RQty'] = (df['Inventory']+df['InTransit']-df['Committed']-df['SuggestedForecast']-(df['SuggestedForecastToDate']*(1-DayOfMonth/DaysOfMonth))-df['SecurityStock']).abs()
                  df['ReorderQty'] = df[['ReorderStatus','RQty']].apply(lambda x: x['RQty'] if (x['ReorderStatus']=='Purchase') else 0 , axis=1 )
                  df['ReorderQty']    = df[['ReorderQty','ReorderStatus']].apply(lambda x: (1 if (x['ReorderQty'] <= 1) else x['ReorderQty']) if(x['ReorderStatus']=='Purchase') else x['ReorderQty'], axis=1)
                  
                  df['MinQty'] = (df['Inventory']+df['InTransit']-df['Committed']-(df['NextSuggestedForecast'])-(df['NextSuggestedForecastToDate']*(1-DayOfMonth/DaysOfMonth))-df['SecurityStock'] ).abs()   
                  df['MaxQty'] = (df['Inventory']+df['InTransit']-df['Committed']-(df['BackSuggestedForecast'])-(df['BackSuggestedForecastToDate']*(1-DayOfMonth/DaysOfMonth))-df['SecurityStock'] ).abs()
                  
                  df['MinReorderQty'] = df[['ReorderStatus','MinQty','MaxQty']].apply(lambda x: (x['MinQty'] if (x['MinQty']<x['MaxQty']) else x['MaxQty']) if (x['ReorderStatus']=='Purchase') else 0 , axis=1 )
                  df['MinReorderQty'] = df[['MinReorderQty','ReorderStatus']].apply(lambda x: (1 if (x['MinReorderQty'] <= 1) else x['MinReorderQty']) if(x['ReorderStatus']=='Purchase') else x['MinReorderQty'], axis=1)
                  
                  df['MaxReorderQty'] = df[['ReorderStatus','MinQty','MaxQty']].apply(lambda x: (x['MinQty'] if (x['MinQty']>x['MaxQty']) else x['MaxQty']) if (x['ReorderStatus']=='Purchase') else 0 , axis=1 )
                  df['MaxReorderQty'] = df[['MaxReorderQty','ReorderStatus']].apply(lambda x: (1 if (x['MaxReorderQty'] <= 1 )else x['MaxReorderQty']) if(x['ReorderStatus']=='Purchase') else x['MaxReorderQty'], axis=1)
                  
                  df['RQtyToDate'] =(df['Inventory']+df['InTransit']-df['Committed']-(df['SuggestedForecastToDate']*(1-DayOfMonth/DaysOfMonth))-df['SecurityStock']).abs()
                  df['ReorderQtyToDate'] = df[['ReorderStatus','RQtyToDate']].apply(lambda x: x['RQtyToDate'] if (x['ReorderStatus']=='Purchase') else 0 , axis=1 )
                  df['ReorderQtyToDate'] = df[['ReorderQtyToDate','ReorderStatus']].apply(lambda x: (1 if (x['ReorderQtyToDate'] <= 1) else x['ReorderQtyToDate']) if(x['ReorderStatus']=='Purchase') else x['ReorderQtyToDate'], axis=1)

                  df['SuggestedReorderQtyToDate']=df['SuggestedForecast']*(1-DayOfMonth/DaysOfMonth)

                if frequency_ == 'W':
                  df['RQty'] = (df['Inventory']+df['InTransit']-df['Committed']-df['SuggestedForecast']-(df['SuggestedForecastToDate']*(1-DayOfWeek/DaysOfWeek))-df['SecurityStock']).abs()
                  df['ReorderQty'] = df[['ReorderStatus','RQty']].apply(lambda x: x['RQty'] if (x['ReorderStatus']=='Purchase') else 0 , axis=1 )
                  df['ReorderQty']    = df[['ReorderQty','ReorderStatus']].apply(lambda x: (1 if (x['ReorderQty'] <= 1) else x['ReorderQty']) if(x['ReorderStatus']=='Purchase') else x['ReorderQty'], axis=1)
                  
                  df['MinQty'] = (df['Inventory']+df['InTransit']-df['Committed']-(df['NextSuggestedForecast'])-(df['NextSuggestedForecastToDate']*(1-DayOfWeek/DaysOfWeek))-df['SecurityStock'] ).abs()   
                  df['MaxQty'] = (df['Inventory']+df['InTransit']-df['Committed']-(df['BackSuggestedForecast'])-(df['BackSuggestedForecastToDate']*(1-DayOfWeek/DaysOfWeek))-df['SecurityStock'] ).abs()
                  ...
                  df['RQtyToDate'] =(df['Inventory']+df['InTransit']-df['Committed']-(df['SuggestedForecastToDate']*(1-DayOfWeek/DaysOfWeek))-df['SecurityStock']).abs()
                  df['ReorderQtyToDate'] = df[['ReorderStatus','RQtyToDate']].apply(lambda x: x['RQtyToDate'] if (x['ReorderStatus']=='Purchase') else 0 , axis=1 )
                  df['ReorderQtyToDate'] = df[['ReorderQtyToDate','ReorderStatus']].apply(lambda x: (1 if (x['ReorderQtyToDate'] <= 1) else x['ReorderQtyToDate']) if(x['ReorderStatus']=='Purchase') else x['ReorderQtyToDate'], axis=1)
                  
                  df['SuggestedReorderQtyToDate']=df['SuggestedForecast']*(1-DayOfWeek/DaysOfWeek)
                
                df.drop(columns=['RQty','MinQty','MaxQty','RQtyToDate'],inplace=True)
                ...
                df['SuggestedStockoutDays']=(df['SuggestedAvailability']-df['SecurityStock'])/df['AvgDailyUsage']
                df['SuggestedStockoutDays'] = df['SuggestedStockoutDays'].fillna(0)
                df['SuggestedStockoutDays'] = df['SuggestedStockoutDays'].map(lambda x: 0 if x < 0 else x)
                df['SuggestedStockoutDays'] = df['SuggestedStockoutDays'].astype(str).str.replace('-inf', '0').str.replace('inf', '0').str.replace('nan', '0')
                df['SuggestedStockoutDays'] = df['SuggestedStockoutDays'].astype(float)
        ...
        def reorden_point_date(self,df_invopt,timestamp,wd):
            """
            Return a dataframe with the column Reorder Point Date      
            : param df_invopt: Inventory's Dataframe 
            : param timestamp: current day   
            : param wd: Boolean to allow the use of working days 
            
            >>> df_inv = reorden_point_date(df_inv,timestamp,wd=False)  
            >>> reorden_point_date =
                                                Item    Location   Inventory   ...     ReorderPointDate
                                    idx0          85      905         23       ...        2022-06-20
                                    idx1          102     487         95       ...        2022-03-12
            """
            try:
              year = int(timestamp[0:4])
              month = int(timestamp[4:6])
              day = int(timestamp[6:8])
              df_invopt = df_invopt.reset_index(drop=True)
              df_invopt['Diff'] = (df_invopt['SuggestedReorderQty']-df_invopt['ReorderPoint']) / (df_invopt['AvgDailyUsage'])
              df_invopt.loc[df_invopt['Diff'] == np.inf,'Diff'] = 0
              df_invopt['Diff'] = df_invopt['Diff'].astype(str)
              df_invopt['Diff'] = df_invopt['Diff'].str.replace('-inf', '0')
              df_invopt['Diff'] = df_invopt['Diff'].str.replace('inf', '0')
              df_invopt['Diff'] = df_invopt['Diff'].str.replace('nan', '0')
              df_invopt['Diff'] = df_invopt['Diff'].astype(float).astype(int) 
              ROP = datetime.datetime(year, month, day)
              df_invopt['ReorderPointDate']=ROP

              for b in df_invopt.index:
                ROP = datetime.datetime(year, month, day)
                count =int(df_invopt['Diff'].iloc[b])
                if wd == True:      
                  if count>0:
                    for a in range(0,count):
                      ROP = ROP + datetime.timedelta(days=1)
                      if datetime.date.weekday(ROP) == 5: 
                        ROP = ROP + datetime.timedelta(days = 2) 
                      elif datetime.date.weekday(ROP) == 6: 
                        ROP = ROP + datetime.timedelta(days = 1)
                    df_invopt['ReorderPointDate'].iloc[b] = ROP
                  if count<0:
                    for a in range(0,count):
                      ROP = ROP - datetime.timedelta(days=1)
                      if datetime.date.weekday(ROP) == 5: 
                ...
                if wd == False:
                  if  count<1000:
                    ...
                    df_invopt['ReorderPointDate'].iloc[b] = ROP
                  if  count>1000:
                    ROP = ROP + datetime.timedelta(days= 1000)
                    df_invopt['ReorderPointDate'].iloc[b] = ROP      

              df_invopt['ReorderPointDate'] = pd.to_datetime(df_invopt['ReorderPointDate'])
              df_invopt.drop(columns=['Diff'],inplace=True)
        ...
        def extract_stockout (self,df_invopt,timestamp,wd): 
            """
            Return a dataframe with the column Stockout Date      
            : param df_invopt: Inventory's Dataframe 
            : param timestamp: current day   
            : param wd: Boolean to allow the use of working days 
            
            >>> df_inv = extract_stockout(df_inv,timestamp,wd=False)  
            >>> extract_stockout =
            ...
            try:
                ...
                month = int(timestamp[4:6])
                day = int(timestamp[6:8])
                StockoutDate = datetime.datetime(year, month, day)
                df_invopt['StockoutDate']=StockoutDate
                ...
                if wd == False:        
                  for a in df_invopt.index:
                    ...
                    if (df_invopt['AvgDailyUsage'][a] != 0):
                        ...
                        if (b < 0):
                            df_invopt['StockoutDate'][a] = StockoutDate
                        else:
                            df_invopt['StockoutDate'][a] = StockoutDate + timedelta(days=b)
                
                if wd == True:        
                  for a in df_invopt.index:
                    StockoutDate = datetime.datetime(year, month, day)
                    if datetime.date.weekday(StockoutDate) == 5: 
                      StockoutDate = StockoutDate + datetime.timedelta(days = 2) 
                    elif datetime.date.weekday(StockoutDate) == 6: 
                      StockoutDate = StockoutDate + datetime.timedelta(days = 1)  
        ...
        # Suggested Stockout Date  -------------------------------------------------

        def extract_sug_stockout(self,df_invopt,timestamp,wd):
            """
            Return a dataframe with the column Suggested Stockout Date      
            : param df_invopt: Inventory's Dataframe 
            : param timestamp: current day   
            : param wd: Boolean to allow the use of working days 
            
            >>> df_inv = extract_sug_stockout(df_inv,timestamp,wd=False)  
            >>> extract_sug_stockout =
            ...
            try:
                ...
                day = int(timestamp[6:8])
                StockoutDate = datetime.datetime(year, month, day)
                df_invopt['SuggestedStockoutDate']=StockoutDate
                ...
                if wd == False:        
                  for a in df_invopt.index:
                    ...
                    if (df_invopt['AvgDailyUsage'][a] != 0):
                        ...
                        if (b < 0):
                            df_invopt['SuggestedStockoutDate'][a] = StockoutDate
                        else:
                            df_invopt['SuggestedStockoutDate'][a] = StockoutDate + timedelta(days=b)
