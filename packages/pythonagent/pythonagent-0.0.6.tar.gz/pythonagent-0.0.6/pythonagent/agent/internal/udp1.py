...
class UDPConnection(object):
    def __init__(self):
        ...
        try:
            ...
            #p = Process(target=incoming_message_processor, args=(self.UDPClientSocket,))#child_conn))
            #p.start()
            #print(parent_conn.recv())
            #p.join()


            havoc_thread = threading.Thread(target=incoming_message_processor, name="havoc_thread", args=(self.UDPClientSocket,), daemon=True)
            havoc_thread.start()
