...
from fastapialchemycollector.utils.once import Once
from fastapialchemycollector.version import __version__
...
from fastapialchemycollector.utils.env_var import extract_additional_tags_from_env_var
from fastapialchemycollector.utils.singleton_class import SingletonMeta
...
def _build_resource(
    ...
    try:
        ...
        logger.exception("Cloudnt get hostname for service")

    if resource_tags:
        attrs.update(resource_tags)

    metis_tags_env_vars = extract_additional_tags_from_env_var()
    if len(metis_tags_env_vars) > 0:
        attrs.update({'app.tag.' + key: val for key, val in metis_tags_env_vars.items()})
...
class MetisInstrumentor(metaclass=SingletonMeta):
    def __init__(self,
        ...
        self.api_instrumentor_app = None
    ...
    def uninstrument_app(self):
        if self.api_instrumentor is not None:
            FastAPIInstrumentor.uninstrument_app(self.api_instrumentor_app)
