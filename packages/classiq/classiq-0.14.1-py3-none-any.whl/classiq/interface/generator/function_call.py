...
class FunctionCall(pydantic.BaseModel):
    ...
    @pydantic.validator("should_control", always=True)
    def _validate_should_control_flag(
        cls, should_control: bool, values: Dict[str, Any]
    ):
        if not should_control and values.get("control_state") is not None:
            raise ValueError("Cannot have a control state if shouldn't be controlled")
